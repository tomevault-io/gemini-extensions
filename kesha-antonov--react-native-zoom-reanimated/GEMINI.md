...
- **Fix pattern**: Run `yarn autofix`, then manually fix remaining TypeScript issues
...
- ❌ React Native CLI commands that require devices (`yarn ios`, `yarn android`)
...
|---------|--------------|----------------|-------|
| `yarn install` | 45-60 seconds | 300 seconds minimum | Root and example directories |
| `yarn start` (Metro) | 15-20 seconds | 60 seconds minimum | Shows ASCII logo when ready |
...
**Linting failures:**
- Use `yarn autofix` or `yarn eslint . --fix` to auto-fix most issues
- Common issues: line length, semicolons, spacing, TypeScript any types
- Check for React hooks dependency warnings

**Metro bundler won't start:**
- Clear Metro cache: `cd example && yarn react-native start --reset-cache`
- Check for port conflicts (default 8081)
- Verify all dependencies installed with `yarn install`
...
```bash
# Setup (run once)
yarn install                    # 45-60 seconds
cd example && yarn install      # 40-50 seconds

# Validate library (always run after library changes)
yarn tsc --noEmit                                              # 1-2 seconds
yarn eslint -c .eslintrc.js --ext .ts,.tsx --fix src/        # 2-3 seconds
yarn eslint -c .eslintrc.js --ext .ts,.tsx src/              # Shows remaining issues

# Validate example app (always run after example changes)
cd example
yarn tsc --noEmit               # 2-3 seconds
yarn eslint . --fix             # 1-2 seconds
yarn lint                   # 1-2 seconds

# Test builds (always run to verify integration)
yarn start                      # 15-20 seconds, shows ASCII art, then Ctrl+C
yarn react-native bundle --platform android --dev false --entry-file index.js --bundle-output bundle.js --assets-dest /tmp/  # 4-25 seconds
rm bundle.js
```
