...
- **Every verification tool is development and CI only, and none may reach production** (`docs/decisions/0026-verification-tools-are-instruments.md`). veraPDF, qpdf, pyHanko, Arlington's `testgrammar`, `pdfsig`, `pdftoppm` and Ghostscript are instruments: nothing in `src/` may invoke one (`tests/Project/ArchTest.php`), and nothing built for testing may ship (`tests/Project/DistributionTest.php` asks `git archive` what a release contains).
