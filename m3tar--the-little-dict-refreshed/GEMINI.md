...
- 公开发布 `TLD.mdx` 前要单独确认词典正文的授权范围。
- 未经用户明确要求，不执行 commit、push、tag 或 GitHub Release 发布。

## 11. 文档与变更控制

- `README.md` 面向最终用户；不要放入冗长开发流水账。
- `AGENTS.md` 保存跨任务都必须遵守的稳定约束。
- `docs/` 保存详细计划、测试证据、构建记录、实机结果和未验证项。
- 每个测试构建记录：变更内容、预期 JS/CSS 编号、自动测试、人工截图、实机结果和未验证项。
- 每个正式版本记录：发布结论、保持不变的行为、验证范围、包名和 SHA-256。
- 新需求若会改变信息顺序、删除原始数据、改变配置含义或扩大功能范围，先向用户说明并确认。
- 用户在当前任务中的明确要求优先；确认后同步更新相关代码、测试和文档，避免约束彼此矛盾。

## 12. 第二阶段已确认约束

页面显示设置按 `docs/TLD_Refreshed_第二阶段_页面显示设置开发规范.md` 实施，以下边界不得自行改变：

- 横向三滑杆按钮放在 `The Little Dict · Refreshed` 右侧、宿主折叠箭头左侧；使用靠右展开的轻量浮层，不使用底部抽屉。
- 浮层挂载到 `body` 顶层并按按钮视口坐标定位，不得成为宿主折叠或裁切容器的后代。
- 浮层的 `top`、`left` 和动态最大高度只由 JavaScript 定位函数负责；兼容样式不得用 `!important` 固定这些运行时坐标。
- 浮层不显示“显示设置”标题和关闭按钮；点击滑杆按钮或浮层外部关闭。页面外点击由透明、无视觉遮罩的顶层点击区域直接承接，关闭后必须隐藏且不得误触底层内容。
- 只提供 `iWeb`、`COCA`、`Spoken`、`Phrase Frequency`、`考试标签与星级` 五项独立开关，不提供总开关。
- 当前查询页不存在某类真实内容时不显示对应设置行；数据存在但用户已关闭时，设置行必须保留以便重新开启。
- 五项全部不可用时保留滑杆按钮，浮层显示 `当前词条暂无可配置内容`。
- 设置即时生效、自动保存，对后续查询和重新打开欧路继续生效；`config.ini` 只提供首次使用默认值，已保存的页面偏好优先。
- 浮层底部使用低强调文案 `自动保存 · 后续查询生效`，不增加“保存”按钮。
- 本轮不实现简洁查询模式、页面内恢复默认入口、主题设置、释义占比胶囊开关或中文释义开关。
- Android/iOS 的实际持久化能力必须通过目标宿主验证；存储失败不得破坏查词，但重新打开后不能保持偏好时不得宣称第二阶段完成。
