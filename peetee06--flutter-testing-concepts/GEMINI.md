...
> Follow these rules when writing Flutter or Dart code
...
16. Object patterns (`ClassName(field1: subpattern1, field2: subpattern2)`) match named object types and destructure their data using getters. Extra fields in the object are ignored. Example: `var Foo(:one, :two) = myFoo;`
...
25. Wildcard patterns (`_`, `Type _`) match any value without binding. Useful for ignoring values or type-checking without binding.
26. All pattern types can be nested and combined for expressive and precise matching and destructuring.
...
4. You can register multiple observers if needed by providing a list to the `observers` parameter. See the [official `ProviderObserver` docs](https://riverpod.dev/docs/api/providerscope) for full API details.
