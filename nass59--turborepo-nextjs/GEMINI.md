...
**Tailwind class order:** Must satisfy Biome sorting (extends `ultracite` preset). Run `pnpm format` to auto-sort. Manual order: custom utilities → layout/sizing/spacing → visual (bg/text/border/shadow) → state variants → responsive breakpoints. Note: Biome ignores `design-system/src/components/{ui,magicui}/*` and `design-system/src/{hooks,lib}/*` (external shadcn components).
...
## 8. Build & Tasks
**Core commands:** `pnpm dev` (all apps), `pnpm --filter=web dev`, `pnpm --filter=storybook dev`, `pnpm typecheck`, `pnpm lint`, `pnpm format`, `pnpm build`.

**Utilities:** `pnpm bump-deps` (update all deps except geist), `pnpm bump-ui` (sync shadcn components to design-system), `pnpm check-dependencies` (sherif validation), `debug:next`/`debug:storybook` (Node inspector), `scripts/adr-helper.sh` (create/manage ADRs).

**Turbo:** Config in `turbo.json` declares env passthrough for builds. When adding build-time env vars: update both `env.mjs` AND `turbo.json` `build.env` array. Turborepo caches based on inputs defined per task.
...
**tRPC specific:** Always use `protectedProcedure` for authenticated routes; `baseProcedure` for public. Context includes `clerkUserId`. Throw `TRPCError` with appropriate codes. Client usage requires `'use client'` directive + `useTRPC()` hook.
