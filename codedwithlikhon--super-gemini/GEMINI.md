...
- **agent-core/**: Implements agentic planning, execution, memory management, and OS integration.
  - `agent.py`: Main agent logic and orchestration.
  - `planner.py`: Task planning and decomposition.
  - `executor.py`: Executes planned actions across runtimes.
  - `memory_manager.py`: Persistent memory via JSON vector store.
  - `ubuntu_manager.py`: OS-level automation for Ubuntu/Termux.
- **runtimes/**: Runtime wrappers for Bash (`bash_runner.sh`), Python (`python_runner.py`), and Node.js (`node_runner.js`).
- **tools/**: Modular tools for API calls, browser automation, database, file system, and model execution. Each tool is a Python module.
- **webapp-builder/**: Scaffolds and deploys web applications using templates for React, Next.js, Svelte, Laravel.
- **android-app/**: Android integration and packaging (Gradle, JS, Kotlin placeholder).
- **memory/**: Embedding and vector store logic.
- **test_scripts/**: Example scripts for runtime validation.

## Developer Workflows
- **Run agent locally**: Execute `main.py` for core agent loop.
- **Add new tool**: Place Python module in `tools/`, update imports in agent logic if needed.
- **Scaffold webapp**: Use `webapp-builder/scaffold_generator.py` with templates in `webapp-builder/templates/`.
- **Test runtimes**: Use scripts in `test_scripts/` to validate Bash, Python, Node.js execution.
- **Setup environment**: Use `setup.sh` or `setup_ubuntu.sh` for dependencies and environment prep.

## Project-Specific Patterns
- **Agent Architecture**: Uses a three-component loop pattern:
  1. **Planner** (`planner.py`): Breaks down user requests into executable tasks
  2. **Executor** (`executor.py`): Runs tasks across different runtimes (Bash/Python/Node.js)
  3. **Memory** (`memory_manager.py`): Maintains preferences and conversation history
- **Tool Interface**: Tools are Python modules in `tools/` with a consistent interface pattern:
  ```python
  ...
  class MyTool:
      def execute(self, params):
          ...
          pass
  ```
- **Runtime Dispatch**: Executor auto-detects script type by extension:
  - `.sh` → Bash
  - `.py` → Python
  - `.js` → Node.js
- **Webapp Templates**: Follow standardized structure in `webapp-builder/templates/` for each framework

## Integration Points
- **Android**: Uses Termux for Linux compatibility and package management.
- **External APIs**: Use `tools/api_caller.py` for HTTP/API integration.
- **Emulator/VM**: Controlled via `tools/emulator.py`.

## Memory & State Management
- **Preferences**: Stored in `memory/preferences.json`, accessible via `MemoryManager`
- **Vector Store**: Located in `memory/vector_store.json` for conversation history
- **Runtime State**: Executor tracks script execution across runtimes
- **Ubuntu Status**: Agent checks Termux Ubuntu installation on startup

## Conventions
- Prefer stateless, modular Python tools.
- Use JSON for persistent memory and configuration.
- Scaffolded webapps should follow the template structure in `webapp-builder/templates/`.
- Runtimes should be validated with scripts in `test_scripts/` before agent integration.

## Example: Adding a New Tool
1. Create `tools/my_tool.py` with a minimal Python interface.
2. Update agent logic to import and invoke the new tool.
3. Add usage example in `test_scripts/` if needed.

---
For questions or unclear patterns, review `README.md` and key files in `agent-core/`, `tools/`, and `webapp-builder/`.
