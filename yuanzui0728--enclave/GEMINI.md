...
**群聊**：Group · GroupMember · GroupMessage · GroupReplyTask · ReplyArtifactJob · MediaInsightJob
...
- `MediaInsightJob`：用于持久化单聊 / 群聊附件理解任务，当前承载 `audio/video` 转写与 `document` 文本抽取的状态、重试、取消与结果回写，并额外记录 insight `provider` / `errorCode`，避免附件理解结果只存在于请求期临时旁路
...
- `Character` 表现已扩展字段：`modelRoutingMode`、`inferenceProviderAccountId`、`inferenceModelId`、`allowOwnerKeyOverride`、`modelRoutingNotes`，用于承载角色级模型路由、Provider 账户绑定与世界主人 Key 覆盖策略
- 会话管理路由：
...
- `inference-page.tsx`：模型与路由页，管理多个 Provider 账户、模型目录、默认推理路由，以及模型人格角色的批量安装与批量换绑
...
- `GET /api/admin/inference/overview`
- `POST /api/admin/inference/providers`
