...
- **Don't combine layouts from different templates.** Each template is a closed visual system. Mixing slide A from `rough-grid` with slide B from `neon-yellow` will look amateur. (This is **different** from §5: extending one template is fine; mashing two templates is not.)
