...
* Every component must remain available to consumers without disruption during upgrades and reconfiguration
  * Disruption caused by node restarts is allowed but must respect disruption policies (PodDisruptionBudget) and should be optimized to reduce total disruption
    ...
    * PodDisruptionBudgets (PDBs) should use `.spec.unhealthyPodEvictionPolicy=AlwaysAllow`. If an application loses its health (`Ready` condition is `False`) due to a bug or misconfiguration, PDB `.status.currentHealthy` may fall below `.status.desiredHealthy` which could block pod eviction. This will block a cluster upgrade. Therefore, it is  recommended to use the `AlwaysAllow` policy for most components. The only exceptions are critical components (e.g. etcd) that do not handle disruption well or do not get any benefit from this policy. With this policy, the disruption could occur (even affecting all replicas), for example during the workload start-up when no pod is ready, or if pods experience temporary loss of readiness and the application is expected to recover.
