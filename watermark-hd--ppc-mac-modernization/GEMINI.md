...
1. 環境情報の取得（上記コマンド）
2. Aquafox の `application.ini` と `omni.ja` の地雷チェック → 日本語化 ✅ 完了
3. libsmb2 のビルドが通るかだけ確認 ✅ 完了（SMB3接続まで実証済み。`smb3/README.md` 参照）
4. C案（マウントアプリ）の実装
   - 第1段（ファイルブラウザ型） ✅ 完了
   - 第2段（WebDAVループバックでFinderにアイコン） ✅ 完了

**タスク2は当初ゴール達成につき完了。** NAS(SMB3)が iBook の Finder に通常のボリュームとして
マウントされ、ドラッグ&ドロップ・コピー&ペーストで操作できる状態になった。詳細は `smb3/README.md`。

**追加タスク2b: iBookのNAS化(逆方向) ✅ 完了。** 実際の用途(iBook内の古いRAW写真を
現代のMac/Windowsから直接編集したい)を受けて追加。`LocalWebDAVServer` でiBookのローカル
フォルダをBasic認証付きWebDAVとしてLANに公開し、現代のMac Finderからの実マウント・
ブラウズ・ダウンロードまで実機確認済み。詳細は `smb3/README.md`。

**Windows側も2026-08-19に実機確認済み。** 双方向のドラッグ&ドロップコピーまで動作。
ただし手順は複雑(レジストリ変更・専用スクリプトの実行が必要)。AquaLinkの共有設定画面に
「Windows用接続ガイド」ボタンを追加し、実際のIPアドレス・共有名を埋め込んだ手順を
日本語で表示するようにして緩和した。`smb3/AquaLink/windows-setup/` に対話式のセットアップ
スクリプト(`connect-aqualink.bat` / `setup-aqualink.ps1`)を同梱。

**A案（ブリッジサーバ）は既定の進行順から外す。** C案が自己完結で動くことを最終目標とし、
A案は「詰まったときの保険」としてのみ温存する。

---

## タスク3（構想）: ブラウザ完結型のNASアクセス

2026-08-19、着想。現状のWebDAV方式(タスク2b)は「Finderやエクスプローラーに
常時ドライブとして表示される」という本物のNASらしい体験を実現できた一方、Windows側の
セットアップが複雑(レジストリ変更、専用スクリプトの実行)という代償がある。これはWindows
標準のWebDAVクライアント自体の制限に起因しており、これ以上の簡略化には限界がある。

**別解として、常時マウントを諦め、ブラウザだけで完結するファイルアクセスを検討する。**
現代はOfficeもクラウド対応でブラウザ完結が当たり前になっている時流を踏まえた発想。

- WebDAV(OSのファイルシステム層に統合する方式)をやめ、素のHTTP + HTMLでファイル一覧・
  ダウンロード・アップロード(ドラッグ&ドロップ)を提供する
- 利点: OSのWebDAVクライアントの癖に一切左右されない。Windows/Mac/スマホ問わずブラウザが
  あれば繋がる。レジストリ変更や専用スクリプトが完全に不要になる
- 欠点: エクスプローラー/Finderに常時ドライブとして表示されない。ブラウザを開く一手間が残る
- 実装コストは低い見込み。`LocalWebDAVServer.m` の GET/PUT/ディレクトリ一覧ロジックは
  既にあるので、WebDAVプロトコル部分(PROPFINDのXML応答など)を、素のHTML/JSのファイル
  ブラウザ画面に置き換える形で流用できる

**まだ着手していない。** タスク2b(WebDAV方式)は現役で動いているので、これは並行する
「第2の選択肢」として位置づける。「どちらを先にすべきか」という問いも含め、次回以降
検討する。
