...
| **`db:reset` requires running postgres container**    | Uses `DROP SCHEMA public CASCADE` + `drizzle-kit push` (not `docker compose down -v`). Preserves Zitadel database and postgres volume. Container must be running before reset        |
