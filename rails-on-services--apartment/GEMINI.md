...
- **A method-level rescue in a prepended method also covers its own `return super` guards**: `Patches::ConnectionHandling#connection_pool` opens with four `return super` guards (nil tenant, default tenant, no pool manager, pinned model on a separate pool), and its `rescue` used to sit at method level — so a stock-Rails connection failure on the **default** path, which is most of what a Rails app does, came back as `Apartment::ApartmentError: Failed to resolve connection pool for tenant ''`. The rescue is now an explicit `begin` around the tenant-resolution portion only, and stays inline rather than extracted: the block calls `super`, which resolves only from the prepended method's own scope. `TenantNameValidator.validate_common!` sits **inside** that `begin` — it is tenant-scoped work, and the `ApartmentError` clause re-raises its `ConfigurationError` unchanged. Two consequences downstream: `MigrationRole.wrap` no longer needs the `ApartmentError` clause and one-layer unwrap it carried (a `ddl_role` failure arrives as a bare `ActiveRecord::ConnectionNotEstablished`, because it surfaces from the default-path lookup and because the tenant path establishes a pool for the role itself and so never fails on an unregistered one), and `spec/integration/v4/ddl_role_resolution_spec.rb` asserts a **one-link** cause chain, which is what fails if the relabelling ever returns.
