...
- **Two-tsconfig split:** `tsconfig.json` checks everything (incl. tests, `jsx: preserve`); `declaration.tsconfig.json` extends it and emits `.d.ts` for `lib/**` only — its **`"files": []` override is load-bearing**: `index.d.ts` is hand-authored (see below), and inheriting `files: ["index.js"]` would emit over it. Don't merge them, don't "tidy" the empty array.
...
- **`index.d.ts` is hand-authored and git-tracked** (mirrors voxpelli/node-module-template) — the declaration emitter mishandles its surface (TS ≤6.0 drops the quotes from the `'module.exports'` interop export, producing invalid syntax; every emitter incl. 7.0 drops the `@deprecated` tags on the compat properties). `clean:declarations-top` and `.gitignore` both exclude it. Keep it in sync with `index.js` when the public surface changes; the repo's own tests type-check against it on every `check:tsc`.
