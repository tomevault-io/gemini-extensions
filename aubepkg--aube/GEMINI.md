...
github_interactions:
  ai_disclosure:
    surfaces: pull request descriptions, reviews, pull request comments, and discussion posts
    format: "*AI-assisted — Tool: <tool>; model: <provider>/<model>; version: <version-or-unavailable>.*"
    identifiers: use the exact model and version identifiers exposed by the runtime
    unavailable: never infer or guess identifiers; use unavailable when either value is not exposed
...
benchmarks:
  ...
  bump_sync[2]:
    ...
    - README.md BENCH_RATIOS block (regenerated by benchmarks/update-readme.mts, runs as the last step of `bench:bump`)
