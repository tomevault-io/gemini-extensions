...
- `delegate_worktrees` 一次只接受 bounded named batch；每项从 clean parent 的 exact HEAD 创建独立
  ...
  `pony agents merge <id>` 或有序 batch 的 `pony agents merge-all <batch-id>` 可把已封存 revision 的普通文件改动合入
  当前 trusted、clean branch；batch 在任何 parent 写入前预检完整顺序。未合入 terminal child 只能以显式
  `pony agents cleanup <id> --discard` 回收。
