...
- 構造を移行している最中のコードは「特性化テストで現行動作を固定 → 置き換え → テスト緑」の順で進める。移行中に挙動変更の判断を持ち込まないための**作業手順**であって、現行動作を製品仕様として承認するものではない。移行が済んだ領域を設計しなおすときは、現行動作は前提ではなく検討対象
...
- **UI 要素の `id` は E2E のセレクタ契約**(`#install-package` / `#uninstall-package` / `#check-packages-list` / `#clear-cache` / `#install-aviutl` / `#installation-path` / `#packages-list` / `#data-url` / `#set-data-url` ほか)。見た目のリファクタで消しても PR 前の 3 コマンド(lint / lint:ts / test)は緑のままで、気づくのは Windows の `yarn package` → `yarn test:e2e` を通したときになる。手元で確かめるには同じ 2 コマンドが要る
