...
- For slide deck presentations (when `--slides` is requested or the `generate-slides` prompt is used): read `./templates/slide-deck.html` and `./references/slide-patterns.md`
...
**Output location:** Write to `~/.openclaw/workspace/diagrams/`. Use a descriptive filename based on content: `modem-architecture.html`, `pipeline-flow.html`, `schema-overview.html`. The directory persists across sessions.

**Open in browser:**
- macOS: `open ~/.openclaw/workspace/diagrams/filename.html`
- Linux: `xdg-open ~/.openclaw/workspace/diagrams/filename.html`

**OpenClaw tool pattern:**
1. Use the `write` tool to create the HTML file directly.
2. Use `exec` to ensure the output directory exists (`mkdir -p ~/.openclaw/workspace/diagrams`).
3. Use `exec` to open the file (`open ...` on macOS, `xdg-open ...` on Linux).
...
An alternative output format for presenting content as a magazine-quality slide presentation instead of a scrollable page. **Opt-in only** — generate slides when the user invokes the `generate-slides` prompt, passes `--slides` to an existing workflow (e.g., `diff-review --slides`), or explicitly asks for a slide deck. Never auto-select slide format.
...
**`--slides` flag on existing prompts:** When a user passes `--slides` to `diff-review`, `plan-review`, `project-recap`, or other prompts, gather data using the workflow's normal data-gathering instructions, then present the content as a slide deck instead of a scrollable page. The slide version tells the same story with different structure and pacing — but the same breadth of coverage. Don't use the slide format as an excuse to summarize or skip sections that the scrollable version would have included.
...
**Example:**
```bash
bash {{skill_dir}}/scripts/share.sh ~/.openclaw/workspace/diagrams/my-diagram.html
...
**Requirements:**
- vercel-deploy helper (optional). Install/configure it separately in your OpenClaw environment before using `scripts/share.sh`.
