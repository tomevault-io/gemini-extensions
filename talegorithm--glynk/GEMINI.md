...
POST /api/publications                   → 从 URL 导入（?update_of={id} 走原地更新）
POST /api/publications/upload            → 从文件（epub/pdf/html/md/zip）导入（同上支持 update_of）
POST /api/publications/media/init        → 音频/视频：拿 OSS presigned URL + 分配 unit_id
