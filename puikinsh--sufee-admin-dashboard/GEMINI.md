...
- [src/styles/main.scss](src/styles/main.scss) — entry: Bootstrap source imports + body/layout
  shell + scrollbar + component imports. ~110 lines.
...
- `nav#sidebar.sidebar` (injected by the `sidebar` runtime partial) + `.main-content`, both inside a
  `div.d-flex.min-vh-100` shell. Sidebar width via CSS var (280px / 70px collapsed).
...
- Keyboard shortcuts: `Ctrl/Cmd+K` (focus search), `Ctrl+\` (toggle sidebar), `Esc` (close mobile
  sidebar / search).
...
- Targets last 4 versions of major browsers; legacy bundle generated via `@vitejs/plugin-legacy`
  (`targets: ['defaults', 'not IE 11']`).
- Internet Explorer is not supported. Do not add `<meta http-equiv="X-UA-Compatible">` or IE shims.

## Conventions

- Add new JS as ES modules under `src/scripts/components/` or `src/scripts/utils/`. Components are
  self-contained classes with their own init/destroy.
- `App` exposes `getComponent(name)`, `addComponent(name, component)`, `removeComponent(name)`, and
  `showNotification(message, type)` for cross-component coordination.
