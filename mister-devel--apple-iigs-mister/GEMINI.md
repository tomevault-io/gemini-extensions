...
- **notes.txt**: Memory layout, bank map, soft switches, and the Apple II text
  screen-hole addressing table that `lineaddr()` in `rtl/vgc.v` implements.
- **vsim/sdram_accel/**: Design docs for the SDRAM burst controller, line-buffer
  cache, and CPU speed control, plus the prototype RTL and the behavioral SDRAM
  chip model used by `make SDRAM=sim` and the `vsim/sdram_tb/` testbenches.

Note: the reference emulator sources (KEGS/GSplus), Apple documentation, and the
annotated IIgs ROM source are **not** part of this repository — they are
third-party material that cannot be redistributed. Some comments in the RTL still
cite them by name (e.g. "KEGS-based chr.mem"); those are historical references,
not files you will find here.
