...
- **Version**: kept in sync across `package.json`, `Cargo.toml`, `tauri.conf.json` — CI auto-syncs.
- **Adding a new command**: implement in `commands/*.rs`, register in `lib.rs` invoke_handler, add invoke wrapper in `commands.ts`.
