...
- Before running PowerShell commands that may print or consume Chinese text, force the session to UTF-8 so Codex does not read mojibake from console code page mismatches:
...
- For one-off commands, prepend the UTF-8 setup in the same PowerShell invocation before the command that emits Chinese output.
- When writing text files from PowerShell, do not rely on default encoding. Use explicit UTF-8 options such as `Set-Content -Encoding utf8`, `Out-File -Encoding utf8`, or `Export-Csv -Encoding utf8` when shell-based writes are unavoidable.
- If Chinese output appears garbled, rerun the source command with the UTF-8 setup above and verify the real text before editing code, tests, docs, or configuration based on that output.
