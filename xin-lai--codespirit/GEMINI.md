...
> CodeSpirit CSS 样式规范 - 命名约定、主题系统、动画效果、AMIS样式覆盖
...
- ❌ 不要使用 `!important` 覆盖 AMIS 核心功能样式（除非必要）
- ❌ 不要使用内联样式代替 CSS 类
- ❌ 不要使用绝对单位 `px` 定义字体大小（使用 `rem`）
- ❌ 不要创建过于复杂的选择器（超过 3 层嵌套）
- ❌ 不要在动画中使用会触发重排的属性（`width`, `height`, `top`, `left`）
- ❌ 不要忽略暗色主题适配
