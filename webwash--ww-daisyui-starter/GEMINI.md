...
**Note**: Even though Drupal core templates sometimes use `<div{{ attributes }}>`, this pattern is not allowed in WebWash daisyUI Starter theme components.
...
**Rule**: This theme uses **pnpm**, not npm. Always use `pnpm` for installing dependencies and running scripts. Node is pinned via `.nvmrc` (run `nvm use` first).

### Tailwind v4 + daisyUI Configured in CSS

**Rule**: Tailwind v4 and daisyUI are configured in `src/css/style.css`, not in `tailwind.config.js`. Any new CSS file must be `@import`ed from `src/css/style.css` — Vite will not pick up arbitrary new entries otherwise.

**Rule**: When adding new template/markup locations, update the `@source` lines in `src/css/style.css` or Tailwind will purge those classes. Use `tailwind-classes.txt` to safelist classes that are only referenced dynamically.

### Theme Hooks Use the OOP Hook API

**Rule**: There is **no `.theme` file**. Hook implementations live in `src/Hook/ThemeHooks.php` as a `final class` using `#[Hook(...)]` attributes. Add new hooks there following the existing pattern (preprocess + class injection) rather than overriding system templates.

## Workflow

### Run Format and Build After Changes

**Rule**: After completing any changes to the WebWash daisyUI Starter theme, always run `pnpm run format` and `pnpm run build` to ensure code is properly formatted and the build artifacts are up to date.

**Required Steps:**

1. **Format code**: Run `pnpm run format` to format all code according to the project's formatting rules
2. **Build assets**: Run `pnpm run build` to compile CSS and other assets into `dist/`
...
13. **Use pnpm and configure Tailwind/daisyUI in CSS** - Run scripts with `pnpm`; new CSS must be `@import`ed from `src/css/style.css`
14. **Run `pnpm run format` and `pnpm run build`** - After completing changes, format code and rebuild assets
