...
- this is a svelte web project AND a chrome extension
- it's built w/ vite
- remove the legacy streaming stores
- ALWAYS USE $STATE and $DERIVED!!!
- ALWAYS USE THE TAB DATA MODEL!!
- Don't use a global page data model as it pollutes tabs across each other
  - only universal settings and bookmarks should be global; anything related to a page or tab should be scoped to that url of the tab / open window! Otherwise you'll cross-contaminate
- avoid tight coupling; functions should focus on ONE thing; use secondary functions like message transports and functions
- any running component jobs like summary should ALWAYS save to a specific tab, not the active tab
