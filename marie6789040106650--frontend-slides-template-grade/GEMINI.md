...
> Create stunning, animation-rich HTML presentations, reusable deck templates, public template galleries, or web-style pages from scratch or by converting PowerPoint files. Use when the user wants to build a presentation, convert a PPT/PPTX to web, create slides for a talk/pitch, or produce high-completion HTML template samples. Helps non-designers discover their aesthetic through visual exploration rather than abstract choices.
...
**Template-grade mode:** If the user asks for a reusable template, demo deck, public gallery item, premium sample, website-style variant, or anything that should match a high-end HTML template library, read [TEMPLATE_GRADE.md](TEMPLATE_GRADE.md) before generating. In that mode:

1. Pick a style-specific demo topic instead of using the user's abstract placeholders.
...
4. Register layout ids and image slots using `PPT_PRODUCTION_GUARDRAILS.md` and `IMAGE_SLOT_GUIDE.md`.
