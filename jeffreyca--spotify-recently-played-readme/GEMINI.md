...
- **`isValidUserId` and `databaseKey` are one security boundary.** With the Admin SDK the user ID was a key; over REST it's a path segment in `/{key}.json`, so `/`, `..` and anything that ends the path early matter. Validate first, then `databaseKey`, then `encodeURIComponent` - encoding alone is not the boundary. Production contains legacy Spotify IDs with Unicode and punctuation (`+`, `*`, `?`, `!`, `;`, and `.` in email-style IDs), so the rule rejects only `/`, `\`, `%`, `$`, `#`, `[`, `]`, whitespace, controls, malformed Unicode and all-dot IDs. `databaseKey` then escapes the Realtime Database forbidden characters (`. $ # [ ] /`) as `%XX` - only `.` can reach it today, the other five branches are defensive and must stay; it is the identity for every ID that was valid before dots were admitted, so existing nodes never move, and `%` stays rejected in raw IDs so the mapping is injective. Do not narrow the rule back to ASCII, admit `/` or `%`, or change the key scheme.
