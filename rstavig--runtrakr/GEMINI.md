...
Edit `db/sample-data.ts` then run `ts-node db/seed.ts`. Uses `hashSync()` from bcrypt-ts-edge for password hashing (synchronous function, no await needed).
...
- NextAuth auto-configuration via `auth.ts`
