...
- `tests/` — Test suite, mirroring `src/` one-to-one (`tests/<path>.test.ts` ↔ `src/<path>.ts`); imports resolve via the `@/*` → `src/*` alias
...
- **Test layout**: Tests live in a top-level `tests/` tree mirroring `src/` (**not** co-located), so `src/` reads as pure production code. A test imports source through the `@/*` → `src/*` path alias (`tsconfig.json` `paths`, no `baseUrl` needed under `moduleResolution: "bundler"`) — `tests/core/foo.test.ts` does `import { x } from "@/core/foo"`. The all-Bun toolchain honors the alias natively everywhere: `bun test` auto-discovers `tests/`, and `tsc --noEmit` + `bun build --compile` both resolve `@/`. Two config touch-points beyond the alias, both required or `tests/` goes silently unlinted (effective set = CLI paths ∩ `files.includes`): Biome's `files.includes` in `biome.json` **and** the four `biome` scripts in `package.json` each list `./tests`. **Caveat:** a test that spawns a child process importing a real source module must anchor that path at `process.cwd()` (repo root, where `bun test` runs), **not** `import.meta.dir` — see `tests/core/session.test.ts` / `tests/core/solutions.test.ts`, whose child scripts `import` the on-disk `src/core/*` module.
...
Everything else can be edited **in-app** via the **settings editor** (`,` or `Ctrl+P` → "Settings", browse mode; `src/ui/components/SettingsEditor.tsx`) instead of hand-editing the TOML — a list of the 7 user-editable knobs (`editor.command`, `editor.detach`, `git.ui`, `language.default`, `theme.name`, `scroll.jump_rows`, `update.auto`) where enums cycle in place and text/number values edit inline. Writes go through the generic comment-preserving `persistSetting(section, key, value)` (`src/config/index.ts`), which picks a **quoted** string vs a **bare** number/boolean writer — the latter is why `jump_rows`/`detach` round-trip as real scalars; the metadata table is `src/config/settings.ts`. Changes apply live (theme via the `themeVersion` repaint; the rest re-read on the next accessor call), no relaunch. `paths.db`/`paths.solutions` are intentionally **not** in the editor — the solutions dir has its own migrate-on-change flow (`Ctrl+P` → "Change solutions directory").
...
`[update] auto` (default `true`) lets the TUI download + install new releases in the background (see the `update` subcommand bullet above); `false` = notify-only banner. Read via `getUpdateAuto()` (coerced by the pure `coerceUpdateAuto` — anything that isn't clearly `false` defaults to on); the other gates (release build, platform support) live in `core/update`'s `shouldAutoDownload`.
