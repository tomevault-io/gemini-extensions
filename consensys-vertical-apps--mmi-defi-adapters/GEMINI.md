...
> DeFi Adapter Development Rules and Patterns
...
### Key Points:
- `metadata` specifies which event argument to extract as additional metadata
- This metadata gets stored in the database and passed to adapters as `tokenIds`
- The underlying implementation supports multiple metadata for future expansion
- Use this for protocols like ETH2 staking (pubkey) or Uniswap V4 (tokenId)
