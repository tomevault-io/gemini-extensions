...
`editorconfig-checker` — это обёртка вокруг бинарника, который скачивается в `node_modules` при первом запуске и кешируется там до следующего `npm ci`. Обёртке нужна нода не ниже 24, как в `.nvmrc`: на более старой она падает с `TypeError` в минифицированном коде вместо отчёта о нарушениях. Версия закреплена в npm-скрипте переменной `EC_VERSION`: обёртка по умолчанию тянет `latest`, и тогда локально остаётся навсегда та версия, которую скачали однажды, а в CI приезжает свежая — отчёты расходятся. Формат вывода тоже задан явно (`--format gcc`): с версии 4 чекер сам переключается в CI на формат аннотаций GitHub, и в логе от ошибки остаётся один текст без файла и строки.
