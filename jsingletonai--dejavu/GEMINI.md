...
- **Entry point:** `dejavu = "dejavu_cli.app:main"`
- **Source layout:** `src/dejavu_cli/`
- **Optional dependency:** `dejavu-memory` (for OSS mode, via `[oss]` extra)
...
- **Hot reload:** Dev Dockerfile mounts `server/` and `dejavu/` for live changes
...
```bash
cd evaluation
make run-dejavu-add                  # Run dejavu add experiments
make run-dejavu-search               # Run dejavu search experiments
make run-dejavu-plus-add             # With graph memory
make run-dejavu-plus-search          # With graph memory
...
|-----------------|---------|--------|
| `Memory` | Self-hosted memory (sync) | `from dejavu import Memory` |
| `AsyncMemory` | Self-hosted memory (async) | `from dejavu import AsyncMemory` |
| `MemoryClient` | Hosted platform client (sync) | `from dejavu import MemoryClient` |
| `AsyncMemoryClient` | Hosted platform client (async) | `from dejavu import AsyncMemoryClient` |
...
|--------|---------|--------|
| `MemoryClient` | Hosted platform client | `import { MemoryClient } from 'dejavu-memory'` |
| `Memory` | Self-hosted OSS memory | `import { Memory } from 'dejavu-memory/oss'` |
...
| What | Import |
|------|--------|
| Core memory classes | `from dejavu import Memory, AsyncMemory` |
| Platform client | `from dejavu import MemoryClient, AsyncMemoryClient` |
| Configuration | `from dejavu.configs.base import MemoryConfig` |
| LLM providers | `from dejavu.llms.<provider> import <ProviderLLM>` |
| Embedding providers | `from dejavu.embeddings.<provider> import <ProviderEmbedding>` |
| Vector store providers | `from dejavu.vector_stores.<provider> import <ProviderVectorStore>` |

### TypeScript

| What | Import |
|------|--------|
| Hosted client | `import { MemoryClient } from 'dejavu-memory'` |
| OSS memory | `import { Memory } from 'dejavu-memory/oss'` |
| Specific providers (OSS) | `import { OpenAIEmbedding } from 'dejavu-memory/oss'` |
...
- **Remote:** MCP server at `mcp.dejavu.ai`
- **Local:** MCP server in `openmemory/api/` (FastAPI-based)
- **Plugin:** MCP tools in `dejavu-plugin/` — 9 tools: `add_memory`, `search_memories`, `get_memories`, `get_memory`, `update_memory`, `delete_memory`, `delete_all_memories`, `delete_entities`, `list_entities`

### Plugin & Skills System

- `dejavu-plugin/` provides integrations for Claude Code, Cursor, and Codex via MCP server connections and lifecycle hooks for automatic memory capture.
- `skills/` contains structured skill definitions for AI agents, split into two categories:
  - **Reference skills** (always-on SDK knowledge): `dejavu` (Python + TS SDKs, framework integrations), `dejavu-cli` (terminal workflows), `dejavu-vercel-ai-sdk` (Vercel AI provider).
  - **Pipeline skills** (run on demand): `dejavu-integrate` wires Deja Vu into an existing repo via a TDD pipeline; `dejavu-test-integration` verifies what the integrator produced on the same branch. The two are loosely coupled via `.dejavu-integration/` artifacts.

### Adding a New Provider

To add a new LLM, embedding, vector store, or reranker provider:

1. Create `dejavu/<category>/<provider_name>.py`
2. Inherit from the abstract base class in `dejavu/<category>/base.py`
3. Add configuration to `dejavu/<category>/configs.py` (if the category uses one)
4. Register the provider in `dejavu/<category>/__init__.py`
...
|----------|------|----------|-------|
| Python SDK | `ci.yml` | Push to main, PRs on `dejavu/`, `tests/`, `pyproject.toml` | Ruff lint + pytest on Python 3.10, 3.11, 3.12 |
| TypeScript SDK | `ts-sdk-ci.yml` | Push to main, PRs on `dejavu-ts/` | Prettier + build + jest on Node 20, 22 |
...
|----------|------|------------|--------|
| Python SDK | `cd.yml` | `v*` | PyPI (`dejavu-memory`) |
| TypeScript SDK | `ts-sdk-cd.yml` | `ts-v*` | npm (`dejavu-memory`) |
| Python CLI | `cli-python-cd.yml` | `cli-v*` | PyPI (`dejavu-cli`) |
| Node CLI | `cli-node-cd.yml` | `cli-node-v*` | npm (`@dejavu/cli`) |
| Vercel AI SDK | `vercel-ai-cd.yml` | `vercel-ai-v*` | npm (`@dejavu/vercel-ai-provider`) |
| OpenClaw | `openclaw-cd.yml` | `openclaw-v*` | npm (`@dejavu/openclaw-dejavu`) |
