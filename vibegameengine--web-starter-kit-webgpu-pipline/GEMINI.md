## web-starter-kit-webgpu-pipline
...
- Contact occlusion is gone from the default pipeline (2026-09-16): the pass, its GUI folder and its two hooks. It lives on under `?pipeline=legacy`, which is what `check-contact.mjs` now asks for. The frame traces one tree, the GI's own; the full-detail tree is built only when a bake runs (`TraceStages.detailedTree`), because a visibility ray between two lightmap texels starts inside the proxy box the GI budget left in place of its surface.
...
- Iteration 28 implements [bake-light-leaks-design.html](public/bake-light-leaks-design.html). `?leak=1`
  captures the atlas after every bake stage - raw, each denoise pass, the blit, the padding, the
  resident texture - and reports where texels moved and where a stage lit a texel the previous one had
  measured as black (`src/shared/gi/bake/leakStages.ts`, hooks `__leak.*`, split view `leak`, chip
  `bake-leak` on the corridor card, `scripts/check-bake-leak.mjs` with `?leakMutation=denoiseAll`).
  `?scene=leak-room` is the oracle: a sealed box whose true interior irradiance is zero, `?gap=` opens
  a slit of that many millimetres, `?scale=` builds the same room at any size
  (`scripts/check-leak-room.mjs`). Three real leaks were found and closed - the denoise mixing texels
  that cannot see each other (`filterLinks.ts`, `?filterLinks=0`), one epsilon guarding the
  determinant, the barycentrics **and the ray distance** at `1e-5` where the distance was the whole
  leak (`?triDetEps= ?triBaryEps= ?triTEps=`), and a ray offset fixed in metres that put every ray
  outside a room built a thousand times smaller (`?spawnEps=radius`). Separately the white line along
  every wall foot was `sun.shadow.bias = -0.0003` plus a shadow texel wider than the lit sliver; the
  map is now sized to a 4 mm target texel (`?shadowBias= ?shadowNormalBias= ?shadowSide=
  ?shadowFit=`). Three changes moved nothing and ship behind flags that default to the cheaper
  behaviour, with the null result in the commit: `?donorGate=`, `?bakeExactReuse=1`,
  `?bakePlacement=0.35`. Sealed room 0.00007 at the fine atlas and 0.00001 at 0.23 m/texel, from
  0.0313. Audit hooks `__audit.atlasIntensity / probeIntensity / shadow / pick / sunOccluded /
  meshBounds`. Details, traps and what is still open in
  [lessons-bake-light-leaks.md](docs/lessons-bake-light-leaks.md).

- Iteration 30 (2026-09-16) makes the lightmap's density a decision instead of a leftover.
  `?lmDensity=` is metres of world per texel (0.05 by default) and the atlas grows in 512²
  pages to hold it, as many as the scene needs up to `floor(8192 / atlasSize)`; `?lmDensity=0`
  goes back to the density solved from the scene's area. The bake is one rasterisation and one
  integration of the whole stack (`153a7de` restored after an unexplained revert), so pages
  cost pages and not bakes. Light is measured on a lattice every `?sample=` metres (0.1),
  counted from each chart's own corner, and carried to the texels between along the links
  `filterLinks` traces - so `MAX_SURFELS` limits the spacing, not the atlas resolution.
  Village 4 pages: 182.5 → 80.3 s cold, 75 → 22.5 s warm. Traps, and the five defects a harsh
  critic found in the first version of this - a link pass that was square and therefore dead on
  every page but the first, a bake cache key that ignored the density, a packer that lost
  charts silently - are in [lessons-lightmap-density.md](docs/lessons-lightmap-density.md).
...
- Iteration 31 (2026-09-17): physical sky. `src/shared/render/sky/` implements Hillaire 2020 from the
  ...
  sun's shadow filter, and are traced into the panorama (512x256) so reflections and a new bake see them.
  Aerial perspective: a 32x32x16 froxel volume to 32 km applied before the fog (`FrameGraph.setAerialPerspective`,
  `?aerial=0 aerialScale=`); never use a TSL `select` over the composite beauty node there, it blacks the sky. Host opt-in `sky: {}`, `?sky=0 skyAltitude= sunDiscScale=
...
- **Work in your own worktree instance, and give yourself a scene that boots in seconds.**
  User rule (2026-09-11). Several agents share this checkout, so before editing, create an
  instance: `git worktree add worktrees/<name> -b <name> pipeline-v3`, then junction
  `node_modules`, `vendor`, `dashboard`, `config` and `public` to the root (junctions, not
  copies - `dashboard` holds the Vite plugins `vite.config.ts` imports and is untracked, so
  a fresh worktree cannot start Vite without it). `git checkout -- vite.config.ts` inside a
  worktree restores the committed 13-line config with no `bakeCachePlugin`, and the scene
  then re-bakes every boot. The dev server takes the next free port after 5188: read it out
  of the log and pass `PORT=` to the checks; write new checks with
  `process.env.PORT ?? '5188'`. Uncommitted work in the root tree does not come with the
  worktree - commit it there first or copy the files over. **A 90-second scene is not a test
  rig:** four boots of the village was a ten-minute measuring loop, and it was stopped. Build
