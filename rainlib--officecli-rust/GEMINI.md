...
officecli view report.pdf --mode issues         # Empty pages, missing content
officecli get report.pdf '/page[1]' --json      # Get page content
officecli set report.pdf '/page[1]' --prop text="New content"  # Replace text
officecli remove report.pdf '/page[3]'          # Delete a page
officecli save report.pdf                       # Save modifications
```

PDF path system: `/page[N]`, `/page[N]/text[N]`, `/page[N]/image[N]`, `/page[N]/annotation[N]`.

### extract-text — Text/Offset → Path Mapping

Every format (docx, xlsx, pptx, pdf) can emit a TextOffsetMap — full text plus character-offset→path-ID mapping. AI agents read the map, find text to change, get the exact document path, and use `set` to modify precisely. No guessing, no regex parsing.
...
Output includes `full_text`, `spans` (each span has `start`, `end`, `path`, `text`, `element_type`), and `meta`.
