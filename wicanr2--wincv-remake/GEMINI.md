...
> **產物內嵌**原版的半形 `.FON` 與倚天的全形字庫(見 `NOTICE`),
> 所以解開就是與原版對齊的畫面。沒有它們也跑得起來:半形改用系統的
> **等寬**字型現場產,尺寸照原版的三種,版面與按鍵完全一樣。
>
> **Big5 以外的字**靠 `internal/ttf` 的後備鏈:掃系統的字型目錄(清單 +
> 目錄掃描,去重、跳過同家族的其他字重),補簡體、日文、韓文、西里爾、
> 希臘、阿拉伯、泰文與各種符號。完整版另外把 Noto Sans CJK 與幾份語系
> 子集嵌進執行檔(SIL OFL,可散布);**不嵌 unifont** —— 它是 GPL-2+,
...
remake 會記住上一次的字級、放大倍率與格數。連續跑好幾個倍率量視窗大小時,
第二次以後量到的是**上一次**的值 —— 而畫面本身完全正常,只是不是你要的那個
倍率,所以看不出來。實測 1.0–2.0 十一個倍率時,不加這個旗標會得到整齊
但整體位移一格的一張表。

同一件事也適用於 `cmd/celldump` 以外的任何「跑起來看看」:程式有狀態,
而狀態存在 `$HOME` 底下,容器內重跑不會自動乾淨。
