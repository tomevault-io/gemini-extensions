...
Cross-component integration tests live in `fr-conformance`.
...
cargo test -p fr-protocol
cargo test -p fr-command
cargo test -p fr-store
cargo test -p fr-expire
cargo test -p fr-persist
cargo test -p fr-repl
cargo test -p fr-eventloop
cargo test -p fr-config
cargo test -p fr-runtime
cargo test -p fr-conformance
```
...
| Crate | Focus Areas |
|-------|-------------|
| `fr-protocol` | RESP2/RESP3 parsing, serialization round-trips, malformed frame rejection |
| `fr-command` | Command dispatch, argument validation, error replies |
| `fr-store` | Data structure operations, memory limits, key enumeration |
| `fr-expire` | TTL tracking, lazy vs active expiration, edge cases |
| `fr-persist` | AOF format, RDB snapshots, recovery ordering invariants |
| `fr-repl` | Replication state machine, ordering guarantees |
| `fr-eventloop` | Event dispatch, client lifecycle |
| `fr-config` | Configuration parsing, defaults, validation |
...
## FrankenRedis — This Project

**This is the project you're working on.** FrankenRedis is a ground-up Rust reimplementation of Redis with strict command semantics, deterministic latency, and recoverable persistence pipelines.
...
**CRITICAL NON-REGRESSION RULE:** Redis command semantics and replication ordering are core contracts for V1 scope.
...
├── crates/
│   ├── fr-protocol/                   # RESP parser and wire types
│   ├── fr-command/                    # Command dispatch and execution
│   ├── fr-store/                      # In-memory data engine
│   ├── fr-expire/                     # Key expiration and TTL management
│   ├── fr-persist/                    # AOF and RDB persistence pipelines
│   ├── fr-repl/                       # Replication protocol and state machine
...
Compatibility focus: preserve Redis-observable replies, side effects, and ordering guarantees for scoped command sets.
...
| Scenario | Tool | Why |
|----------|------|-----|
| "How does the RESP parser work?" | `warp_grep` | Exploratory; don't know where to start |
| "Where is replication state tracked?" | `warp_grep` | Need to understand architecture |
| "Find all uses of `CommandRouter::dispatch`" | `ripgrep` | Targeted literal search |
...
mcp__morph-mcp__warp_grep(
  repoPath: "/dp/frankenredis",
  query: "How does the RESP protocol parser handle inline commands?"
)
```
