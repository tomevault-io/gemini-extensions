...
| [`skills/`](skills/) | Nine skills, **all** namespaced `talksmith:<skill>` in their SKILL.md frontmatter (the namespace is the convention — an un-namespaced `name:` collides with a user's own same-named skill instead of coexisting with it): `ingest` (web capture), `ascii-to-svg` (one ASCII block → one SVG; owns the viewBox and rasterizer contracts), `polish-ascii` (Step-6 mechanical scan / sidecar / stamp / cleanup for diagrams — `stamp-renders` arms render idempotency), `polish-images` (its **sibling** — the same staged scan / sidecar / stamp / cleanup for `<!-- generate-image: … -->` aside directives) and `generate-image` (tool-agnostic backend the image-illustrator dispatches; produces a raster or returns `unavailable`, never blocks), `feedback-cycle` (Step-5 bookkeeping CLI), and `md-to-deck` (Step-7 render plus the PDF and `.pptx` exports measured from it). Shared code lives **once** under [`skills/_shared/`](skills/_shared/), `sys.path`-imported by its consumers: [`_context.py`](skills/_shared/_context.py) (the slide-context scanner — **both** polish skills), [`_plan.py`](skills/_shared/_plan.py) (the plan-file envelope, `--final/--plan/--dry-run`, and the `gc` live set — **both** polish skills, which are otherwise near-twins), and [`_write.py`](skills/_shared/_write.py) (the atomic in-place write every skill that edits `draft.md`/`final.md` must use — currently `feedback-cycle`). The model-reading audits share [`skills/md-to-deck/audits/_model.py`](skills/md-to-deck/audits/_model.py) (the `_`-key-excluding model walk). Two more are **anti-slop passes the Editor applies to a Talk's prose**, one per language: `stop-slop` (English, vendored from Hardik Pandya) and `desrobotizar` (Spanish) — both are **explicit-invocation only**, so neither fires on ordinary drafting or editing; the dispatch table lives in [`agents/editor.md`](agents/editor.md). **No skill requires Cowork.** Everything else lives in each SKILL.md — don't restate it here. |
...
| The render, the two exports, progress checklists and suppression vocabulary | [`skills/md-to-deck/SKILL.md`](skills/md-to-deck/SKILL.md) |
| The model walk shared by the coverage audits | [`skills/md-to-deck/audits/_model.py`](skills/md-to-deck/audits/_model.py) |
...
| The deck palette a diagram may draw from | [`config/diagram-style.md`](config/diagram-style.md) |
...
| Change how the `.pptx` or PDF export behaves | [`skills/md-to-deck/export_pptx.py`](skills/md-to-deck/export_pptx.py) / [`export_pdf.py`](skills/md-to-deck/export_pdf.py), and [`templates/html/harvest.js`](skills/md-to-deck/templates/html/harvest.js) if the display list needs to carry something new |
| Add a deck skin | drop a `:root[data-deck-style=<name>]` token block at [`templates/html/styles/<name>.css`](skills/md-to-deck/templates/html/styles/) — it is discovered from disk, so nothing else changes |
...
7. **Exports — nothing to do.** This is the step that used to mean "write the recipe a second time in EMU." The `.pptx` and the PDF are measured from the rendered HTML, so a new template exports the moment it renders. Confirm it: run `export_pptx.py` on the fixture and look at the new slide. If something about the type is genuinely new to the display list (a construct no other template paints), that shows up as a warning from the harvester rather than as silence.
