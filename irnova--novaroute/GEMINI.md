...
- DB file location resolves via `src/lib/db/paths.js` (`DATA_DIR`, else `~/.novaroute/`).
- Usage/logs (`src/lib/usageDb.js`, `usage.json` + `log.txt`) still live under `~/.novaroute` and do **not** follow `DATA_DIR`.
