...
| `list [--no-tui]` | `run_list()` | プラグイン一覧表示。デフォルトは TUI で `[S] sync / [R] sync --rebuild / [u/U] update / [d] remove / [e] edit / [s] set / [c] config.toml / [b] browse / [?] help` のアクションキー対応。**1 行目は `[ Global hooks ]` sentinel** — `e` で global edit (init/before/after) に飛べる、`u/d/s` は no-op。ナビ: `j/k/g/G/Ctrl-d/u/f/b`、検索: `/n/N`。`--no-tui` で pipe-friendly な plain text 出力 |
