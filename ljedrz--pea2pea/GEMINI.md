...
- Cleanup is RAII-guard-based throughout (`ConnectionGuard`, `DisconnectOnDrop`, `SenderCleanup`, `SchedulingGuard`, `UnregisteredTask`, `DisconnectFinalizer`): every claimed resource has an owner whose `Drop` completes or rolls back on any exit path. Stale-guard protection is keyed by `Connection::id` (globally unique); do not introduce parallel identity schemes.
