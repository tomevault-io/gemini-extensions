...
- 如果主题拿不准，可在正文定稿后运行 `python3 {skill_dir}/toolkit/cli.py gallery {article_dir}/article.md --no-open -o {article_dir}/theme-gallery.html`，对比后再确定最终主题
...
- 先读取：`{skill_dir}/references/template-workflow.md`，发草稿时一律以这份规则为准。
...
| 热点抓取 | WebSearch 替代；若素材不足导致 warning，必须补足后继续 |
| 选题为空 | 请用户手动给选题 |
| SEO 脚本 | LLM 判断仅作临时分析；最终 `seo-report.json` 不得有 warn |
| 素材采集（WebSearch） | LLM 训练数据中可验证的公开信息；必须明确标注素材不足，不能作为发布通过 |
