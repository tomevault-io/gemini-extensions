...
| CODEX.md content | `docs/CODEX.md` (end-user instructions) |
...
```typescript
// Entry point
import { createSisyphusSession } from 'oh-my-codex';
...
| `~/.codex/.omc/` | Legacy state (auto-migrated) |
...
| Package | Purpose |
|---------|---------|
| `@anthropic-ai/claude-agent-sdk` | Codex integration |
...
Settings in `~/.codex/.omc-config.json`:
