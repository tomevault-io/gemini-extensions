...
### 13. Autogenerated Files
- Packages shipping a `version.dart` must include `build_runner` and `build_version` in their `dev_dependencies` (align versions with existing packages in the repo)
- `version.dart` files (typically `lib/src/version.dart`) MUST always be autogenerated
- The generated file must be excluded from analysis in `analysis_options.yaml` under `analyzer: exclude:`
