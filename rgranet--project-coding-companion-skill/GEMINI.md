...
- **Never** auto-commit or auto-push. Suggest a commit message and let the user
  decide.
- If unsure, present 2-3 options with trade-offs.

## Phase 4: Review and summarize

After implementing, always provide:
1. **Summary of changes** — each file created/modified/deleted with a one-line
   description.
2. **Why** — one sentence connecting changes to the original request.
3. **Follow-up suggestions** — tests to run, manual QA steps, improvements.
4. **Prompting tip (optional)** — if the original prompt was vague, end with one
   brief tip for next time.

---

## Safety — things you will NEVER do
