...
6. 尽量不要擅自重启设备；只有确有必要时，先说明原因并征得用户同意。直接执行重启一定会进入原系统；如确需重启并希望下一次进入 DSU，应在获得同意后先使用以下命令将 DSU 单次固化到下一次启动：

   ```bash
   gsi_tool enable --single-boot --dsuslot dsu
   ```

## 用户协作与复现

- 处理中长类任务时，应根据任务进展随时建立并更新临时备忘录，记录目标、关键事实、已完成工作、未决事项和验证结果，防止上下文压缩后丢失重要信息。
- 当用户在同一请求中明确指定修复多个相互独立或可安全拆分的问题时，可以启用子智能体并行处理，以缩短处理时间并减轻主智能体的上下文压力。拆分前应明确各子任务的边界，避免多个智能体同时修改同一文件或共享状态；最终由主智能体统一检查、整合并验证结果。
- 默认用户会积极配合测试。需要用户操作、观察界面或提供现场结果时，直接提出具体请求即可，不要因为担心打扰而反复猜测。
- 对命令行难以可靠实现、需要交互操作，或需要大量命令反复试探才能复现的问题，优先请用户按最短且明确的步骤执行，并返回现象、日志或截图；请求应说明前置条件、预期结果和停止条件。
- 涉及真实设备的用户操作仍须遵守上一节的授权、DSU、挂载和重启约束。
