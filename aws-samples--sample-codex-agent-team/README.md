# sample-codex-agent-team

> Source: [aws-samples/sample-codex-agent-team](https://github.com/aws-samples/sample-codex-agent-team) — distributed by [TomeVault](https://tomevault.io)

Team of Codex agents that collaborate through a spec-driven development process. Full Stack Developer parent orchestrates dynamically sized pools of specialists: Coding, DevOps, Review, and Solutions Architect agents.

## Gemini CLI Config

The `GEMINI.md` file in this directory is the project config converted for Gemini CLI.
Original source: `AGENTS.md` in [aws-samples/sample-codex-agent-team](https://github.com/aws-samples/sample-codex-agent-team).

## Also available for

- **Claude Code** — `CLAUDE.md`
- **GitHub Copilot** — `copilot-instructions.md`
- **Cursor** — `project-config.mdc`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

Quality verified by TomeVault's automated analysis pipeline. Source: [github.com/aws-samples/sample-codex-agent-team](https://github.com/aws-samples/sample-codex-agent-team)

---

Own this repo? Install the TomeVault Relay to keep every platform's copy in sync on every push: [https://tomevault.io/install](https://tomevault.io/install).

<!-- genome:d-c-q -->
