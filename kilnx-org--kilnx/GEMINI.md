...
> Kilnx language — complete reference for generating and editing .kilnx files
...
| `{{if !field}}`...`{{end}}` | Negated conditional |
| `{kilnx.js}` | Inject htmx script tag (use once in layout) |
| `{page.title}` | Current page title (use in layout) |
| `{page.content}` | Current page body (use in layout) |

### Available filters

`upper` `lower` `date` `timeago` `truncate:<n>` `currency` `escape`
...
- Named parameters: `:name` (never string concatenation)
- `:current_user` → authenticated user's ID
- Multi-query: name each one — `query posts: SELECT ...`, `query count: SELECT count(*) ...`
- SQL is validated at compile time by `kilnx check`
...
Import directives must be at column 0 (no indentation).
...
kilnx build <file.kilnx> [-o output]    Compile to standalone binary (~15MB)
kilnx check <file.kilnx>               Static analysis
kilnx test <file.kilnx>                Run test blocks
kilnx migrate <file.kilnx>             Apply DB migrations
kilnx lsp                              LSP server (stdin/stdout)
kilnx mcp                              MCP server for AI tools (stdin/stdout)
```
...
- Do not use `{{each}}` without a named query — the query name is the variable
- Do not use `/` prefix on `after login:` paths — just the path: `after login: /dashboard`
- `password` fields hash automatically — do not call bcrypt yourself
- `timestamp auto` sets the value on INSERT — not on every UPDATE
- `option` type requires brackets: `status: option [a, b, c]`
- Fragment paths must not use `layout` — fragments are partial HTML only
- `requires auth` goes on the `page`/`action` line, not indented under it
