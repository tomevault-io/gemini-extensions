...
29. **`verification-metadata.xml` regen task list is `help build apiCheck cyclonedxBom`** (canonical command in `RELEASING.md`). `cyclonedxBom` is mandatory — it pulls the SBOM transitive-POM closure the others don't reach. The dogfood predicate in `settings.gradle.kts` auto-disables composite mode under `--write-verification-metadata`, so the published harness graph (invariant I1's CI side) is always captured — no `-Pfluxo.dogfood=false` discipline needed. Two manual-pin edge cases remain (rare; surface on CI as `One artifact failed verification: <name>`): (a) **transform-input classpath artifacts** (e.g., `jackson-module-kotlin` via KGP's `ClasspathEntrySnapshotTransform` — invisible to `dependencyInsight` because they're transform inputs, not deps); (b) **non-host-OS platform binaries** (`node-*-{linux,win}-*` from a macOS-host regen — pin all CI-matrix variants from the upstream `SHASUMS256.txt`).
