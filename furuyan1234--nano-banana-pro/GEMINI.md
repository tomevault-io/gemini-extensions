...
- **【役割の分離と監査引き継ぎ (Inspector Handoff)】**: デプロイ、バックアップ、システム構成の変更などクリティカルなバッチ処理を行った後、実行担当AIは直接ユーザーに完了を報告してはならない。必ず `HANDOFF.md` に状態を「Pending Verification」として書き出し、タスクを停止して検証担当AI（Inspector）へとコンテキストを引き継ぐこと。実行と検証を同一の思考ループで行うことを固く禁ずる。
- **【メタ監視とオーディター層 (Auditor Layer)】**: 検証担当AI（Inspector）は、実行担当AIの残したログを監査し、もし「実行が終わっていないのに終わったと報告する形跡」や「ガードレール違反」を発見した場合は、直ちに作業を却下し、ユーザーにアラート（不正報告の検知）を通知しなければならない。AI同士の馴れ合い（エラーの見逃し）は反逆行為とみなされる。
