...
- Masking semantics: `MaskedConv1D` takes `mask_mode` (`any` default, `majority`, `strict`) controlling how the output mask is derived from the k window inputs. `strict` is the legacy rule (valid only if all k inputs are valid — one N kills ~k positions per layer, compounding through deep stacks and erasing N-heavy or short/right-padded fragments). `any` keeps a position valid if at least one window input is valid — Ns only kill N-runs of length ≥ k. Fully-masked samples (zero valid positions) pool to a zero vector via `MaskedGlobalMaxPooling` (never a sentinel), which maps `pooling: max`/`average` in the builder.
- `residual_block` takes `norm_type` (`masked_batchnorm` default, `masked_layernorm`, `masked_dyt`) to swap the block's normalization layers; `return_nmd: true` requires `masked_batchnorm`.
- `representation_learner.pooling` options: `max`/`average` (masked variants of the Keras poolers — padded/fully-masked samples pool to zeros), `masked_max`/`masked_average` (aliases), `last`/`masked_last` (`MaskedLastPooling` — last valid position per frame averaged over frames, for causal models like hyena; mask-aware so right padding and N-runs never leak in), `gatedframe`, and the 1D Keras variants `max1d`/`average1d`.
