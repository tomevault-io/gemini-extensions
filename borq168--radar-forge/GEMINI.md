...
- RSS Feed: `feed.xml` at the repo root. Generated by `src/generate-manifest.ts` in the same `pnpm manifest` step. Contains the latest 30 items (newest first) across all report types. Item links use hash routing: `https://borq168.github.io/radar-forge/#YYYY-MM-DD/report`.
...
- Rollup source selection is now derived from `src/report-registry.ts` via `includeInRollups`, instead of a separate hardcoded list in `src/rollup.ts`. `ai-daily.md` is the first rollup source for new dates; detailed reports remain available as fallback for dates before integrated daily existed.
