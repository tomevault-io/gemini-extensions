...
**Default**: Use `skills/canghe-image-gen/SKILL.md` (unless user specifies otherwise).

1. Read `skills/canghe-image-gen/SKILL.md` for parameters and capabilities
...
**Skill Selection**:
1. Check available image generation skills (e.g., `canghe-danger-gemini-web`)
...
## Style Maintenance (canghe-comic)

When adding, updating, or deleting styles for `canghe-comic`, follow this workflow:

### Adding a New Style

1. **Create style definition**: `skills/canghe-comic/references/styles/<style-name>.md`
2. **Update SKILL.md**:
   - Add style to `--style` options table
   - Add auto-selection entry if applicable
3. **Generate showcase image**:
   ```bash
   npx -y bun skills/canghe-danger-gemini-web/scripts/main.ts \
...
4. **Compress to WebP**:
   ```bash
   npx -y bun skills/canghe-compress-image/scripts/main.ts screenshots/comic-styles/<style-name>.png
   ```
...
1. **Update style definition**: `skills/canghe-comic/references/styles/<style-name>.md`
...
1. **Delete style definition**: `skills/canghe-comic/references/styles/<style-name>.md`
...
test -f .canghe-skills/<skill-name>/EXTEND.md && echo "project"

# Then user-level (cross-platform: $HOME works on macOS/Linux/WSL)
test -f "$HOME/.canghe-skills/<skill-name>/EXTEND.md" && echo "user"
\`\`\`

┌────────────────────────────────────────────┬───────────────────┐
│                    Path                    │     Location      │
├────────────────────────────────────────────┼───────────────────┤
│ .canghe-skills/<skill-name>/EXTEND.md       │ Project directory │
├────────────────────────────────────────────┼───────────────────┤
│ $HOME/.canghe-skills/<skill-name>/EXTEND.md │ User home         │
└────────────────────────────────────────────┴───────────────────┘
...
**Notes**:
- Replace `<skill-name>` with actual skill name (e.g., `canghe-cover-image`)
