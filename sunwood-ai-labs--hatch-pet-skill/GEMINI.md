...
## Post-Generation Chroma Spill Cleanup

If a completed package shows green, cyan, or other chroma-key colored fringe along the transparent edge of sprites, repair the existing generated frames with deterministic cleanup before considering row regeneration. This path is only for transparent-edge chroma spill left by background removal; it must not be used to redraw, invent, or synthesize pet art.

```bash
SKILL_DIR="${CODEX_HOME:-$HOME/.codex}/skills/hatch-pet"
python "$SKILL_DIR/scripts/cleanup_chroma_spill.py" \
  --package-dir /absolute/path/to/pet-package \
  --sync-codex-pets \
  --json-out /tmp/<pet-name>-chroma-spill-cleanup.json
```

For batch repairs, repeat `--package-dir` once per pet package. The script reads `frames/frames-manifest.json`, uses each run's recorded chroma key, removes chroma-colored spill pixels only near transparent edges, zeros hidden RGB under fully transparent pixels, rebuilds `spritesheet.webp`, regenerates `qa/contact-sheet.png` and `final/validation.json`, and optionally syncs the installed `${CODEX_HOME:-$HOME/.codex}/pets/<pet-name>/` package.

Review the JSON report after cleanup. `after_residual` should normally be `0`; if it is not, inspect the remaining pixels before accepting the package. Then visually review the regenerated contact sheet and a white/dark composite preview, and rerun the normal atlas validation for the affected package. If cleanup removes real pet markings or effects, revert that package and repair the affected row through `$imagegen` instead.
