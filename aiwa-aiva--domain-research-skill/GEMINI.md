...
3. **运行转换脚本**：
   ```bash
   python [skill目录]/scripts/md_to_pdf.py input.md output.pdf --title "研究对象名称" --author "微信公众号:熵息茶馆" --qr "$(pwd)/references/shangxichaguan.png"
   ```
4. 脚本会自动生成中间HTML文件（便于调试）和最终PDF
5. **PDF引擎自动选择**：脚本优先使用 WeasyPrint；若 WeasyPrint 缺少系统库（macOS 常见），自动 fallback 到 Chrome headless，无需手动干预
...
### PDF引擎说明（WeasyPrint vs Chrome headless）

脚本支持两种 PDF 引擎，按优先级自动选择：

| 引擎 | 优先级 | 页眉页脚支持 | 适用场景 |
|------|--------|------------|---------|
| WeasyPrint | 优先 | 完整支持 CSS Paged Media（`@top-center`/`@bottom-center`） | 已安装 pango/gobject 系统库时 |
| Chrome headless | Fallback | 渲染 CSS 内容页眉页脚，去除 Chrome 自带的时间戳/路径 | macOS 未安装 WeasyPrint 系统依赖时 |

**⚠️ Chrome headless 关键参数**：必须使用 `--no-pdf-header-footer`，**不能**用 `--print-to-pdf-no-header`。

- `--no-pdf-header-footer`：Chrome 112+ 的正确参数，去除 Chrome 默认附加的「时间戳 + 文件路径 + 页码」
- `--print-to-pdf-no-header`：旧参数，Chrome 112+ 已失效，使用后首页仍会出现时间戳和文件路径

两者的区别：`--no-pdf-header-footer` 去除的是 Chrome 自己加的那一行；HTML 里 CSS `@page` 规则中的 `@top-center`/`@bottom-center` 属于页面内容本身，Chrome 会正常渲染，不受此参数影响。
...
- 用 `##` 作为主要章节标题，内容可根据研究对象来调整，比如（时间深度分析、同期广度分析、交汇洞察等）
- 用 `###` 和 `####` 作为子章节
- 表格使用标准Markdown表格语法
- 引用使用 `>` 语法
- 加粗使用 `**文本**`
- 首页在正负标题下方，展示"微信公众号:熵息茶馆"的二维码，位置在 `reference/shangxichaguan.png`
...
PDF文件命名为 `[研究对象名称]_深度分析报告.pdf`，保存到用户的工作目录中。

---

## 不同研究对象类型的适配

核心原则不变（时间深度，同期广度），但侧重点不同：

**研究产品时**：深度重点关注版本迭代、技术路线演变、用户增长曲线、关键产品决策；广度重点关注功能对比、性能对比、用户体验、定价。

**研究公司时**：深度重点关注创始团队、融资历程、战略转向、组织变革、关键人事变动；广度重点关注商业模式差异、市场份额、营收对比、组织架构差异。

**研究概念时**（技术范式、商业模式、文化现象）：深度重点关注概念的起源（谁提出的、基于什么理论/需求）、如何流行起来、经历了哪些争论和演变；广度重点关注与相近概念的区别、各自适用场景、不同阵营的论证。

**研究人物时**：深度重点关注个人经历、职业轨迹、关键决策、成长曲线、公开言论变化；广度重点关注与同领域其他人物的对比（做事方式、风格、成就、影响力、路线选择差异）。
...
| 部分 | 字数范围 | 说明 |
|-----|---------|------|
| 深度分析 | 6,000 - 15,000字 | 报告主体，不要蜻蜓点水 |
| 广度分析 | 3,000 - 10,000字 | 视竞品数量调整 |
| 交汇洞察 | 1,500 - 3,000字 | 精华段，给出新判断 |
