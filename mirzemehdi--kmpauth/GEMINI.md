...
- **iOS dependencies via SwiftPM** (`swiftPMDependencies {}` DSL, Kotlin 2.4+): GoogleSignIn-iOS (google), facebook-ios-sdk products `FacebookCore`/`FacebookLogin` (facebook), firebase-ios-sdk as a closed range 11.8.0…12.999.999 (firebase; SPM `from` would cap at <12.0.0 and conflict with consumers on 12.x — GitLive was built against the lower bound). No CocoaPods anywhere. `cocoapods.FirebaseAuth.*` imports in kmpauth-firebase iosMain are GitLive's **bundled cinterop** — never rewrite them.
...
`.github/workflows/build_and_publish.yml`: apiCheck → `testAndroid jvmTest` + androidApp APK (ubuntu) and `iosSimulatorArm64Test` (macOS) → on `v*` tag: Dokka docs to GitHub Pages + `publishAndReleaseToMavenCentral` (vanniktech, GPG-signed) + GitHub release. PR builds trigger only for PRs targeting `main`. CI does **not** run `./gradlew build`, so failures outside `apiCheck`/tests (e.g. a stale `kotlin-js-store/yarn.lock` after dependency bumps — fix with `kotlinUpgradeYarnLock`) surface only locally. The release job additionally runs `./gradlew :dokkaGeneratePublicationHtml`, whose common/intermediate **metadata compilations** are stricter than the per-target ones (an `expect object : Interface` must redeclare the interface's non-default members or metadata compile fails) — run it locally before tagging.
...
**Registration is automatic with Firebase**: `kmpauth-firebase` on the classpath auto-registers via `ServiceLoader` on JVM/Android (service class `FirebaseAuthBackendService` + META-INF/services resources + consumer R8 keep rule; core discovers lazily in `KMPAuthBackend.activeBackend()`) and via `@EagerInitialization` top-level vals on iOS (`kotlin.native.EagerInitialization`, deprecated-but-functional) and JS/wasm. The firebase-resident states and deprecated containers also self-register lazily. Registry is KEYED by `AuthProviderBackend.backendId` (firebase/supabase/custom): first registered = default (dual Firebase+Supabase -> Firebase default), `defaultBackendProvider(id)`/`setDefaultBackendProvider(id)` switches, `requireBackendProvider(id)` fetches, `ProvideKMPAuthBackend(id) {}` scopes a subtree; `registerDefault` never clobbers an explicit same-id instance. `supabase(...)` NO LONGER supersedes the Firebase default. `FirebaseAuthBackend` is an `expect object`; its wasm actual delegates to the REST engine (email/anonymous/id-token/email-link/reauth work; browser web flows and phone fail with a reason, config via `firebase(apiKey=...)`).

Credentials: `AuthCredential.IdToken` (Google; Apple with `rawNonce`; Facebook — `rawNonce` set means Limited-Login OIDC, otherwise classic access token) and `AuthCredential.EmailPassword` are exchanged directly by the backend (`signIn`, `reauthenticate`); `AuthCredential.OAuthWebFlow` is a backend op too: every engine serves `signIn(OAuthWebFlow)` (GitLive platform flows on android/ios, DesktopWebAuthFlow on jvm, supabase-kt browser flow, wasm fails), which is what makes `rememberOAuthState`/`rememberGithubAuthState`/`rememberMicrosoftAuthState`/`rememberAppleAuthState` backend-generic. Backend interface additions must ship default implementations (unsupported failure) so custom backends stay source-compatible.
