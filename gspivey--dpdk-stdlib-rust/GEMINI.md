...
Before considering any work done, you must validate it. Follow this order:

1. **Local checks** (fast, catches most issues):
   ```bash
   cargo build && cargo test
   ```

2. **Integration tests** (when changes touch networking, backends, deployment, or CI scripts):
   ```bash
   ./scripts/ci-validate.sh
   ```
   This script runs `cargo build` + `cargo test` locally, pushes the branch, triggers
   the `integration-tests.yml` GitHub Actions workflow, and polls until it finishes.
   It exits 0 only if everything passes.

   Options:
   - `--skip-local` — skip the local cargo build/test (only trigger CI)
   - `--skip-integration` — skip integration tests (only run local checks)

3. **If integration tests fail**, `ci-validate.sh` automatically downloads and prints
   the `failure-summary.json` and relevant log excerpts. Read the actual output to
   diagnose — do not guess. Fix the code, then re-run.

4. **Only after all checks pass** should you consider the work complete.

For changes that don't touch networking code (docs, CI config, scripts only), you can
skip integration tests with `./scripts/ci-validate.sh --skip-integration`.
...
| File | Purpose |
|------|---------|
| `dpdk-udp/src/lib.rs` | UdpSocket, packet build/parse, socket backend dispatch |
| `dpdk-udp/src/arp.rs` | ARP handler, cache, packet parsing |
| `dpdk-udp/src/icmp.rs` | ICMP handler, echo reply |
| `dpdk-udp/src/backend.rs` | PacketBackend trait, BackendConfig, BackendType |
| `dpdk-udp/src/backend_dpdk.rs` | DPDK backend implementation |
| `dpdk-udp/src/backend_raw.rs` | AF_PACKET raw socket backend |
| `dpdk-udp/src/ring_buffer.rs` | PACKET_MMAP ring buffer structures |
| `dpdk/src/port.rs` | Port config, MAC address, offloads |
| `dpdk/src/mbuf.rs` | Mbuf and Mempool wrappers |
| `dpdk-tokio/src/lib.rs` | Async UDP trait, macros, bind helpers |
| `dpdk-tokio/src/compat/` | Drop-in std/tokio socket replacements |
