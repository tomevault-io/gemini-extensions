...
- `lib/store.js` — the single source of truth for data. Every tool and pipeline stage reads and
  writes the community's data through here: id resolution (`@lid` ↔ phone ↔ name), the grow-only
  name ledger, messages/reactions, the roster, scores, the protect layer, and dossiers. One
  correct copy = consistent scores, no double-counted reactions, no lost names.
- `data/` — messages, reactions, identities, scores. Local, gitignored, never pushed.
- `data/dossiers.json` — what you've learned about people (per-person notes with provenance,
  tags, history, protect flag). Distinct from `memory/` (that's about *you*); this is about *them*.
- `memory/` — what you've learned about the work. Local, yours.
