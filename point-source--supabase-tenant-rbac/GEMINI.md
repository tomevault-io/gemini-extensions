...
| Table | Purpose |
|-------|---------|
| `permissions` | Canonical permission registry. `name text PK`, `description`. All permission strings used anywhere must be registered here first. Managed by service_role only. |
| `roles` | Global role definitions. `name text PK`, `description`, `permissions text[]`, `grantable_roles text[]`. Pre-seeded with `'owner'` (grantable_roles = ['*']). |
...
| `user_claims` | Claims cache. `user_id uuid PK`, `claims jsonb`. Format: `{"<gid>": {"roles": [...], "permissions": [...], "grantable_roles": [...], "grantable_permissions": [...]}}`. Auto-managed by triggers. |
...
- `get_claims()` → jsonb
- `_version()` → text (auto-generated by `tools/install.sh`, not in source SQL)
...
- `create_role(p_name text, p_description text, p_permissions text[], p_grantable_roles text[])` [service_role]
- `delete_role(p_name text)` [service_role]
- `list_roles()` → table (includes `grantable_roles` column) [service_role]
...
- `set_role_grantable_roles(p_role_name text, p_grantable_roles text[])` [service_role]
- `create_permission(p_name text, p_description text)` [service_role]
- `delete_permission(p_name text)` [service_role]
- `list_permissions()` → table [service_role]
...
- **Permission validation**: All permissions used in role definitions (`roles.permissions[]`) and member_permissions must exist in `rbac.permissions`. Register them via `create_permission()` before creating roles or granting overrides.
...
- **No pg_tle dependency**: The extension installs as plain SQL. The `_version()` function tracks the installed version. `pg_dump`, `supabase db diff`, and `supabase db pull` all work normally since objects are not extension members.
