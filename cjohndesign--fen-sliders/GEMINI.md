...
3. **Missing intro paragraphs**: Every slide section needs a title paragraph (v-click 0)
4. **Table descriptions**: Don't forget to describe tables that appear on click 1
5. **Deck ID mismatches**: Update ALL references when duplicating decks
6. **Port conflicts**: Always check port 3030 availability before video export
7. **Using "comprehensive"**: Script sanitization catches this, but avoid it entirely
...
```
decks/<DECK_ID>/
├── slides.md              # Slidev presentation (CRITICAL: must match audio script)
├── pdf.md                 # PDF content summary
├── audio/
│   ├── audio_script.md    # Narration script (CRITICAL: must match slides.md clicks)
│   └── config.json        # Audio timing configuration
└── img/
...
- `node scripts/deckSyncCounter.js <DECK_ID>` - Check synchronization
- `node scripts/export-steps/03-export-pdf.js <DECK_ID>` - Export PDF
- `node scripts/export-steps/04-generate-audio.js <DECK_ID>` - Generate audio
- `node scripts/export-steps/05-export-video.js <DECK_ID>` - Export video
- `npm run dev:<DECK_ID>` - Start development server
- `npm run export:<DECK_ID>` - Export to PDF

### Documentation References

- Complete export workflow: `docs/complete-export-workflow.md`
- Quick reference: `docs/complete-export-quick-reference.md`
- Script/slide sync guide: `docs/script-slide-synchronization.md`
- Video export details: `docs/video-export-quickstart.md`
