...
- **Switchers**: `<x-theme-switcher>` cycles `pinion` ↔ `pinion-dark` by default; `<x-theme-tune-switcher>` shows the whole grouped lineup with a light/dark mode toggle; `<x-settings-switcher>` consolidates theme × tune × lang into one panel for tight (mobile) chrome; `<x-lang-switcher>` is the matching standalone locale picker. The theme/tune ones carry a faint "pinion-ui" attribution link pinned at the dropdown's top-right (default → the GitHub repo; `link="site"` → pinion-ui.dev; `:attribution="false"` removes it). **Switcher chrome is tune-neutral** (v0.8.2): the picker renders identically under every tune — only the per-option previews react. The mechanism is the `.tune-exempt` class (`tune.css`), which you can put on your own site chrome too (navbar, admin bars) to pin the base font and lift effect-level tune styling like `draft`'s rough filter.
