...
`.editorconfig`, `.gitignore`, `.gitattributes`, `.github/*`, `build.ps1`, `build.sh` and
`LICENSE` originally came from the `AngleSharp.GitBase` repository. That repository is archived
(last pushed June 2021), so there is no sync to preserve — edit these files here like any other.

CI (`.github/workflows/ci.yml`) builds on Linux and Windows; on Windows it selects the Fallout
target from the branch (`main` → `Publish`, `devel` → `PrePublish`, otherwise the default). It
invokes the bootstrap scripts only, so it needs no changes when the build engine is swapped.
