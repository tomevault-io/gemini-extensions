...
- Do not resolve conflicts by automatically keeping CrossDiTo's current implementation or by discarding the upstream change wholesale. Preserve or adapt the upstream intent unless it is already fully implemented, would introduce a regression, or would substantially and unjustifiably change CrossDiTo's UX or behavior. When rejecting an upstream change, state the concrete reason.
