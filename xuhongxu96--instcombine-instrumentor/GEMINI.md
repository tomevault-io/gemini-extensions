...
### Older-LLVM compatibility

`patch_llvm` first calls `detect_llvm_major_version` (reads `LLVM_VERSION_MAJOR` from `llvm/CMakeLists.txt` or `cmake/Modules/LLVMVersion.cmake`) and applies version-gated source fixups so the same patcher + driver build cleanly from old LLVM releases through tip:

- **`patch_signals_header_for_older_llvm`** (LLVM ≤ 13) — prepends `#include <cstdint>` to `llvm/include/llvm/Support/Signals.h`; older headers use `uint8_t`/etc. without including the header, which newer libstdc++ rejects.
- **`patch_path_inc_for_older_llvm`** (LLVM ≤ 8) — teaches `llvm/lib/Support/Unix/Path.inc` about Emscripten: extends the `#if defined(__NetBSD__)…` guard before `STATVFS_F_FLAG` to also cover `__EMSCRIPTEN__`, and inserts an `#elif defined(__EMSCRIPTEN__) return true;` branch into `is_local_impl` (the generic POSIX branch references `statfs::f_flags`/`MNT_LOCAL`, which emcc lacks). Upstream added native Emscripten support only in the LLVM 9+ cycle.
- The runtime and wasm driver pick `raw_fd_ostream` OpenFlags spellings by `LLVM_VERSION_MAJOR` (see runtime section).

`clone_llvm.sh` also runs `git reset --hard` before the detached checkout so re-cloning over a previously patched tree starts clean.
...
- For cross-version portability the cpp includes `llvm/Config/llvm-config.h` and selects the `raw_fd_ostream` OpenFlags by major version: `F_None`/`F_Append` for LLVM < 7, `OF_None`/`OF_Append` for ≥ 7 (the `F_*` aliases were renamed in LLVM 7 and removed in LLVM 13). The wasm driver does the same dance for `F_Text`/`OF_Text`.
...
JSONL (not a single JSON document) matches the existing append-on-each-iteration write model: a process that dies mid-iteration leaves an unfinished trailing line, but every prior iteration remains parseable. The webapp's `web/src/trace/parse.ts` defensively skips malformed lines, and recomputes each new value's `rule` via `web/src/trace/rule.ts` (`computeRule`) rather than trusting the JSON `rule` field. The runtime's `compute_rule` walks only the captured call frames (`#1, #2, …`) and ignores frame `#0` — the function that actually produced the value — so a value created directly inside a `visit*`/`fold*` body whose stack has no further InstCombine call site gets mis- or un-attributed; `computeRule` treats frame `#0` (`func_name` at `loc`) as the innermost candidate, then the frames, and returns the first whose file path contains `InstCombine`. (The text view and the raw JSONL `rule` field still carry the runtime value.) Pointers are emitted as `"0x<hex>"` strings so they double as DOM ids / anchor targets in the structured view.

## WebAssembly build (webapp at `web/`)

The repo also produces an in-browser InstCombine debugger: `web/` is a Vite + React + Monaco SPA that loads a wasm InstCombine driver, runs it on user-pasted IR, and shows the optimized IR plus the resulting `llvm_fuzz_info.txt`. The layout is three resizable panes (`react-resizable-panels`): input IR top-left, post-pass `output.ll` bottom-left, trace on the right. A toolbar across the top carries the version picker, an artifact-branch input, **Run**, **Share** (copies a permalink — see "Share URLs" below), a **manual** link (to the hosted `USER_MANUAL`), a status line, and a system/light/dark theme picker; each pane header has a `CopyButton` (`web/src/components/OutputIrPane.tsx`). The trace pane has a Text / Structured toggle — "Structured" parses `llvm_fuzz_info.json` and renders collapsible iterations with opcode/rule/function pills, replacement rows with clickable pointer cross-links, and a filter bar (text / opcode / rule / function). Build pipeline:
...
### Share URLs and analytics

The toolbar's **Share** button (`App.tsx` `onShare`) builds a permalink that encodes the current state in query params, copied to the clipboard:
- `branch` — the artifact branch (omitted when it's the default `wasm-pkgs`).
- `tag` — the selected wasm version.
- `irz` — the IR, IR-aware-compressed (preferred). `web/src/share/irCompress.ts` runs a static-dictionary token substitution (`web/src/share/irDictionary.ts`: common LLVM IR fragments → single bytes `0x80..0xFE`, `0xFF` reserved as a future escape) then browser-native `deflate-raw` via `CompressionStream`, prefixes a 1-byte version (`0x01` dict / `0x02` UTF-8 passthrough for non-ASCII inputs), and base64url-encodes. Decode is the inverse. The wire format is versioned, so any reorder/removal of dictionary entries MUST bump the version byte (appending is backward-safe).
- `ir` — legacy plain base64url of the IR, used as the fallback when `CompressionStream` is unavailable, and still decoded on load for old links.

On load, `readInitialIr` prefers `irz` (async-decoded after mount) over `ir`. When either IR param is present (the marker of a shared link), `readInitialIr` sets `fromShare`, which seeds `shouldAutoRunRef`; an effect then fires `onRun()` exactly once as soon as the shared IR has decoded *and* the selected wasm has reached the `ready` state (the two conditions can resolve in either order — `onRun`'s `[ir, state]` deps recreate it when the decoded IR lands, so the auto-run uses the shared IR). Opening the app without an IR param does not auto-run. The webapp is instrumented with a privacy-light GoatCounter snippet in `web/index.html`.

`web/` carries a Vitest suite (`npm test`) — `irCompress.test.ts` (compression round-trips + edge cases), `compressBench.test.ts` (skipped by default), and `rule.test.ts` (the `computeRule` frame-#0 logic above).

A user-facing manual lives at `docs/USER_MANUAL.md` (Part I: using the webapp; Part II: operating the CI), with screenshots under `docs/images/` regenerated by `docs/scripts/capture-screenshots.mjs` (Playwright against the live site). The hosted copy is linked from the toolbar.
...
- `.github/workflows/native-release-auto.yml` — Monday cron plus `workflow_dispatch`. Delegates tag selection to `.github/scripts/native-release-auto/list_missing_tags.sh`, which scans `git ls-remote --tags https://github.com/llvm/llvm-project.git` for stable `llvmorg-X.Y.Z` tags not yet present as `release/llvmorg-X.Y.Z` **and newer than the newest already-released stable** (a cutoff mirroring `wasm-publish.yml` weekly-stable, so the auto scanner never backfills versions we deliberately skipped — that's `native-release-manual`'s job), then takes the newest `max_tags` (default 1). For each such tag `tags.sh` pushes a `release/<llvm-tag>` tag on a detached commit that bumps `llvm_commit.txt`, pre-creates the GitHub Release, then explicitly dispatches `native-build.yml` against that tag's ref via `gh workflow run` (the dispatch sidesteps GitHub's loop-prevention rule that suppresses workflow runs triggered by tag pushes from `GITHUB_TOKEN`). Wasm for the same tag is published *independently* by `wasm-publish.yml`'s weekly cron — they aren't coupled.
...
**Helper scripts are grouped by owning workflow under `.github/scripts/`:** `shared/` (`ccache_keys.sh`, `install_toolchain.sh`, `resolve_llvm_commit.sh`, `retry.sh`) is used by multiple workflows. Everything else lives under the subdirectory named after its sole consumer — `native-release-auto/`, `native-release-manual/`, `wasm-publish/`, `wasm-pages/`. Inside each subdir filenames drop the redundant workflow prefix (so the wasm-publish dispatch step lives at `wasm-publish/dispatch.sh`, not `wasm-publish/wasm_publish_dispatch.sh`). `wasm-publish/resolve_custom_source.mjs` is the helper for publishing from a GitHub fork/source URL; it accepts `https://github.com/<owner>/<repo>/tree/<branch-or-sha>`, `https://github.com/<owner>/<repo>/commit/<branch-or-sha>`, and `https://github.com/<owner>/<repo>/commits/<branch-or-sha>`, and classifies any of those forms as branch-backed unless the ref is a 7-40 hex SHA.
