...
- **SVG** — the in-library `Svg/` renderer (`SvgImage`, `SvgPathData`, `SvgPaint`) parses SVG and draws onto `Microsoft.Maui.Graphics`; `MarkdownView` calls `SvgImage.RenderToPng` to rasterize remote `.svg` URLs to a PNG `ImageSource`. Rasterization uses the platform `PlatformBitmapExportService`, so that step is behind `#if ANDROID || IOS || MACCATALYST || WINDOWS` (the plain `net10.0` target returns null — it isn't used at runtime). The parser covers paths (full command set incl. arcs), basic shapes, transforms, hex/rgb/named fills+strokes, and linear/radial **gradient fills** (`SvgGradient` → `LinearGradientPaint`/`RadialGradientPaint` via `ICanvas.SetFillPaint`, with `gradientUnits`/`gradientTransform`/multi-stop/`href` inheritance); **not** filters, text, or clip/mask. Gradient *strokes* approximate to a solid stop colour (MAUI has no gradient stroke), and radial `fx`/`fy`/`spreadMethod` are ignored. Local `.svg` app resources still go through MAUI's native `MauiImage` pipeline, not this renderer.
- `BuilderExtension.UseMarkdownView()` no longer calls `UseSkiaSharp()`. It now registers the native `ZoomImageView` handler (under `Handlers/ZoomImageViewHandler.*.cs`) that powers `MarkdownView.AllowImagePopup` — a full-screen, pinch/double-tap zoomable image overlay opened when a rendered image is tapped. Consumers must call it in `MauiProgram` for the image popup to work; if they don't use `AllowImagePopup` (default `false`), nothing breaks when it's omitted. The overlay is a fullscreen MAUI modal page (`Controls/ImagePopup.cs`) hosting the handler-driven `ZoomImageView` (`UIScrollView` on iOS/MacCatalyst, matrix `ImageView` in a `FrameLayout` on Android, `ScrollViewer` with `ZoomMode` on Windows). Native delegate/gesture work uses weak-reference proxies to avoid the iOS retain-cycle leaks noted in the conventions.
