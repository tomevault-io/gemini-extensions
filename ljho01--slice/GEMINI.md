...
- `sounds/` 디렉토리는 테스트용 WAV 파일이므로 커밋하지 않는다
- Splice DB는 읽기 전용(`SQLITE_OPEN_READ_ONLY`)으로 접근 — 원본 변경 금지
- `~/Slice/slice.db`가 앱의 자체 데이터베이스
- `waveform_peaks`는 samples 테이블에 JSON 문자열로 캐시됨
- 모든 응답과 UI 텍스트는 한국어로 작성
