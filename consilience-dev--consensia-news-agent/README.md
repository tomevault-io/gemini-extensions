# consensia-news-agent

> Source: [consilience-dev/consensia-news-agent](https://github.com/consilience-dev/consensia-news-agent) — distributed by [TomeVault](https://tomevault.io)

A Python agent for searching news articles by topic and date using NewsAPI and Google ADK. Includes robust error handling and LLM-friendly interfaces.

## Gemini CLI Config

The `GEMINI.md` file in this directory is the project config converted for Gemini CLI.
Original source: `.windsurf/rules/*.md` in [consilience-dev/consensia-news-agent](https://github.com/consilience-dev/consensia-news-agent).

## Also available for

- **Claude Code** — `CLAUDE.md`
- **Codex** — `AGENTS.md`
- **GitHub Copilot** — `copilot-instructions.md`
- **Cursor** — `project-config.mdc`
- **Gemini CLI** — `GEMINI.md`

Available for 6 platforms including Claude Code, Cursor, Windsurf, and more. Source: [github.com/consilience-dev/consensia-news-agent](https://github.com/consilience-dev/consensia-news-agent)

---

Explore more instruction files on [TomeVault](https://tomevault.io) — the open index of AI config files for every platform.

<!-- genome:d-e-p -->
