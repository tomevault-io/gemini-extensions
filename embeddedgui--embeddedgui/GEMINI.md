...
核心要点：
- 资源放在 `example/{APP}/resource/src/`
- Designer 生成写入 `app_resource_config_designer.json`，用户自定义覆盖写在 `app_resource_config.json`，资源构建会自动 merge 两者
- 公用字体库位于 `scripts/tools/build_in/`，在资源配置里直接写 `build_in/字体名.ttf` 引用；只有 build-in 不满足时才把字体放到 `resource/src/`
...
关键约束：
- **WASM 构建强制禁用录制模式**（`EGUI_CONFIG_FUNCTION_RECORDING_TEST=0`），用户直接与 canvas 交互
...
## Skills 与 Workflow 说明

项目技能文件位于 `.claude/skills/`，项目级工作流文件位于 `.claude/workflow/`。**在当前 VS Code + Copilot 环境下不会自动触发**，需要 AI 在任务开始时根据目标主动判断、读取并执行对应文件。

使用规则：
- 遇到下表场景时，先读取对应 skill，再按其中的固定流程、门禁、命令和验收要求执行。
- 如果一个 skill 要求切换到其他 skill（例如 APP 生成后进入性能或体积优化），必须继续读取被点名的 skill，不能凭记忆执行。
- 如果任务属于 HTML / Figma / 设计稿转换链路，优先切换到 `EmbeddedGUI_Designer` 并阅读那里的迁移说明；当前仓库只保留迁移入口说明。
- 如果任务涉及 HelloBasic / HelloVirtual 控件渲染、录制动作或回归检查，除相关 skill 外，还要读取 `.claude/workflow/hello_basic_render_workflow.md`。

| 技能文件 | 触发时机 |
|----------|----------|
| `ai-generate-egui-app.md` | 用户提供效果图/参考图和切图资源，要求新建 EmbeddedGUI APP，或继续完善半成品 APP、做 1:1 视觉还原、像素级截图对比、后端数据接口和 PC 预览 |
| `resource-generation.md` | 添加图片/字体/图标；资源构建报 undefined reference；需要配置 `app_resource_config.json` 或刷新资源生成文件 |
| `runtime-verification.md` | 修改代码后验证；截图检查渲染；诊断黑屏、控件缺失、布局错位、运行时卡死或崩溃 |
| `build-and-debug.md` | 构建失败排查、链接错误、平台移植问题、PC 模拟器调试、全量构建收尾验收 |
| `recording-simulation.md` | 配置点击、拖动、滑动等录制动作用于 CI 运行时验证或 GIF/截图回归 |
| `performance-optimization.md` | 针对具体 APP / APP_SUB / 页面 / 交互做性能优化，分析卡顿、整屏刷新、dirty 区域、PFB/SPI、文本或图片绘制耗时 |
| `performance-analysis.md` | 框架级性能测试、帧率分析、QEMU 基准、PFB 矩阵、SPI 矩阵等量化分析 |
| `code-size-optimization.md` | 针对具体 APP / APP_SUB 做 ROM、Resource、RAM、PFB、heap、stack、feature 宏、资源格式或控件组合的体积分析和优化 |
| `github-pages-web.md` | 添加或修改 GitHub Pages 在线 Demo 页面、demo 数据、WASM 展示和 web 布局 |
| `html-to-egui.md` | 已迁移到 `EmbeddedGUI_Designer`，用于 HTML / JSX / TSX 设计转换入口说明 |
| `figmamake-to-egui.md` | 已迁移到 `EmbeddedGUI_Designer`，用于 Figma Make 全流程转换入口说明 |
| `figma-mcp-to-egui.md` | 已迁移到 `EmbeddedGUI_Designer`，用于 Figma MCP / REST 导入入口说明 |
| `dynamic-effects.md` | 已迁移到 `EmbeddedGUI_Designer`，用于设计稿驱动的动效补齐入口说明 |

| Workflow 文件 | 触发时机 |
|---------------|----------|
| `.claude/workflow/hello_basic_render_workflow.md` | 修改 HelloBasic / HelloVirtual 控件、录制动作、渲染逻辑或运行时回归检查时使用；需要按 workflow 执行 suite、审计录制动作并检查输出报告 |
