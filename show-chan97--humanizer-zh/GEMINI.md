...
**问题：** LLM 使用破折号（—）比人类更频繁，模仿"有力"的销售文案。在实践中，大多数可以更干净地改写为逗号、句号或括号。
...
**需要注意的词汇：** 真正的问题是、从本质上讲、事实上、真正重要的、根本上来讲、更深层的问题、问题的核心

**问题：** LLM 使用这些短语假装在切中要害地揭示某些更深层的真相，而后面的句子通常只是用额外的仪式感重述一个普通观点。

**改写前：**
> 真正的问题在于团队能否适应。从本质上讲，真正重要的是组织准备就绪。
...
**需要注意的词汇：** 让我们深入了解、让我们探索、让我们来拆解、您需要了解的如下、现在让我们看看、不多废话

**问题：** LLM 宣布它们将要做什么，而不是直接做。这种元评论使写作变慢，并给它一种教程脚本的感觉。

**改写前：**
> 让我们深入了解 Next.js 中缓存的工作原理。您需要了解的如下。
...
**问题：** 标题后跟着一个段落，仅仅简单重复标题内容，然后才开始真正的内容。这种引导句通常毫无意义，使文章感觉被 padding 了。
...
>
> AI 辅助编程作为大型语言模型变革潜力的持久证明，标志着软件开发演进史上的关键时刻。在当今快速变化的技术格局中，这些处于研究与实践交汇处的突破性工具——正在重塑工程师构思、迭代和交付的方式——凸显了它们在现代工作流程中的重要作用。
>
> 从本质上讲，价值主张是明确的：简化流程、增强协作、培养一致性。这不仅仅是关于自动完成；这是关于释放大规模创造力，确保组织能够保持敏捷，同时为用户提供无缝、直观和强大的体验。该工具充当催化剂。助手扮演合作伙伴的角色。系统是创新的基础。
>
> 行业观察者注意到，采用已从爱好者实验加速到企业范围推广，从独立开发者到跨职能团队。该技术已被《纽约时报》、Wired 和 The Verge 报道。此外，生成文档、测试和重构的能力展示了 AI 如何为更好的结果做出贡献，凸显了自动化与人类判断之间复杂的相互作用。
>
> - 💡 **速度：** 代码生成显著更快，减少摩擦，赋能开发者。
> - 🚀 **质量：** 通过改进训练提高了输出质量，为更高标准做出贡献。
> - ✅ **采用：** 使用持续增长，反映更广泛的行业趋势。
>
> 虽然基于可用信息，具体细节有限，但可以认为这些工具可能有一些积极影响。尽管面临新兴技术典型的挑战——包括幻觉、偏见和责任问题——生态系统仍在蓬勃发展。为了充分实现这一潜力，团队必须与最佳实践保持一致。
>
> 总之，未来是光明的。激动人心的时代即将来临，因为我们继续追求卓越的旅程。如果您想让我扩展任何部分，请告诉我！

**改写草稿：**
> AI 编程助手可以加快一些任务。在 Google 2024 年的一项研究中，使用 Codex 的开发者在完成简单函数时比对照组快 55%，但在调试或架构决策上没有改善。
>
> 这些工具擅长样板文件：配置文件、测试脚手架、重复的重构。它们也擅长在出错时听起来是对的。我接受过一些建议，编译通过了，lint 也过了，但还是没达到目的——因为我停止注意了。
>
> 我采访过的一位金融科技初创公司的工程师 Mira 说，她用 Copilot"就像用自动补全做无聊的代码"，但每次提交前都会审查。Jake 在一家较大公司担任高级开发人员，在它不断建议已弃用内部库的代码模式后禁用了它。
>
> 生产率声明很难验证。GitHub 说 Copilot 用户"接受 30% 的建议"，但接受不等于正确，正确不等于有价值。2024 年 Uplevel 研究发现，有 AI 助手和没有 AI 助手的团队在拉取请求吞吐量上没有统计学显著差异。
>
> 这并不意味着这些工具没用。这意味着它们只是工具。它们不替代判断，也不消除测试的需要。如果你没有测试，你就无法判断建议是否正确。

**这段文字哪里明显是 AI 写的？**
- 节奏仍然有点过于整齐（干净的对比、均匀的段落）
- 提到的人名和研究引用可能读起来像捏造的占位符
- 结尾有点口号式（"如果你没有测试……"）而不是像一个人在说话

**现在让它不再像 AI：**
> AI 编程助手可以让你在无聊的部分更快。不是一切。绝对不是架构。
>
> 它们擅长样板：配置文件、测试脚手架、重复的重构。它们也擅长在出错时听起来对。我接受过建议，编译通过，lint 通过，但还是没理解我的意思——因为我停止注意了。
>
> 我交谈过的人通常属于两类。有些人用它像做家务的自动补全，每行都审查。有些人禁用它，因为它不断建议他们不想要的模式。两种做法都合理。
>
> 生产率指标很模糊。GitHub 可以说 Copilot 用户"接受 30% 的建议"，但接受不等于正确，正确不等于有价值。如果你没有测试，你基本上就是在猜。

**所做更改：**
- 删除了 chatbot 痕迹（"好问题！""希望这对您有帮助！""如果您想让我扩展任何部分……"）
- 删除了意义膨胀（"持久证明""关键时刻""不断变化的格局""重要作用"）
- 删除了宣传性语言（"突破性的""坐落""无缝、直观和强大"）
- 删除了模糊归因（"行业观察者"）
- 删除了肤浅 -ing 短语（"凸显""强调""反映""为……做出贡献"）
- 删除了否定式排比（"这不仅仅是 X；这是 Y"）
- 删除了三段式法则和同义词循环（"催化剂/合作伙伴/基础"）
- 删除了虚假范围（"从 X 到 Y，从 A 到 B"）
- 删除了破折号、表情符号、粗体标题和弯引号
- 删除了系动词回避（"充当""扮演……角色""是……的基础"），改用"是/有"
- 删除了公式化挑战部分（"尽管面临挑战……继续蓬勃发展"）
- 删除了知识截止日期对冲（"虽然具体细节有限……"）
- 删除了过度限定（"可以认为……可能有一些"）
- 删除了填充短语和说服性措辞（"为了""从本质上讲"）
- 删除了通用积极结论（"未来是光明的""激动人心的时代即将来临"）
- 让声音更个人化、更少"组装"感（变化的节奏，更少的占位符）
...
- **标题大小写问题**：中文标题通常不涉及英文 Title Case 问题，此模式在中文中不太适用
- **弯引号表现**：中文通常使用中文引号（「」或""），此模式在中文中表现为英文引号的使用
- **连字符词组**：中文中的复合词使用方式与英文连字符不同，但过度使用的判断标准相似
- 添加了适合中文语境的示例
- 调整了部分表达以符合中文习惯
...
本技能基于 [Wikipedia:Signs of AI writing](https://en.wikipedia.org/wiki/Wikipedia:Signs_of_AI_writing)（v2.5.1），由 WikiProject AI Cleanup 维护。那里记录的模式来自对维基百科上数千个 AI 生成文本实例的观察。
