...
> General knowledge about the repo
...
When adding a new database table, don't create one-off tables in TypeScript. Each table should have:

1. Its own SQL file in `backend/supabase/` (e.g. `backend/supabase/my_table.sql`)
2. Its own initialization script in `backend/scripts/` named `init-<table-name>.ts` (e.g. `backend/scripts/init-my-table.ts`)

This ensures that database schema is properly versioned and can be reproduced consistently.

**Row Level Security (RLS):** Be cautious about adding "public read" policies to tables. Only add public read access if the data genuinely needs to be accessible from the client. Tables that are only accessed by the backend (via service role) should have RLS enabled but no public policies. This follows the principle of least privilege.
...
**Sweepstakes / CASH markets are deprecated.** Do not implement support for sweepstakes or CASH markets in any new features. Legacy code may still reference these, but they should be ignored for all new development.
