...
### Provider Behavioral Differences (GitHub vs GitLab)

**CRITICAL**: GitHub and GitLab have fundamentally different behaviors for label management. Understanding these differences is essential to avoid breaking functionality.

#### Label Addition Behavior
- **GitHub (`gh pr edit --add-label`)**:
  - FAILS if the label does not exist
  - Returns error code, triggering `provider_create_pr_label()`
  - Simple implementation: just call `gh pr edit --add-label` and handle errors

- **GitLab (`glab mr update --label`)**:
  - AUTO-CREATES the label if it doesn't exist
  - **BUT**: Creates it WITHOUT color or description
  - **CONSEQUENCE**: Labels lose their visual identity and metadata
  - **SOLUTION**: Must check label existence BEFORE adding:
    ```bash
    # Check if label exists first
    if glab label list | grep -F "${label_name}"; then
        # Label exists, safe to add
        glab mr update --label "${label_name}"
    else
        # Label doesn't exist, force creation with colors
        return 1  # Triggers provider_create_pr_label()
    fi
    ```

#### Label Ownership Protection (GH-75)
Both providers now check if labels are managed by Badgetizr before modifying them:
- Labels created by Badgetizr have description: `"Generated by Badgetizr - do not edit"`
- Existing team labels are respected and used as-is with a warning
- `provider_is_label_managed()` checks label description to determine ownership

#### Implementation Differences
- **GitHub `provider_add_pr_label()`**: Simple, relies on API error handling
- **GitLab `provider_add_pr_label()`**: Complex, requires explicit existence check to prevent auto-creation without metadata

**IMPORTANT**: When modifying provider code, ALWAYS verify behavior on BOTH platforms. A simplification that works on GitHub may break GitLab label colors/descriptions.
...
- **GitLab label auto-creation**: Never simplify GitLab's `provider_add_pr_label()` to skip existence check - it will break label colors
...
1. Run: `shellcheck -x configure *.sh providers/*.sh badgetizr`
