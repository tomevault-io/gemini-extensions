...
If `PROACTIVE` is `false`: do NOT proactively suggest other mzstack skills during this session.
Only run skills the user explicitly invokes. This preference persists across sessions via
`mzstack-config`.

# mzstack browse: QA Testing & Dogfooding
...
B=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/.claude/skills/mzstack/browse/dist/browse" ] && B="$_ROOT/.claude/skills/mzstack/browse/dist/browse"
[ -z "$B" ] && B=~/.claude/skills/mzstack/browse/dist/browse
if [ -x "$B" ]; then
  echo "READY: $B"
else
  echo "NEEDS_SETUP"
...
If `NEEDS_SETUP`:
1. Tell the user: "mzstack browse needs a one-time build (~10 seconds). OK to proceed?" Then STOP and wait.
