...
- Frontend gate: `npm run typecheck` + `npm test` (Vitest). CI (`.github/workflows/ci.yml`) runs these same gates on every PR/push to `main` — a PR cannot merge until both the frontend and backend CI jobs pass.
...
- **Open Local Paper flow** — `+ Open Local Paper` (sidebar) opens `OpenLocalPaperDialog` with two tabs sharing the `paper_uploads` backend: Upload (pdf.js parses embedded metadata — DOI from `info.Custom`, abstract via a 3-page sweep; see spec §5.4 — → `[Edit]`/`[Try LLM enrichment]`/`[Looks good]`) and Zotero (library search → `importFromZotero` → `fetch_attachment_bytes` reads the item's PDF attachment, or 400 "no attachment" → fall back to manual upload). A `PaperCard` whose paper has no arxiv_id AND no oa_pdf_url renders an **unfetchable** card with 3 buttons (`Upload Local PDF` / `Import from Zotero` / `Open source page`) — UI-determined, not model-emitted; the buttons pre-seed the dialog with the card's metadata so the upload attaches bytes to the EXISTING global `Paper` row. The general-chat system prompt tells the model to proactively explain when it can't open a PDF. See `docs/superpowers/specs/2026-07-01-local-paper-upload-design.md`.
- **Zotero PDF import is local-first** (`zotero.py::fetch_attachment_bytes`) — importing a Zotero PDF reads it straight off the **local disk** via the local Zotero API's `file://` redirect (`GET /api/users/0/items/<key>/file` → 302 → `file:///<storage>/<key>/<file>.pdf`), falling back to the (retried, 30s-capped) S3 cloud download only when local is unavailable. Local-first dodges both the flaky S3 throttle (the old "stuck on Importing… forever" ~5min hang) AND the cloud-storage-quota gap (a file not synced to zotero.org is still on local disk). `_LocalUnavailable` is a silent sentinel — never surfaced; the user only sees an error if BOTH paths fail. Docker needs `LAX_ZOTERO_LOCAL_BASE=http://host.docker.internal:23119` (the container can't reach the host loopback; Zotero's anti-CSRF rejects non-localhost `Host` headers, so the code always sends `Host: 127.0.0.1:23119`) + `LAX_ZOTERO_STORAGE_DIR` + a read-only `/zotero-storage` mount (the `file://` host path is rewritten to the mount). Native (`run.bat`) needs **no** config. See `docs/superpowers/specs/2026-07-02-zotero-local-first-import-design.md`.
...
- `docs/superpowers/specs/2026-07-02-zotero-local-first-import-design.md` — local-first Zotero PDF import (read off local disk via the `file://` redirect, cloud fallback). Read before touching `fetch_attachment_bytes` / `download_local_attachment_bytes` / the `LAX_ZOTERO_LOCAL_BASE` + `LAX_ZOTERO_STORAGE_DIR` Docker wiring.
