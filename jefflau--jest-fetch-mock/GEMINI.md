...
- **Route precedence (4.2+)**: queued once-implementations > registered routes > conditional gate + ambient response. Routes are consulted only by *persistent* implementations (`normalizeResponse(..., consultRoutes=true)`); once-implementations pass `false`, which is what makes the once-queue win. A matching route fires even under `dontMock()` (deliberate — enables selective mocking); `mockReject`/`mockAbort` replace the whole implementation and therefore override routes. `fetchMock.resetMocks()` clears routes; Jest's `resetMocks: true` does not (routes are closure state, not jest-mock state), and the re-arm hook restores a route-aware default implementation.
