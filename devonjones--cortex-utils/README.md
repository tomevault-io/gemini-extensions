# cortex-utils

> Source: [devonjones/cortex-utils](https://github.com/devonjones/cortex-utils) — distributed by [TomeVault](https://tomevault.io)

Operational tooling for Cortex: queue management, partitioning, dead letter handling

## Gemini CLI Config

The `GEMINI.md` file in this directory is the project config converted for Gemini CLI.
Original source: `CLAUDE.md` in [devonjones/cortex-utils](https://github.com/devonjones/cortex-utils).

## Also available for

- **Codex** — `AGENTS.md`
- **GitHub Copilot** — `copilot-instructions.md`
- **Cursor** — `project-config.mdc`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

From [devonjones/cortex-utils](https://github.com/devonjones/cortex-utils) — a repo with 0+ stars on GitHub.

---

Own this repo? Install the TomeVault Relay to keep every platform's copy in sync on every push: [https://tomevault.io/install](https://tomevault.io/install).

<!-- genome:d-c-s -->
