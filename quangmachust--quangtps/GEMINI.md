...
## Các loại Objective (Mục tiêu)

### 1. DVH-based Objectives

- **Minimum Dose Objective**: Đảm bảo liều tối thiểu cho một cấu trúc
- **Maximum Dose Objective**: Giới hạn liều tối đa cho một cấu trúc
- **Mean Dose Objective**: Kiểm soát liều trung bình cho một cấu trúc
- **Dose-Volume Objective**: Đảm bảo một phần trăm thể tích nhận một liều nhất định (ví dụ: V95% > 95%)
- **Volume-Dose Objective**: Đảm bảo một liều nhất định bao phủ một thể tích nhất định (ví dụ: D95% > 95%)

### 2. Conformity Objectives

- **Conformity Index Objective**: Tối ưu hóa mức độ phù hợp của phân bố liều với cấu trúc mục tiêu
- **Gradient Index Objective**: Kiểm soát gradient liều xung quanh mục tiêu
- **Homogeneity Index Objective**: Đảm bảo tính đồng nhất của liều trong mục tiêu

### 3. Biological Objectives

- **EUD Objective**: Tối ưu hóa Equivalent Uniform Dose
- **NTCP Objective**: Giảm thiểu xác suất biến chứng cho mô bình thường
- **TCP Objective**: Tối đa hóa xác suất kiểm soát khối u

## Luồng công việc tối ưu hóa

### 1. Thiết lập kế hoạch ban đầu

```
1. Nhập kế hoạch điều trị với các chùm tia ban đầu
2. Thiết lập các thông số kỹ thuật (năng lượng, góc, thời gian chiếu xạ)
3. Tạo hình dạng MLC ban đầu từ cấu trúc mục tiêu
```

### 2. Thiết lập mục tiêu và ràng buộc

```
1. Xác định các cấu trúc mục tiêu và OARs
2. Thiết lập mục tiêu liều lượng cho từng cấu trúc
3. Thiết lập các ràng buộc cho OARs dựa trên giao thức lâm sàng
4. Thiết lập độ ưu tiên cho mỗi mục tiêu và ràng buộc
```

### 3. Tối ưu hóa kế hoạch

```
1. Chọn thuật toán tối ưu hóa (Fluence, IMRT, VMAT, MCO, KBP)
2. Thiết lập tham số tối ưu hóa (số lần lặp, ngưỡng hội tụ)
3. Chạy quy trình tối ưu hóa với hiển thị tiến trình
4. Hiển thị kết quả trung gian theo thời gian thực
```

### 4. Đánh giá và tinh chỉnh

```
1. Đánh giá kế hoạch với DVH và các chỉ số đánh giá
2. Kiểm tra các ràng buộc và mức độ đạt mục tiêu
3. Tinh chỉnh tham số tối ưu hóa và trọng số nếu cần
4. Chạy lại quy trình tối ưu hóa cho đến khi đạt kết quả mong muốn
```

### 5. Hoàn thiện kế hoạch

```
1. Chuyển đổi fluence tối ưu thành các phân đoạn MLC thực tế
2. Tính toán lại liều dựa trên cấu hình MLC thực tế
3. Kiểm tra tính khả thi của kế hoạch (kiểm tra va chạm, giới hạn máy)
4. Xuất kế hoạch cuối cùng cho quá trình phê duyệt
```
...
- **V95%**: Phần trăm thể tích nhận 95% liều chỉ định
- **D95%**: Liều bao phủ 95% thể tích mục tiêu
- **D2%**: Liều nhận bởi 2% thể tích (gần như liều tối đa)
- **Dmin**: Liều tối thiểu trong mục tiêu
- **Dmax**: Liều tối đa trong mục tiêu
- **Dmean**: Liều trung bình

### 2. Chỉ số đánh giá nâng cao

- **CI (Conformity Index)**: Đánh giá mức độ phù hợp của phân bố liều
- **HI (Homogeneity Index)**: Đánh giá tính đồng nhất của liều
- **GI (Gradient Index)**: Đánh giá gradient liều giữa mục tiêu và mô lành
- **COIN (Conformal Index)**: Kết hợp đánh giá bao phủ mục tiêu và tránh OARs
...
1. Giao diện thân thiện để thiết lập mục tiêu và ràng buộc:
   - Hỗ trợ đặt mục tiêu từ giao thức lâm sàng
   - Hiển thị danh sách mục tiêu và ràng buộc dạng bảng
   - Cho phép điều chỉnh trọng số trực quan

2. Hiển thị quá trình tối ưu hóa:
   - Thanh tiến trình và thời gian ước tính
   - Biểu đồ hội tụ của hàm cost
   - Hiển thị DVH theo thời gian thực

3. Công cụ đánh giá trực quan:
   - So sánh DVH của các phiên bản kế hoạch
   - Hiển thị mức độ đạt được của từng mục tiêu
   - Bản đồ nhiệt cho phân bố liều

4. MCO Pareto Browser:
   - Hiển thị mặt Pareto cho các mục tiêu đối lập
   - Cho phép người dùng khám phá không gian giải pháp
   - Lưu và so sánh các điểm trên mặt Pareto
