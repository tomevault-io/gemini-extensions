...
- **Docs**: `docs/*.md` need leading-fence frontmatter (`title`, `order`, `audience`); only `audience: user` pages render in-app at `/docs/:slug`. `web/scripts/check-docs.mjs` (runs in `npm run build`) fails on bad frontmatter, duplicate `order`, or broken relative links. See `docs/README.md`.
  - **After editing any `docs/*.md`, run `task docs:sync` and commit the mirror.** The CLI embeds the corpus from `api/internal/uzidocs/embed/` (`go:embed` cannot reach `..`), and `TestEmbeddedDocsMatchSource` in `test:api` asserts byte-equality with `docs/*.md`, so a docs-only commit without the sync reddens `gate:api` (measured 2026-09-03, PR #1096). `docs:sync` is deliberately not a gate target: it writes the tree.
