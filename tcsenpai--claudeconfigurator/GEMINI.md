...
## Project Goals & Rules (claudeconfigurator)

Core project values — apply to every change:

1. **Lean code** — minimum code that works. No speculative abstractions, no boilerplate "for later".
2. **Ultra efficiency** — small footprint, fast build, fast runtime. Prefer stdlib / native / already-installed deps over new ones.
3. **Best practices** — production quality, correctness first, path-jailed filesystem access, validate before writing user config.
4. **Smart tricks** — favor elegant, minimal solutions over verbose ones, but never at the cost of readability.
5. **Readable** — code reads like the surrounding code; comment non-obvious logic only.

Stack (locked): Tauri 2 + Svelte 5 + CodeMirror 6, `bun` for frontend, Rust owns all filesystem access (path-jailed to `~/.claude`). Rotating `.bak` backups on write (cap N per file). Global `~/.claude` scope for v1.
