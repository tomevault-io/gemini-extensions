...
When launching from a terminal, run `open ZoomacIt.app` (LaunchServices). Direct binary execution (`./ZoomacIt.app/Contents/MacOS/ZoomacIt`) makes the terminal the "responsible process", causing Screen Recording permission prompts for the terminal instead of ZoomacIt.
...
Uses `CGPreflightScreenCaptureAccess()` / `ScreenCaptureKit`. If denied, the app should degrade gracefully (e.g., black background). Accessibility permission is **not needed** (Carbon hotkey API).
...
Tests live in `src/ZoomacItTests/`. Test files: `ZoomMathTests`, `ShapeRendererTests`, `FreehandRendererTests`, `StrokeManagerTests`, `DrawingStateTests`, `SettingsTests`, `BreakTimerStateTests`. UI/overlay classes are not unit-tested (require a running app context).
