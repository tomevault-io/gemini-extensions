...
- Result Statement Hygiene: theorem, proposition, lemma, and corollary statements should state the result, not explain the result. Keep formal statements minimal, clean, and formal; put nonessential interpretation, proof intuition, caveats, examples, and why-it-matters material in the theorem setup, proof roadmap, remark, or post-result discussion.
- Only truth-critical mathematical conditions belong in a formal result statement. Do not hide weak results by stuffing statements with clauses, qualifications, or narrative; if many conditions are needed, audit whether they are genuine assumptions, proof dependencies, or defensive statement bloat.
...
- Before a full manuscript draft, major restructuring, or full style pass, require an anchor-based manuscript architecture audit. Use `manuscript_architecture_plan.md` to compare the paper's section count, section order, appendix boundary, and section functions against target-, field-, and genre-matched published papers. Every section must have a job.
...
- In ordinary autonomous or goal-driven runs, never let the AI proxy the human for the Micro-Example Gate, Minimal Model Base Gate, main theorem gate, novelty claim, contribution lock, target decision, or invest/pivot/kill decision. The AI may recommend; the human must confirm.
- If the user explicitly enables guarded full-auto goal mode, the system may pass through gates by recording `AI-delegated provisional` decisions in `auto_decisions.md` and the controlled artifacts. These decisions are not human confirmations, must not be written as human approvals in `human_decisions.md`, and must be collected in `final_ratification_report.md` before the paper is treated as confirmed or submission-ready.
- Use an artifact budget during long automatic runs. Keep canonical source-of-truth files small and few, put exploratory scratch in `scratch_runs/YYYYMMDD_HHMM/`, and end each long run with `run_summary.md` plus an updated `active_context.md`. Do not create hundreds of top-level files as a substitute for research progress.
...
- Every real human gate decision must be written to the relevant persistent artifact before the system treats it as durable state. Create `human_decisions.md` if it is missing, use it for append-only decision history, and update the active state file such as `project_state.md`, `discovery_state.md`, `field_profile.md`, `target_journal_profile.md`, `manuscript_architecture_plan.md`, `contribution_lock.md`, `revision_tree.md`, or `risk_register.md` as appropriate. In guarded full-auto goal mode, write AI-delegated provisional choices to `auto_decisions.md` instead, then wait for later human ratification before treating them as confirmed.
