...
### Excel与下载文件一致性检查
**重要：必须确保下载的文章与Excel文件中的文章完全一致。**

#### 数量一致性
- 下载的Markdown文件数量必须等于Excel中的文章数量
- 下载的PDF文件数量（如果生成）必须等于Excel中的文章数量
- 实现验证机制：下载完成后统计文件数量并与Excel行数对比

#### 文件名一致性
- 每个下载文件的文件名必须与Excel中对应文章的标题一致
- 文件名格式：`{Excel中的文章标题}_{时间戳}.md` 或 `.pdf`
- 文件名中的标题部分必须与Excel中的标题完全匹配（去除非法字符后）
- 实现验证机制：下载完成后检查每个Excel条目是否都有对应的文件
...
#### 实现建议
- 在下载过程中维护一个已下载文件的映射表
- 下载完成后遍历Excel，检查每个URL是否都有对应的输出文件
- 比较文件名中的标题部分与Excel中的标题是否匹配
- 提供验证报告，列出缺失或不匹配的文件
