...
- **New `NASTECH_*` env vars for non-secret config.** `.env` is for secrets
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `nastech tools`
...
- **Third-party products / other people's projects integrated into the core
  ...
  **standalone plugin repo** users install into `~/.nastech/plugins/` (or via a
  pip entry point), and promote them in the Nastech Research Discord
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `nastech <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `nastech webhook`, `nastech cron`, `nastech tools`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.nastech/plugins/` or a pip package, discovered at runtime.
...
`$HOME/.nastech/nastech-agent/venv` (for worktrees that share a venv with the
main checkout).
...
├── nastech_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # Agent internals (provider adapters, memory, caching, compression, etc.)
├── nastech_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
│   ├── nastech-achievements/  # Gamified achievement tracking
...
├── ui-tui/               # Ink (React) terminal UI — `nastech --tui`
...
**User config:** `~/.nastech/config.yaml` (settings), `~/.nastech/.env` (API keys only).
**Logs:** `~/.nastech/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_nastech_home()`.
Browse with `nastech logs [--follow] [--level ...] [--session ...]`.

## TypeScript Style

Applies to TypeScript across Nastech: desktop, TUI, website, and future TS packages.
...
- **Skin engine** (`nastech_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `NastechCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `agent/skill_commands.py` scans `~/.nastech/skills/`, injects as **user message** (not system prompt) to preserve prompt caching

### Slash Command Registry (`nastech_cli/commands.py`)
...
- **Slack** — `slack_subcommand_map()` generates `/nastech` subcommand routing
- **Autocomplete** — `COMMANDS` flat dict feeds `SlashCommandCompleter`
- **CLI help** — `COMMANDS_BY_CATEGORY` dict feeds `show_help()`

### Adding a Slash Command

1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `nastech_cli/commands.py`:
```python
CommandDef("mycommand", "Description of what it does", "Session",
           aliases=("mc",), args_hint="[arg]"),
```
2. Add handler in `NastechCLI.process_command()` in `cli.py`:
```python
...
npm run dev       # watch mode (rebuilds nastech-ink + tsx --watch)
npm start         # production
npm run build     # full build (nastech-ink + tsc)
...
### TUI in the Dashboard (`nastech dashboard` → `/chat`)

The dashboard embeds the real `nastech --tui` — **not** a rewrite.  See `nastech_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `nastech_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `nastech --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `nastech --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMAND_SPECS` (the built-ins and their Desktop surfaces) plus `NO_DESKTOP_SURFACE` block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known Nastech built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
be core tools. For custom or local-only tools, do **not** edit Nastech core.
Use the plugin route instead: create `~/.nastech/plugins/<name>/plugin.yaml`
and `~/.nastech/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core Nastech tool that should ship in the base system.
...
**2. Add to `toolsets.py`** — either `_NASTECH_CORE_TOOLS` (all platforms) or a new toolset. **This step is required:** auto-discovery imports the tool and registers its schema, but the tool is only *exposed to an agent* if its name appears in a toolset. `_NASTECH_CORE_TOOLS` is not dead code — it's the default bundle every platform's base toolset inherits from.

Auto-discovery: any `tools/*.py` file with a top-level `registry.register()` call is imported automatically — no manual import list to maintain. Wiring into a toolset is still a deliberate, manual step.

The registry handles schema collection, dispatch, availability checking, and error wrapping. All handlers MUST return a JSON string.

**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_nastech_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `NASTECH_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_nastech_home()` for the base directory — never `Path.home() / ".nastech"`. This ensures each profile gets its own state.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `nastech_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `nastech_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `nastech tools`, `nastech setup`, most CLI subcommands | `nastech_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`nastech_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
nastech_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.nastech/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
- `default` — Classic Nastech gold/kawaii (the current look)
...
Add to `_BUILTIN_SKINS` dict in `nastech_cli/skin_engine.py`:
...
Users create `~/.nastech/skins/<name>.yaml`:
...
Nastech has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.nastech/plugins/` and pip-installed entry points.

### General plugins (`nastech_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.nastech/plugins/`, `./.nastech/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `nastech` at startup so
  `nastech <pluginname> <subcmd>` works with no change to `main.py`
...
`post_setup(nastech_home, config)` for setup-wizard integration.

**CLI commands via `plugins/memory/<name>/cli.py`:** if a memory plugin
defines `register_cli(subparser)`, `discover_plugin_cli_commands()` finds
it at argparse setup time and wires it into `nastech <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `nastech --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `nastech_cli/main.py`, etc.).
...
into `~/.nastech/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `nastech memory setup` / `post_setup()` without
...
`~/.nastech/plugins/` (or via pip entry points). They register through
...
don't own. Promote standalone plugins in the Nastech Research Discord
...
2. User: `$NASTECH_HOME/plugins/model-providers/<name>/`
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `nastech skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.nastech.tags`, `metadata.nastech.category`,
`metadata.nastech.related_skills`, `metadata.nastech.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.nastech.*` by the loader.
...
2. **Tools referenced in SKILL.md prose must be native Nastech tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Nastech Agent" is the secondary collaborator. If the
   contributor's commit shows "Nastech Agent" as author (because they
   used Nastech to draft the skill), replace it with their actual name
...
lives in the `nastech-agent-dev` skill at
...
`"messaging"`); `_NASTECH_CORE_TOOLS` is the default bundle most
...
Enable/disable per platform via `nastech tools` (the curses UI) or the
...
Nastech returns a delegation id immediately and the result re-enters the
...
go to `~/.nastech/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `nastech_cli/curator.py` wires `nastech curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.nastech/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `nastech cron <verb>`
...
- File lock at `~/.nastech/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `nastech kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `nastech_cli/kanban.py` wires `nastech kanban` with verbs
...
- **Plugin assets:** `plugins/kanban/dashboard/` (web UI) +
  `plugins/kanban/systemd/` (`nastech-kanban-dispatcher.service` for
  standalone dispatcher deployment).

Isolation model:
- **Board** is the hard boundary — workers are spawned with
  `NASTECH_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
Nastech-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
in config.yaml (or `NASTECH_BACKGROUND_NOTIFICATIONS` env var):
...
Nastech supports **profiles** — multiple fully isolated instances, each with its own
`NASTECH_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `nastech_cli/main.py` sets
`NASTECH_HOME` before any module imports. All `get_nastech_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_nastech_home()` for all NASTECH_HOME paths.** Import from `nastech_constants`.
   NEVER hardcode `~/.nastech` or `Path.home() / ".nastech"` in code that reads/writes state.
   ```python
   # GOOD
   ...
   # BAD — breaks profiles
   config_path = Path.home() / ".nastech" / "config.yaml"
   ```

2. **Use `display_nastech_home()` for user-facing messages.** Import from `nastech_constants`.
   This returns `~/.nastech` for default or `~/.nastech/profiles/<name>` for profiles.
   ```python
   # GOOD
   from nastech_constants import display_nastech_home
   print(f"Config saved to {display_nastech_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.nastech/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_nastech_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_nastech_home()`,
   not `Path.home() / ".nastech"`.

4. **Tests that mock `Path.home()` must also set `NASTECH_HOME`** — since code now uses
   `get_nastech_home()` (reads env var), not `Path.home() / ".nastech"`:
   ```python
...
6. **Profile operations are HOME-anchored, not NASTECH_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".nastech" / "profiles"`, NOT `get_nastech_home() / "profiles"`.
   This is intentional — it lets `nastech -p coder profile list` see all profiles regardless
   of which one is active.

## Known Pitfalls

### DO NOT hardcode `~/.nastech` paths
Use `get_nastech_home()` from `nastech_constants` for code paths. Use `display_nastech_home()`
for user-facing print/log messages. Hardcoding `~/.nastech` breaks profiles — each profile
has its own `NASTECH_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### DO NOT introduce new `simple_term_menu` usage
Existing call sites in `nastech_cli/main.py` remain for legacy fallback only;
the preferred UI is curses (stdlib) because `simple_term_menu` has
ghost-duplication rendering bugs in tmux/iTerm2 with arrow keys. New
interactive menus must use `nastech_cli/curses_ui.py` — see
`nastech_cli/tools_config.py` for the canonical pattern.
...
with actual imports (not mocks) against a temp `NASTECH_HOME`.

### Tests must not write to `~/.nastech/`
The `_isolate_nastech_home` autouse fixture in `tests/conftest.py` redirects `NASTECH_HOME` to a temp dir. Never hardcode `~/.nastech/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_nastech_home()` resolve within the temp dir.
Use the pattern from `tests/nastech_cli/test_profiles.py`:
```python
@pytest.fixture
...
subprocess (`--file-retries`, default 1; `NASTECH_TEST_FILE_RETRIES=0` to
...
| HOME / `~/.nastech/` | Your real config+auth.json                  | Temp dir per test                         |
...
Nastech supports Linux, macOS and native Windows, and plenty of its behaviour
