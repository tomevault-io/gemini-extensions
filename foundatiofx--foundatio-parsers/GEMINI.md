...
- **Exceptions**: Throw `FormatException` for parse errors with cursor position info. Use `ArgumentException.ThrowIfNullOrEmpty(parameter)` for validation. Throw `ArgumentNullException`, `ArgumentException`, `InvalidOperationException` with clear messages for general validation and operation errors.
...
1. **Builds successfully**: `dotnet build Foundatio.Parsers.slnx` exits with code 0
2. **All tests pass**: `dotnet test Foundatio.Parsers.slnx` shows no failures
...
- `ParseAsync_WithRangeQuery_ReturnsTermRangeNode`
- `GenerateQueryVisitor_WithGroupNode_GeneratesCorrectQuery`
- `ValidateAsync_WithInvalidField_ReturnsValidationError`
...
```csharp
[Fact]
public async Task ParseAsync_WithRangeQuery_ReturnsTermRangeNode()
{
    // Arrange
    var parser = new LuceneQueryParser();
    string query = "field:[1 TO 2]";
...
[InlineData("field:[1 TO 10]")]
[InlineData("(a OR b) AND c")]
public async Task ParseAsync_WithValidQuery_CanRoundTrip(string query)
{
    var parser = new LuceneQueryParser();
    var result = await parser.ParseAsync(query);
    string generatedQuery = await GenerateQueryVisitor.RunAsync(result);
...
# Specific test file
dotnet test --filter "FullyQualifiedName~QueryParserTests"
...
- [docs/](docs/) - Query and aggregation syntax documentation
