## claude-skill-appstore-screenshots
...
Default to **1320 x 2868px** (iPhone 6.9") unless the user specifies otherwise. Ask the user which size(s) they need — iPhone only, iPad only, or both. Up to 10 screenshots can be uploaded per display size.

For iPad screenshots, pass `--device ipad` to compose.py. The output will be 2048×2732px using the iPad 12.9" device frame template.
...
```bash
SKILL_DIR="$HOME/.claude/skills/appstore-screenshots" && \
...
This outputs pixel-perfect 1320×2868 PNGs with:
...
- iPhone 6.9" (default): `TARGET_W=1320 TARGET_H=2868`
...
- **Target display size**: e.g., iPhone 6.9" (1320x2868)
...
python3 "$SKILL_DIR/showcase.py" \
  ...
  --github "github.com/NaufalRusada" \
