...
## End of session: file remaining work

Before ending a session, file every loose end as a Backlog task. This applies whether the user explicitly asks or not - the goal is that no idea, todo, deferred fix, or surfaced bug walks out of the conversation in chat history alone.

What counts:

- Features the user asked for that didn't ship this session.
- Bugs the user reported that were diagnosed but not fixed.
- Refactors or cleanups discussed but deferred ("we can do that later").
- Follow-up tasks from a fix you just shipped (e.g. "needs a test", "consider Linux behavior").
- Edge cases and risks the user surfaced as concerns.

What does NOT count:

- Already-shipped commits (the git log is the record).
- Things the user explicitly decided NOT to do.
- Vague "could be nicer" thoughts you had unprompted.

How to file:

```bash
backlog task create "<short title>" \
  -d "<why we want this, what triggered it, links to commits or discussions>" \
  --ac "<verifiable criterion 1>" \
  --ac "<verifiable criterion 2>"
```

Title each task so it reads on its own without context from this session. The description is the spot for conversational background.

When you're done, run `backlog task list --plain` and surface a short list of what you filed to the user, so they have a chance to push back on framing or scope before you wrap.

This applies to both Claude Code and Copilot CLI sessions equally.

### Also log shipped work as Done tasks

When you ship a substantive change (a new feature, a real bug fix - not docs touch-ups or trivial commits), file it as a Backlog task and immediately mark it Done. The git log is authoritative, but the Backlog kanban is what the user looks at to see progress and velocity.

```bash
backlog task create "Title - matches the commit subject" \
  -d "One paragraph: what was wrong, what changed, where to look (commit hash)." \
  --ac "Behavior X works as described"
backlog task edit <id> --check-ac 1 --final-summary "Shipped in <hash>" -s Done
```

Don't file a task per cosmetic commit (whitespace, doc tweaks, dependency bumps); group those into a single 'Misc fixes' task at session end if at all. Substantive feature/fix commits get one task each.
