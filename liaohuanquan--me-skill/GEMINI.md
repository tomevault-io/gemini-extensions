...
> Distill a myskill into an AI Skill. Auto-collect Feishu/DingTalk data, generate Work Skill + Persona, with continuous evolution. | 把你的数字分身蒸馏成 AI Skill，自动采集飞书/钉钉数据，生成 Work + Persona，支持持续进化。
...
**1. 创建目录结构**（用 Bash）：
```bash
mkdir -p me/{slug}/versions
mkdir -p me/{slug}/knowledge/docs
mkdir -p me/{slug}/knowledge/messages
mkdir -p me/{slug}/knowledge/emails
```
...
路径：`me/{slug}/persona.md`

**4. 写入 meta.json**（用 Write 工具）：
路径：`me/{slug}/meta.json`
内容：
```json
{
  "name": "{name}",
...
1. 按 Step 2 的方式读取新内容
2. 用 `Read` 读取现有 `me/{slug}/work.md` 和 `persona.md`
3. 参考 `${CLAUDE_SKILL_DIR}/prompts/merger.md` 分析增量内容
4. 存档当前版本（用 Bash）：
   ```bash
   python3 ${CLAUDE_SKILL_DIR}/tools/version_manager.py --action backup --slug {slug} --base-dir ./me
   ```
...
`/list-myskills`：
```bash
python3 ${CLAUDE_SKILL_DIR}/tools/skill_writer.py --action list --base-dir ./me
```

`/myskill-rollback {slug} {version}`：
```bash
python3 ${CLAUDE_SKILL_DIR}/tools/version_manager.py --action rollback --slug {slug} --version {version} --base-dir ./me
```
...
# Myskill.skill Creator (Claude Code Edition)

## Trigger Conditions

Activate when the user says any of the following:
- `/create-myskill`
- "Help me create a myskill skill"
- "I want to distill a myskill"
- "New myskill"
- "Make a skill for XX"
...
List all generated me when the user says `/list-myskills`.
...
**Base directory**: Skill files are written to `./me/{slug}/` (relative to the project directory).
For a global path, use `--base-dir ~/.openclaw/workspace/skills/me`.

---

## Main Flow: Create a New My Skill
...
Path: `me/{slug}/work.md`

**3. Write persona.md** (Write tool):
Path: `me/{slug}/persona.md`

**4. Write meta.json** (Write tool):
Path: `me/{slug}/meta.json`
Content:
```json
{
  "name": "{name}",
...
2. `Read` existing `me/{slug}/work.md` and `persona.md`
