...
> Self-learning — recognize a hard-won "golden path" during a task and persist it as a reusable Cursor rule so the next session already knows it. Applies whenever a task succeeds only after several tries, uses non-obvious tooling or commands, reveals project facts (how to reach the DB, where creds/env vars live, how to deploy, run migrations, or verify a change live), or the user says "remember this".
