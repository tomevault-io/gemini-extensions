...
- Share terminal parsing through a minimal `zeus-terminal-state` crate; do not make `zeus-remote` depend on the full Engine or create a second parser implementation.
...
- `zeus-remote` must remain a Helper, not a second Zeus Engine. Hooks, MCP forwarding, artifacts, ports, usage, handoff, checkpoints, resource governance, and multiple observers remain separate enhancements unless `zeus/REMOTE_PORT.md` is explicitly revised.
...
- Preserve the local Helper/UDS gates from `zeus/REMOTE_PORT.md`: snapshot p90 <= 100 ms, input-to-PTY p95 <= 10 ms, output-to-diff p90 <= 50 ms, and loopback interaction median <= 75 ms / p90 <= 150 ms.
...
- Keep SSH protocol stdin exclusively for Helper frames. On macOS, route OpenSSH prompts through the packaged Rust `zeus-ssh-askpass`; do not parse passwords or host-key answers in the Engine.
...
CI must also build and execute `zeus-remote probe` natively on Linux x86_64/aarch64 and macOS arm64, and run a disposable ordinary-user OpenSSH detach/reconnect soak. These are release gates for the completed transport. macOS x86_64 and Rosetta are not supported Remote Helper targets.
...
- Update `zeus/REMOTE_PORT.md` when a design decision, phase boundary, dependency, platform matrix, or acceptance condition changes.
