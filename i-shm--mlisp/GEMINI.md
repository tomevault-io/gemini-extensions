...
See `packages/interpreter/mlisp.opam` for full dependency list.
...
├── dune-project          # Project metadata (required)
├── bin/
...
- This project uses Dune 3.3+ (see `packages/interpreter/dune-project`)
...
```bash
mlp init <name>      # Initialize a new MLisp package
mlp install <path>   # Install a package from local path
mlp test             # Run tests in the test/ directory
mlp --help           # Show help
```
...
```
my-package/
├── package.mlisp    # Package metadata
├── src/             # Source modules (*.mlisp)
├── test/            # Test files (*.mlisp)
└── modules/         # Local module search path
```
...
(eval. (read-file "packages/interpreter/stdlib/testing.mlisp"))

;; Define tests with deftest
(deftest "addition works" (== (+ 2 2) 4))
(deftest "list operations" (== (length (list 1 2 3)) 3))
```

### Testing Framework API

- `(deftest "name" body)` - Define a named test that prints result
- Each test outputs: `TEST_RESULT|passed|name|message`

### Architecture

- **OCaml CLI (mlp)**: Command handling, test discovery, pretty reporting
- **MLisp (testing.mlisp)**: Test definitions, assertions, result output
- **Separation**: OCaml handles terminal I/O, MLisp handles test logic

## Module-Test Framework

MLisp provides a `module-test` macro for organizing tests.
...
(module-test factorial
  (deftest "factorial of 0 is 1" (== (factorial 0) 1))
  (deftest "factorial of 5 is 120" (== (factorial 5) 120)))
```
...
1. `module-test` wraps a group of `deftest` calls
2. Each `deftest` executes the test body and prints the result
3. mlp runs `mlisp` on each test file, parses results, and displays grouped report
