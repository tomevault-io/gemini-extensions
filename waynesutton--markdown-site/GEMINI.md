...
The `stats:heartbeat` mutation uses a 45-second dedup window to prevent rapid updates:
...
export const heartbeat = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (existingSession) {
      // Early return if recently updated (idempotent)
...
### Frontend Cross-Tab Coordination with BroadcastChannel

The `usePageTracking` hook uses BroadcastChannel to coordinate heartbeats across tabs (only leader tab sends):
...
export function usePageTracking(): void {
  ...
  useEffect(() => {
    ...
    // Claim leadership and handle messages from other tabs
  ...
  const sendHeartbeat = useCallback(
    async (path: string) => {
      ...
      if (broadcastChannelRef.current && !isLeaderTabRef.current) return;
      if (Date.now() - lastHeartbeatTime.current < HEARTBEAT_DEBOUNCE_MS) return;
      ...
      try {
        ...
        broadcastChannelRef.current?.postMessage({ type: "heartbeat_sent", tabId: tabIdRef.current });
      } finally {
...
1. **Backend idempotency**: Early return when session was updated within 45 seconds
2. **Frontend debouncing**: 45-second debounce window matching backend dedup
3. **Cross-tab coordination**: BroadcastChannel ensures only leader tab sends heartbeats
4. **Tab leadership election**: Claim/close/heartbeat_sent messages for coordination
5. **Config-driven disable**: `isStatsEnabled` check completely disables heartbeat when `statsPage.enabled: false`
6. **Pending state tracking**: `isHeartbeatPending` ref prevents overlapping calls
7. **Indexed queries**: Uses `by_sessionId` index for efficient lookups
8. **Event records pattern**: Page views use `pageViews` table instead of counters
...
3. Debounce rapid user inputs (300-500ms for typing, 45s for heartbeats)
4. Use event records for high-frequency counters
5. Add backend dedup windows for idempotency (45s for heartbeats)
6. Use BroadcastChannel for cross-tab coordination (only leader tab sends)
7. Use refs to track pending mutations and prevent duplicates
8. Monitor your dashboard for conflict patterns
