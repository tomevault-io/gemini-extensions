...
modules/<name>/
  ...
  transport/http/
    ...
    dependencies.py      only what's still needed directly: non-command, module-owned transport
                        ...
                        needed it.
  composition.py        register_<name>_handlers(mediator, session, stack) — builds this
                        module's repositories/UoW against the request's session, enters the UoW
                        into the shared AsyncExitStack, registers its command/query handlers on
                        the mediator. Called from api/v1/dependencies.py::get_mediator, not from
                        composition/container.py (that file only handles HTTP router mounting).
                        Also where a module builds *other* modules' facades it depends on (e.g.
                        `auth`'s takes a `users_facade` param built once in `get_mediator` and
                        passed down — facade construction has no ordering constraint here, unlike
                        a process-wide DI container, since the mediator only dispatches after
                        every module's handlers are already registered).
...
- **Commands/queries are dispatched through the mediator, not injected directly.** `api/v1/dependencies.py::get_mediator` builds a fresh `InProcessMediator` per request (handlers close over a request-scoped session/UoW, so there's no app-wide singleton), opens an `AsyncExitStack`, and calls every module's `register_<name>_handlers`. Routers depend on `MediatorDep` and call `mediator.send(SomeCommand(...))` / `mediator.query(SomeQuery(...))`. `shared/application/mediator.py`'s `Mediator` Protocol is what domain/application/transport code depends on; `shared/application/in_process_mediator.py::InProcessMediator` is the only implementation.
...
- Unit of Work (`shared/data/unit_of_work/BaseUnitOfWork`) wraps one `AsyncSession` per request and exposes module repositories as attributes (e.g. `uow.chats`, `uow.members`); handlers call `uow.commit()` explicitly after mutating through repositories.
