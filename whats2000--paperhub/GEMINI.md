...
- **Agent/LLM prompt authoring (MANDATORY skill — applies to the main session AND every dispatched subagent):** ANY agent that will write or edit an agent/LLM prompt — a `src/paperhub/llm/prompts/*.yaml`, an eval prompt variant under `benchmark/agent/prompts/`, or any change to how a model is instructed — MUST **invoke the `writing-agent-prompts` skill FIRST (via the Skill tool) and follow its principles**: clear & direct first line, **concise without over-explaining a concept**, **XML-delimited injected context** (wrap `{user_message}` and runtime state in descriptive tags — the existing `llm/prompts/*` files are NOT style exemplars, only format exemplars), multi-shot examples for the ambiguous corners, and the **≥2 variants × query set × judged comparison** loop. Pasting a *paraphrase* of the principles into a subagent's dispatch is NOT sufficient — the authoring agent itself must invoke the skill. When dispatching a subagent to author a prompt, instruct it to invoke `writing-agent-prompts` before writing. **A prompt authored without invoking the skill is invalid and must be redone.**
...
- `docs/superpowers/specs/` — SRS (**v2.39.0 current**; shipped through **v2.39.0**)
