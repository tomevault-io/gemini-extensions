...
| Refresh raw app screenshots after a UI change | Re-capture via Stage 1, replace files in `public/screenshots/`, re-run batch capture |
| Add a new slide | Add `Slide7` component, append entry to `SLIDES` in `page.tsx`, add matching `{ param, name, w, h }` row to `capture.mjs` (e.g. `name: "07-something"`) |

First install on a fresh machine needs `bun pm trust puppeteer` so Chromium downloads.
