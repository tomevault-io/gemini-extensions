...
- If an OpenAPI field is wrong, push the backend to fix `workspace/api-spec/openapi.json` — don't work around it on the frontend
...
- `status` values: `pending` (not started) | `in-progress` (in progress) | `done` (completed) | `blocked` (blocked)
