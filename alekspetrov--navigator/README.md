# navigator

> Source: [alekspetrov/navigator](https://github.com/alekspetrov/navigator) — distributed by [TomeVault](https://tomevault.io)

Finish What You Start — Context engineering for Claude Code.         Sessions last 20+ exchanges instead of crashing at 7.

## Gemini CLI Config

The `GEMINI.md` file in this directory is the project config converted for Gemini CLI.
Original source: `CLAUDE.md` in [alekspetrov/navigator](https://github.com/alekspetrov/navigator).

## Also available for

- **Codex** — `AGENTS.md`
- **GitHub Copilot** — `copilot-instructions.md`
- **Cursor** — `project-config.mdc`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

## Bundled Skills (19)

- [backend-endpoint](https://github.com/alekspetrov/navigator/tree/main/skills/backend-endpoint)
- [backend-test](https://github.com/alekspetrov/navigator/tree/main/skills/backend-test)
- [frontend-component](https://github.com/alekspetrov/navigator/tree/main/skills/frontend-component)
- [frontend-test](https://github.com/alekspetrov/navigator/tree/main/skills/frontend-test)
- [nav-compact](https://github.com/alekspetrov/navigator/tree/main/skills/nav-compact)
- [nav-diagnose](https://github.com/alekspetrov/navigator/tree/main/skills/nav-diagnose)
- [nav-init](https://github.com/alekspetrov/navigator/tree/main/skills/nav-init)
- [nav-multi](https://github.com/alekspetrov/navigator/tree/main/skills/nav-multi)
- [nav-onboard](https://github.com/alekspetrov/navigator/tree/main/skills/nav-onboard)
- [nav-release](https://github.com/alekspetrov/navigator/tree/main/skills/nav-release)
- [nav-simplify](https://github.com/alekspetrov/navigator/tree/main/skills/nav-simplify)
- [nav-skill-creator](https://github.com/alekspetrov/navigator/tree/main/skills/nav-skill-creator)
- [nav-sop](https://github.com/alekspetrov/navigator/tree/main/skills/nav-sop)
- [nav-start](https://github.com/alekspetrov/navigator/tree/main/skills/nav-start)
- [nav-task](https://github.com/alekspetrov/navigator/tree/main/skills/nav-task)
- [nav-task-mode](https://github.com/alekspetrov/navigator/tree/main/skills/nav-task-mode)
- [product-design](https://github.com/alekspetrov/navigator/tree/main/skills/product-design)
- [social-media-post](https://github.com/alekspetrov/navigator/tree/main/skills-local/social-media-post)
- [visual-regression](https://github.com/alekspetrov/navigator/tree/main/skills/visual-regression)

Quality verified by TomeVault's automated analysis pipeline. Source: [github.com/alekspetrov/navigator](https://github.com/alekspetrov/navigator)

---

Explore more instruction files on [TomeVault](https://tomevault.io) — the open index of AI config files for every platform.

<!-- genome:d-e-q -->
