...
| WC3 Polymorph (`Aply`), Ply1-Ply5 data, reversible morph presentation, dispel/JASS/save behavior | [docs/games/warcraft-3/polymorph.md](docs/games/warcraft-3/polymorph.md) |
...
| WC3 player/team/unit color ownership, MDX TeamColor/TeamGlow precedence, projectile/effect/UI inheritance, lobby and JASS overrides | [docs/games/warcraft-3/team-colors.md](docs/games/warcraft-3/team-colors.md) |
...
| WC3 Human Barracks unit abilities: Defend toggle/animation, Long Rifles range, Animal War Training max life, remaining fidelity gaps | [docs/games/warcraft-3/human-barracks-unit-abilities.md](docs/games/warcraft-3/human-barracks-unit-abilities.md) |
...
| WC3 Stasis Trap Asta arm/detect/detonate, DataA–D and Bsta stun | [docs/games/warcraft-3/stasis-trap.md](docs/games/warcraft-3/stasis-trap.md) |
...
- **Split gameplay behaviors into their owning abilities.** An ability owns its orders, validation, state transitions, animation moves, timers, interruption/inverse paths, and cleanup. Movement is an ability too: reusable steering, stepping, routing, and support-height behavior belong to Move. Keep `g_monster.c`, `m_unit.c`, `g_ai.c`, and equivalent general-purpose machinery focused on lifecycle, dispatch, and shared mechanisms; do not grow them with individual ability rules or direct spell-specific update calls. Reuse or extend generic function-table hooks so the implementation stays in `skills/s_*.c`, following `s_raven.c` and `s_move.c`. Existing misplaced behavior is precedent to refactor, not a pattern to copy. See [ability ownership](docs/games/warcraft-3/ability-implementation-plan.md#ability-owned-orders-and-persistent-behavior).
- **Flat ability procedures.** Keep classname, procedure, behavior flags, target shape and optional order names directly in each `ability_t` registry row. `CAbility*` names are functions using `abilityProc_t`, not descriptor globals. Pass `abilityitem_t` (actual rawcode plus registry row) through typed `abilityCall_t` messages and read authored data/profile text by that rawcode. A concrete procedure handles its own switch cases and delegates unhandled messages by directly calling its TFT parent procedure, as a Win32 window procedure calls its default procedure; do not add runtime parents, inherited callback tables, descriptor objects, or compatibility bridges. `AB_SPELL` selects the shared cast processor; combine independent policies such as `AB_CHANNEL`, `AB_TOGGLE`, and `AB_AUTOCAST`. Register concrete AbilityData row IDs or implementation codes and necessary internal commands; never register abstract retail classes. Runtime indices are rebuilt with the registry. Run `python3 tools/wc3_ability_class_audit.py --format=coverage` to audit registration against the TFT registry; registration alone does not prove implemented behavior.
...
- **Never guess at a bug fix; reproduce it in a test first.** Confirm the root cause from a failing regression test, authoritative data, and the actual code path before changing production behavior. Recreate the gameplay situation in the test, including the relevant data, entry points, timing, and event order. Assertions are evidence; runtime logs and a game launch are not mandatory. Use targeted logs inside the test or CLI diagnostics when needed, then remove investigative traces. If existing fixtures or tools (`mpqtool`, `dbctool`, `mdxtool`) cannot answer the question, extend them rather than guessing or defaulting to another game launch. See [test-first verification](CONTRIBUTING.md#test-first-behavior-verification).
 - **Never leave debug logs in production code.** The only `fprintf(stderr, ...)` calls that belong unconditionally in source are lifecycle messages ("loading map %s", "R_Init") and genuine errors/warnings. Investigative logs added while tracing a bug must be removed when the fix lands. If a log is still needed because the feature is in-progress, define one `SUBSYSTEM_LOG(...)` macro in the subsystem's common header (`stderr` with a `"PREFIX "` tag under `<GAME>_DEBUG_<SYSTEM>`, `((void)0)` otherwise) and replace every `#ifdef` block with it; never let an `#ifdef` change control flow. Keep one trace per stage (init, datagram, cache rebuild, growth tick); delete per-element traces, world dumps, and counter helpers nothing else uses. Never commit bare per-call trace logs (e.g. printing every trigger fire, every unit order, every tile resolve) outside such a guard.
...
- Use `bzFieldType_t` from `common/shared.h` for shared parser conversion/storage contracts. Keep pointer-valued `BZ_FIELD_CSTR` distinct from bounded inline `BZ_FIELD_CHAR_ARRAY`, and use explicit color channel-order types. Source readers still own byte order, token conversion, bounds, and accepted subsets. Shader descriptors follow the same table pattern but retain renderer-owned `uniformType_t`, because that enum specifies GLSL declarations and `glUniform*` ABI rather than file/text decoding.
 - Prefer format-driven parsing when the data has a fixed syntax. Use `sscanf(text, "%f,%f,%f", ...)` instead of hand-writing character walkers, separator loops, and ad hoc token logic. Every `%s`/`%[...]` width must equal the destination capacity minus 1 (e.g. `%255[^\r\n]` for a 256-byte `PATHSTR`/`MAX_PATHLEN` buffer); never scan a wider field than the buffer holds. Extract line parsing into a pure function and test it with an oversize line plus a sentinel byte after the buffer; drop unused `%n`.
...
- **Verify behavior with automated tests instead of launching the game wherever possible.** Do our best to reproduce in tests the situation we would otherwise inspect in a gameplay session. Use the real production entry points and scheduler/callback path, deterministic simulation time, and representative fixtures; test the action sequence and observable result rather than copying the implementation into the test. Extend the harness or fixture when coverage is missing. A headless test runner may use the game binary without opening the game or playing through a map.
- **Game launches are an exception for a named coverage gap.** Before launching, identify the specific property that cannot be verified through tests or diagnostic tools, such as framebuffer output or platform input behavior. First try to make it testable. If a launch is still needed, exercise only that gap in a bounded run and turn any discovered gameplay failure into a regression test. Once the scenario is covered, use the test; do not repeat game launches just because code was rebased, moved, or rebuilt. See [verification workflow](CONTRIBUTING.md#test-first-behavior-verification).
- Verify every Warcraft III data/UI change in both ROC (default archives) and TFT (`-tft`). TFT archives override ROC
  paths and may replace whole FDF/string/data files rather than extending them, so cover both variants with fixtures and tests.
- **Fixtures must not hide the authored contract.** Use a non-stock Data cell so the test cannot pass on a hardcoded
  retail constant, *and* a stock-shaped case from the brief (`ability_audit -raw`): omitted ROC columns, sentinel zeros,
  and the actual BuffID/`targs` tokens. Injecting the fourcc or scale the consumer already hardcodes is not coverage.
  See [ability tests](docs/games/warcraft-3/ability-implementation-plan.md#4-tests-first-from-the-brief).
- **Assert the gameplay lock, not a proxy.** Expiry, cancel, and inverse tests check the observable a player would
  notice (channel code, movement flags, current order), not only the buff or helper execute already set. If another
  ability can strip that status, that path is in scope.
...
- **Audit every WC3 `edict_t` field and C callback against save/load.** Any field added, removed, moved, or retyped in `games/warcraft-3/game/g_local.h` must update the persistence contract in `games/warcraft-3/game/g_save.c` in the same change. Plain value fields are retained by the raw `edict_t` record; entity/client pointers must use a stable `fields[]` fixup, and process-owned pointers must be cleared and rebuilt explicitly. Roster every new `think`/`stand`/`die` in the append-only `save_cfunctions[]` table and add a round-trip while that pointer is live; advancing the thinker with `G_RunEntities` does not prove `WriteGame`. Add a focused ROC/TFT round-trip test for the field and an invalid-reference test when applicable; never rely only on the save header's `sizeof(edict_t)` check.
...
- **Compile affected code and run tests before finishing implementation work.** Start with the regression reproducer,
  verify it fails before the fix and passes afterward, then run `make test`. Build every game whose shared renderer path
  changed and test the affected contracts. A game launch is not a completion requirement; report any specific coverage gap.
- **Exercise the changed behavior, not just startup.** For order, networking, and UI logic, drive the real command,
  serialization, dispatch, and lifecycle paths from the test harness and assert the resulting state or output. A successful
  main-menu startup proves none of those contracts. If a visual/platform gap still needs a game run, reach the relevant
  connected state and capture the changed behavior, following the exception rule above.
...
- **Verify the transport before assuming loss.** Before adding resync or ack logic, read `Netchan_Transmit`, `NET_SendPacket` and `sv_user.c`. If frames are actually reliable or acked, stop and report instead of building the fix.
- **Unreliable datagrams must self-heal.** Never clear a row's dirty bit just because it was written into an unreliable datagram; one dropped packet then desyncs the client permanently. Prefer full snapshots (weather, tints) that converge on the next send. When deltas are needed, send dirty rows first, then run a per-client background sweep: a cursor per player, every `INTERVAL` frames send the next band with at most `BYTES` of payload, wrapping at the last row. Define both as `#defines` with units, reset the cursor on full-mark, route sweep chunks through the existing datagram path and size budget, and bump client generation only when a cell actually changed. Test: dropping one datagram still converges after the sweep; dirty rows take priority; prove the sweep through the real scheduler entry point (`G_RunFrame`), not by calling the drain directly.
- **Client mirrors accept multiple chunks per frame.** Never let a later chunk overwrite an earlier chunk's dirty range before render. Add a test that parses two chunks before any render and asserts both ranges are present and generation advanced.
- **Prefer full recompute over incremental caches.** On generation change, recompute all derived corners/active flags from the full mask in a GL-free pure function, plus a second pure function for the tile mask. Remove dirty-band and seam-neighbour logic and delete the removed dirty fields from every owner (`client_state`, `viewDef`, `cl_parse.c`, `cl_view.c`). Unit-test a single corner, a map edge, and a seam at a former chunk boundary.
- **One shared struct, one lookup, one flag registry.** Define the shared mask once (e.g. `terrainMask_t {width, height, origin, cell_size, cells, generation}` in `common/`) and use it in both `client_state` and `viewDef` so the handoff is one assignment. Provide one point-to-cell `static inline` in the same common header (origin, cell_size, width, height) used by both server and client; outside the grid means not present. Test server/client agreement at cell boundaries, the max edge, and outside the grid. Keep all datagram flags in one common registry header with a `COUNT` mask and static asserts; never build ad-hoc `~(A|B)` masks in `cl_parse.c`, and never include one feature header from another just for flags.
- **Wire structs are explicit.** Replace implicit padding with an explicit `USHORT reserved` field written as 0; size and offsets stay identical.
- **Failed parses must not move the cursor.** `CL_Parse*` helpers must not touch `readcount` on failure; the frame parser discards the rest of the message.
- **Check query results before acting.** Call follow-up hooks (e.g. `CL_GameModifyBuildPathing`) only when the query (e.g. `CM_GetPathingFlagsAt`) succeeded.
- **Exact-size renderer buffers.** Count emitted tiles first and allocate exactly N vertices each, or reuse a growing persistent buffer. No world-sized worst-case allocation per rebuild. Keep loud error reporting.
- **Factor, don't fork, existing renderer paths.** Keep existing signatures (e.g. `R_MakeSplatTile`, `R_GenerateSplatTiles`) with no per-feature identity checks; factor shared quad construction into one static helper used by the old path and the new tile maker. Delete dead declarations/definitions and fix error strings that name them; make single-use helpers static.
- **Hoist per-cell work to the mutator.** Compute masks/lookups once per public mutator (point, radius, rect, set-state) and pass the result down, not once per changed cell.
...
- **Wire format stays bit-compatible unless explicitly approved.** Flags and chunk layouts are unchanged; no save-format version bump. State the protocol impact (none, or exactly what changed) in the PR description.
...
- **Write the PR description:** summary, protocol impact, test commands, and a proposed split with dependency order (e.g. (1) server state, JASS, save and tests; (2) wire and client mirror; (3) renderer). Propose the split; do not split or squash the PR yourself.
- **Docs move with code; plans do not survive.** Fold durable plan content (target model, constraints) into the subsystem doc, delete the plan file, fix its links, and grep for other references. Update coverage docs to list only what the subsystem doc still calls remaining, and check adjacent docs (construction, race mechanics, save/load, JASS) for stale statements.
