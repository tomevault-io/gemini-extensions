...
- Never hardcode role/block/model strings — use `engine/translator/schema/` and `engine/config/` constants. Config-driven and DRY is enforced by convention here.

### Provider registry (`engine/providers/registry/*`)
...
- DB file location resolves via `src/lib/db/paths.js` (`DATA_DIR`, else `~/.nexus/`).
- Usage/logs (`src/lib/usageDb.js`, `usage.json` + `log.txt`) still live under `~/.nexus` and do **not** follow `DATA_DIR`.

### RTK token saver (`engine/rtk/`)
