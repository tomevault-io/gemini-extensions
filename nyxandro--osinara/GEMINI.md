...
Патчи применяются через `postinstall`; `node_modules/eve` вручную не редактировать.
Ошибка несовпадения патча требует проверки исходников, а не обхода защитной проверки.
Официальная документация: [eve.dev/docs](https://eve.dev/docs).

## Разработка и проверка

Для локальной разработки использовать `npm run dev` и существующий цикл обновления кода.
В режиме `eve dev` расписания автоматически не срабатывают; способ проверки описан в локальном guide Eve.
...
После изменений интеграции с Eve проверять чистую установку `npm ci` и сборку.
После изменения tools/channels проверять результат `npm run build` и свежий discovery:
`.output/.eve/discovery/agent-discovery-manifest.json`; старые корневые `.eve/discovery/` не использовать.
...
В рабочей копии `AGENTS.md` и `CLAUDE.md` должны быть хардлинками одного файла.
После редактирования проверять сохранность связи; Git не сохраняет хардлинки между клонами.
