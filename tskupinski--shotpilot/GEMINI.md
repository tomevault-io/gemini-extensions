...
(with music: `<name>-final.mp4`). Version workflow: the `version` skill.

## Skills (workflows)

Workflows live as skills in `.claude/skills/<name>/SKILL.md` (plain markdown,
open Agent Skills format). The same directory is exposed as `.codex/skills`
(symlink) for Codex CLI; OpenCode discovers `.claude/skills/` natively. Skills
auto-activate when the request matches their description; in Claude Code they
can also be invoked explicitly as `/<name>`.
...
the agent renders thumbnail candidates with `--out` and inspects the images;
...
- `output/<stem>/` — per video: `summary.json` (+`warnings[]`), `review.png` and
  `contact.png` (**inspect the images**), `report.html` (for humans),
...
- `output/grade-preview/` — before/after grids from `shot grade --preview`
  (**inspect the images**); regenerable, safe to delete
...
- `output/publish/` — YT assets: `thumbnail.jpg` (1280×720 thumbnail, **inspect
  the image**), `description.txt` (ready-to-paste description), working `cand-*.jpg`
...
- Done with the footage → suggest the `project-archive` skill (`shot archive`).
...
`CLAUDE.local.md` — it is gitignored; Claude Code loads it automatically
alongside this file, other agents read it at session start (see the top of this
