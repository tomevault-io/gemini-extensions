...
gets. To release, upload `dist/`'s `.exe`, `.blockmap`, and `latest.yml` to that repository's
Release; there is no other sync step. Runtime resolution lives in
`src/main/services/updater/updateFeed.ts`.
