...
- Tests use table-driven style with `{name, source, input, output}`
  ...
  Prefer `if i > 0 { print(" ") }` over trailing `print(" ")`
  for separators in test output.
  Consolidate related tests into fewer, richer test cases
  rather than many single-assertion tests.
...
- Use `allocCells(n)` for result cells, never separate
  `allocCell()` calls -- the free list may return
  non-consecutive cells.
- Multi-byte return values use `ReturnSizes` for per-value
  cell counts. Stride through `returnDst`/`retCells` by
  each return's size.
- Use temp copies when navigating pointer cells --
  never modify pointer variables directly.
- N-byte carry/borrow must track whether a byte WRAPPED,
  not just whether it IS zero.
