...
> Verify that Android Compose implementation matches Figma designs. Uses the Figma MCP server to extract design specs, tokens, and component structure, then compares against the live implementation.
...
- Navigation bar handling (WindowInsets) ✅

### Token Gaps Found
The following Figma Variables have no code equivalent:
- `Color/Surface/Variant` → add `onSurfaceVariant` to AppColors
- `Spacing/Avatar/Large` → add `avatarLarge = 64.dp` to AppSpacing
```

---

## Checking Design Handoff Quality

Before implementing, use Figma MCP to extract the full spec:

```
Before I implement the CheckoutScreen, extract from this Figma frame:
https://www.figma.com/file/ABC123?node-id=99:200

Give me:
1. Full layout breakdown (containers, scroll behaviour, padding)
2. All text styles used (map to our AppTypography tokens)
3. All colors used (map to our AppColors tokens)
4. Component instances used (map to our existing components in :core:ui)
5. Any interactive states shown (default, loading, error, empty)
6. Responsive behaviour (if any breakpoints or min-widths defined)
7. Figma Variables used — flag any that don't exist in our token system yet
```

This replaces manual design handoff inspection and gives Claude exact values before writing a single line of Compose code.
