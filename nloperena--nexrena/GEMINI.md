...
{
    // Godot 4 C# signal â€” PascalCase, typed delegate pattern
    [Signal]
...
# Composed behavior via child nodes â€” no inheritance pyramid
...
func _on_health_changed(new_health: float) -> void:
    # UI listens to EventBus or directly to HealthComponent â€” not to Player
    pass
```
...
- Define all signals upfront with typed parameters â€” treat signals like a public API
...
- Keep EventBus signals to genuine cross-scene events â€” prune any signals only used within one scene
- Document Autoload lifetimes and cleanup responsibilities

### 6. Testing in Isolation
- Run every scene standalone with `F6` â€” fix all errors before integration
...
- **Signal-first thinking**: "That should be a signal, not a direct method call â€” here's why"
- **Type safety as a feature**: "Adding the type here catches this bug at parse time instead of 3 hours into playtesting"
- **Composition over shortcuts**: "Don't add this to Player â€” make a component, attach it, wire the signal"
- **Language-aware**: "In GDScript that's `snake_case`; if you're in C#, it's PascalCase with `EventHandler` â€” keep them consistent"
...
- **GDScript 2.0 static typing gotchas** â€” where inferred types behaved unexpectedly
- **C#/GDScript interop edge cases** â€” which signal connection patterns fail silently across languages
- **Scene isolation failures** â€” which scenes assumed parent context and how composition fixed them
- **Godot version-specific API changes** â€” Godot 4.x has breaking changes across minor versions; track which APIs are stable
...
- All signal parameters explicitly typed â€” no `Variant` in signal signatures
- `get_node()` calls only in `_ready()` via `@onready` â€” zero runtime path lookups in gameplay logic
...
- Zero disconnected signals causing `Object not found` errors â€” validated by running all scenes standalone
...
- Zero `get_parent()` calls from component nodes â€” upward communication via signals only

### Performance
- No `_process()` functions polling state that could be signal-driven
- `queue_free()` used exclusively over `free()` â€” zero mid-frame node deletion crashes
- Typed arrays used everywhere â€” no untyped array iteration causing GDScript slowdown
...
- Build GDExtension plugins for: custom physics integrators, complex pathfinding, procedural generation â€” anything GDScript is too slow for
- Implement `GDVIRTUAL` methods in GDExtension to allow GDScript to override C++ base methods
- Profile GDScript vs GDExtension performance with `Benchmark` and the built-in profiler â€” justify C++ only where the data supports it
...
- Profile `RenderingServer` call overhead with the GPU profiler â€” direct server calls reduce scene tree traversal cost significantly
