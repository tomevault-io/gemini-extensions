...
dependency/<name> - for dependencies (ie. version bump of `pystromligning` and others)
```

All new branches MUST be based on `main`, unless the user explicitly instructs otherwise.
...
When a branch is merged it must also be deleted both local and remote, and changes merged to main must be pulled
