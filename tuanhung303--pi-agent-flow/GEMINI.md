...
> **Tier** (lite / flash / full) only affects **model selection** — which LLM candidate to use. It does **not** restrict tools or access. See [`docs/FLOWS.md#trace-snapshot-transparency`](docs/FLOWS.md#trace-snapshot-transparency) for why `trace` shows `—` in the Context Profile column despite having `tier: lite`.
>
...
| `PI_FLOW_SUB_AGENT_MAX_RETRIES` | Connection-error retries after model failover is exhausted (default 3, max 10; set 0 to disable). |
