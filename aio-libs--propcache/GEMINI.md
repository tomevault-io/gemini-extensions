...
Example (`CHANGES/207.packaging.rst` style):

```rst
Started shipping reproducible wheels by injecting
``-ffile-prefix-map`` into the default tmp-dir build
...
- **If the change has a linked issue, name the fragment after
  the issue number** (e.g. `CHANGES/68.packaging.rst` for a fix
  that closes `#68`). The issue number is stable and known
...
- **If both an issue and a PR number are in play and you want
  ...
  ```bash
  ln -s 68.packaging.rst CHANGES/218.packaging.rst
  ```
...
- The repo does **not** use Conventional Commits as a CI gate.
  ...
  prose (e.g. `Make wheel builds reproducible`,
  `Drop Python 3.9 support`,
  `Loosen the Cython build dependency specifier to >= 3.2.0`).
  Match that style; do not force `feat:` / `fix:` prefixes onto
  every commit.
...
benchmark leg for the descriptor hot path. Do not regress
`tests/test_benchmarks.py` without flagging the trade-off in the
PR body.

### Every line in a test must be covered

The coverage gate applies to test code too, not just
`src/propcache/`. A test that contains a branch or statement the
...
fixture. From the perspective of a unit suite all of those lines
are dead code, and the coverage report flags them the same as
dead code in `src/propcache/`.
...
`src/propcache/_helpers_c.pyx` is the compiled implementation;
`src/propcache/_helpers_py.py` is the pure-Python equivalent.
They must stay behaviourally identical: any change to one must
land in the other in the same PR, and the tests under
`tests/test_cached_property.py` and
`tests/test_under_cached_property.py` exercise both paths via
the parametrized fixture in `tests/conftest.py`. If you change
the public descriptor surface, update the dispatcher in
`src/propcache/_helpers.py` (and the re-exports in
