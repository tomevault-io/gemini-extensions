...
`CLAUDE.md` refers to Claude Code "skills" (`tdd`, `diagnosing-bugs`, `code-review`, `research`,
`gui-screenshots`, `safe-optimization`, `release`, `external-api-change`). Those are convenience
shortcuts for one particular tool — but the *workflows they encode* apply to every agent:
disciplined TDD, methodical debugging, a clean-context review of each diff, verifying an external
API's real current behavior before coding against it (and mocking external-service tests against a
captured fixture rather than the live service), attaching a dialog screenshot to any visible GUI
change (rule 9; `packaging/capture_gui_dialog.py` is the provider-neutral engine),
profiling-then-proving a performance change is safe (rule 5; `packaging/bench_equal.py` is the
provider-neutral equality/benchmark engine), and (maintainer-only) the ordered
version-bump/release checklist. Use whatever tooling you have to achieve the same outcomes.
