...
> All project rules consolidated into a single file — MUST be loaded at conversation start
...
- Two project roots in the workspace:
  - `/home/gamer/git/eros_n` — main project (iOS/Android)
  - `/home/gamer/git/eros_n_ohos` — HarmonyOS adaptation
- **Primary development target is ohos.** When pointing the user to code, always reference the ohos project path first.

## HarmonyOS (ohos) Platform

- Rendering engine: **Skia** (NOT Impeller)
- Build/deploy: `./dev.sh` script, `hvigorw`, `hdc`
- Flutter attach: `./dev.sh --attach`
- Package name: `com.erosteam.erosn`
- Device log: `hdc shell cat /data/app/el2/100/base/com.erosteam.erosn/haps/entry/files/debug.log`

## Key Libraries

- **State management**: Riverpod
- **Routing**: auto_route
- **Liquid Glass**: `liquid_glass_widgets` v0.8.4 (both projects; ohos overrides `meta: ^1.17.0` in dependency_overrides)
  - On Skia: uses `lightweight_glass.frag` single pass, NO native shader refraction
  - For refraction on Skia: must wrap with `LiquidGlassScope` + `GlassRefractionSource`
  - `GlassQuality.premium` refraction only works on Impeller
  - `refractiveIndex` controls refraction strength (separate from `blur` and `thickness`)
  - `blur` controls backdrop blur sigma
  - `thickness` controls frosted/matte appearance
  - `glassColor` controls tint overlay color and opacity
  - `ShaderMask` wrapping `BackdropFilter` will kill the blur effect

## Workflow Rules

- Commits: ohos project first, then main project only when user says "同步"
- Do NOT push to remote unless explicitly asked
- Use hot reload for UI iteration, do NOT reinstall the app for UI changes
- Run `dart format .` and `dart fix --apply` before committing code changes

## UI Modification Rules

- **Before modifying any UI**: first identify ALL related components that share the same visual style (height, spacing, color, font). List them out, define unified parameters (constants or shared values), then apply changes to ALL of them in one pass.
- **NEVER modify only one instance** of a repeated UI pattern. If three capsules should be the same height, change all three at once.
- **NEVER use different approaches** (e.g. padding vs IntrinsicHeight vs SizedBox) for the same visual requirement across components. Pick ONE approach and apply it consistently.
- When pointing to code files, always reference the ohos project path.

---

# 6. Cross-Project Rules

两个项目（`eros_n` 和 `eros_n_ohos`）共享同一套规则。当对话涉及 `eros_n_ohos` 项目时，如果 ohos 项目有自己的 `.cursor/rules/all-rules.mdc`，以 ohos 的为准（可能包含 ohos 特有的补充规则）。

---

# 7. Avoid Usage Policy Blocks

This project targets a site whose name is a content-policy keyword. Letting that
string flow through tool output / responses repeatedly causes Anthropic's Usage
Policy filter to abort the turn with:
> We are unable to complete this request because it was blocked under
> Anthropic's Usage Policy.

## 7.1 Filter sensitive strings out of `Shell` tool output

When invoking commands that may print site URLs, page titles, response bodies,
log output, or HTTP headers, **always pipe through `sed`** to scrub them before
the result reaches the assistant context.

Replace these substrings with neutral placeholders:

| Substring | Replace with |
|-----------|--------------|
| `nhentai.net` | `<SITE>` |
| `nhentai` (case-insensitive, when standalone) | `<SITE>` |
| `hentai` | `<H>` |
| `doujinshi` | `<D>` |
| `manga` | `<M>` |
...
❌ BAD — passes raw output through:

```bash
adb logcat -d | grep flutter | tail -30
```

✅ GOOD — scrubs sensitive substrings:

```bash
adb logcat -d | grep flutter | tail -30 \
  | sed -E 's/nhentai\.net/<SITE>/g; s/nhentai/<SITE>/Ig; s/hentai/<H>/Ig; s/doujinshi/<D>/Ig; s/manga/<M>/Ig'
```

❌ BAD — curl prints HTML body containing those words:

```bash
curl -sI https://nhentai.net/
```

✅ GOOD — only print headers we care about, scrub anyway:

```bash
curl -sI https://nhentai.net/ \
  | grep -iE '^(http|cf-|server|content-type|set-cookie)' \
  | sed -E 's/nhentai\.net/<SITE>/g; s/nhentai/<SITE>/Ig'
```

## 7.2 Don't quote sensitive substrings in assistant text

When discussing, refer to the site as **"the target site"** or **`<SITE>`**.
Don't paste page titles, response bodies, or full URLs that include the
filtered words above into the response.

## 7.3 Code edits are exempt

Editing actual project source files is fine — the constants stay in code as-is
(`NHConst.baseUrl = 'https://nhentai.net'`). The filter is only for **tool
output and assistant text** that gets streamed back through Anthropic.
