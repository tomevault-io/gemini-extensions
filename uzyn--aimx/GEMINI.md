...
- `hooks.rs`: `aimx hooks list | create | delete` CLI (with `hook` clap alias). Owner-gated, not root-gated: the central `auth::authorize` predicate (gating `Action::HookCrud`) runs as a local pre-flight only when the config is readable; otherwise the daemon's `SO_PEERCRED` authz is authoritative. Dispatched directly from `main.rs` (NOT the eager-config `dispatch_with_config` arm) and loads the config *optionally* via `mailbox::load_config_optional`, mirroring `mailbox.rs` — so a non-root mailbox owner on a `0640 root:root` install never hits `Permission denied` before the hooks code runs. All three subcommands prefer the daemon UDS path: `list` falls back to the `HOOK-LIST` verb (already ownership-filtered) when the config is unreadable; `create` / `delete` submit `HOOK-CREATE` / `HOOK-DELETE` so `Arc<Config>` hot-swaps live without a restart. When the daemon is down, root falls back to a direct `config.toml` edit + restart hint; non-root hard-errors with the canonical *"daemon must be running"* hint because it cannot read or write the root-owned config. `--cmd` takes the argv as a JSON array. Delete resolves against effective names (locally when the config is readable, else via the daemon's `HOOK-LIST` rows for the confirmation prompt); operator-origin hooks can only be removed via CLI, MCP-origin hooks can also be removed over UDS. The UDS helpers `submit_hook_create_via_daemon` / `submit_hook_delete_via_daemon` / `submit_hook_list_via_daemon_for_cli` live in `src/mcp.rs` and are shared by both the CLI and the MCP server.
...
- `hook_list_handler.rs`: daemon-side handler for the `AIMX/1 HOOK-LIST` verb. Returns a JSON array `Vec<HookListRow>` filtered to hooks on mailboxes the caller's uid owns; root sees every hook on every mailbox. Mirrors `MAILBOX-LIST`'s frame shape. Consumed by both the MCP `hook_list` tool and (via `submit_hook_list_via_daemon_for_cli`) the `aimx hooks list` CLI fallback when the local config is unreadable.
