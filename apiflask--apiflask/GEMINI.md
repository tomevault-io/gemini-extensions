...
- Python target version: 3.9+ (note: pre-commit currently runs `pyupgrade --py38-plus`).
...
If a significant part of a commit is generated by an AI tool (with minimal changes), it must be disclosed via an `Assisted-by:` commit message trailer.
