...
> 在 Windows 中配置、修复或说明 Git Bash zsh 终端环境，涵盖 Windows Terminal、Oh My Zsh、Starship、Nerd Font、VS Code 终端、fzf、常用 zsh 插件，以及 bat、ripgrep、lsd、yazi 等现代 CLI 工具。适用于 Windows Bash/zsh/Starship 安装、MSYS2 zsh 包安装、.bashrc/.zshrc 修改、UTF-8 或 PATH 问题、Windows Terminal Git Bash profile、CLI alias、插件报错，以及回退到 Bash。
