...
**recording on**, session advisory/limit **300k/400k** estimated tokens, **auto-compaction on
at 300k and edge-triggered**, **multi-agent on** with `maxWorkers` 2 (hard max 8). Existing
...
- Canonical messages live in `messages.json`, keyed by logical id. They are not appended
  forever to `events.jsonl`.
...
test/*.test.ts                37 suites, named for the subsystem they cover
scripts/*                     build-time icon / tunnel-client / ripgrep fetchers
electron-builder.yml          Windows package contents and NSIS policy
```

Older connector-native helpers survive: `process-manager.ts`, `patch.ts`, `patch-files.ts`,
`exec.ts`. Some are still live dependencies, some are adapters. **Trace the actual caller
before editing a similarly named old subsystem.**
...
`codex-apply-patch-invocation-parity`, `codex-view-image-parity`, `mcp`, `process-manager`.
...
%APPDATA%\chat-on-steroids\sessions\<id>\
  ...
  messages.json       canonical user/assistant messages, keyed by logical id
...
`messages.json` **replaces by stable identity** — never turn that back into blind appends.
...
| exec / PTY / stdin / output / session | `unified-exec.ts`, `shell.ts`, `ownership.ts`, `exec-output.ts` | `codex-runtime-parity`, `process-manager`, `mcp` |
...
36 suites, named for the subsystem they cover. Vitest uses real filesystem, real processes
...
| `patch` / `patch-files` | connector-native patch parser and transactional writes |
| `process-manager` | the older managed-process subsystem |
