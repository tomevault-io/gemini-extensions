...
- If the default model is `gpt-5.4` / `openai-codex`, remember that chat depends on a live local Codex CLI login (`codex login`).
  |- **AI Second Brain vault** (`~/obsidian/memo/`, outside this repo — only the ingestion scripts live here): whenever new raw files are added to `~/obsidian/memo/01-Audio/` or `~/obsidian/memo/02-Documents/`, run `./scripts/sync-second-brain.sh` to transcribe/extract them into `~/obsidian/memo/03-Notes/`, consolidate durable facts into Hermes Agent's own `MEMORY.md`/`USER.md` (`scripts/consolidate_memory.py`, via `hermes -z ... -t memory --yolo`), and refresh the `graphify` code graph. Override the vault location with the `SECOND_BRAIN_VAULT` env var. See `~/obsidian/memo/AI_CONTEXT.md` for the full structure and limitations. This is independent of the `km-agent` row above — `km-agent` is defined in the roster but not yet provisioned (no profile, no `gbrain` skill installed); the vault and its scripts don't depend on it being live.
  ||- **Automatic Second Brain Integration**: When user asks to "pelajari" or "study" something and wants it saved for reference, follow the workflow: (1) Research/learn the topic, (2) Create a note in `~/obsidian/memo/03-Notes/Extracted-Docs/`, (3) Run `bash ~/.hermes/hermes-agent/scripts/second-brain/sync-second-brain.sh`, (4) After successful wiki ingest, the source file is **automatically deleted** by `cleanup_sources.py`. Notes MUST go in `Extracted-Docs/` (not `03-Notes/` directly) because `wiki_ingest.py` only processes that folder. This behavior is permanent and applies to all future sessions. See `~/.hermes/skills/workflow/automatic-second-brain-integration/SKILL.md` for the complete skill.
  ||- **Auto-Didactic Learning Rule**: When user asks to "pelajari"/"study" without clear source, or gives a command requiring unknown knowledge, or asks about a topic without specifying a source → **Automatically search the web first, learn the material, then execute the command**. Workflow: (1) Search web for relevant materials, (2) Study and extract key concepts into `~/obsidian/memo/03-Notes/Extracted-Docs/<Topic>.md`, (3) Run `sync-second-brain.sh`, (4) Execute user's command ONLY after learning is complete, (5) Source file auto-deleted after ingest. This rule is permanent and applies to all future conversations regardless of session or agent. Triggered for: "pelajari/study" requests without source, unknown knowledge commands, ambiguous topics without clear source.
