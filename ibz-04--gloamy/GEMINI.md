...
- If identity-like naming is required in tests/examples, use Gloamy-native labels only (`GloamyAgent`, `gloamy_user`, `gloamy_node`).
...
- If identity-like context is unavoidable, use Gloamy-scoped roles/labels only (for example: `GloamyAgent`, `GloamyOperator`, `gloamy_user`) and avoid real-world personas.
- Recommended identity-safe naming palette (use when identity-like context is required):
    - actor labels: `GloamyAgent`, `GloamyOperator`, `GloamyMaintainer`, `gloamy_user`
    - service/runtime labels: `gloamy_bot`, `gloamy_service`, `gloamy_runtime`, `gloamy_node`
    - environment labels: `gloamy_project`, `gloamy_workspace`, `gloamy_channel`
