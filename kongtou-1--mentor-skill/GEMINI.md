...
- 提取：研究方向、方法论、指导流程、论文与实验规范、输出标准、经验知识
- 重点关注：选题判断、组会节奏、论文修改、数据严谨性、学术底线、学生培养方式

**线路 B（Persona）**：
- 参考 `${CLAUDE_SKILL_DIR}/prompts/persona_analyzer.md` 中的提取维度
- 将用户填写的标签翻译为具体行为规则（参见标签翻译表）
- 从原材料中提取：表达风格、批注习惯、决策模式、人际行为、边界与雷区
...
---
name: mentor-{slug}
description: {name}，{school} {department} {title}
user-invocable: true
---

# {name}

{school} {department} {title}{如有研究方向则附上}
...
1. 按 Step 2 的方式读取新内容
2. 用 `Read` 读取现有 `mentors/{slug}/work.md` 和 `persona.md`
3. 参考 `${CLAUDE_SKILL_DIR}/prompts/merger.md` 分析增量内容
4. 存档当前版本（用 Bash）：
   ```bash
   python3 ${CLAUDE_SKILL_DIR}/tools/version_manager.py --action backup --slug {slug} --base-dir ./mentors
   ```
...
List all generated mentors when the user says `/list-mentors`.
...
**Base directory**: Skill files are written to `./mentors/{slug}/` (relative to the project directory).
For a global path, use `--base-dir ~/.openclaw/workspace/skills/mentors`.
...
1. **Mentor name / codename** (required)
2. **Basic info** (one sentence: school, department/lab, title, research area — say whatever comes to mind)
   - Example: `South China Normal University School of Psychology Professor cognitive neuroscience`
3. **Guidance style** (one sentence: supervision habits, feedback style, standards, impressions)
   - Example: `gentle but demanding, weekly group meetings, detailed paper comments, encourages students to think first`
...
- Extract: research areas, methodology, supervision workflow, paper/experiment standards, output preferences, experience
- Emphasize topic selection, group meeting rhythm, revision standards, data rigor, academic boundaries, and student training principles
...
- Extract from materials: communication style, annotation habits, decision patterns, interpersonal behavior, and boundaries
...
Work Skill Summary:
  - Research areas: {xxx}
  - Methodology: {xxx}
  - Paper / experiment standards: {xxx}
  ...
...
Path: `mentors/{slug}/work.md`

**3. Write persona.md** (Write tool):
Path: `mentors/{slug}/persona.md`

**4. Write meta.json** (Write tool):
Path: `mentors/{slug}/meta.json`
Content:
```json
{
  "name": "{name}",
...
{school} {department} {title}{append research area if available}
...
2. PART A executes: use your academic guidance capabilities to complete the task
...
Inform user:
```
✅ Mentor Skill created!

Location: mentors/{slug}/
...
2. `Read` existing `mentors/{slug}/work.md` and `persona.md`
...
2. Determine if it belongs to Work (academic/workflow) or Persona (communication/interaction)
