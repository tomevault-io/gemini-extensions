...
> Dashboard JavaScript patterns inferred from web UI bugfix commits
...
- **CSS**: Scope under `#report-modal-content`; extend [`oasis/static/css/report_preview.css`](oasis/static/css/report_preview.css) and reuse shared classes (`report-toc`, chart wrappers, section spacing) before adding unrelated class names in [`dashboard.css`](oasis/static/css/dashboard.css).
- **JS**: Report “kind” detection and one-shot setup (TOC wiring, chart init, which assistant variant to show) live in **one** initializer invoked from [`modal.js`](oasis/static/js/dashboard/modal.js) (`_finalizeReportModalView` path) — do not scatter stem/`report_type` checks across unrelated modules.
- **Server**: HTML preview composition and meta endpoints belong in [`oasis/web.py`](oasis/web.py) with parsing/helpers under [`oasis/helpers/`](oasis/helpers/) so executive, audit, and vulnerability stay DRY unless a genuinely different document shape requires a separate template.
- **Assistant**: Any new request field or UI mode is implemented in [`assistant.js`](oasis/static/js/dashboard/assistant.js), [`api.js`](oasis/static/js/dashboard/api.js), and [`web.py`](oasis/web.py) together; copy strings and labels go through [`assistant-constants.js`](oasis/static/js/dashboard/assistant-constants.js).
