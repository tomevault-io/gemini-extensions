...
Key data structures:
- **genome**: continuous genes (branchiness, branchFactorN, tillering, radialOrder, succulence, stemGirth, taper, rigidity, verticality, ribbing, spininess, segmentation, appendageBreadth/Density, branchAngle, lengthRatio, apicalBias, droopBias) + cosmetic (pigment, leafSize, leafWidth, jitter) + `structuralSeed`. Grass/cactus/kelp/tree are **regions of this continuous space**, reached by interpolation — there are NO `if (type === ...)` branches. Schema (tiers, ranges, distance metric) lives in `src/genomeSchema.js` (`FLORA_SCHEMA`).
  - **NO archetypes / no "type" concept** — and not even soft identity bundles: a gene NEVER drives another gene's value. The only cross-parameter coupling allowed is continuous physical *law* (allometry, in `allometry.js`), never identity. A conifer is just what you get from needle-leaf + whorled-branching set together; nothing co-activates them.
  - **`appendageDensity` is the single foliage-density gene** (range widened to `[0,1.5]`); **`leafSize` is the single leaf-size gene** (range widened to `[0.6,4.0]`). The former `leafDensity` and `leafScale` were **removed** as pure-product duplicates and folded into these two. (`leafWidth`/`appendageBreadth` are still two overlapping breadth knobs — a deferred merge, since it changes the leaf *sprite* render.)
...
- **Branch mesh**: `buildBranchGeometry(graph)` (`branchMesh.js`) walks the graph into ONE merged tapered-tube `BufferGeometry` (parallel-transport framed, fork joints bridged, tip apex-collapse, per-vertex `ao`). It bakes the bark-coordinate inputs: **`aTangent`/`aFrameU`** (the parallel-transport frame, **carried across forks** so a child chain inherits its parent's frame → continuous bark around-coordinate), and **UV.v = a GLOBAL arc length** from the root (continuous across forks, also inherited at fork nodes → no bark seam at junctions). Material: `createBarkMaterial()` (`barkMaterial.js`) = `MeshStandardMaterial` + `onBeforeCompile` injecting **furrow-guided FBM** bark (see "Bark" under tuning) for albedo/normal/roughness/AO with real PBR lighting + IBL.
...
- **Render-mode toggle** (`renderModes.js` + the `#rendermode-panel` in `index.html`): lit / unlit / wireframe / normals / **furrows** / ao — swaps per-mesh materials (instancing-aware), persists across regenerate. `viewer.attachRenderModeController(...)` / `viewer.setRenderMode(mode)`. The **furrows** mode is a bark debug view: it sets `barkCtl.setDebugFurrows(true)` so the real bark material outputs the flow-line furrow field directly (the line traces), for dialling the furrow structure before judging the lit result.
...
- **Bark**: `BARK_*` constants in `barkMaterial.js`. The bark is **furrow-guided FBM** (NOT voronoi — see the memory): `barkFurrowHeight(around, alongArc, alongGlobal)` is the relief that drives normal/colour/AO. `around` = a SEAMLESS angle from the radial normal · the carried branch frame (not `uv.x`, which has a wrap-seam quad). The FBM is a ridged-multifractal sampled on a **cylinder embedding** (`cos/sin` of the furrow angle → seamless wrap) in furrow-flow coordinates so its organic ridges/furrows FOLLOW the branch + furrow guides; the **furrow lines guide the FBM, they don't replace it**. Knobs: `BARK_FURROW_MIN/MAX` (density range, driven by the `barkScale` gene = "Furrow density" slider, rounded to integer for a seamless wrap), `BARK_FURROW_WARP` (organic wander), `BARK_FURROW_ALONG_FREQ` (vertical break-up), `BARK_FURROW_MEANDER_FREQ`/`WANDER` (furrow drift), `BARK_FURROW_DEPTH` (relief/normal strength), and the **horizontal cross-fractures (S1)** `BARK_XCRACK_AROUND`/`FREQ`/`DEPTH` (thin recessed cracks, sampled on the SAME seamless cylinder, that chop the vertical ridges into stacked oak scales — driven by the `barkPlates` gene × `uBarkRelief`; `barkPlates=0` is a strict no-op). The normal is built analytically (finite-difference the furrow height in around/along, tilt the VIEW-space normal via `vViewTangent` — `modelMatrix` is NOT available in the fragment, so the tilt is done in view space). Dead-but-present: `barkVoronoiEdge`/`barkCrackMask`/`barkReliefHeight`/`barkPerturbNormal`/`barkHeightField` (no longer called; left for reference).
...
- **Allometry / cross-parameter coupling (`allometry.js`).** Genes used to be siloed: moving `trunkHeight` alone gave a spindly reed (girth/leaf/tier counts didn't follow). The fix is `deriveTraits(genome)` — continuous scaling LAWS, identity-centered at `sizeFactor=1` so default-stature plants are byte-identical. It couples: girth `∝ sizeFactor^1.3` (proportions), leaf-cluster card size `∝ sizeFactor^0.5` (foliage), whorl tier count `∝ sizeFactor^0.7` (skeleton), branch-recursion depth `+round(log10(size)·2)` (skeleton, gated so an unbranched plant stays unbranched), root extents `∝ sizeFactor` (roots). **This is the ONE place a value is derived rather than a free slider — and it's a physical law, not an archetype.** Other one-directional couplings live in their stages: `apicalBias` suppresses laterals (skeleton), `rigidity`→girth + `weep`→strand-thinning (proportions), and `needleLeaf`/`frondLeaf`/`rosette` continuously blend the foliage distribution from broadleaf→clothed-shaft (foliage `frondyness`, replacing the old binary 0.5 flip). Tune the exponents via the named constants in `allometry.js` (`GIRTH_EXP`, `LEAF_AREA_EXP`, `TIER_EXP`, `BRANCH_ORDER_GAIN`).
...
- **Bark genes** (cosmetic; drive `barkMaterial` uniforms via `setGenome`). The relief is now **furrow-guided FBM** (see "Bark" in the tuning section); the live UI sliders are **`barkHue`** (grey→warm brown/red), **`barkLightness`** (dark→pale/birch), **Furrow density** (the `barkScale` gene → furrow count), and **`barkPlates`** (ridges↔plates morphology — now drives the **horizontal cross-fracture** term in `barkFurrowHeight` (S1): thin recessed cracks, sampled on the same seamless cylinder embedding, that chop the vertical furrow ridges into stacked oak scales. Gated by `uBarkRelief` (smooth-bark presets stay smooth) and `0 = strict no-op`. Knobs: `BARK_XCRACK_AROUND/FREQ/DEPTH`). The `barkRelief`/`barkOrient`/`barkLenticels`/`barkShed`/`barkUnderHue` genes still exist in the genome (determinism), but their sliders were **removed** (relief is generated, not slider-driven; lenticels/shed/under-hue are inert for the furrowed-oak look). All bark genes remain in the schema so `randomGenome`/fixtures stay byte-identical.
