...
- `data` - Large/dynamic data that will be stringified (use for data you want to read but not search by)
- `entityId` - ID of entity to search by (use instead of `userId`, `organizationId`, etc.)
- `entity` - Name of entity related to `entityId` (e.g., `'User'`, `'Organization'`, `'Ticket'`)
- `count` - Numeric value for aggregation (sum/avg/percentiles)
- `status` - HTTP status code, task status, or any status indicator
- `err` - Error object (automatically serialized)
...
logger.info(`Message ${message.id} delivered via ${transport}`)
```

**2. Use `data` field for large/dynamic content:**
```javascript
...
- `/packages/keystone/logging/serializers.js` - All standard fields and their serializers
- `/packages/keystone/logging/getLogger.js` - Logger initialization
- `/packages/keystone/logging/index.js` - Exports
...
- Use Redis or Valkey for caching and session storage
