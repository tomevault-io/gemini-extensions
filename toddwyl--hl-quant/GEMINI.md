...
## （附录）编程行为准则：Andrej Karpathy 风格 Skills 指南

以下节译自开源仓库 **[forrestchang/andrej-karpathy-skills](https://github.com/forrestchang/andrej-karpathy-skills)** 中的 [`CLAUDE.md`](https://github.com/forrestchang/andrej-karpathy-skills/blob/main/CLAUDE.md)，作为与本仓库前述条款**并行**的补充行为准则。**取舍**：偏重谨慎而非速度；对鸡毛蒜皮的任务可自行把握松紧。

### 1. 先想再写

**不要想当然。不要装作懂。要把取舍摊在桌面上。**

动手实现之前：

- 把假设说清楚；拿不准就先问。
- 若有多种解读，摊开说明，勿默默选一条路。
- 若有更简单的路子，要主动说；必要时顶回去。
- 若仍不清楚：**停**。点明迷惑之处，再问。
...
- 未要求时不要加「灵活性」「可配置性」。
- 不要为假想中的不可能分支写容错。
- 若写了 200 行但能 50 行搞定：**重写**。

自检：资深工程师会不会觉得你这版「过度复杂」？若是，就简化。
...
- 不要为了「更好看」顺带改相邻注释、排版或顺手重构无关逻辑。
- 没坏的东西不要重构。
- 风格与上下文一致——即便你本会写成别的样子。
- 若发现与本次无关的死代码：**提及即可**，别擅自删。

只有因**你的改动**产生的孤儿时才清理（多余 import / 变量 / 函数）。
...
验收标准越强，越能独立完成闭环；只有「做出来就行」则难免反复拉扯澄清。

---

**这些准则起效时通常表现为：** diff 里无关紧要改动更少、因过度设计返工更少、多在实现前发问而不是做错后再问。
