...
The content editor JavaScript is written as **native ES modules** (vanilla JS,
no build step) in `content_editor/static/content_editor/`:

- `index.js` - Entry point, loaded as `type="module"`. Runs the single,
  ordered initialization: parses the context, constructs and wires the
  subsystem classes, moves the plugin inlines into the order machine, then runs
  the initial state pass and restores persisted UI state.
- `context.js` - Parses the JSON config into the singleton `window.ContentEditor`
...
- `machine.js` - The order machine: scaffolding, `reorderInlines`, ordering
  math, the Django formset add/remove lifecycle, and collapse/expand.
- `sections.js`, `dragdrop.js`, `cloning.js` - Single-instance classes for
  nested sections, drag & drop, and the clone dialog.
- `save_shortcut.js`, `tabbed_fieldsets.js` - Standalone scripts (Ctrl/Cmd-S
  save; tabbed fieldsets for `RefinedModelAdmin`).
...
`django.jQuery(init)` in `index.js` as the DOM-ready hook, so initialization
runs *after* Django's admin `inlines.js` setup (which runs on the jQuery ready
queue). `admin/js/jquery.init.js` is declared as a `Media` dependency for this.
Requires Django >= 4.1 (the project targets >= 4.2), since formset events are
...
- JavaScript receives configuration via the `#content-editor-context` JSON
  script tag, generated by `_content_editor_context()` in admin.
- Context includes plugins, regions, messages, permissions.

**Rights handling:** plugins can only be added to a valid, declared
(non-`_unknown_`) region. The visibility pass always runs once during init (even
with zero regions), and `addContent` refuses without a valid current region.
