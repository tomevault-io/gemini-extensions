...
- During implementation, run the narrowest relevant test command by itself. Run `cargo fmt --all`, `git diff --check`, and broader validation during final verification instead of chaining them before each test run.
- Use `cargo check -p <crate>` when only compile validation is needed; use `cargo build` only when a linked artifact is required.
- Keep the Cargo profile, features, target, `RUSTFLAGS`, and `CARGO_TARGET_DIR` consistent between iterative runs; changing them creates separate artifacts and can recompile the dependency graph.
- Do not set `CARGO_BUILD_JOBS` above Cargo's default. Cargo already uses the available logical CPUs, while oversubscription increases contention and memory use without accelerating large single-crate frontend work.
...
## Coding executor task sizing

- Split implementation and verification work into small, bounded executor tasks with one coherent objective each.
- Choose a generous but finite timeout for the first executor call, sized for the task and its expected test or build time, instead of relying on a timeout retry.
