...
one function (`api.api._selection`) so the reduction the table shows is the one
...
same `api.api._selection`, so a *selection* is a name the server gives a file
...
src/api/                # Flask, a package of its own: create_app, the /api
                        #   blueprint, problem.py (RFC 9457), health
```
...
`Timestamp` at the market's edge, `api.api` on an ISO string arriving from the
