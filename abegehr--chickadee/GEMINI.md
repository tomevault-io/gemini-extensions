...
@plugin "daisyui/theme" {
  ...
  --border: 1px; /* borderepth: 1; /* only 0 or 1 – Adds a shadow and subtle 3D effect to components */
...
#### RuleER} is optional and can have one of the modifier class names
...
### alert
Alert informs users about imps
...
[avatar docs](https://daisyui.com/avatar/)
...
[badge docs](https://daisyui.com/components/ba names
- component: `badge`
...
#### Syntax
```html
<span class="badge {MODIFIER}">Badge</s## Rules
...
#### Class names
- component: `btn`
- color: `btn-neutral`, `btn-primary`, `btn-secondary`, `btn-accent`, `btn-info`, `btn-success`, `btn-warning`, style: `btn-outline`, `btn-dash`, `btn-soft`, `btn-ghost`, `btn-link`
...
- set `tabindee="button" aria-disabled="true"` if you want to disable the button using a class name
...
- can use `sm:card-horizontal` for responsi
...
[chat docs]ui.com/components/chat/)
...
<div class="chat {PLACEMENT}">
  ...
  <div class="chat-bubble {COLOR}">Message<div class="chat-footer"></div>
</div>
```
...
- size: `checkbox-xs`, `checkbox-md`, `checkbox-lg`, `checkbox-xl`
...
- component: `collapse`
- part: `collapse-title`, `collapse-content`
- modifier: `collapse-arrow`, `collapse-plus`, `collapse-open`, `collapse-closentax
```html
...
[countdos://daisyui.com/components/countdown/)
...
#### Class names
- component: `diff`
- part: `diff-item-1`, `diff-item-2`, `diff-re## Syntax
```html
<figure class="diff">
...
#### Syntax
```html
<div class="drawer {MODIFI id="my-drawer" type="checkbox" class="drawer-toggle" />
...
[dropdown docs](https://daisyui.com/componen
#### Class names
- component: `dropdown`
...
### fieldset
Fieldset is a containing related form elements. It includes fieldset-legend as a title and label as a description
...
<form class="filter">
  ...
  <input class="btn" type="radio" name="{NAME}" aria-label="Tab 
</form>
```
Without HTML form
```html
...
- Each set of radio inputs must have uniqbutes to avoid conflicts
...
```
where content can contain several `<nav>` tags with `footer-title` and li
#### Rules
...
[indicator docs](https://daisyui.com/components/indicator/)ss names
- component: `indicator`
- part: `indicator-item`
...
- {placement} is optional and can have one of zonal/vertical class names. default is `indicator-end indicator-top`
...
- component: `input`
- style: `input-ghost`
- color: `input-neutral`, `input-primary`, `input-secondary`, `input-accent`, `input-info`, `input-success`, `input-warningrror`
...
### kbd
Kbd is used to display kertcuts
...
- Use `floating-lthe parent of an input field and a span that floats above the input field when the field is focused
...
#### Rules
- {MODId can have one of the style/size class names
...
Horizontal menu:
```html
<ul class="menu menu-horizontatem</button></li>
</ul>
```
...
- Use `menu-title` for list iteme `<details>` tag to make submenus collapsible
...
[mockup-phone docs](https://daisyui.com/componehone/)
...
### mockup-window
Window mockup shows a box that looks like an  window
...
#### Class names
- component: `modal`
- part: `modal-boion`, `modal-backdrop`, `modal-toggle`
- modifier: `modal-open`
...
[navbars://daisyui.com/components/navbar/)
...
#### Rules
- Use `join-ith button or link inside the pagination
...
#### Rules
- {Moptional and can have one of the color class names
...
- you need to add `aria-valuenow="{value}"`, `aria-valuenow={value}` so screen readers can properly read value a that its a progress element to them
...
- size: `radio-xs`, `radio-sm`, `radiog`, `radio-xl`
...
- Replace {name} with a unique name for the radEach set of radio inputs should have unique `name` attributes to avoid conflicts with other sets of radio inputs on the same page
...
-nge-xs`, `range-sm`, `range-md`, `range-lg`, `range-xl`
...
<div class="rating {MODIFIER}">
  <input type="rrating-1" class="mask mask-star" />
</div>
```
...
- component: `select`
- style: `select-ghost`
elect-neutral`, `select-primary`, `select-secondary`, `select-accent`, `select-info`, `select-success`, `select-warning`, `select-error`
...
### stack
Stack visually puts n top of each other
...
#### Rules
- {MODIFIER} is optional and can have one of the directionmes
...
- This component does not render anyth
...
- To display content` ,use `data-content="{value}"` at the `<li>`
...
- To show something when the checkbox is indeterminate, use `swap-indetermin# tab
...
### table
Table can be used to show a list of data inrmat
...
- Color: `textarea-neutral`, `textarea-primary`, `textarea-searea-accent`, `textarea-info`, `textarea-success`, `textarea-warning`, `textarea-error`
