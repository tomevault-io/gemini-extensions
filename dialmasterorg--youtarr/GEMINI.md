...
- `modules/`: class-based singletons holding business logic. Top-level modules include `channelModule`, `downloadModule`, `plexModule`, `jobModule`, `configModule`, `videosModule`, `videoMetadataModule`, `videoSearchModule`, `webSocketServer`, `databaseHealthModule`, `notificationModule`, `channelSettingsModule`, `videoDeletionModule`, `nfoGenerator`, `cronJobs`, `apiKeyModule`, `ytdlpModule`, `messageEmitter`, `playlistModule` (YouTube playlist lifecycle + auto-create source channels; every fetch pages the full playlist via yt-dlp's default webpage path (full pagination needs yt-dlp >= 2026.07.04), capped at 5000 entries, retrying once via InnerTube (`youtubetab:skip=webpage`, which YouTube caps at ~200 for playlist views) when the default path fails or falls short of the playlist's reported count; auto-download uses seed-then-track: the first auto run per playlist downloads the latest N (legacy position window) and stamps `auto_download_baseline_at`, after which runs select the newest undownloaded rows by first-seen `added_at`, ordering-proof against owner-controlled playlist order; playlist removal is a soft delete (`enabled=false`) that hides the playlist from every id-addressed route, and re-subscribing restores it with its saved settings; every fetch ends with `backfillFromDownloadedVideos`, reconciling rows against already-downloaded videos to fill channel attribution and stamp `added_at` with the real download time; an idle auto-download sweep (>=1 auto-enabled playlist checked, nothing enqueued, no errors) records one synthetic Complete 'Playlist Downloads' job with an empty videos array; like other no-video jobs it only appears in Download History when "Show jobs with no videos" is checked), `m3uGenerator` (writes `__playlists__/*.m3u` fallback files; entries prefer the file matching the playlist's Download Type - mp3 for MP3 Only playlists, video otherwise - and fall back to the other so no downloaded item is dropped; entries follow the playlist's saved `sort_order` ('default' = YouTube playlist order, 'reversed' = flipped)), `playlistDownloadGrouper` (resolves per-video download settings for playlist downloads (override -> channel -> playlist -> global) and groups videos by resolved settings), `subfolderModule` (subfolder registry: `getAll` returns the union of the `subfolders` table + config default + Plex-mapping subfolders; `register` idempotently upserts a name; `delete` guards removal against non-empty disk paths and active references), `channelDownloadAllModule` ("download all videos for a channel tab": selects every known never-downloaded channel video (excluding ignored/YouTube-removed/members-only/live, and anything ever downloaded even if since deleted - download-all is not a re-download mechanism), previews count + total duration for the confirmation dialog, and queues one `doSpecificDownloads` job labelled `Channel Download All: <title>`; these jobs are exempt from the 6-hour absolute runtime cap since large channels legitimately take days, with the 30-minute activity timeout remaining as the hang guard), `manualDownloadGrouper` (buckets manually pasted URLs by resolved per-channel resolution/audio format before job creation; per-URL channel attribution prefers the channelId captured during URL validation (server cache merged over the client echo), then videos.channel_id, then channelvideos listers, gated to tracked+enabled channels; multi-bucket pastes share one downloadRunTracker run labeled 'Manual downloads'; file structure is NOT grouped - manual jobs always download nested to temp and the post-processor resolves flat-vs-subfolder per video from its real channel at finalize, so bulk imports are structurally correct regardless of pre-download attribution), `youtubeUrlParser` (pure YouTube URL -> video-id parsing shared by videoValidationModule and manualDownloadGrouper), `playlistVideoFilters` (resolves the playlist videos listing's downloadState/watchedState filters into a youtube_id constraint for the paginated page query; models and watchStatusQueries are passed at call time so the playlists route keeps its dependency injection), `channelSearchModule` (YouTube channel search: YouTube Data API search.list type=channel + channels.list enrichment when a key is configured, yt-dlp channel-filtered results-page fallback otherwise; stamps subscribed from enabled Channel rows; videoCount is API-path only).
- `modules/download/`: download orchestration (`downloadExecutor`, `ytdlpCommandBuilder`, `DownloadProgressMonitor`, `tempPathManager`, `videoMetadataProcessor`, `customArgsParser`, `ytdlpValidator`, `downloadRunTracker` (aggregates the many jobs of one channel+playlist sweep into a single end-of-run summary + notification), `ytdlpEnvBuilder` (yt-dlp process env construction), `DownloadTimeoutController` (per-run activity/absolute timeouts + graceful shutdown), `YtdlpErrorTracker` (per-run ERROR-line classification and failure bookkeeping), `downloadStatusResolver` (pure job-status/final-state decision functions), `downloadResultProcessor` (post-run URL resolution, success/failure partitioning, archive reconciliation), `downloadCleanup` (in-progress video and .part/fragment file cleanup), `downloadCompletionEffects` (post-completion side effects: temp-file/tracking cleanup, poster backfill, playlist hooks, Plex refresh, next-job kickoff), `downloadJobFinalizer` (terminal status persistence, final WebSocket broadcast, run-tracker reporting, notification dispatch), `YtdlpOutputRouter` (per-run stdout/stderr routing + throttled progress emission), `transient403RetryPlanner` (pure decision logic for auto-retrying videos that failed with a transient mid-stream HTTP 403; the finalizer enqueues an "Auto-retry" URL-list job via a callback injected from downloadModule, capped by the `downloadAutoRetryCount` config, and handed-off failures are tagged `autoRetryQueued` so run summaries report the post-retry outcome), `failureAdvisor` (pure registry mapping known persistent-failure signatures (403 with/without cookies, bot check) to plain-language diagnoses; the finalizer stamps a `diagnosisKey` on reportable failed videos and emits a deduped `diagnoses` list that flows into job data, run summaries, the Download History UI, and notification "Likely cause" lines)).
...
- `modules/mediaServers/`: pluggable adapter registry for native playlist sync (`serverRegistry`, `mediaServerSync` orchestrator, `adapters/{plexAdapter,jellyfinAdapter,embyAdapter,baseAdapter}`). Adapters wrap server-specific REST APIs; `mediaServerSync` decides which adapters apply for a given playlist and updates `playlist_sync_state`. Playlists are media-typed by the playlist's Download Type SETTING, not by what is on disk: an MP3 Only playlist syncs as an audio/music playlist using each item's `Video.audioFilePath` (Plex resolves tracks by listing artist sections with type=10), every other playlist syncs `Video.filePath`, in the playlist's saved `sort_order` ('reversed' flips YouTube playlist order); items lacking a file of the playlist's type are left out (count surfaced as `unsyncable_count` on the playlist detail route and shown on the playlist page, but only while the playlist syncs to at least one configured server). With zero resolved items the sync defers instead of creating or emptying a server playlist - the one exception is a deliberately emptied playlist (every video ignored/removed) with an existing server playlist, which is emptied. Audio syncs also trigger a scan of Plex's music sections before resolving (video sections rely on the post-download refresh instead; Jellyfin/Emby refresh all libraries in one call). `watchStatusSync` (pulls per-video, per-user watch state - played/percent/last-watched - from every enabled server via the adapters' `fetchWatchStates()` contract `{ entries, users }`, matches items to Videos primarily by the trailing `[<youtube-id>].<ext>` filename token with exact `filePath` basename as the fallback (a video matches ALL user entries of its best-scoring path; videos marked missing/removed still sync, since their file may have moved out of Youtarr's view but live on in a server library), and upserts `video_watch_status` rows keyed (video, server, user) plus a `media_server_users` name directory; only items actually returned are written, so a missing row means unknown, never unwatched; Jellyfin/Emby list every user via /Users + per-user /Items (full fidelity), Plex reads the owner account from section listings (account id '1'; anonymous session on unclaimed-server setups) and all OTHER accounts from the server-local play history endpoint - played-only fidelity, incremental via a viewedAt watermark from stored rows; per-server all-users toggles `plexWatchStatusAllUsers`/`jellyfinWatchStatusAllUsers`/`embyWatchStatusAllUsers` default on; video files only), `watchStatusQueries` (stateless read side of the watch-status rows: `getWatchedByMap()` aggregates listing-page `watchedBy` honoring `watchStatusWatchedRule` ('any' default / 'primary' = owner + configured user only), `buildWatchedExistsSql()` expresses the same watched rule as a raw-SQL EXISTS fragment for videosModule's paginated listing, and `getStatusesForVideo()` shapes the per-user modal API rows with names from `media_server_users`; the three listing pages filter on watched state through this rule - VideosPage/ChannelPage via a tri-state `watchedFilter` query param (`off`/`only`/`exclude`), the playlist videos route via `watchedState` (`all`/`watched`/`not_watched`), where unwatched always includes unknown/never-synced videos) and `watchStatusScheduler` (config-driven cron: `watchStatusSyncEnabled`/`watchStatusSyncFrequency`, default every 4 hours, reschedules on config change).
...
- `components/`: feature directories and pages. Complex features pair a top-level `FeatureName.tsx` with a same-named `FeatureName/` directory holding `components/`, `hooks/`, and `__tests__/`. Examples of this sibling-file layout: `Subscriptions.tsx` + `Subscriptions/` (renamed from `ChannelManager`), `Configuration.tsx` + `Configuration/`, `ChannelPage.tsx` + `ChannelPage/`, `PlaylistPage.tsx` + `PlaylistPage/`. Newer features (e.g. `SubscriptionImport/`) put the main component at `FeatureName/index.tsx` instead; either layout is acceptable for new features.
- `components/shared/`: reusable components used across multiple features (e.g. `VideoModal/` for the video detail modal, `ThumbnailClickOverlay` for clickable thumbnail hotspots, `DeleteVideosDialog`, `AddChannelDialog` for confirming a channel add from search results or an unsubscribed channel name click).
...
- `components/Settings/`: Settings page wrapper and splash index (`SettingsIndex.tsx`) listing the per-section routes under `/settings/<key>`. `MaintenanceSection.tsx` hosts the manual filesystem rescan trigger. `/settings/watch-status` routes to `Configuration/sections/WatchStatusSection.tsx`, which surfaces the sync toggle/frequency and a manual "Sync Now" trigger.
- `components/FindChannels/`: channel search page at `/subscriptions/find`; clicking an unsubscribed card opens the shared `AddChannelDialog`, whose confirm hands off to `Subscriptions` via `location.state.addChannelUrl` and auto-runs the add flow.
...
- **File size targets for new files**: backend modules under ~500 lines, frontend components under ~300 lines. Several legacy files exceed these limits (notably `jobModule.js`, `channelSettingsModule.js`, `videoDeletionModule.js`, `VideosPage.tsx`, `ChannelVideos.tsx`, `DownloadProgress.tsx`) - do not use them as examples. The codebase is migrating toward these targets over time.
