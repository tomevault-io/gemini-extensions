...
```bash
# Run the enhanced init script (fixes upstream issues)
./scripts/claude-flow-init.sh

# After restarting Claude desktop, verify connections
./scripts/claude-flow-init.sh --verify

# Load convenience aliases (optional)
source scripts/aliases.sh
```

**What the init script does:**
- ✅ Runs `claude-flow init`
- ✅ Creates missing `.claude/sparc-modes.json`
- ✅ Fixes MCP server version mismatches
- ✅ Configures `claude-flow`, `ruv-swarm`, and `flow-nexus`
...
See [docs/claude-flow-init-guide.md](docs/claude-flow-init-guide.md) for detailed documentation.
