...
**数据准备**（Python）：
1. 获取本地最新行情快照：`TdxClient.get_realtime_quote('代码')`
2. 获取 K 线数据：`TdxClient.get_daily()`
...
from modules.tdx_client import TdxClient
...
**指标工具**：
- `TdxClient.get_realtime_quote('代码')` → 本地最新收盘、涨跌幅
- `TdxClient.get_daily()` → 日线 OHLCV
- `analyze_stock(ts_code, days)` → MACD/KDJ/RSI/布林带/砖形图等
- 财务、资金流、涨停列表不再走外部行情 API；需要时先说明本地 TDX 数据源不覆盖这些维度，再用公开资料辅助判断。
