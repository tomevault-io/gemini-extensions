...
- `packages/next-openapi-gen` CLI bundling in `tsup.config.ts` must externalize non-workspace runtime dependencies; bundling CommonJS packages like `commander` into the ESM CLI breaks sample-app `generate` with Node's `Dynamic require` error. A committed bin wrapper at `packages/next-openapi-gen/bin/cli.mjs` delegates to the built CLI so the `next-openapi-gen` / `openapi-gen` commands resolve in clean checkouts before any workspace build runs. Playwright e2e (`playwright.config.ts`) prebuilds the CLI workspace graph with `pnpm exec turbo run build --filter=next-openapi-gen...` so internal packages have `dist/` outputs.
