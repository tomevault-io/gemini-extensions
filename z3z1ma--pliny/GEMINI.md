...
### 6. Keep Pliny as the Index

Pliny remains the index even when work occurs through other tools, skills, workflows, conversations, or documents.

If anything outside `.pliny/` has the shape or force of a specification, plan, ticket, decision, evidence record, review, research finding, skill, or durable knowledge record, create the corresponding Pliny record as soon as it exists. Capture plans as parent tickets. The external artifact may remain canonical. In that case, the Pliny record may be thin: include its status headers, enough context to classify it, and a durable pointer to the canonical source.

The Pliny record is still mandatory. Facts that exist only in chat, tool output, external documents, or subagent reports are invisible to the project's durable memory.
...
Records live under `.pliny/` in directories named by type. Each record is a Markdown file. Active work remains at the top level of its type directory; terminal states move into their designated subdirectories.
...
Store source materials—PDFs, papers, exported pages, and datasets—in `.pliny/research/.storage/` and reference them by file path from the research record.
...
Store binary artifacts—screenshots, recordings, exported files, and build outputs—in `.pliny/evidence/.storage/` and reference them by file path from the evidence record.
...
Skills must be strictly self-contained. To preserve modularity and prevent execution drift, a skill must not reference other `.pliny/` record categories. The sole exception is a `knowledge` record used for shared vocabulary.

Skills are distilled operational memory: repeated friction converted into institutional capability.

Skills are the only Pliny records that do not use the common text headers. Use YAML frontmatter with exactly these fields:
...
- This repo ships Pliny: a Markdown protocol for project memory and execution discipline.
- The protocol lives in `PROTOCOL.md`. Users copy-paste it into their agent instructions.
- `skills/pliny/SKILL.md` is a copy of `PROTOCOL.md` with frontmatter for the skills ecosystem (`npx skills add z3z1ma/pliny`).
...
- `.pliny/` is this repo's own dogfood workspace — it is not part of the shipped product surface.
- `pliny-atrium/` is a companion web application for visualizing and interacting with `.pliny/` records.

## Pliny Atrium Dev Servers

Human dogfood defaults are reserved: backend `127.0.0.1:8765`, frontend
`127.0.0.1:5173`, and the committed `pliny-atrium/frontend/.env.development`
pointing at that backend. These defaults are for `npm run dev` in the frontend
only. Agents MUST NOT rely on that file.

CRITICAL: When starting Atrium dev servers, ALWAYS use nohup with full stdout/stderr
...
CRITICAL: Agents MUST use random non-default ports exclusively. Ports `5173` and
`8765` are reserved for an external developer dogfooding Pliny Atrium. NEVER start
servers on those ports, and NEVER kill processes on those ports. Do not assume a
process on a default port belongs to you.

CRITICAL: Agents MUST NOT run bare `npm run dev`. The committed frontend
`.env.development` is human-default-only; every agent frontend run MUST override
it with explicit random `VITE_API_URL` and `VITE_WS_URL` values that point at the
agent's tracked backend port.

CRITICAL: Track every port and PID you allocate in a run-local port file so you
can clean up only your own servers. Never kill by broad process name (`pkill
vite`, `pkill uvicorn`) and never kill an untracked port. Multiple agents may be
running verification at the same time.

Choose ports and create a run-local tracking file:

```bash
RUN_ID="pliny-atrium-${USER:-agent}-$$-$(date +%s)"
PORT_FILE="/tmp/${RUN_ID}.ports"
...
printf 'BACKEND_PORT=%s\nFRONTEND_PORT=%s\n' "$BACKEND_PORT" "$FRONTEND_PORT" > "$PORT_FILE"
```

If you start backend/frontend from separate shells or tool calls, keep the
`PORT_FILE` path in your notes and reload it before using the variables:

```bash
set -a; . "$PORT_FILE"; set +a
```

Start backend:

```bash
nohup uv run python -m uvicorn pliny_atrium.app:app --host 127.0.0.1 --port "$BACKEND_PORT" > "/tmp/${RUN_ID}-backend.log" 2>&1 < /dev/null &
printf 'BACKEND_PID=%s\n' "$!" >> "$PORT_FILE"
```
(Run from `pliny-atrium/` directory)

Start frontend:

```bash
nohup env VITE_API_URL="http://127.0.0.1:${BACKEND_PORT}" VITE_WS_URL="ws://127.0.0.1:${BACKEND_PORT}" npm run dev -- --host 127.0.0.1 --port "$FRONTEND_PORT" --strictPort > "/tmp/${RUN_ID}-vite.log" 2>&1 < /dev/null &
printf 'FRONTEND_PID=%s\n' "$!" >> "$PORT_FILE"
```
(Run from `pliny-atrium/frontend/` directory)
...
- Start servers on default ports `5173` or `8765`
- Kill ports `5173` or `8765`
- Run bare `npm run dev` or rely on `pliny-atrium/frontend/.env.development`
- Kill a port or PID that is not in your run-local port file
- Use broad process-name cleanup such as `pkill -f vite` or `pkill -f uvicorn`
- Run two subagents that both try to start servers (coordinate or share)

## Editing Checks

- When changing `PROTOCOL.md`, verify the `.pliny/` directory structure and existing records still conform.
