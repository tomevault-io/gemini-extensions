...
**v5.0.0 is the current version.** v5.0 reframes the cockpit as a research-action monitor. Top-of-screen **phase strip** shows the derived current phase (`idle / explore / select / experiment / verify / prove / review / narrate`) over the last 200 events, with anti-flicker. The main grid pane is now **ActivityPane** which aggregates `cockpit_events` into per-action cards (graph / bt / verify / prove / lean / intervention / narrate / risk families) with critical → low severity bands. The right-side tabs gain a **Focus** tab as the first cross-group entry, derived live from the same event tail. The original EventStreamPane is preserved verbatim as a collapsible bottom-docked **audit log** (toggle with `A`). Two new optional MCP atomic tools — `cockpit__set_phase(phase, focus_nodes, intent)` and `cockpit__narrate(text, scope)` — give SOP authors a sanctioned channel for surfacing decision context without inflating the event vocab. Per ADR 0007, both tools are descriptive: agents may skip them and the cockpit derives meaning regardless. Settings additions: `phase_strip_visible` (key `P`) and `animations_enabled` (key `M`). Legacy `focused_pane="events"` is healed to `"activity"` on load. No schema migration. See [ADR 0011](docs/adr/0011-cockpit-activity-streaming.md) and [architecture.md §14](docs/architecture.md#14-cockpit-activity-streaming-v50).
