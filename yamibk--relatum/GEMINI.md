...
| `assets/note-live-editor.js` | 基于离线 CodeMirror 6 / Lezer 的源码保真 Markdown Live Preview；负责分帧补齐当前视口语法树、可见富块增量重扫、Obsidian 式源码标记染色、围栏代码语言高亮与语言标签复制、当前语法单元显露、Callout/任务/双链/图片/行内与块公式/Mermaid/表格/derive 组件、IME、括号自动闭合、最高优先级空引用退出、加粗/围栏代码等快捷键与编辑器快照接口，并向阅读模式提供同一安全 Markdown/本地资源渲染入口。IME 以 CodeMirror 原生 composition 状态为准，由 `idle / composing / settling` 单一会话协调：候选输入期间只映射已有投影，禁止重新解析和保存预编辑拼音，结束后只重扫一次可见区；同步快照始终返回最后已提交文档，工作区保存和切页通过 `whenInputSettled()` 等待最终候选。笔记图片在 Live Preview 中始终保持可视投影，独占行图片贴齐正文左缘；点击选中隐藏源码范围，右下角手柄等比例缩放并在松手时用单次 CodeMirror 事务写回 Obsidian 像素宽度语法；缩放预览统一启用显式宽度，让原始无尺寸图片也与外框同步，取消恢复原始尺寸约束。图片文字编辑及切框等待期间拦截缩放且不提交或打断草稿，编辑时角点变淡并显示禁用光标，退出文字编辑后恢复缩放。图片与未激活富块、行内图片/公式使用原子光标范围；点击渲染公式会显露源码并把空光标放进定界符内。独占行本地图片还可在显式“图片文字”模式中创建、编辑和拖动多个纯文字框；图片文字 textarea 自己等待 compositionend，候选窗触发 blur 时不得提交拉丁预编辑串；若明确点击图片内外（同一图片上的另一个文字框除外），则在指针按下时同步冻结当时可见值（如 `t`）并立即隐藏 textarea/工具栏，但保留原生 textarea 到 compositionend 后再提交，避免 WebView2 丢失预编辑值；编辑中单击另一个文字框改为同步提交当前文字、保持图片文字模式与工具栏，并在提交后通过当前适配器按 ID 定位被点文字框，直接进入编辑且光标放在末尾；切换按下时用当前草稿同步恢复旧框展示层，覆盖指针释放、输入法结束与提交等待期间，避免隐藏输入框后文字空白闪烁，提交事务自身的重钉选中不得把它抢回原框；组合输入中该次提交延后到 compositionend，且已排定的提交不得被 `cancelDraft()` 丢弃；`Esc` 取消候选不得把它重新保存。显式打开的图片文字工具栏会固定绑定当前图片：焦点变化或富块重绘导致的短暂选区漂移只恢复选区，不得自动关闭。编辑态由同样式隐藏文字镜像实时测量宽高，展示态与编辑态必须保持同一中心、字号、行高和内容尺寸；输入、改字号、拖动和键盘微调都把完整文字框约束在图片范围内。选中文字后直接打字或粘贴在末尾追加并进入编辑，原生输入及事务保护不得让正文选区覆盖隐藏图片源码；清空已有框提交会删除对应元数据，新建空框不保存，Esc 取消编辑；编辑框内右键提交当前可见文字并退出编辑，保留对象选中与工具栏，不弹原生菜单，组合输入等待 compositionend 后提交；选中态 Delete/Backspace 删除整框并保留撤销，最后一框删除移除完整注释，撤销/重做需同步图片范围。选中文字后支持 `Enter` 编辑、方向键 1px 微调和 `Shift+方向键` 10px 微调。平时文字层只显示不命中，每个视图共用一个 `ResizeObserver` 同步比例字号，每次内容/位置/样式修改只提交一次事务。只有源码模式显示完整原语法。实时预览与源码模式通过 CodeMirror `Compartment` 原位重配置，不能重建文档状态或清空撤销历史。Markdown 字符串始终是唯一真源，原始 HTML/SVG 只显示源码。 |
| `assets/vendor/codemirror/` | 固定版本的 CodeMirror 6 / Lezer 离线浏览器 bundle、完整依赖锁和第三方许可证；只在笔记工作区首次交互或空闲预热时加载，运行和桌面打包均不需要 Node/npm 或网络。 |
| `assets/editor.js` | 编辑器页面编排：加载/保存、模式切换、模板、导出、背景、AI/图谱入口和参考画布父页协调；标题栏不显示“未保存 / 正在保存 / 已保存”文字，但脏状态、自动保存与关闭提醒逻辑保持运行；未使用附件清理先显示中英文自定义确认浮窗，明确删除范围与不可撤销提示，支持 Esc、遮罩取消和键盘焦点闭环，执行期间锁定操作。主画布坐标模式开启时由本机偏好临时接管辅助底纹：保留全局底纹选择但隐藏视口/顶栏底纹并禁用背景面板入口，关闭坐标后原样恢复，不写背景 API。 |
...
| `assets/i18n.js` | 起始页与编辑器共用的界面语言层；归并动态变更并按新增 UI 子树翻译，整棵跳过正文、SVG 几何和小地图节点；输入提示保留翻译但不改 value。全局 lang/语言控件只在初始化和切换语言时同步，不能因动态节点重复查询全局控件或重写 lang。 |
| `assets/tooltip.js` | 全局自定义说明框层；接管静态与动态 `title`，同步中英文文案，并处理悬停/键盘焦点与视口避让。 |
| `assets/canvas.js` | 核心画布引擎，节点/边/手写/附件/批注/选择/历史/导图/独立表格/AI 计划执行和双屏选区导入导出。主画布用独立高 DPI Canvas2D 坐标层绘制随相机移动的 1/2/5 自适应方格、数学方向 x/y 轴与可选数字；`canvas:coordinatesVisible:v1`、`canvas:axisOpacity:v1`、`canvas:coordinateLabelsVisible:v1` 只存本机，原点相机保留缩放并把 `(0,0)` 移到视口中心。左侧直线工具共用同一入口创建纯直线或终点箭头，两者都可在选中后添加转折点；新建对象在 `ink.arrows[]` 中用 `arrowhead:"none"|"end"` 显式保存端点类型。 |
| `assets/canvas-changes.js` | 无 DOM 的内部变更集合与记录恢复层；比较约 50 步完整历史快照，区分位置、内容、结构与任务簿/墨迹/尺子/计时器变化，恢复时保留记录对象身份并深拷贝可变字段。不改 HTTP API 或 `.canvas` 格式，主编辑器和参考查看器在 canvas.js 前加载。 |
...
| `assets/study-palette.js` | 目标树阶段、学习任务卡片与记账卡片共用的 12 色主题色库和浮层控制器（单一事实来源）；持久化的十六进制值保持兼容。浅色 `light` 使用柔和粉彩并保留暖金 `#fafec3`；蔷薇、青瓷、雾蓝分别使用较明显的玫瑰粉、青绿与长春花蓝，拉开与赤霞、薄荷、天空的差异。`lightAccent` 独立保存描边与根节点的浅色主题强调色。深色 `dark` 使用略带粉彩感、清透柔和的完整彩色卡面；字体为“智能”时按实际混合表面动态校准到至少 4.5:1，也允许用户按页强制纯白或纯黑。树状页与学习页各自的底色、边框/侧条强度和字体只影响深色卡片，不影响调色盘、图例、功能进度和完成材质。调色盘与树状页/学习卡片共用映射，深色调色盘用亮色选择环。`window.RelatumStudyPalette` 提供 `COLORS`、`toneFor()`、`applyColorTones()` 与 `createPopoverController()`，统一 4×3 DOM、主题色映射、定位、焦点恢复和出入场，不写用户内容。 |
| `assets/tree-page.js` | 起步页独立“树状页”的交互层；直接使用 `study-goal-tree.js` 的目标树模型，并以 `study-route.js` 为运行时母版保留布局、常驻 DOM、FLIP、整枝拖动、相机、编号栏、弹层与教程手感。它只调用树状页独立 API，另加卡片形状、每树自由文本/重点便签层，以及不进入共享模型和布局的右键拖拽视觉关系线，不读取学习页数据，也不提供接入已有任务。新建重点便签与文本框的正文均为空；便签纸张始终为固定黄色，工具栏 10 个色块只改变文字颜色，无历史偏好时默认选中最右侧黑色。普通文本框的垂直居中由外层 `.decor-content` 承担，可编辑 `.text-box-content` 保持普通块级行盒，不得恢复为空内容时光标与输入后文字使用不同垂直定位的 Flex 编辑容器。文本溢出仍允许滚轮滚动，但隐藏 WebView2 原生滚动条外观。KoseFont 与树状页数据共用现有空闲预载；若首次打开时字体仍未就绪，必须等字体 Promise 完成后才一次性渲染权威数据，不允许先显示备用字体再跳变。对象在首次编辑中开始拖动或缩放时，必须先将创建命令排入独立请求队列，再持久化几何变更。自由对象跟随树状页镜头但不参与自动布局或“适应”边界，文本框需要时才通过 `font-loader.js` 加载 KoseFont。树状页调用共享目标树模型时必须显式启用空根标题；学习页仍保留“我的学习路线”的缺省名，不能用共享规范化再把树状页的空标题回填。任务与黑色根节点的进度条复用学习任务的宽度/颜色缓动、达标材质揭示、有限光流与低动态降级；有量化进度的任务会常驻固定宽度的控制槽和独立完成材质层，手动勾选不改真实数值，而从当前比例揭示到持久 100% 金色完成态，取消勾选时金色材质从 100% 收束回真实比例后重新露出原进度；未设置量化进度时卡片不显示“设置进度”占位文字。右侧数字栏支持滚轮翻页：编号与滑块即时追随，画面只追最新目标，持久化在后台串行合并；深色主题使用高不透明度墨色栏面与暖白活动滑块，不继续沿用浅色玻璃底。退场期间同方向滚动只替换透明点目标，中间树不渲染；反向或入场中改向则继承当前帧连续转向，最终树保留完整入场，写盘延迟不阻塞动画。关闭后在途写入仍需追到最终目标。重建编号栏时必须先无过渡钉住黑色滑块，不能短暂闪回第 1 页。卡片右键调色盘覆盖卡片内部的文字与按钮子控件，里程碑仍不提供外观菜单。树状页只适配卡片尺寸和独立请求队列。裸按 `Tab` 会按本机偏好开合右侧进度明细面板；任务选择按树记忆，空白点击清除。面板只派生量化进度的 `1..target` 行，超过 200 行时使用固定行高窗口化；方框完成态只取决于数值进度，双击卡片只编辑既有 `progress.milestones[]` 名称。名称按 350ms 合并并串行写入 `set-progress-point-name`；切树、离页或关面板只冲刷队列不等待。缩短或取消进度会原子清理越界名称：附加里程碑依赖删除，主路线依赖降级为整任务完成。 |
| `assets/study-graph.js` | 活跃页/学习页星图，可视化学习活动和任务结构。 |
| `assets/sticky-palette.js` | 速记墙、起步页跨页便签与画布便签共用的20色色库和随机候选偏好；负责按色系均衡抽色，不写用户内容。 |
| `assets/notes.js` | 起步页速记墙，独立便签数据、拖拽、连线、箭头、归档与按纯文本视觉长度分档的正文排版。控制台“？”旁的圆形勾选按钮只禁用“创建与连接”和“浏览与搜索”两组键盘快捷键，偏好保存在 `canvas:notesCreateBrowseShortcutsDisabled:v1`，缺失时默认禁用；禁用状态下画布上的 `Tab` / `Shift+Tab` 会被完整吞掉，不触发创建，也不执行浏览器默认焦点跳转。起步页启动后会在空闲时间读取并隐藏预渲染速记墙，首次进入时只校准受 `content-visibility` 影响的连线几何；若用户先进入则复用同一加载任务。 |
| `assets/start-sticky-notes.js` | 起步页跨页便签：安全空白创建、纯文本编辑、轻量拖动、键盘换色/旋转/删除。 |
| `assets/calendar.js` | 日历页“日历 / 记账”双视图协调层，以及日历、日记、右侧三栏（画布活动 / 每日打卡 / 学习任务）、倒数日与倒数日进度条。启动空闲时只预取首次打开所需的当天 JSON，进入时复用同一请求并开始渲染与入场；已在日历页时重复点击书脊切换内部视图，两个视图使用固定叠层并同步 `hidden` / `inert` / `aria-hidden`。日历内部错峰入场未完成时离页会冻结 CSS / WAAPI 当前帧，外层退场隐藏后再清理。 |
| `assets/ledger.js`、`assets/ledger-model.js` | 日历页第二视图的极简人民币流水：无 DOM 模型层用 BigInt 定点运算统一金额解析、四位十进制倍率、半入舍入、筛选与汇总；页壳、日期组和账目卡使用常驻 Map 索引、渲染指纹与按 ID 增量同步。右缘页栏提供 1–99 虚拟数字页，月份在页间共享；账本一次全量加载后由本地按页/月派生，切页即时完成并保留未保存草稿。整版错峰入场未完成时切换到日历视图或离开日历页，会先冻结当前动画帧，等交接退场结束后再清理。每页可在当前页设置中独立记忆新建账目的默认“支出/收入”，未设置时默认支出；点击 `＋` 只按该偏好创建当前页本地草稿。保存有效金额时编辑框立即关闭并把带稳定客户端 ID 的乐观账目放入列表，后端在后台落盘，失败时只撤回该乐观条目并恢复为可重试草稿。切月或离页丢弃尚未提交的草稿。写接口只回 revision 与目标增量；同账目请求串行，不同账目只合并各自响应，失败只回滚目标并在修订跳号或结果不确定时合并重载。账目卡使用并列的主编辑按钮和选项按钮，不嵌套交互控件。仍不包含预算、趋势、分类、搜索、筛选、批量、多账户、多币种或周期账单。 |
| `assets/countdown.html`、`assets/countdown.js` | 独立倒数日页面；事件管理、轻量翻页时钟、空状态、返回日历过渡与桌面背景只读模式。 |
| `assets/review.js` | 独立复习卡片页面，负责计划复习、无限随机自由复习、卡片库、卡组/标签、批量管理和评分。启动空闲时只预取卡池 JSON，首次进入复用该请求后再生成卡片并按需加载公式运行时。 |
| `assets/focus.js` | 专注钟、正计时/番茄钟、每日任务绑定、音效/噪音、记录编辑。每日任务新增控制条展开与收起均播放过渡，低动态偏好下即时切换；每日任务视图错峰入场中离页时冻结当前帧，外层退场后再清理；计时器视图保持原有退场行为。 |
| `assets/trash.js` | 回收站页面，按目标分组恢复、键盘恢复、一键清空确认。 |
| `assets/desktop-shell.js` | 前端到 pywebview 的桥接、窗口按钮、dirty 标记；桌面关闭按钮支持注册异步保存前置钩子。笔记专注模式收起顶栏后，桌面壳在右上角提供浮动最小化、最大化／还原和关闭按钮，复用原顶栏的窗口操作；关闭仍经过笔记保存前置钩子，连续点击只执行一次在途关闭。 |
| `assets/styles.css` | 全局视觉系统和所有页面样式；顶层“画布 / 笔记 / 生涯”工作区以固定公共底景上的双层淡化与水平位移交接，进入层始终覆盖离开层，避免沉浸模式亮度跳变和整屏三维变换卡顿，并尊重低动态偏好。深色“沉浸”主页下，独立树状页与速记页的全屏台面保持透明，让公共极光背景透出；“简洁”仍使用实色暗场，节点、侧栏与弹层继续使用高不透明度墨色表面。 |
...
| 记账流水 | `data/ledger.json`（v2）；顶层保存单调递增安全整数 `revision`、严格 `entries[]` 和 `pageUnits`。每笔必须保存 `id/type/amountCents/ledgerPage/date/note/color/createdAt/updatedAt`，可选 `multiplier` 是规范十进制字符串；`ledgerPage` 只接受 1–99。金额为正整数分、倍率留空按 1，非空大于 0、不超过 1000000 且最多四位小数，最终金额按十进制半入舍入。文件内非法账目、重复 ID、非法 revision、页键或单位均视为整本损坏，不静默丢条目；主文件隔离后尝试恢复严格有效的备份。读取层 `OSError` 直接上报，不把暂时不可读误判为内容损坏。缺失时返回未落盘空 v2；有效写入前维护上一份有效备份；不兼容版本只报错且不覆盖。 |
...
- 笔记工作区处于前台且页面可见时，以 2 秒起步、失败后最长 30 秒退避的串行轻量轮询读取最新文件树元数据；离页或隐藏立即停止，回焦、恢复可见与重新进入时立即复检。路径、名称、类型、子级结构或当前排序下的可见顺序变化才重建文件树 DOM；仅修改时间/大小但顺序不变时不重建行，元数据未变也不重读当前正文或重复预取。自动复检失败保持现有树并静默退避，右键菜单手动刷新才显示结果。`tests/note-view-state-browser.js` 覆盖无变化零 DOM 改动、自动外部增删、非活动停轮询、修改时间变化、重命名焦点和错误恢复。

- 笔记库左侧齿轮的“恢复默认”经面板内二次确认后只移除 `canvas:noteTreeSort:v1` 与 `canvas:noteNewName:v1` 两项本机偏好，并立即恢复文件树“修改时间从新到旧”和新建笔记本地时间戳命名；已有 Markdown 文件、右侧笔记设置和专注状态都不改。排序实际变化时才重建文件树，取消确认或按 Esc 不重置。

- 编辑位置独立于 24 篇正文缓存，使用本机 `canvas:noteViewStates:v1` 保存最近 200 篇笔记的 `[库内相对路径, {anchor, head, scrollTop}]`，不保存正文。正常保存与切文档边界更新位置，750ms 合并写入；窗口失焦、页面隐藏/卸载、离开工作区及桌面关闭前同步冲刷位置偏好。重启、关闭标签后重开、刷新失效或缓存淘汰后的重读均恢复选区和滚动；位置按新正文长度夹紧，反向选区的 `head:0` 必须保留。文件/文件夹成功移动后迁移位置键，失败不迁移；新建文件即使复用旧名字也从头开始。损坏偏好忽略，存储失败不阻断正文保存。不在窗口每次获得焦点时强制改写选区。

- 笔记库是起步页顶层“笔记”工作区，不是画布内 `markdownNotebook` 笔记坞，也不读取 `data/notes.json` 速记墙。默认只显示文件树与多标签编辑区，所有标签共用一个 CodeMirror 实例与现有保存链；从“画布”或“生涯”返回且当前已有笔记时，下一帧重新聚焦共享编辑表面，让 CodeMirror 保留的光标立即可见，不重建文档状态；链接/历史从右侧覆盖展开而不挤压正文，不建立常驻第三列。
- 所有笔记 API 只接受相对 `notes/` 的 POSIX 路径。绝对路径、反斜杠、`.` / `..`、Windows 保留名、大小写冲突、内部 `.relatum-*`、符号链接和 Windows 重解析点都拒绝；不能处理笔记根。图片 `src` 可在解析后仍留在库内的前提下使用 `../`。外部直接复制、删除或重命名 `.md`/文件夹由前台轮询自动同步；删除是磁盘事实，会静默清理整棵子树对应的标签与缓存并就近切到有效标签。右键“移到系统回收站”必须先在点击帧乐观移除树行，再异步冲刷正文并调用后端；后端失败才恢复原树与展开/选择状态。已加载笔记的保存若发现目标消失必须返回 `404/not_found`，前端停止重试，不能用尚未落盘的编辑器内容复活文件。
...
- 输入法候选期间，编辑范围仍只取决于选区，不能把 composing 当成整篇显露源码的开关。行内投影与富块保留既有显示，仅随原生输入映射位置；视口强制解析和富块重扫暂停，输入完成后再补齐。候选文字使用原生光标，暂时隐藏跟随已提交选区的绘制光标，避免光标停在拼音前面。结束回调必须等待原生输入提交，并校验输入批次与文档批次；新一轮输入、切笔记和销毁取消旧回调，失焦负责收尾。源码/实时预览切换在候选提交后执行，不能中途重配编辑表面。
- 笔记正文基线为 16px / 1.65 行高，最大有效行宽 880px，宽窗左边距最多 112px；正文和行内文件标题优先使用系统 UI 字体栈，Noto Sans SC / 微软雅黑只作跨机器后备，且禁用伪造字重。实时预览与阅读模式的代码块使用相对正文 .94em 字号、1.65 行高和轻中性底色；代码与源码模式共用带明确中文无衬线后备的 `--note-font-mono`，避免中文注释落到默认衬线字体。行内文件标题保持 32px，Markdown H1 为 1.75em 并逐级收敛，不能让 H1 与文件标题同大；齿轮中的百分比滑条只缩放正文与行内文件标题。文件路径栏保持固定，行内文件标题及其顶部占位属于正文滚动内容：滚动时通过共享编辑表面的 `scrollTop` 同步移出并裁切，不能常驻挤压正文。词数/字符数统计定位为正文右下角的无交互悬浮层，不得作为 flex 子项独占一行；图片文字按解码后的真实可见文本计数，不计 Base64 元数据。普通输入的字符数可用 CodeMirror 文档长度即时更新，词数在 350ms 快照或其他既有保存边界更新，不能为了统计恢复逐键全文 `toString()`。
- 普通代码块的行装饰只覆盖当前可见范围，由保护区遍历统一生成；Markdown 投影只处理围栏的直接定界符子节点，不重复遍历整块代码行或语言语法树。富块扫描先按语言与长度判断，普通代码不提取整段源码；语言标签保留不可变文档引用，只在实际点击复制时读取正文，未闭合围栏不得丢弃最后一行。无序任务项只显示复选框，不额外显示圆点；有序任务仍保留编号，进入编辑后完整显露原始标记。
- 左侧齿轮管理文件树排序与新建命名，右侧齿轮继续管理正文外观和快捷键。排序按钮代替工具栏刷新，提供名称、修改时间、创建时间各正反序；默认修改时间从新到旧，文件夹始终在前并按名称升序，各目录内笔记分别排序。后端树项提供 `createdNs`，本机 `canvas:noteTreeSort:v1` 保存排序；弹层仅做短促透明度/位移过渡，低动态无动画，文件树重排无逐项动画。`canvas:noteNewName:v1` 保存时间戳或自定义基础名；普通新建统一走 `/api/note-create` 的自动命名，重名加 `-2/-3`，指定名称创建和重命名仍拒绝重名。创建接口直接回传空白正文和修订号，不追加一次 GET；普通新建会聚焦并全选正文上方的行内文件标题，用户改名时只移动 `.md` 与伴生素材，不把标题写入 Markdown 正文，重名返回浮动警告并保留原名。新建文件夹、`F2` 和右键重命名继续共用树内输入。文件树使用常驻路径索引更新选中态，目录展开、结构或排序变化才重建树；展开子目录必须保留缩进和竖向层级线，正文顶栏用 `notes › 文件夹 › 文件.md` 面包屑显示归属。文件树内拖放乐观移动，Windows Explorer 拖入使用 begin/upload/commit/abort 暂存会话复制 Markdown、目录和安全栅格图片。
...
- 起步页：当前分组、主题、背景风格（`canvas:startBackgroundStyle`，unset 时默认“简洁”）、翻页速度、速记页拖拽惯性、叠放展开延迟、日历倒数日开关、生涯揭示等待（`canvas:careerScrollIdleMs:v1`，20–160ms，默认 50ms，生涯页滚动停稳到批量揭示的等待）、学习/树状/速记前台计时开关（`canvas:startPageActivityEnabled:v1`，**默认开启**；显式存为 `'0'` 时关闭）及绿色附加统计显示开关（`canvas:startPageActivityStatsVisible:v1`，新用户默认关闭）；两项独立保存 `'1'` / `'0'`，旧用户缺少显示偏好时仍继承其已有计时开关一次并立即保存，之后互不影响。画布名称搜索开关（`canvas:librarySearchEnabled`，**默认关闭**：只有显式存为 `'1'` 才在画布列表标题旁显示搜索工具）、树状页根标题隐藏开关（`canvas:treePageRootTitleHidden:v1`，**默认关闭**：仅显式 `'1'` 隐藏）与根标题字号（`canvas:treePageRootTitleSize:v1`，16–36px，默认 25px；滑条在 45% 位置显示灰色默认值提示线）、隐藏特殊页开关（`canvas:hideSpecialPages`，**默认关闭**：只有显式存为 `'1'` 才隐藏，unset 或其他值都正常显示全部页面；开启后书脊只留最近/收藏/分组，滚轮翻页与点击都不进 7 张前置页）、本地数据说明看过状态 `canvas:dataGuideClicked:v1`。左上角齿轮设置面板展开和收起使用方向一致的有限缩放/淡入淡出，收起期间立即停止交互，快速重复点击可反向重开；低动态偏好下立即完成。齿轮面板顶部标题行「设置」右侧提供整面板「恢复默认」：只删除本面板各项偏好键（主页背景、足迹星图动画、日历倒数日、学习/树状/速记计时与统计显示、隐藏特殊页、精简目标树编辑、强制按解锁顺序、隐藏根节点标题与根标题字号、画布名称搜索、翻页速度、生涯滚动手感与揭示等待），键缺失即出厂默认，随后各控件、CSS 变量与运行状态立即刷回默认；不写回默认值，也不触碰主题、语言、当前分组等面板外偏好。左下角 `?` 打开中英文各四页的本地数据说明，解释 `canvases/`、`notes/`、`data/` 中各类文件和浏览器/WebView2 本机偏好的用途、删除后果与安全清理顺序；其中将独立树状页的 `tree-page.json` 与学习页的 `study.json` 明确区分，并说明任务页颜色、树镜头和未完成专注计时等只存在本机的状态。说明中不展示盘符、用户名或固定绝对路径，仅使用相对数据目录和便携式存储名称。它不再承载旧版功能教程。说明浮层打开和关闭都使用一次有限过渡，低动态偏好下立即完成；除了细书脊本身，还把正文滚动区最左侧约 64–88px 作为滚轮翻页热区，右侧其余区域继续纵向惯性滚动，且不通过扩大书脊 DOM 遮罩阻挡顶部目录按钮。右侧正文滚动条使用无原生上下按钮的细圆角滑块，深浅主题分别控制对比度。三类便签共用 `canvas:stickyPalette:v1`，以 `{version:1,disabled:[]}` 只保存不参与随机生成/换色的色名；缺失或非法值启用全部20色。全部禁用会保留为控制台“视觉全不选”状态，但随机生成/换色仍安全使用全部20色；新颜色未来默认加入，不写 `.canvas` 或便签数据。
...
- 日历页使用 `calendar:viewMode:v1` 记住 `calendar` / `ledger`，缺失或非法时默认日历；已在该页时重复点击“历”切换。记账当前月份只存在 `ledger.js` 本次文档会话内：同一会话离页再回来保持刚查看月份，应用重新启动后回到本月。当前数字页用 `ledger:page:v1` 保存在 `localStorage`，只接受 1–99，非法时回到第 1 页；数字页共用当前月份。月份/累计视图按页独立，用 `ledger:viewByPage:v1` 的 `{version:1,views:{页码:'cumulative'}}` 只保存显式设为累计的页，未记录的页（含全部月份页）缺失或非法时默认 `month`，删除整个键后每页都恢复月份视图；累计视图不改写原账目日期。隐藏小数点同样按页独立，用 `ledger:hideDecimalsByPage:v1` 的 `{version:1,pages:{页码:true}}` 只保存开启的页，未记录的页显示完整小数；只改变显示（账目行与汇总直接截断为整数，不四舍五入），编辑弹层仍可输入两位小数，账目金额数据（分）不变。记账页四个无文字图例色块用 `ledger:legend:v1` 的 `{version:1,colors:[4 个空串或 #rrggbb]}` 保存在 `localStorage`，只作视觉备忘且不筛选账目；单笔颜色存在 `ledger.json`，收入/支出仍必须用文字和金额正负号表达。账目行或图例色块已经打开调色盘时，再次在同一项上右键会收起调色盘；右键另一项则直接切换到新项。重复点击已经展开设置的账目行或三点按钮会自然收起弹层。删除当前视图最后一条流水或本地待填写草稿时，账目退场完成后再以分层淡入揭示空状态；页设置改变视图、金额显示或单位时复用数字页原位交叉淡化；低动态偏好下立即切换。
...
- 日历、记账与倒数日：`/api/calendar`、`/api/ledger`、`/api/countdown`；记账 GET 不带参数，一次返回整本账本快照 `{version, revision, highestPage, pageUnits, entries}`，流水按日期/创建时间倒序，页面/月份/累计汇总与当页单位由前端精确模型本地派生。
- 模板：`/api/templates`
...
- 记账：`/api/ledger-entry-create`、`/api/ledger-entry-update`、`/api/ledger-entry-delete`、`/api/ledger-page-unit`；创建必须传 1–99 的 `ledgerPage`，更新不支持跨页移动；倍率使用规范十进制字符串，传 `null` 清除。每页单位接口只接受 `page/unit`，空 `unit` 恢复人民币。有效写入递增 revision；创建/更新只回 `{revision,entry}`，删除回 `{revision,deletedId}`，单位回 `{revision,page,unit,changed}`，不回整本快照。
...
左侧“连接锚点”工具只在画布/导图模式可用：双击空白放置，拖动空白执行框选，普通拖动锚点移动，按住 `Alt` 从锚点或其他可连线节点拖出连线；同一锚点可连接多个不同目标。工具激活时显示全部锚点，并允许框选同时纳入普通节点和当前可见锚点；在“选择”工具中对已选锚点按 `Ctrl/Cmd+D` 复制时自动切回“连接锚点”工具。连接锚点的连线几何端点始终落在锚点正中心；显形时由锚点图标覆盖中心段，离开工具后锚点隐藏、不参与命中且退出框选范围，相邻连线不留下图标外框造成的断口。选中相邻连线时只临时显示它的锚点端点，端点本体可直接拖动，但不套用锚点自身的选中反馈。直接选中锚点时显示双环与一次性弹跳，视觉反馈不得修改节点尺寸或连线几何；放置、框选、连线和删除说明统一收在右下角“？”快捷键速查中，不在画布旁常驻文字。删除锚点必须连带删除相邻连线并作为一次历史操作；缩略图和 PNG 不显示锚点本体，PNG 仍保留有效连线。
...
- 历史栈限制约 50。AI 计划应用、模板实例化等批量操作应保持可整体撤销。快照恢复按 id 增量同步：未变化节点、图片/PDF/Markdown 运行时及连线几何继续复用，类型变化或删除才释放对应节点。记录对象也保持身份，避免旧事件闭包写回失效对象；marks/waypoints 不与历史共享可变引用。修改与撤销都要刷新受影响的多层目录、折叠和任务簿派生状态，任务卡的 strike 变化必须重算根完成状态。快速删除后撤销要清理尚未结束的离场副本。
- 双屏外观只在可见且未退场时读取；背景来源签名变化后按帧合并为一份 payload。关闭后，观察器、迟到的 iframe load/ready 以及待执行 RAF 都不能继续读取或推送外观；重新打开同步当前背景。
...
- 首页是书本式工作台，不是营销页。
- 顶栏“画布 / 笔记 / 研究 / 生涯”只切换现有起步页壳中的直属工作区面板；画布工作区 DOM 不卸载，所以当前特殊页、分组、书脊位置和滚动状态必须保留。切换使用同一 `--start-turn-*` 速度变量与低动态降级；研究的初始直属面板保持为空，第一次进入才创建 `research.html` iframe，且不得加入空闲预热。离开研究先调用 `suspend`，返回调用 `activate`，`pagehide` 或加载失败调用 `dispose`；失败时回到画布。笔记和生涯保持既有按需加载/空闲预热。离开笔记前必须等待保存链；按钮选择器只能命中 `button[data-start-workspace]`，不能把带当前状态的 `body[data-start-workspace]` 误绑为点击入口。四个顶层入口支持方向键与 Home/End 切换。
- 学习、树状、速记三页在画布工作区可见且窗口有焦点时，由 `start.js` 共用一个 30 秒前台计时器写入 `/api/start-page-activity`；切页、切到笔记/生涯工作区、失焦、隐藏或 `pagehide` 时立即结算，不设键鼠闲置超时。计时开关默认开启，用户显式关闭后会先结算当前片段再停用，但不再改变绿色统计的显示状态。独立的“显示三页统计数字”开关对新用户默认关闭，只控制四张卡片中的绿色附加值，不启停计时或清除既有账本。三页计时写入后不得自动重绘活跃页；已有缓存保持到用户点击右上角“更新”，按钮先等待计时队列至多 3 秒再统一读取最新活跃数据，避免翻页时二次渲染闪烁。活跃页画布镜头的热力图、当天明细和星图仍只表示真实画布；四张统计卡以同字号绿色 `+值` 单独显示三页的月/年时长、统一连续天数、当年使用页面数和累计时长，时长两边各用自己的自然单位。
- 主要页面顺序包括复习、日历、节奏/活跃、速记、树状、学习、专注、最近、收藏、自定义分组和固定在其后的未分组；另有回收站、帮助、主题/背景设置。
- 左侧书脊保留两层游标：黑白实体表示真实当前页，彩色细条与柔光层负责悬停预览；实体浅色模式固定纯黑 `#000`、深色模式固定纯白 `#fff`，显示时完全不透明，不做透明度过渡或亮度滤镜，呼吸和搜索提示只改变外侧光晕。书脊功能按钮不使用背景模糊；非激活按钮位于实体下方，激活升层时背景、边框与阴影立即清空，不做这些属性的过渡，避免快速滚轮改道时覆盖滑块。彩色层初载直接落在当前页，悬停时连续改道，离开书脊、焦点离开或滚动后回到当前页并常驻为细条 + 微光轮廓，不得重新改成淡出消失。深色主题的书脊底板使用均匀、不透明的中性黑，不叠加环境色或局部彩色光晕。动态分组重建和窗口缩放必须无飞入地校准，低动态模式立即完成位置与形状切换。
- 左侧书脊黑白实体滑块由齿轮面板“翻页速度”下方的独立“滑块速度”滑条控制，范围 100–800ms、步长 1ms，显示实际动画时长；默认 414ms，比此前固定的 460ms 缩短 10%。`assets/start.js` 即时设置 `--spine-motion-ms`，仅供 `.spine-active-orb` 的位置和形状过渡使用，以 `canvas:spineMotionMs:v1` 保存本机偏好；缺失或非法值使用默认值，超出范围钳制，设置总重置恢复默认。页面翻页和其他游标仍沿用原速度偏好，低动态模式仍关闭滑块过渡。
...
- 研究相机定向复用主画布的目标/当前双状态、帧率归一化 RAF 缓动、指针锚定滚轮缩放、空格/中键拖拽 EMA 测速与时间无关惯性、连续方向键/WASD 平移；不把主画布整体抽成共享模块。低动态下缩放即时到达且无松手惯性。右下角相机图标保留当前缩放，把世界原点平滑移到视口中心；低动态时立即到达。六项设置独立保存为 `research:panSpeed:v1`（默认 8）、`research:panInertia:v1`（默认 0.15）、`research:zoomSpeed:v1`（默认 1）、`research:coordinatesVisible:v1`、`research:axisOpacity:v1`（默认 1，仅控制 x/y 轴线）与 `research:coordinateLabelsVisible:v1`（坐标显示与数字默认关闭），不得读写对应 `canvas:*` 键。坐标层在 Canvas2D 中随相机绘制自适应方格、x/y 轴和可选刻度数字，显示采用 x 向右、y 向上的数学方向，不写入研究文档。
- 研究右下角的空白页删除、原点相机、设置和帮助使用统一角落操作区，不得用重叠的绝对坐标互相覆盖；第 1 页永不显示删除，后续页仅在节点和连线都为空时显示。正常自动保存全程静默，不显示“正在保存”或“已保存”；失败、冲突和恢复警告继续显示。
...
- M1–M4 默认保持 DOM 节点、Canvas2D 静态连线和临时 SVG 活动线。WebGL 只有在 1200/3000 基线或 10000 简单节点概览未达性能目标时才进入混合后端原型；完整 WebGL 不是默认路线。计算、历史和保存只操作研究模型，任何渲染器不得成为第二份业务真源。
- 研究正文只保存到 `data/research-workspace/workspace.json`，有效 V3 备份为同目录 `workspace.backup.json`；损坏主文件先隔离为 `workspace.corrupt-时间.json`，再恢复备份或空白文档。前端写入携带读取时的 SHA-256 修订号，409 冲突不得覆盖；服务端保存前必须读取同一节点声明并严格校验，拒绝原位改写已发布的子电路修订，只允许追加更大的修订号，再沿用 `_atomic_write_json`。事件队列、投影、编译缓存、性能统计、历史、工具状态和调度句柄不得进入文档。
- 子电路定义/实例、无环嵌套、状态隔离、修订固定/手动升级、从选区封装和 M5.4 选区整体复制已完成；图分析和动作节点仍未实现。分层 CPU 公开交互验收也已完成：4 位 CPU 的 ROM/Core r1→r2、旧实例固定、升级清空状态、Probe 顺序及 25 Core/1,006 递归展开节点均通过，实测见 `docs/research-cpu-evaluation-2026-09-21.md`。651 次公开操作表明重复配置、拖线和端口命名存在摩擦；选区复制现可复用节点配置与内部连线，下一步用同一 CPU 工作流量化改善，再决定是否设计跨页剪贴板。1,000 个同步主动 Clock 的真实 Edge 分片验收也已完成：同刻批次保持原子，计算按 256 源分片，连续投影按帧限量刷新后 RAF 中位/P95 为 16.7/17 ms；仍记录一次 55 ms Long Task，详见 `docs/research-time-slicing-evaluation-2026-09-21.md`，当前不引入 WebGL。显式 Graph Snapshot/节点引用延期到出现具体分析场景后，分析节点仍禁止偷偷依赖整页。契约验证在既有命令外增加 `node tests/research-model-regression.js` 与 `node tests/research-subcircuits-regression.js`；真实浏览器验收为 `node tests/research-circuit-browser.js <本地服务 URL>`、`node tests/research-subcircuits-browser.js <本地服务 URL>`、`node tests/research-cpu-browser.js <本地服务 URL> [report.json]` 与 `node tests/research-time-slicing-browser.js <本地服务 URL> [report.json]`，都必须拦截研究读写，不接触真实研究数据。
...
- 目标树拖动到视口边缘时，自动平移按 RAF 的真实时间间隔推进，保留原 60Hz 速度并在高刷新率屏幕上逐帧更新，不能按每帧固定距离导致 240Hz 四倍加速。指针回到中央后停止帧循环，再进入边缘时重置时间基准；单次延迟最多按 40ms 计算，避免卡顿或恢复后跳动。与独立树状页共用的行为由 `node tests/tree-page-motion-regression.js` 验证 60/120/144/240Hz、停顿与边缘重入。
...
- 深色彩色卡片呈现还需运行 `node tests/start-dark-card-presentation-contract.js` 与 `node tests/study-palette-contract.js`，并手测树状页/学习页各自 0%、15%、100%、“智能/白/黑”字体、浅色禁用和刷新恢复；0% 只去掉彩色呈现，不删除任务或阶段的持久化颜色。
...
- 右侧编号栏支持滚轮前后切树；树较多、编号列表可滚动时，先滚动栏内列表，到边缘后继续滚才切换。快速滚轮采用视觉与持久化双通道：编号、滑块和画面即时追随最新目标，不等待手势停稳或本地写盘；同方向退场时只替换透明点即将显示的树，中间页不渲染，反向退场或入场中改向则继承当前透明度与水平位移连续转向，并按剩余可见度缩短退场，最终树仍完整入场。树状态在后台按单请求串行落盘且只追最新目标，关闭页面后也要完成在途链；失败回到最后确认树并使乐观缓存失效。编号栏 DOM 重建时先将黑色滑块无动画钉在正确起点，不得短暂回到第 1 页再跳回。
...
- 阶段汇总进度条与任务、根节点进度条一样常驻复用同一 fill DOM；任务 `＋/−` 的乐观渲染与后端校准都只更新其宽度，不能替换节点并截断正在进行的过渡动画。
...
- 树状页不绑定 `Alt + 左键` 的额外交互；阶段或任务卡片任意区域（包含卡内按钮）右键移动超过 6px 后进入视觉连线，松到合法目标时添加，同方向重复拖拽删除；普通右键单击仍打开外观面板。增删在松手时先乐观同步本地数据和 SVG，再沿现有串行命令链静默保存，失败时才回滚，不能先撤掉预览再等待后端造成闪烁。视觉线与解锁箭头使用同一深色，且只在共享布局完成后按当前可见端点追加；端口轻量错位但不移动节点，也不进入解锁、进度或下一步计算。结构拖拽的基础树快照必须通过 `preserveTreeExtensions()` 保留视觉线、自由对象与树外观，松手预览及失败回滚不得用仅含 `nodes/links` 的瘦快照覆盖扩展字段。
...
- 连线和箭头按 ID 复用常驻 SVG 元素。几何变化时先批量测量，每张被引用便签只读取一次中心，再统一更新 SVG；相机平移/缩放只投影内存中的世界坐标，不得重新测量便签或重建线条，线宽、阴影与箭头大小保持屏幕尺寸。只观察被连线引用的便签尺寸，叠摞/新建的有限位移动画期间才运行几何跟随帧，完成、取消、离页或隐藏时停止；缓存和观察器不写入用户数据。缩放缓动与方向键平移按真实帧间隔推进，60/120/144/240Hz 不得使用抬高高刷新率步长的下限。
...
- 验证：`node tests/notes-camera-contract.js` 包含虚拟 60/120/144/240Hz 时间步长回归。真实浏览器使用测试主机已有 Playwright 与 Edge，设置 `RELATUM_PLAYWRIGHT`、`RELATUM_PYTHON` 后运行 `node tests/notes-browser-performance.js <报告.json> regression`；工具自建隔离数据根、使用确定性速记替身并在结束时关闭服务，覆盖缩放开销、各起步页静置、端点跟随、编辑、叠摞、删除/撤销、箭头与划线、搜索、窄窗和低动态。不得连接真实 `data/notes.json`。
...
- 页头保留四个无文字图例色块、上月/下月、“…”当前页设置和“＋记一笔”；设置卡包含一个可留空单位输入框、月份/累计视图、含小数/隐藏小数和默认记录支出/收入三组二选一与保存/取消。后三项都是按数字页独立保存在本地的界面偏好，只有单位变化才发送写请求；单位留空使用人民币 `¥`，非空单位仅替换当前数字页的金额显示。月份视图的三张汇总纸面按结余/收入/支出显示，流水按日期倒序分组；累计视图隐藏月份切换、日期分组、编辑卡日期项和月份辅助文字，展示当前数字页的总收入/总支出/总结余。桌面三卡并排、窄窗 `auto-fit/minmax` 换行，流水始终单列且没有侧栏。
- 右侧边缘悬停或键盘聚焦显示 1–99 数字页栏，复用学习页按钮、活动滑块、隐藏滚动条和滚轮边缘切页手感；页栏至少铺满当前可用高度，并覆盖当前页及 `highestPage`。当前页写入 `ledger:page:v1`，切页保留月份与未保存草稿（草稿跟随创建页）并关闭设置/调色盘；全量数据仍在本地直接过滤，右侧黑色滑块继续提供方向反馈，页头、汇总卡外壳和流水纸面完全静止。旧/新汇总文字与流水使用临时只读克隆层在原位 220ms 交叉淡化，不位移、不缩放、不模糊、不重播整版错峰；流水舞台在当前可见会话内只增高不缩短，离页或窗口变化时重置，避免不同页账目长度导致底边跳动。快速连点只让最后一次生效。
...
- 行右键或键盘菜单键打开学习页共享 12 色盘；账目行（含草稿、乐观条目）与页头四个图例色块通过 `study-palette.js` 的 `applyColorTones()` 共用深浅呈现色，原始保存色值不变。浅色使用柔和纸面与同色侧边标记，深色使用完整彩色卡面和墨色文字；收入金额（含正号）与支出金额（含负号）使用鲜明柔和的绿色和珊瑚红，默认深底与彩色卡面分别使用较亮和较深的对应色。待保存彩色行保留 90% 不透明度。两套色值随常驻 DOM 一起缓存，切主题由 CSS 完成；恢复默认或回滚时同步清理/恢复呈现变量，色库外的旧自定义色沿用原有回退。四个图例色块右键或 Enter/Space 使用同一色盘。保存失败必须保留服务端已确认状态或回滚乐观颜色，不能让失败请求改写缓存。
- 整本账本在首屏空闲 `requestIdleCallback` 预热，首次翻入直接消费；权威重载使用 `AbortController` 与递增序号。乐观操作只改目标条目或页单位，同目标请求串行，响应按增量合并；失败只回滚当前目标，revision 跳号或请求结果不确定时等待 mutation 清空后合并一次 GET。页视图、隐藏小数与新建默认收支类型是按页本地偏好，单位未变时不得发 POST。汇总、行增删改、颜色、跨视图与月份动画均服从 `prefers-reduced-motion`。
...
node .\tests\ledger-model-regression.js
node .\tests\calendar-day-columns-contract.js
node .\tests\calendar-countdown-progress-contract.js
```

并使用隔离的 `RELATUM_DATA_ROOT` 验证本月空状态、精确倍率、未来月份新增、跨月修改、同槽删除确认、全量加载后本地切页即时切换与草稿保留、不同账目增量交错与 revision 重载、窄窗、键盘、深浅主题和低动态；结束后关闭测试服务，不得读取或写入现有 `data/ledger.json`。
...
排序、新建命名与文件树空白处取消文件夹选择另用 `node tests/note-tree-sort-browser.js` 验证六种顺序、偏好恢复、元数据重排、自动编号、根目录新建目标和弹层操作；同样只连接隔离数据根。
笔记专注状态与桌面浮动关闭按钮用 `node tests/note-focus-browser.js` 验证冷启动首绘、跨工作区与重载恢复、快速开合、低动态、深浅主题、窄窗以及关闭前保存；设置相同的 Playwright、Python 和 Edge 环境变量，脚本使用隔离数据根与模拟桌面桥接。

实时预览排版与命中用 `node tests/note-live-presentation-browser.js` 验证；设置上述 Playwright 与 Edge 路径即可，不需要 Python。脚本只托管静态资产和合成笔记，在临时目录输出深浅截图与 JSON 报告，覆盖长代码块可见装饰数量、重复 Callout/表格/分隔线下方代码的真实鼠标点击、窄窗与 135% 字号、多阶段拼音预编辑、段落中间选词、图片相邻输入、公式源码进入、模式切换后的撤销、任务框和未闭合围栏复制，并复用长文档视口/公式测试与 4MiB 文档输入耗时探针；图片文字候选输入、编辑/展示几何、恢复默认样式与键盘微调用 `node tests/note-image-text-browser.js` 做聚焦浏览器回归，覆盖选中后英文/原生中文输入、粘贴、切框、清空删除、撤销/重做和切框拖动；`node tests/note-image-text-workspace-browser.js` 使用隔离库验证真实自动保存、删除最后一框后重开、合并与清理，另需人工覆盖多框创建、拖动边界、字号/字色、缩放、保存重开、Live/源码/阅读往返、深浅主题、窄窗、低动态，以及合并 PNG 的真实文字像素与透明背景、原图与屏外文字保留，以及清理后的 Markdown/历史/撤销记录。不接触真实笔记库。Windows 桌面交付仍需用微软拼音手测真实候选窗，确认正文、图片旁和公式内均不出现拉丁串残留、光标错位或跳行。
...
图片文字工具栏与保存/同步的联合回归使用 `node tests/note-image-text-workspace-browser.js`（同样配置上述三个测试环境变量）。脚本运行真实页面和隔离笔记 API，覆盖候选输入被空白点击打断后立即重开、收尾事件先后顺序、Chromium 原生合成输入与连续开关，并断言保存返回的修订号等于磁盘读取值、外部检查不重建自身刚保存的文档。`test_notes_library.py` 另覆盖 LF / CRLF 的原样落盘与重复保存零改写；原生微软拼音候选窗仍需桌面手测。
...
node .\tests\canvas-changes-regression.js
...
性能夹具只能写入一次性隔离运行目录，禁止指向仓库真实 `canvases/` 或 `data/`。生成器提供 1200/2359、3000/2970 节点/连线夹具，以及连线交互、折叠、图片、Markdown、PDF 夹具。真实浏览器验收使用测试主机已有的 Playwright 与 Edge，无应用依赖或构建链变化：设置 `RELATUM_PLAYWRIGHT` 为模块绝对路径、`RELATUM_EDGE_PATH` 为浏览器路径，再对以该隔离目录为 `RELATUM_DATA_ROOT` 的测试服务运行 `node tests/canvas-browser-performance.js <本地服务 URL> <隔离目录> <report.json> [all|regression|benchmark]`。默认每种规模各 5 次新浏览器冷启动及同页热启动，固定 1440×900、DPR 1、相机 100% 与中心 (330,280)；操作覆盖局部/25% 总览缩放、平移、拖动收尾和撤销。回归另覆盖 DPR 2、减少动态效果、深浅语义、SVG 回退、中文合成、导图/分组、任务簿、附件、双屏、导出与起步页反向切页；功能夹具的保存请求由浏览器替身接收，基准场景保留正常 HTTP 缓存和真实隔离服务。
