...
describe('Product Detail Page', () => {
    ...
    it('allows adding product to cart', () => {
        ...
        cy.get('[data-testid="cart-notification"]').should('contain', 'Product added to cart');
    });
});
```
...
class ProductTest extends TestCase
{
    public function testProductQuery(): void
    {
        $query = '
            query {
                product(uuid: "' . $this->getReference(ProductDataFixture::PRODUCT_PREFIX . '1')->getUuid() . '") {
                    name
                    price {
                        priceWithVat
                    }
        ...
        $this->assertResponseContainsArrayOfDataForGraphQlQuery($query, $response);
        $this->assertArrayHasKey('product', $response['data']);
        $this->assertNotEmpty($response['data']['product']['name']);
    }
}
```
...
3. **Mock external dependencies** (GraphQL, APIs)
4. **Test accessibility** with screen reader queries
5. **Use waitFor** for asynchronous operations

### Backend Testing

1. **Use `@inject` annotation** instead of `$this->getContainer()->get()`
2. **Test business logic** in unit tests
3. **Test integration** in functional tests
...
│   ├── Functional/        # Database integration tests
│   └── Smoke/            # Basic smoke tests
├── FrontendApiBundle/     # GraphQL API tests
...
protected function setUp(): void
{
    ...
    $this->productData->name = 'Test Product';
}
```

## Configuration

- **Frontend**: `project-base/storefront/vitest.config.ts`
- **Backend**: `project-base/phpunit.xml`
- **Acceptance**: `project-base/tests/cypress.config.ts`
