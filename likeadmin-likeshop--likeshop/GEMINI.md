...
- 用户 API 可通过 `server/.env` 的 `[consume_token] enabled = true` 开启附加消费票据校验，`ttl_seconds` 范围为 1 到 600 秒。`POST /api/account/consumeToken` 使用现有登录 token 签发与用户会话绑定的短时票据；uni-app 与 PC 请求层缓存票据并通过 `X-Consume-Token` header 携带。票据在 TTL 内可复用，不影响免登录 action；开启前必须先部署包含新请求层的 PC/mobile 构建。多实例部署必须使用共享缓存（通常为 Redis），不能让签发和校验落在彼此隔离的 File cache。
...
- 登录 token、消费票据、回跳和平台登录逻辑集中在 `utils/request.js`、`utils/consumeToken.js`、`utils/login.js`、`config/cachekey.js`，修改认证流程时必须一起追踪。
