...
- **Thread priority** (`audio_output::thread_priority`): render threads we own take `RenderThreadPriority::acquire` as their first statement — MMCSS on Windows, where `cpal` also carries the `audio_thread_priority` feature so its shared streams get the same treatment; CoreAudio grants its own HAL/IOProc threads real-time priority for free. Linux has no promotion **by decision**: the only route is rtkit over DBus, which drags system `libdbus` back in against the pure-Rust zbus policy `media_integration` was built around. Nothing on Linux needs it: both the shared path (cpal) and the native-sample-rate path (`exclusive/linux`, a PipeWire client through the `pipewire` ALSA PCM) only feed a ring buffer — the real-time thread belongs to PipeWire, which already runs it with the right priority. The decode/engine thread only gets `boost_decoder_thread()` (macOS `QOS_CLASS_USER_INITIATED` / Windows above-normal / Linux nice) — never real-time: it blocks on file I/O and allocates per packet. On macOS deliberately one class below the UI thread's `USER_INTERACTIVE`: the 128 ms ring buffer makes the deadline soft, and `USER_INITIATED` is already the lowest class exempt from I/O throttling, which is what actually protects the refill.
