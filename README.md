# TomeVault Gemini Extensions

AI instruction files in `GEMINI.md` format, packaged as Gemini CLI extensions. Each is graded against the public Tome Standard and kept in sync across every major AI platform by [TomeVault](https://tomevault.io).

## Install

Browse the per-extension directories and copy a plugin's `GEMINI.md` into your project root, or follow the per-extension README for Gemini CLI setup steps.

## Browse

Search, filter, and discover instruction files across every platform at [tomevault.io](https://tomevault.io).

---

> Every package links its original creator. Graded against the public [Tome Standard](https://tomevault.io/standards) and kept in sync across platforms by [TomeVault](https://tomevault.io). TomeVault watches AI instruction files for silent drift on every push and every model release, and signs the verdict.
