...
|----------|-----------------|
| `docs/agent-context/reading-guide.md` | Paper structure, three core claims, notation glossary (35 symbols with LaTeX), equation quick-reference (22 equations), dataset divergence log, six suggested reading paths |
| `docs/agent-context/variables.md` | Plain-text variable definitions grouped by family |
| `docs/agent-context/method.md` | Methodology summary: data construction, interpolation pipeline, conditional protocol |
| `docs/agent-context/results-summary.md` | Key results tables with headline numbers |
| `docs/agent-context/strategies.md` | Seven strategy templates with legs, exposure, payoff profile |
| `docs/paper/paper-digest.md` | Full section-by-section paper digest with exhibit map |
| `docs/paper/paper-annotated.md` | Full paper text with `@section-type` annotations for machine navigation (generated by `tools/latex2md.py`) |
| `docs/manifests/exhibits.csv` | Machine-readable exhibit-to-script mapping |
| `docs/manifests/paper-claims.csv` | Machine-readable claim-to-evidence mapping |
| `docs/manifests/data-files.csv` | Data file inventory with row counts and key columns |
| `docs/manifests/code-map.md` | Script-to-exhibit map + data-flow diagram |

When an agent needs to discuss paper math, start with the **Notation Glossary** and **Equation Quick-Reference** in `reading-guide.md`. When an agent needs to trace a claim to code, use `paper-claims.csv` and `exhibits.csv`.
...
6. **Dependencies**: Base packages are in `requirements.txt` (with pinned upper bounds). Model-zoo extras (`torch`, `lightgbm`, `xgboost`, `catboost`) are in `requirements-modelzoo.txt` and are optional unless the user needs Tables 8–9.

7. **Health check**: Run `python tools/doctor.py` to verify environment. Use `--quick` for dependencies only, `--verbose` for detailed file validation.

8. **Converter**: `tools/latex2md.py` regenerates `paper-annotated.md` from LaTeX source. In the public repo, the full paper `.tex` is not shipped, so end-to-end converter tests use `tests/fixtures/sample_paper.tex`.
