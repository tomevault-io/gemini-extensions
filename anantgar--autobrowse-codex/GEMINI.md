...
- `OPENAI_API_KEY` can optionally be in the environment (or in a `.env` file in CWD — `evaluate.mjs` auto-loads it). Users with a Codex subscription do not need to set this. The Codex CLI (`npm install -g @openai/codex`) must be installed.
...
```bash
node ${CODEX_SKILL_DIR}/scripts/evaluate.mjs --task <task-name> --workspace ./autobrowse
# or for bot-protected sites:
node ${CODEX_SKILL_DIR}/scripts/evaluate.mjs --task <task-name> --workspace ./autobrowse --env remote
```
...
BT_DIR="${CODEX_SKILL_DIR}/../browser-trace"
if [ ! -f "$BT_DIR/scripts/bb-capture.mjs" ]; then
  ...
  echo "into the same parent directory as autobrowse (e.g. ~/.codex/skills/browser-trace/)." >&2
  exit 1
fi
...
node ${CODEX_SKILL_DIR}/../browser-trace/scripts/bb-capture.mjs "$sid" "$RUN_ID" &
sleep 2
...
node ${CODEX_SKILL_DIR}/scripts/evaluate.mjs \
...
node ${CODEX_SKILL_DIR}/../browser-trace/scripts/stop-capture.mjs "$RUN_ID"
node ${CODEX_SKILL_DIR}/../browser-trace/scripts/bisect-cdp.mjs "$RUN_ID"
node ${CODEX_SKILL_DIR}/scripts/unify-trace.mjs \
...
| Ad-hoc grouped query (e.g. top hosts, errors-by-page) | `O11Y_ROOT=./autobrowse/traces/<task-name>/latest/.o11y node ${CODEX_SKILL_DIR}/../browser-trace/scripts/query.mjs <run-id> <cmd>` |
...
```bash
node ${CODEX_SKILL_DIR}/scripts/codegen.mjs \
  --task <name> \
...
runtime requirement is `BROWSERBASE_API_KEY` (plus `OPENAI_API_KEY` for
...
If the task passed on 2+ of the last 3 iterations **or has reached the max iteration limit**, install it as a Codex skill. **Do not just copy strategy.md** — the skill must be self-contained and useful to someone who has never seen this codebase. If graduating at max iterations without a clean pass, note the known failure point but still document everything learned.

Install by writing to `~/.codex/skills/<task-name>/SKILL.md`:

```bash
mkdir -p ~/.codex/skills/<task-name>
```
...
```bash
ls ~/.codex/skills/<task-name>/SKILL.md
```

The skill is now available as `/<task-name>` in Codex.
...
- **Stay in the workspace** — all training writes go to `./autobrowse/`, never to the skill installation directory. The skill source is read-only.
...
- **Graduate to `~/.codex/skills/`** — the only file you write there is the final graduated `SKILL.md`
