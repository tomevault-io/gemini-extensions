...
- Internal-only skills: additional skills under `skills/` are allowed for maintainer tooling (e.g. `skills/benchmark/`) but MUST NOT appear in `.claude-plugin/plugin.json` `skills`, the Claude marketplace listing, `.codex-plugin/plugin.json` `defaultPrompt`, or the Codex install projection at `~/plugins/ui-clone-skills/skills`. `scripts/ci/review.sh`'s `internal_skills` set and `install.sh` `CODEX_PUBLIC_SKILLS` are the enforcement allowlists — update both when introducing public/internal skills.
