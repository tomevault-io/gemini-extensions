...
- **Use parallel versions**: `parallel_search_web`, `parallel_search_arxiv`, `parallel_read_url` for multiple
  queries/sources
- **Limit results**: Don't read more URLs than necessary (typically 5-10 sources)
- **Deduplicate**: Use `deduplicate_strings` or `deduplicate_images` when you have many similar results

### Accuracy

- **Always read sources**: Never rely solely on search snippets
- **Read multiple sources**: Cross-reference information from multiple sources
- **Prioritize relevance**: Use `sort_by_relevance` when you have many documents

### When to Use Jina Tools

- When uncertain about current best practices
- When user doubts your answer
- When you need up-to-date information
- When researching theoretical concepts (use arXiv)
- When you need to-date documentation or guides

## Error Handling

If Jina MCP tools fail:

1. Verify the query is clear and specific
2. Try expanding the query with `expand_query`
3. Try alternative search terms
4. Check if URLs are accessible
5. Consider using fewer parallel operations if hitting limits

## Remember

- **Never use search tools alone** - always complement with `read_url` or `parallel_read_url`
- **Use parallel versions** for efficiency when dealing with multiple queries or sources
- **Combine arXiv and web searches** for comprehensive research on technical topics
- **Read multiple sources** to cross-reference and verify information
- **Use query expansion** for complex research topics to get diverse perspectives
