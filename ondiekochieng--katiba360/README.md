# katiba360

> Source: [OndiekOchieng/katiba360](https://github.com/OndiekOchieng/katiba360) — distributed by [TomeVault](https://tomevault.io)

Katiba360 is a modern, user-friendly platform that makes the Kenyan Constitution accessible, searchable, and understandable for everyone. The project aims to empower citizens, students, and legal professionals with intuitive tools to explore, learn, and interact with constitutional content.

## Gemini CLI Config

The `GEMINI.md` file in this directory is the project config converted for Gemini CLI.
Original source: `CLAUDE.md` in [OndiekOchieng/katiba360](https://github.com/OndiekOchieng/katiba360).

## Also available for

- **Codex** — `AGENTS.md`
- **GitHub Copilot** — `copilot-instructions.md`
- **Cursor** — `project-config.mdc`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

Available for 6 platforms including Claude Code, Cursor, Windsurf, and more. Source: [github.com/OndiekOchieng/katiba360](https://github.com/OndiekOchieng/katiba360)

---

Install this config instantly:
```
npx tomevault install OndiekOchieng/katiba360
```
Source: [github.com/OndiekOchieng/katiba360](https://github.com/OndiekOchieng/katiba360).

<!-- genome:d-i-p -->
