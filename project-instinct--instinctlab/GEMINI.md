...
> Overview of InstinctLab design principles, usage, and expansion guide.
...
2.  **`fill_init_reference_state(self, env_ids, ...)`**:
    - Fill `MotionReferenceState` with the starting pose. `get_init_reference_state(env_ids)` returns `state[env_ids]` (tensors shape `[len(env_ids), ...]`); do not re-index with `env_ids`.
