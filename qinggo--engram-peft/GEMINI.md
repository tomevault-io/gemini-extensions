...
### 需要收集的信息
1. 完整的终端输出（如果太长，请截取最后500行）
2. 峰值显存占用（可通过 `nvidia-smi dmon -s p -d 1` 监控）
3. 最终的Train Loss和Eval Loss值
4. 任何异常警告或错误信息
```
...
> 2. **禁止盲目执行**：绝对不要尝试在本地自动运行任何需要真实 GPU 显存的代码（如 `examples/` 下的大模型训练/推理示例代码）。
