...
## Task
Universal custom-arch support + multi-device/multi-arch training verification + multi-org fix + package/smoke Node .exe + commit (no Cursor tags).

## Plan
- [x] Declarative knobs (GQA, qk_norm, head gating, embedding scale, attn residual, looped layers)
- [x] External HF/local loader + executor/hf_integration wiring
- [x] Unit tests (generic names only)
- [x] Multi-device/multi-arch training verification harness (`tools/verify_multi_device_training.py`), all 9 architecture profiles pass under capped CPU/GPU/RAM
- [x] Fixed multi-org bug: shared `JobSubmissionHandler`, open-by-default allowlist, `DISTRIBAI_ALLOWED_ORGS` override
- [x] Assigned stable per-install `org_id` + surfaced it in node/orch settings pages and job-submission payloads
- [x] Fix packaging spec (binaries dedupe, drop `unittest` exclude), build + smoke-run .exe headless — succeeded
- [x] Full unit suite + ruff + npm test green
- [ ] Commit with conventional subject
