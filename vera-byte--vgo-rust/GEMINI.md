...
- 所有服务都应使用`v`库中的功能，避免重复实现。
- 所有接口使用`actix-web`框架实现。
- 所有API接口响应使用`v::response::respond_any`方法返回数据。
- 所有服务接口都通过`src/api/<版本>/接口分组/接口名.rs` 实现。
...
- 在异步函数中使用`?`运算符传播错误。
- 使用`thiserror`或`anyhow`实现自定义错误类型以提供更具描述性的错误。
- 尽早处理错误和边界情况，适时返回错误。
...
---
> Converted and distributed by [TomeVault](https://tomevault.io/claim/vera-byte) — claim your Tome and manage your conversions.
