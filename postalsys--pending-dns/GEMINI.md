...
- `[skip ci]` is read from the **head commit of the push**, not per commit, and from anywhere in that message including the body. So it suppresses the workflows for everything pushed with it, and even a commit that only *mentions* the marker in its body skips CI. When a batch ends with a docs commit but contains `feat:`/`fix:` commits, either drop the marker or order the docs commit so it is not last.
