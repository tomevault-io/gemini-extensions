...
- Non-Goals: auto issue/PR creation or push (drafts only), a managed always-on monitoring daemon (roadmap — today `bin/follow.ts` gives session-scoped live via `tail -n0 -F ~/.gjc/logs/gjc.$(date +%F).log | bun run follow.ts`, reusing `classifyLogEntry`), changing gjc log/session formats (read-only).
