...
Docs navigation is autogenerated from `website/docs/` (`sidebars.ts` uses `type: "autogenerated"`). Put each page under the right subfolder, set `sidebar_position` in front matter, and use `_category_.json` in a folder to control its label and order among top-level sections.
