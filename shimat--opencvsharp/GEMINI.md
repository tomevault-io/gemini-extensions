...
## Raw array parameters and native mutation

A `T[]` (or `T[,]`) parameter that native code mutates in place — via a pinned `fixed` pointer, or by passing the array directly to a P/Invoke signature typed as a raw pointer/array — looks identical at the call site to a pure read-only input array. No `[In]`/`[Out]` attribute is needed to make this mutation happen: for blittable element types, default P/Invoke array marshaling already pins the managed array and hands native code a pointer into that same memory, so writes are visible to the caller regardless of any attribute. Since the signature alone can't distinguish the two cases, the convention here is doc wording, not a wrapper type or attribute:

- If native code both reads a raw array parameter's incoming contents and writes a new result back into the same array (e.g. an initial guess that gets refined in place), its `<param>` doc must start with **"Input/output"** (e.g. `Input/output vector of distortion coefficients...`). Don't document it as "Output" alone in this case — that hides the fact that the caller's pre-existing values feed into the call.
- If native code only writes into the array (the caller just pre-allocates it for sizing, and its incoming contents are never read), document it as **"Output ..."** — but it must say so; an empty `<param>` or a vague, non-committal description (e.g. "Pointer to the destination array") leaves the reader unable to tell it apart from a pure input. Either way — read-and-write or write-only — the array must never be documented as if it were a plain input with no mutation signal at all.
- If a parameter is genuinely a pure input (native code only reads it, never writes), keep a plain description with no "Input/output"/"Output" wording, and prefer `IEnumerable<T>` over `T[]` to remove the ambiguity altogether — unless it sits directly next to a `T[][]` parameter pinned via `ArrayAddress2<T>` in the same signature, in which case keep it as `T[]` for visual consistency with its `T[][]` sibling (see `Aruco.RefineDetectedMarkers`'s `detectedIds`).
- `InputOutputArray`/`OutputArray`-typed parameters already signal mutation through their type and don't strictly need this doc convention, though matching "Input/output"/"Output" wording there is still accurate and preferred.

See [issue #2036](https://github.com/shimat/opencvsharp/issues/2036) for the background discussion and known instances (`Cv2.CalibrateCamera`'s `cameraMatrix`/`distCoeffs`, `Cv2.Find4QuadCornerSubpix`'s `corners`).
