...
### 4.0 Read the manifest first

`custom-elements.json` at the repo root is the machine-readable description of
this element — the Custom Elements Manifest (schema 2.0), and what
`package.json` points editors at via `"customElements"`. Every observed
attribute (with its type and default) and every DOM event it dispatches is in
there, as data rather than prose:

```bash
jq '.modules[0].declarations[0].attributes[].name' custom-elements.json   # 70
jq '.modules[0].declarations[0].events[].name' custom-elements.json       # 47
```

Prefer it over the lists below when you need the *current* answer. It is
regenerated by `scripts/build-custom-data.mjs` on every `npm run build:ts`,
from `MoviElement.observedAttributes` and the event table in
`docs/api/events.md` — so it cannot quietly drift from the code the way a
hand-written list can, and the build fails when the two disagree.

Two things it does **not** carry, so go elsewhere for them:

- **Properties and methods** — `members` is empty. Use `docs/api/element.md`,
  or read the getters/setters in `src/render/MoviElement.ts`.
- **Why** anything is the way it is. The manifest is a surface; the reasoning
