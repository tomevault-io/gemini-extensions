...
2.  **修复不是重设计 (Fix != Redesign)**
    - 当测试失败或出现 Lint 错误时，你的任务是修复代码以符合现有设计，而不是修改设计来迁就代码。
    - **规则**：如果你发现不改设计就修不好 Bug，必须立即停止并报告，绝不能擅自行动。

3.  **仅有提案权，无决策权 (Proposal Only, No Auto-Decision)**
    - 对于设计层面的问题，你只有提出建议的权利。
    - **规则**：严禁使用“自动决策”机制来执行设计变更。设计变更必须经由用户显式批准。

**记住：保持架构完整性比修复一个 Bug 更重要。未经授权的“优化”就是破坏！**
...
- 核心包示例: `@affectjs/dsl`, `@affectjs/runtime`, `@affectjs/affect`, `@affectjs/fluent-ffmpeg`, `@affectjs/examples`
...
所有包（`packages/dsl` 和 `packages/affect`）都有严格的覆盖率阈值：
...
- **示例**: `0001-dsl-design.md`, `0002-runtime-engine.md`
