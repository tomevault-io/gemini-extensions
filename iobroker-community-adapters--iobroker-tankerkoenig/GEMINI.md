                ...
                return new Promise(async (resolve, reject) => {
                    try {
                        ...
                        const obj = await new Promise((res, rej) => {
                            harness.objects.getObject('system.adapter.tankerkoenig.0', (err, o) => {
                        ...
                        // Verify states
                        const stateIds = await harness.dbConnection.getStateIDs('tankerkoenig.0.*');
...
The tankerkoenig.de API requires a valid API key. For testing:

- Use the demo API key `00000000-0000-0000-0000-000000000002` for basic connectivity tests (placeholder — verify with tankerkoenig.de docs)
- API key must be encrypted before storing in adapter config (uses ioBroker's `protectedNative`)
- Test both valid and invalid API key scenarios
- Verify that the adapter sets `info.connection` state correctly on API failures
...
**Example with fetch (tankerkoenig API pattern):**
```javascript
try {
  // IMPORTANT: Never log the full URL as it contains the API key
  const url = `https://creativecommons.tankerkoenig.de/json/prices.php?ids=${stationIds.join(',')}&apikey=${this.config.apikey}`;
...
**tankerkoenig-specific error codes:**
- `parameter error` — invalid API parameters (log and skip update cycle)
- `limit reached` — API rate limit exceeded (back off and retry)
- Station status: `open`, `closed`, `no prices` — handle all three states
...
- Maintain multilingual support (minimum English and German — see `docs/en/` and `docs/de/`)
...
### Adapter-Specific Patterns (tankerkoenig)

#### API Integration
- tankerkoenig.de API base URL: `https://creativecommons.tankerkoenig.de/json/`
- Endpoints used: `prices.php` (batch price fetch), `list.php` (station search), `detail.php` (single station)
- Always batch station price requests into a single API call (pass comma-separated IDs)
- Respect the minimum query interval of 10 minutes as required by API terms of service
- Handle the `ok: false` response field — log `message` field and skip the update cycle

#### State Structure
- Organize states per station: `stations.<stationId>.diesel`, `stations.<stationId>.e5`, `stations.<stationId>.e10`
- Include station metadata states: `name`, `brand`, `street`, `city`, `postCode`, `status`, `isOpen`
- Track price differences: `cheapest.<fuelType>` combined states for the cheapest available station
- `stations.adapterStatus` — string state reflecting current adapter operational status

#### Configuration Requirements
- `apikey` — stored via `protectedNative`, must be validated as a non-empty UUID v4 format (e.g., `xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx`) before use; never log or expose the raw key value
- `synctime` — update interval in minutes (minimum 10 minutes, default 10 minutes)
- `station` — array of configured station objects with `stationid`, `stationname`, and fuel type flags
- `combinedOptions` — configures display text for closed/no-price/not-found states

#### Price Monitoring
- Compare new prices against previous state values to detect changes
- Log price changes at `info` level for user visibility
- Set `status` state to configured `combinedOptions` text when station is closed or has no prices
- Always set `ack: true` when writing states from API data
