...
The two sections below are layered on top of the upstream skill and do not
replace any of its workflow. Everything above this line still applies.

## Rendering to PNG

The upstream skill outputs only `.excalidraw` JSON, which doesn't render in
...
```bash
node ~/.claude/skills/excalidraw/renderer/render.js <input>.excalidraw [output.png] [--scale=N]
```

- Output defaults to the input path with `.png` extension.
- `--scale=2` (default) gives retina-quality export.

### Workflow integration

After generating the `.excalidraw` JSON via the upstream workflow above:

1. Run the renderer to produce a `.png` alongside the source.
2. Embed `![[diagram.png]]` (Obsidian) or `![](diagram.png)` (Markdown) in
   the user's note.
3. Keep both files: `.excalidraw` is the editable source, `.png` is what
   actually renders cross-platform.
...
The upstream skill suggests Excalidraw's classic primary palette
...
want the eye to land on. Rainbow = noisy; one-accent = clear.

For the louder Excalidraw-classic look, `lib.py` also exports
...
| `text(x, y, w, content, size, color)` | Single-line free text |
| `labeled_box(x, y, w, h, lines, palette, shape="rectangle")` | Shape + multi-line centered text. Most common building block. |
| `arrow(x1, y1, x2, y2, label=None)` | Straight arrow with optional midpoint label |
| `save_doc(elements, path)` | Writes the `.excalidraw` JSON, flattens any nested element lists |

### When to use the helpers vs the upstream by-hand path

- **For simple, common layouts** (linear flow, snake pipeline,
  start-process-end, small mind maps): use the helpers in `lib.py`. They
  enforce a clean visual language and save tokens.
- **For specialized diagrams** (full-fidelity ER, sequence with activation
  boxes, AWS architecture with icons): follow the upstream workflow and
  templates above. The helpers are deliberately small and don't try to
  cover every diagram type.

Both paths produce identical Excalidraw JSON and both render to PNG via
