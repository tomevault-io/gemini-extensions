...
#### 4.0.0 · 主副标题换行 / 溢出三轮自校验（强制）

每一页的主标题和副标题都要检查。只要标题出现自然换行、手写 `<br>`、被迫溢出、压缩到不可读、或视觉上变成 2 行以上，就必须重新分析最佳分界点后改稿并重新生成 HTML / PDF。

**三开三轮要求**:

- **开 1:结构切分** — 按句子内容结构切分,例如“问题 / 结论”“对象 / 动作”“旧状态 / 新状态”。优先保留语义完整,不要把修饰语和中心词拆散。
- **开 2:标点切分** — 如果标题含 `,`、`，`、`:`、`：`、`/`、`｜`、`、`、`；`、括号,优先在标点后断行,但不能让上一行只剩空泛引导词。
- **开 3:均衡切分** — 如果没有清晰语义或标点,按中文字符数寻找接近均匀的断点,同时照顾阅读呼吸感。不要出现“1-2 个字孤行”或第二行明显过短。

每轮都按这个顺序处理:

1. 抽取每页主标题 / 副标题候选。
2. 判断是否可能换行、溢出或视觉拥挤。
3. 对有问题的标题给出 2-3 个切分方案。
4. 选择最自然的一版,改 HTML 文案或 `<br>` 位置。
5. 重新导出 PDF 或截图,再次逐页检查。

可执行预检:

```bash
node <SKILL_ROOT>/scripts/lint-title-breaks.mjs path/to/index.html
```

这个脚本只能做机械预警,不能替代视觉检查。它会标出过长标题、疑似坏 `<br>`、孤行、缺少可读切分点等问题。任何 warning 都要人工看页面,并按“结构 / 标点 / 均衡”三种候选重新切分。
...
7. 如果导出 PDF,每次标题切分或溢出修正后都要重新生成 PDF,不能只改 HTML 后交旧 PDF。
...
8. 生成后先运行 `node scripts/validate-swiss-deck.mjs path/to/index.html`(Swiss 风),再运行 `node scripts/lint-title-breaks.mjs path/to/index.html`,最后读 `checklist.md` 自检
