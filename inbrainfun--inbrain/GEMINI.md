...
- **Track a founder/company over time (v0.35.7):** when an entity has
  ...
  `inbrain eval trajectory <entity-slug>` for the chronological history
  with regressions auto-flagged, or `inbrain founder scorecard <entity-slug>`
  ...
  `inbrain think` now uses this substrate automatically on temporal /
...
Privacy rule in `./CLAUDE.md`. Inbrain pages reference real contacts; public docs must
