...
1. **Never hand-edit generated C++ parser files** - Files under `src/vtlengine/AST/Grammar/_cpp_parser/` (`Vtl{Lexer,Parser,Listener,BaseListener,Visitor,BaseVisitor}.{cpp,h}`) are ANTLR-generated. Change `Vtl.g4` / `VtlTokens.g4` and regenerate. From `src/vtlengine/AST/Grammar/` run: `antlr4 -Dlanguage=Cpp -visitor -o _cpp_parser Vtl.g4`, then rebuild the wheel (`poetry install`). `tokens.py` next to the grammar is hand-maintained, not autogenerated.
...
5. **ANTLR version** - Use 4.13.x to match the bundled C++ runtime (see `scripts/setup_antlr4_runtime.sh`). The legacy `antlr4-python3-runtime` Python dependency was removed when the parser moved to C++ in commit `6ee162c4`.
...
- **ANTLR 4.13.x C++ runtime** (vendored, downloaded by `scripts/setup_antlr4_runtime.sh` and statically linked via `CMakeLists.txt`): backs the compiled parser shipped in the wheel
