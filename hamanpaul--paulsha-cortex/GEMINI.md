...
- **R-23（引擎 pin attestation）**：`policy-check.yml` 的 reusable workflow pin、`policy_engine_ref`、`.project-policy.yml` 的 `policy_version` 必須同步對齊。
- **R-24（moc-alignment）**：僅在 `.project-policy.yml` 宣告 `moc` 時生效；未宣告則 not-applicable。
