# teams-mcp

> Source: [floriscornel/teams-mcp](https://github.com/floriscornel/teams-mcp) — distributed by [TomeVault](https://tomevault.io)

MCP server providing comprehensive Microsoft Teams and Graph API access for AI assistants including messaging, search, and user management.

## Gemini CLI Config

The `GEMINI.md` file in this directory is the project config converted for Gemini CLI.
Original source: `copilot-instructions.md` in [floriscornel/teams-mcp](https://github.com/floriscornel/teams-mcp).

## Also available for

- **Claude Code** — `CLAUDE.md`
- **Codex** — `AGENTS.md`
- **Cursor** — `project-config.mdc`
- **Gemini CLI** — `GEMINI.md`
- **Windsurf** — `project-config.md`

Quality verified by TomeVault's automated analysis pipeline. Source: [github.com/floriscornel/teams-mcp](https://github.com/floriscornel/teams-mcp)

---

Own this repo? Install the TomeVault Relay to keep every platform's copy in sync on every push: [https://tomevault.io/install](https://tomevault.io/install).

<!-- genome:d-c-q -->
