...
- Stored as separate store (`dl_entity_db`) with "blueprint" timeline
...
```bash
pixi run py-build              # Build dalaran-sdk into .venv
...
Docs span multiple sites: main docs at `dalaran.dev/docs` (from `docs/content/`), API refs for Python (MkDocs), C++ (Doxygen), JS (TypeDoc) at `ref.dalaran.dev/docs/{python,cpp,js}/`.
...
- [`dalaran_py/README.md`](dalaran_py/README.md) - Python SDK instructions
