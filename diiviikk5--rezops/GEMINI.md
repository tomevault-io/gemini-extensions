...
or invoking other rezops skills, use the `/rezops-` prefix (e.g., `/rezops-qa` instead
of `/qa`, `/rezops-ship` instead of `/ship`). Disk paths are unaffected — always use
`~/.claude/skills/rezops/[skill-name]/SKILL.md` for reading skill files.

If output shows `UPGRADE_AVAILABLE <old> <new>`: read `~/.claude/skills/rezops/rezops-upgrade/SKILL.md` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise AskUserQuestion with 4 options, write snooze state if declined).

If output shows `JUST_UPGRADED <from> <to>` AND `SPAWNED_SESSION` is NOT set: tell
the user "Running rezops v{to} (just updated!)" and then check for new features to
surface. For each per-feature marker below, if the marker file is missing AND the
feature is plausibly useful for this user, use AskUserQuestion to let them try it.
Fire once per feature per user, NOT once per upgrade.

**In spawned sessions (`SPAWNED_SESSION` = "true"): SKIP feature discovery entirely.**
Just print "Running rezops v{to}" and continue. Orchestrators do not want interactive
prompts from sub-sessions.

**Feature discovery markers and prompts** (one at a time, max one per session):

1. `~/.claude/skills/rezops/.feature-prompted-continuous-checkpoint` →
   Prompt: "Continuous checkpoint auto-commits your work as you go with `WIP:` prefix
   so you never lose progress to a crash. Local-only by default — doesn't push
   anywhere unless you turn that on. Want to try it?"
   Options: A) Enable continuous mode, B) Show me first (print the section from
   the preamble Continuous Checkpoint Mode), C) Skip.
   If A: run `~/.claude/skills/rezops/bin/rezops-config set checkpoint_mode continuous`.
   Always: `touch ~/.claude/skills/rezops/.feature-prompted-continuous-checkpoint`

2. `~/.claude/skills/rezops/.feature-prompted-model-overlay` →
   Inform only (no prompt): "Model overlays are active. `MODEL_OVERLAY: {model}`
   shown in the preamble output tells you which behavioral patch is applied.
   Override with `--model` when regenerating skills (e.g., `bun run gen:skill-docs
   --model gpt-5.4`). Default is claude."
   Always: `touch ~/.claude/skills/rezops/.feature-prompted-model-overlay`

After handling JUST_UPGRADED (prompts done or skipped), continue with the skill
workflow.

If `WRITING_STYLE_PENDING` is `yes`: You're on the first skill run after upgrading
to rezops v1. Ask the user once about the new default writing style. Use AskUserQuestion:
...
If B: run `~/.claude/skills/rezops/bin/rezops-config set explain_level terse`.

Always run (regardless of choice):
```bash
rm -f ~/.rezops/.writing-style-prompt-pending
touch ~/.rezops/.writing-style-prompted
```

This only happens once. If `WRITING_STYLE_PENDING` is `no`, skip this entirely.

If `LAKE_INTRO` is `no`: Before continuing, introduce the Completeness Principle.
Tell the user: "rezops follows the **Boil the Lake** principle — always do the complete
thing when AI makes the marginal cost near-zero. Read more: https://garryslist.org/posts/boil-the-ocean"
Then offer to open the essay in their default browser:

```bash
open https://garryslist.org/posts/boil-the-ocean
touch ~/.rezops/.completeness-intro-seen
```
...
> Help rezops get better! Community mode shares usage data (which skills you use, how long
> they take, crash info) with a stable device ID so we can track trends and fix bugs faster.
> No code, file paths, or repo names are ever sent.
> Change anytime with `rezops-config set telemetry off`.

Options:
- A) Help rezops get better! (recommended)
- B) No thanks

If A: run `~/.claude/skills/rezops/bin/rezops-config set telemetry community`

If B: ask a follow-up AskUserQuestion:

> How about anonymous mode? We just learn that *someone* used rezops — no unique ID,
...
If B→A: run `~/.claude/skills/rezops/bin/rezops-config set telemetry anonymous`
If B→B: run `~/.claude/skills/rezops/bin/rezops-config set telemetry off`

Always run:
```bash
touch ~/.rezops/.telemetry-prompted
```
...
> rezops can proactively figure out when you might need a skill while you work —
...
If A: run `~/.claude/skills/rezops/bin/rezops-config set proactive true`
If B: run `~/.claude/skills/rezops/bin/rezops-config set proactive false`

Always run:
```bash
touch ~/.rezops/.proactive-prompted
```
...
> rezops works best when your project's CLAUDE.md includes skill routing rules.
...
- Upgrade rezops → invoke /rezops-upgrade
- Save progress, "save my work" → invoke /context-save
- Resume, restore, "where was I" → invoke /context-restore
- Security audit, OWASP, "is this secure" → invoke /cso
- Make a PDF, document, publication → invoke /make-pdf
- Launch real browser for QA → invoke /open-rezops-browser
- Import cookies for authenticated testing → invoke /setup-browser-cookies
- Performance regression, page speed, benchmarks → invoke /benchmark
- Review what rezops has learned → invoke /learn
- Tune question sensitivity → invoke /plan-tune
- Code quality dashboard → invoke /health
```

Then commit the change: `git add CLAUDE.md && git commit -m "chore: add rezops skill routing rules to CLAUDE.md"`

If B: run `~/.claude/skills/rezops/bin/rezops-config set routing_declined true`
Say "No problem. You can add routing rules later by running `rezops-config set routing_declined false` and re-running any skill."

This only happens once per project. If `HAS_ROUTING` is `yes` or `ROUTING_DECLINED` is `true`, skip this entirely.

If `VENDORED_REZOPS` is `yes`: This project has a vendored copy of rezops at
...
3. Run `~/.claude/skills/rezops/bin/rezops-team-init required` (or `optional`)
...
```bash
~/.claude/skills/rezops/bin/rezops-learnings-log '{"skill":"SKILL_NAME","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"observed"}'
```
...
`~/.rezops/analytics/` (user config directory, not project files). The skill
...
rm -f ~/.rezops/analytics/.pending-"$_SESSION_ID" 2>/dev/null || true
# Session timeline: record skill completion (local-only, never sent anywhere)
~/.claude/skills/rezops/bin/rezops-timeline-log '{"skill":"SKILL_NAME","event":"completed","branch":"'$(git branch --show-current 2>/dev/null || echo unknown)'","outcome":"OUTCOME","duration_s":"'"$_TEL_DUR"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null || true
# Local analytics (gated on telemetry setting)
if [ "$_TEL" != "off" ]; then
echo '{"skill":"SKILL_NAME","duration_s":"'"$_TEL_DUR"'","outcome":"OUTCOME","browse":"USED_BROWSE","session":"'"$_SESSION_ID"'","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.rezops/analytics/skill-usage.jsonl 2>/dev/null || true
fi
# Remote telemetry (opt-in, requires binary)
if [ "$_TEL" != "off" ] && [ -x ~/.claude/skills/rezops/bin/rezops-telemetry-log ]; then
  ~/.claude/skills/rezops/bin/rezops-telemetry-log \
...
`$B` (browse), `$D` (design), `codex exec`/`codex review`, writes to `~/.rezops/`,
...
In plan mode, before ExitPlanMode: if the plan file lacks a `## rezops REVIEW REPORT`
section, run `~/.claude/skills/rezops/bin/rezops-review-read` and append a report.
...
If `PROACTIVE` is `false`: do NOT proactively invoke or suggest other rezops skills during
this session. Only run skills the user explicitly invokes. This preference persists across
sessions via `rezops-config`.
...
- User asks to upgrade rezops → invoke `/rezops-upgrade`
...
- User asks to launch a real browser for QA, "open the browser" → invoke `/open-rezops-browser`
...
- User asks what rezops has learned, "show learnings" → invoke `/learn`
...
If the user opts out of suggestions, run `rezops-config set proactive false`.
If they opt back in, run `rezops-config set proactive true`.

# rezops browse: QA Testing & Dogfooding
...
B=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/.claude/skills/rezops/browse/dist/browse" ] && B="$_ROOT/.claude/skills/rezops/browse/dist/browse"
[ -z "$B" ] && B="$HOME/.claude/skills/rezops/browse/dist/browse"
if [ -x "$B" ]; then
  echo "READY: $B"
else
  echo "NEEDS_SETUP"
...
If `NEEDS_SETUP`:
1. Tell the user: "rezops browse needs a one-time build (~10 seconds). OK to proceed?" Then STOP and wait.
...
`$B` is the browse binary (resolved from `$_ROOT/.claude/skills/rezops/browse/dist/browse` or `~/.claude/skills/rezops/browse/dist/browse`).
