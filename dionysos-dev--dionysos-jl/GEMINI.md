...
| :--- | :--- | :--- |
| `Utils` | [src/utils/utils.jl](src/utils/utils.jl) | Foundational helpers on top of LazySets: sets *are* LazySets (`LazySets.Hyperrectangle` built as `(; low, high)`, `LazySets.Ellipsoid` with shape matrix `Q = P⁻¹`, unions/set-minus), plus what LazySets lacks — exact ellipsoid kernels (`is_included`/`is_disjoint`), periodic splitting (`set_in_period`), quadratic-form bridge (`get_quadratic_form`) — callable cost functions (`functions.jl`), data structures, `search/RRT.jl`, scalar optimization (`numeric/`), plotting recipes, PCLF. |
...
- Validating constructors (e.g. `LazySets.Hyperrectangle` rejects crossed bounds; an empty
...
**Reuse before writing.** Prefer existing primitives — `LazySets.Hyperrectangle` (always built
from bounds by keyword, `(; low, high)` — positionally it takes a *centre and a radius*),
