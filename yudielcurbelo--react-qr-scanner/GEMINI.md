...
- Default `retryDelay` is 500 ms when no tracker, 10 ms with tracker (Scanner.tsx:205)
...
The library requires secure context (HTTPS or localhost) for camera access (useCamera.ts:25-28). This is a browser
security requirement, not a library limitation.
