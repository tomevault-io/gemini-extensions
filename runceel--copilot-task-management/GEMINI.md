...
- Done に変更すると Issue を自動 close する。
- Done 以外に変更すると、Issue が closed の場合は自動 reopen する。
- プロジェクトの自動検出に対応（`-ProjectNumber` で明示指定も可）。

スクリプトでカバーできないケース（フィールド ID 取得など）では `gh api graphql` を直接使う。
プロジェクト番号は **7**（個人タスク管理デモ）。
