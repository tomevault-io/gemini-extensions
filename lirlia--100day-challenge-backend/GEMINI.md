...
- git commit を行う際は、自分が作成・修正・削除したファイルに限定して実施してください(別のLLMが作業をしている可能性があるため `git add .` などは禁止です)
- アプリを作る際は日本のユーザが使いやすいように、日本語を用いてください
- READMEも日本語で書いてください
- 現在の年をCマークにつける場合は date コマンドで現在の時刻を取得してください
...
- **テスト**
    ...
    - またテストが完了したら git コミットしてください
    - コミットメッセージは dayXX : step x/x xxxxxxx(message) でお願いします
...
- 提案は堅実なものを20個、CSの学習に関連するものを20個だしてください
  - ただし堅実なものについては学びの多さと、実践での複雑さを考慮にいれてください
