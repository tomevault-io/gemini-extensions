...
- **Every model request is durably reconstructable**: interactive requests use the session log, while an explicitly approved capability-owned one-shot uses that capability's durable audit before dispatch. A new interactive model-visible input requires a session event ([request reconstruction](.agents/notes/implemented/architecture/2026-07-05-reconstructable-requests.md)).
