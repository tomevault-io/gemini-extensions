...
- 任何新建或更新 `v*` tag 的正式发版，都必须先补齐对应的用户向 release notes：`docs/releases/vX.Y.Z.md`；缺失时不得打 tag
- 用户如果只要求“push / 推送代码”，但没有明确要求“打 tag / 发版 / release”，agent 不得默认执行正式发版；必须先询问是否需要本次顺带正式发版
