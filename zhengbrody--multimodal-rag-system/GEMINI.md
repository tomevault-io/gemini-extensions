...
If those don't reflect reality, they're the user's to correct. NOTE (2026-07-08): the
Chinese-language internal docs `PROJECT_POSITIONING.md`, `TECHNICAL_GUIDE.md`, and
`INTERVIEW_PREP.md` were REMOVED from the repo at the user's request (job-search/interview
strategy must not be public on GitHub; they remain in git history only). Do not recreate
career-strategy or interview-prep documents inside this repository.
...
## Agentic upgrade engagement (July 2026) — COMPLETE

Branch `feature/agentic-rag` (off `feature/rag-eval-k8s-hardening` @ fdbab25). Upgraded the
fixed retrieve→generate pipeline into an agentic loop; README repositioned to
"Agentic Multimodal RAG". ~30 commits, 199 tests passing (1 skip), CI-green.

**Phase structure (P4 was pulled ahead of P3 at the user's call):**
- **P1 hand-written core** ✅ — `src/agents/`: route → retrieve → grade → (rewrite ≤3) →
  generate → verify → (constrained regenerate ≤1). Typed temp-0 JSON decisions (pydantic),
  budget guards (≤3 iters / ≤10 LLM calls with one reserved for generation / 30s cooperative
  deadline), full `AgentTrace`, fallback-to-baseline on any decision failure (never 500).
  `POST /agent/ask`; `/ask` frozen byte-identical as the A/B control (characterization tests).
- **P2 observability** ✅ — trace→Prometheus mapping (`src/utils/prometheus_metrics.py`,
  `GET /metrics/prometheus`; legacy JSON `/metrics` kept for the frontend + gains an `agent`
  section); logger now renders `extra={}` (JSON suffix; `LOG_FORMAT=json` for full JSON);
  `docs/agent_design.md` (state machine, budgets, fallback matrix, trade-offs).
- **P4 quantified eval** ✅ — `eval/agentic/` (40 cases: 12 single-hop / 10 bad-phrasing /
  10 should-refuse / 8 visual) + `run_comparison.py` with DUAL faithfulness judges
  (independent LLM judge + zero-LLM mechanical grounding). Real numbers in
  `eval/results/phase4_agentic_vs_baseline.md`: agent wins refusal 90→100%, visual routing
  8/8, judged faithfulness 42→57% (CIs overlap, n=40); loses over-refusal/assertions;
  baseline is 100% mechanically grounded (strict prompt already works); cost 5.7× calls,
  ~6.4× p50 latency. `--mode smoke` runs offline in CI; `--mode full` needs OPENAI_API_KEY;
  `--rescore <json> --judge-model X` re-judges saved runs without re-running endpoints.
- **P3 LangGraph twin** ✅ — `src/agents/langgraph_impl/` subclasses `AgentLoop`, overrides
  ONLY orchestration (StateGraph); nodes/budgets/trace/fallbacks shared (zero fork).
  Freebies: MemorySaver checkpointing, `run_interruptible()` (pause-before-generate/resume),
  `stream_steps()`. Optional extra (`requirements_langgraph.txt`; NOT in simple/CI; bumps
  pydantic 2.5.0→2.13.x — suite verified under both). `?impl=langgraph`; 13 equivalence
  tests lock identical outputs + step sequences. `docs/langgraph_vs_handwritten.md`.

**Key decisions (defend these; don't silently reverse):**
- `src/agents/` never imports `src/rag` — dependencies injected via Protocols (dodges the
  repo's dual import convention; makes tests offline-trivial). Decision LLM client is
  INJECTED (reuses `pipeline.client`) because `OpenAI()` raises at construction without a key.
- Confidence thresholds are pipeline-owned (`_confidence_from_scores`; real vs mock scales
  differ); the loop only downgrades (partial→cap medium, unfaithful→low).
- `HeuristicDecisionLLM` is product code: USE_MOCK deployments and CI run the FULL loop
  deterministically at zero cost.
- **Evidence-window lesson (found by the eval, fixed in P4):** decision nodes must see
  (nearly) the evidence generation uses. Grader/verifier previews are 1500 chars, verifier
  top-5 docs — judging truncated evidence produced 17/40 false verify-failures whose
  regenerations DELETED true detail. Same principle in the eval: judge against full doc
  text (preview prefix-expansion), never the 200-char API previews.
- The LLM-as-judge is noisy (gpt-3.5 judged 21% faithful what mechanical grounding proved
  100% grounded) — any faithfulness claim needs the mechanical column alongside.
- Frontend untouched by the whole engagement (calls only /ask + legacy endpoints).

**Env knobs:** `AGENT_MAX_ITERATIONS/MAX_LLM_CALLS/TIMEOUT_S/LLM_CALL_TIMEOUT_S/
DECISION_MODEL/ENABLE_VERIFY/ENABLE_REVERIFY/JSON_MODE/RETRIEVAL_THRESHOLD`, `LOG_FORMAT`,
`AGENT_JUDGE_MODEL` (eval only).

**Local envs:** `.venv-simple` = CI-equivalent (py3.12 + requirements_simple + linters
+ langgraph extra installed locally); `.venv-eval` = Flickr30k harness only. Run agent/API
work from `.venv-simple`: `USE_MOCK=true .venv-simple/bin/python -m pytest tests/`.
