...
- **Lint fails.** Read the first failure in the report. Most come from
  ...
  indentation. The server keeps `.rpy` lint-clean for the pieces IT
  writes — if something's off it's almost always a forward-reference
  you can satisfy by creating the target label next.
- **Preview crashes.** Check `get_preview_status` for the last exit
  code, then `get_lint_report` for the actual problem. The engine's own
  traceback lands in `<project>/traceback.txt`.
