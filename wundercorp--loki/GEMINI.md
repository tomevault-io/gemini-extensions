...
- **Outbound telemetry / usage attribution without opt-in gating.** No analytics,
  ...
  (config gate + setup prompt + `loki tools` toggle) exists. Park behind a label.
...
- **Third-party products integrated into the core tree.** Observability backends, vendor
  ...
  backend we don't own. Ship as a **standalone plugin repo** (`~/.loki/plugins/` or pip
  entry point), promoted in the WunderCorp, Inc. Discord `#plugins-skills-and-skins`. This is a
...
2. **CLI command + skill** — config/state/infra expressible as shell commands; the agent runs
   `loki <subcommand>` guided by a skill. Default for subscriptions, scheduled tasks,
   service setup (`loki webhook`, `loki cron`, `loki tools`).
...
4. **Plugin** — third-party/niche/user-specific; lives in `~/.loki/plugins/` or a pip
...
drive a locally spawned backend, one over SSH, one behind URL + token, or Loki Cloud, and
only the first two carry `LOKI_DESKTOP=1`. An env-keyed gate is a silent no-op on the other
topologies — the tool is stripped from the schema while the platform hint tells the model it
is "inside the Loki desktop app". The pattern:

- **The toolset is the surface gate.** Keep such tools off `_LOKI_CORE_TOOLS` and in a named
...
- **Ask which identity you mean.** `LOKI_DESKTOP=1` legitimately means "this backend was
  spawned by the app" (cron ticker, web-dist handling). It does NOT mean "a GUI is watching";
  the embedded terminal pane (`loki --tui` against that backend) is the counterexample.
...
```
`scripts/run_tests.sh` probes `.venv`, then `venv`, then `$HOME/.loki/loki-agent/venv`
...
├── toolsets.py           # TOOLSETS dict, _LOKI_CORE_TOOLS
├── cli.py                # LokiCLI (REPL, slash dispatch) + loki_cli/cli_*_mixin.py
├── loki_state.py       # SessionDB facade; loki_state_*.py siblings
├── loki_constants.py   # get_loki_home(), display_loki_home() — profile-aware paths
├── loki_logging.py     # agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # turn_*.py loop phases, providers, memory, compression, prompt builder
├── loki_cli/           # CLI subcommands, setup, config, plugins loader, skins, updater
...
├── website/              # Doku docs (developer-guide/ holds the long-form area docs)
└── tests/                # Pytest suite (~39k tests / ~3.7k files, Sep 2026)
```

**User state:** `~/.loki/config.yaml` (settings), `~/.loki/.env` (secrets only),
`~/.loki/logs/` (`agent.log` INFO+, `errors.log` WARNING+, `gateway.log`); all
profile-aware via `get_loki_home()`. Browse logs with `loki logs [--follow] [--level] [--session]`.
...
families: `loki_state.py` (21), `gateway/run.py` (15), `tools/mcp_tool.py` (15),
`loki_cli/kanban.py` (14), `loki_cli/web_server.py` (13 + 24 routers), `loki_cli/auth.py`
(12), `tools/browser_tool.py` (11), `cli.py` (12 `loki_cli/cli_*_mixin.py`), `run_agent.py`
...
- **Compat pointers are OFF LIMITS in-tree.** Old import paths kept alive for external plugins
  ...
  `-W error::loki_cli.plugin_compat.LokiPluginCompatWarning` catches them in the suite.
...
- **Never infer process identity from argv substrings** (`"serve" in cmdline`) — the bug class
  ...
  `loki_cli.update_cmd._loki_holder_subcommand`; flag sets are DERIVED from the parser
  (`_holder_value_flags()`), never hand-written; match FULL cmdlines and truncate only for
  display. Details: `loki_cli/AGENTS.md`.
- **Never hardcode `~/.loki`.** `get_loki_home()` for code paths, `display_loki_home()`
  for user-facing text (both from `loki_constants`). Hardcoding breaks profiles (5 bugs in
  PR #3575). Module-level constants are fine — they cache after `_apply_profile_override()`
  sets `LOKI_HOME`. Profile operations themselves are HOME-anchored
  (`_get_profiles_root()` = `Path.home()/.loki/profiles`) so `loki -p x profile list`
  sees all profiles — intentional, not a bug.
- **Argparse alias dispatch:** `add_parser("list", aliases=["ls"])` sets `dest` to the literal
  the user typed (`"ls"`). Dispatch must accept both (caught PTY-testing `loki webhook ls`).
- **Don't wire in dead code without E2E validation.** Unshipped code was dead for a reason;
  E2E the real resolution chain with real imports against a temp `LOKI_HOME` first.
...
vars unset, `TZ=UTC`, `LANG=C.UTF-8`, `LOKI_HOME` → temp dir, and per-file subprocess
...
- **Flake policy:** a failing FILE is retried once in a fresh subprocess (`--file-retries`;
  `LOKI_TEST_FILE_RETRIES=0` disables). Pass-on-retry is green but printed under `⚠ FLAKY`
...
- **Tests must not write to `~/.loki/`.** The autouse `_isolate_loki_home` fixture in
  `tests/conftest.py` redirects `LOKI_HOME`; never hardcode `~/.loki/` in tests. Profile
  tests also mock `Path.home()` so `_get_profiles_root()` / `_get_default_loki_home()` stay
  in the temp dir (pattern: `tests/loki_cli/test_profiles.py`):
  ```python
  @pytest.fixture
  def profile_env(tmp_path, monkeypatch):
      home = tmp_path / ".loki"; home.mkdir()
      monkeypatch.setattr(Path, "home", lambda: tmp_path)
      monkeypatch.setenv("LOKI_HOME", str(home))
      return home
  ```
  Tests that `patch.object(Path, "home", ...)` must ALSO set `LOKI_HOME` — code reads the
  env var, not `Path.home()/.loki`.
...
`tests/loki_cli/test_venv_holder_windows_live.py` on a real `windows-latest` runner (real
...
| `cli.py`, `loki_cli/`, `main.py` | `loki_cli/AGENTS.md` | CLI mixins, `_SLASH_DISPATCH`, slash registry, config system + loaders, skins, `loki update` pipeline, profiles / multiplex |
...
| `cron/`, kanban (`loki_cli/kanban*.py`, `tools/kanban_tools.py`, `plugins/kanban/`) | `cron/AGENTS.md` | Scheduler invariants, job fields, kanban board/dispatcher |
