...
- Retrosheet is fetched directly from `https://retrosheet.org/downloads/alldata.zip` and integrity-pinned via `RETROSHEET_SHA256` in `.env` (the URL is mutable in place per release; `RETROSHEET_RELEASE_DATE` is documentation only). The Dockerfile sends a `Mozilla/5.0 (compatible; boxball-build)` User-Agent — retrosheet.org returns 403 on default `wget`/`curl` UAs. Baseball Databank is pinned by git SHA on `corbtastik/lahman-baseball-db` (SABR Lahman v2025; flat tree of CSVs at the repo root, rehomed under `/baseballdatabank/core/` by the Dockerfile). `chadwickbureau/baseballdatabank` was retired in 2024. See `docs/adr/0002-data-sources.md`.
- Lahman v2025 reorganized two tables: `baseballdatabank.people` and `baseballdatabank.parks` gained a leading sequential `id` column and reordered some existing columns. The schemas absorb the column-position changes while preserving all existing column NAMES and PK identity, so existing queries (`WHERE player_id = …`, `WHERE park_id = …`) keep working. AllstarFull's `starting_pos` keeps its `SmallInteger` type; `parsers/baseballdatabank.py` collapses Negro Leagues `9;9` multi-position values to the first integer.
