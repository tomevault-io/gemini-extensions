...
*   When targeting ES2022+ with [`useDefineForClassFields`](mdc:https:/www.typescriptlang.org/tsconfig/#useDefineForClassFields), you can avoid separating field declaration and initialization when fields read on injected dependencies.

[You can refactor existing code to `inject` with an automatic tool](mdc:reference/migrations/inject-function).
...
See
the [Components guide for details on choosing component selectors](mdc:guide/components/selectors#choosing-a-selector).

### Naming component and directive members

See the Components guide for details
on [naming input properties](mdc:guide/components/inputs#choosing-input-names)
and [naming output properties](mdc:guide/components/outputs#choosing-event-names).

### Choosing directive selectors

Directives should use the
same [application-specific prefix](mdc:guide/components/selectors#selector-prefixes)
as your components.
...
accommodate [JavaScript-like expressions](mdc:guide/templates/expression-syntax).
...
typically with a [computed](mdc:guide/signals#computed-signals)).
...
dependency injection and [queries](mdc:guide/components/queries). Prefer `protected`
...
1. A `@Component`[decorator](mdc:https:/www.typescriptlang.org/docs/handbook/decorators.html) that contains some configuration used by Angular.
2. An HTML template that controls what renders into the DOM.
3. A [CSS selector](mdc:https:/developer.mozilla.org/docs/Learn/CSS/Building_blocks/Selectors) that defines how the component is used in HTML.
...
TIP: Want to know more about Angular components? See the [In-depth Components guide](mdc:guide/components) for the full details.
...
TIP: This guide assumes you've already read the [Essentials Guide](mdc:essentials). Read that first if you're new to Angular.

Every component defines
a [CSS selector](mdc:https:/developer.mozilla.org/docs/Web/CSS/CSS_selectors) that determines how
...
of [basic CSS selector types](mdc:https:/developer.mozilla.org/docs/Web/CSS/CSS_Selectors) in
component selectors:
...
the [descendant combinator](mdc:https:/developer.mozilla.org/docs/Web/CSS/Descendant_combinator)
or [child combinator](mdc:https:/developer.mozilla.org/docs/Web/CSS/Child_combinator).

Angular component selectors do not support
specifying [namespaces](mdc:https:/developer.mozilla.org/docs/Web/SVG/Namespaces_Crash_Course).

### The `:not` pseudo-class

Angular supports [the `:not` pseudo-class](mdc:https:/developer.mozilla.org/docs/Web/CSS/:not).
...
by [the HTML specification](mdc:https:/html.spec.whatwg.org/multipage/custom-elements.html#valid-custom-element-name).
...
See [Advanced component configuration](mdc:guide/components/advanced-configuration) for details on
using [native custom elements](mdc:https:/developer.mozilla.org/docs/Web/Web_Components) in
Angular templates.
...
See [Importing and using components](mdc:guide/components/importing) for more information.
...
including [Sass](mdc:https:/sass-lang.com), [less](mdc:https:/lesscss.org),
and [stylus](mdc:https:/stylus-lang.com).
...
the [`:host`](mdc:https:/developer.mozilla.org/docs/Web/CSS/:host)
and [`:host-context()`](mdc:https:/developer.mozilla.org/docs/Web/CSS/:host-context) pseudo
classes without
using [Shadow DOM](mdc:https:/developer.mozilla.org/docs/Web/Web_Components/Using_shadow_DOM).
...
using [the web standard Shadow DOM API](mdc:https:/developer.mozilla.org/docs/Web/Web_Components/Using_shadow_DOM).
...
with [the `<slot>` API](mdc:https:/developer.mozilla.org/docs/Web/Web_Components/Using_templates_and_slots),
...
use [the `<link>` element](mdc:https:/developer.mozilla.org/docs/Web/HTML/Element/link) to
reference CSS files. Additionally, your CSS may
use [the `@import`at-rule](mdc:https:/developer.mozilla.org/docs/Web/CSS/@import) to reference
...
**Input transform functions should always be [pure functions](mdc:https:/en.wikipedia.org/wiki/Pure_function).** Relying on state outside the transform function can lead to unpredictable behavior.
...
```
`booleanAttribute` imitates the behavior of standard HTML [boolean attributes](mdc:https:/developer.mozilla.org/docs/Glossary/Boolean/HTML), where the
...
See [Two-way binding](mdc:guide/templates/two-way-binding) for more details on two-way binding in templates.
...
When you declare a model input in a component or directive, Angular automatically creates a corresponding [output](mdc:guide/components/outputs) for that model. The output's name is the model input's name suffixed with "Change".
...
See [Custom events with outputs](mdc:guide/components/outputs) for more details on outputs.

### Customizing model inputs

You can mark a model input as required or provide an alias in the same way as a [standard input](mdc:guide/signals/inputs).
...
Always use [camelCase](mdc:https:/en.wikipedia.org/wiki/Camel_case) output names. Avoid prefixing output names with "on".

## Using outputs with RxJS

See [RxJS interop with component and directive outputs](mdc:ecosystem/rxjs-interop/output-interop) for details on interoperability between outputs and RxJS.
...
to [the native `<slot>` element](mdc:https:/developer.mozilla.org/docs/Web/HTML/Element/slot),
...
see [Template fragments](mdc:api/core/ng-template).
...
as [component selectors](mdc:guide/components/selectors).
...
a [`Record`](mdc:https:/www.typescriptlang.org/docs/handbook/utility-types.html#recordkeys-type)
...
[content queries](mdc:guide/components/queries#content-queries). While you can access the initialized
state of these queries, attempting to change any state in this method results in an
[ExpressionChangedAfterItHasBeenCheckedError](mdc:errors/NG0100)
...
of [content queries](mdc:guide/components/queries#content-queries) here, attempting to
change any state in this method results in
an [ExpressionChangedAfterItHasBeenCheckedError](mdc:errors/NG0100).
...
You can use this lifecycle hook to read the results of
[view queries](mdc:guide/components/queries#view-queries). While you can access the initialized state of
...
While you can access the updated state of [view queries](mdc:guide/components/queries#view-queries)
here, attempting to
change any state in this method results in
an [ExpressionChangedAfterItHasBeenCheckedError](mdc:errors/NG0100).
...
an [injection context](mdc:guide/di/dependency-injection-context), typically a
component's constructor.

You can use render callbacks to perform manual DOM operations.
See [Using DOM APIs](mdc:guide/components/dom-apis) for guidance on working with the DOM in Angular.
...
[layout thrashing](mdc:https:/web.dev/avoid-large-complex-layouts-and-layout-thrashing). In order to
...
If you need to pass the [event](mdc:https:/developer.mozilla.org/docs/Web/API/Event) object to your listener, you can use Angular's built-in `$event` variable inside the function call:
...
Angular's uses the `track` keyword, shown in the example above, to associate data with the DOM elements created by `@for`. See [_Why is track in @for blocks important?_](mdc:guide/templates/control-flow#why-is-track-in-for-blocks-important) for more info.

TIP: Want to know more about Angular templates? See the [In-depth Templates guide](mdc:guide/templates) for the full details.
...
You can add listeners for any native events, such as: `click`, `keydown`, `mouseover`, etc. To learn more, check out the [all available events on elements on MDN](mdc:https:/developer.mozilla.org/en-US/docs/Web/API/Element#events).
...
You can specify the key or code that you would like to bind to keyboard events. The key and code fields are a native part of the browser keyboard event object. By default, event binding assumes you want to use the [Key values for keyboard events](mdc:https:/developer.mozilla.org/docs/Web/API/UI_Events/Keyboard_event_key_values).

Angular also allows you to specify [Code values for keyboard events](mdc:https:/developer.mozilla.org/docs/Web/API/UI_Events/Keyboard_event_code_values) by providing a built-in `code` suffix.
...
If your event handler should replace the native browser behavior, you can use the event object's [`preventDefault` method](mdc:https:/developer.mozilla.org/en-US/docs/Web/API/Event/preventDefault):
...
If the event handler statement evaluates to `false`, Angular automatically calls `preventDefault()`, similar to [native event handler attributes](mdc:https:/developer.mozilla.org/en-US/docs/Web/HTML/Reference/Attributes#event_handler_attributes). *Always prefer explicitly calling `preventDefault`*, as this approach makes the code's intent obvious.
...
You can bind to properties on an HTML element's DOM instance, a [component](mdc:guide/components) instance, or a [directive](mdc:guide/directives) instance.
...
You can create a CSS class binding to conditionally add or remove a CSS class on an element based on whether the bound value is [truthy or falsy](mdc:https:/developer.mozilla.org/en-US/docs/Glossary/Truthy).
...
The `@for` block loops through a collection and repeatedly renders the content of a block. The collection can be any JavaScript [iterable](mdc:https:/developer.mozilla.org/docs/Web/JavaScript/Reference/Iteration_protocols), but Angular has additional performance optimizations for `Array` values.
...
Angular's `@let` syntax allows you to define a local variable and re-use it across a template, similar to the [JavaScript `let` syntax](mdc:https:/developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/let).

### Using `@let`

Use `@let` to declare a variable whose value is based on the result of a template expression. Angular automatically keeps the variable's value up-to-date with the given expression, similar to [bindings](mdc:templates/bindings).
...
- a DOM element within a template (including [custom elements](mdc:https:/developer.mozilla.org/en-US/docs/Web/API/Web_components/Using_custom_elements))
- an Angular component or directive
- a [TemplateRef](mdc:api/core/TemplateRef) from an [ng-template](mdc:api/core/ng-template)
...
If you declare the variable on an `<ng-template>` element, the variable refers to a TemplateRef instance which represents the template. For more information, see [How Angular uses the asterisk, \*, syntax](mdc:guide/directives/structural-directives#structural-directive-shorthand) in [Structural directives](mdc:guide/directives/structural-directives).
...
In addition to using template variables to read values from another part of the same template, you can also use this style of variable declaration to "mark" an element for [component and directive queries](mdc:guide/components/queries).
...
See [Referencing children with queries](mdc:guide/components/queries) for more information on queries.
...
This is the primary block that defines the section of content that is lazily loaded. It is not rendered initially– deferred content loads and renders once the specified [trigger](mdc:guide/templates/defer#triggers) occurs or the `when` condition is met.

By default, a @defer block is triggered when the browser state becomes [idle](mdc:guide/templates/defer#idle).
...
While optional, certain triggers may require the presence of either a `@placeholder` or a [template reference variable](mdc:guide/templates/variables#template-reference-variables) to function. See the [Triggers](mdc:guide/templates/defer#triggers) section for more details.
...
| Trigger                       | Description                                                            |
| ----------------------------- | ---------------------------------------------------------------------- |
| [`idle`](mdc:#idle)               | Triggers when the browser is idle.                                     |
| [`viewport`](mdc:#viewport)       | Triggers when specified content enters the viewport                    |
| [`interaction`](mdc:#interaction) | Triggers when the user interacts with specified element                |
| [`hover`](mdc:#hover)             | Triggers when the mouse hovers over specified area                     |
| [`immediate`](mdc:#immediate)     | Triggers immediately after non-deferred content has finished rendering |
| [`timer`](mdc:#timer)             | Triggers after a specific duration                                     |
...
The `viewport` trigger loads the deferred content when the specified content enters the viewport using the [Intersection Observer API](mdc:https:/developer.mozilla.org/docs/Web/API/Intersection_Observer_API). Observed content may be `@placeholder` content or an explicit element reference.
...
Alternatively, you can specify a [template reference variable](mdc:guide/templates/variables) in the same template as the `@defer` block as the element that is watched to enter the viewport. This variable is passed in as a parameter on the viewport trigger.
...
Alternatively, you can specify a [template reference variable](mdc:guide/templates/variables) in the same template as the `@defer` block as the element that is watched for interactions. This variable is passed in as a parameter on the viewport trigger.
...
To render the main content of `@defer` blocks on the server (both SSR and SSG), you can enable [the Incremental Hydration feature](mdc:guide/incremental-hydration) and configure `hydrate` triggers for the necessary blocks.
...
Angular supports a subset of [literal values](mdc:https:/developer.mozilla.org/en-US/docs/Glossary/Literal) from JavaScript.
...
Angular expressions support the following [globals](mdc:https:/developer.mozilla.org/en-US/docs/Glossary/Global_object):

- [undefined](mdc:https:/developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/undefined)
- [$any](mdc:https:/www.typescriptlang.org/docs/handbook/2/everyday-types.html#any)
...
| Operator                        | Example(s)                     |
| ------------------------------- | ------------------------------ |
| [Pipe](mdc:guide/templates/pipes) | `{{ total \| currency }}`      |
...
1. To create a directive, use the CLI command [`ng generate directive`](mdc:tools/cli/schematics).
...
1. Add `ElementRef` in the directive's `constructor()` to [inject](mdc:guide/di) a reference to the host DOM element, the element to which you apply `appHighlight`.
...
`SelectDirective` will have an input naming the data source to be used, which you will call `selectFrom`. The `select` prefix for this input is important for the [shorthand syntax](mdc:#structural-directive-shorthand). The directive will instantiate its `<ng-template>` with a template context providing the selected data.
...
For more information, see the [ng-template API](mdc:api/core/ng-template) documentation.
...
Shorthand syntax is expanded through a set of conventions. A more thorough [grammar](mdc:#structural-directive-syntax-reference) is defined below, but in the above example, this transformation can be explained as follows:
...
That's it - `SelectDirective` is up and running. A follow-up step might be to [add template type-checking support](mdc:#typing-the-directives-context).
...
For more information, see [Template type checking](mdc:tools/cli/template-typecheck "Template type-checking guide").
...
[Including inputs and outputs](mdc:#including-inputs-and-outputs) below for more information.
...
The directive ensures that the loading of the [Largest Contentful Paint (LCP)](mdc:http:/web.dev/lcp) image is prioritized by:

* Automatically setting the `fetchpriority` attribute on the `<img>` tag
* Lazy loading other images by default
* Automatically generating a preconnect link tag in the document head
* Automatically generating a `srcset` attribute
* Generating a [preload hint](mdc:https:/developer.mozilla.org/docs/Web/HTML/Link_types/preload) if app is using SSR
...
* Using [image CDN URLs to apply image optimizations](mdc:https:/web.dev/image-cdns/#how-image-cdns-use-urls-to-indicate-optimization-options)
...
If you're using a background image in CSS, [start here](mdc:#how-to-migrate-your-background-image).
...
A brief guide for setting up a loader can be found in the [Configuring an Image Loader](mdc:#configuring-an-image-loader-for-ngoptimizedimage) section at the end of this page.
</docs-step>
...
<img ngSrc="cat.jpg">
```
If you're using a [built-in third-party loader](mdc:#built-in-loaders), make sure to omit the base URL path from `src`, as that will be prepended automatically by the loader.
</docs-step>
<docs-step title="Mark images as `priority`">
Always mark the [LCP image](mdc:https:/web.dev/lcp/#what-elements-are-considered) on your page as `priority` to prioritize its loading.
...
* Sets `fetchpriority=high` (read more about priority hints [here](mdc:https:/web.dev/priority-hints))
* Sets `loading=eager` (read more about native lazy loading [here](mdc:https:/web.dev/browser-level-image-lazy-loading))
* Automatically generates a [preload link element](mdc:https:/developer.mozilla.org/docs/Web/HTML/Link_types/preload) if [rendering on the server](mdc:guide/ssr).

Angular displays a warning during development if the LCP element is an image that does not have the `priority` attribute. A page’s LCP element can vary based on a number of factors - such as the dimensions of a user's screen, so a page may have multiple images that should be marked `priority`. See [CSS for Web Vitals](mdc:https:/web.dev/css-web-vitals/#images-and-largest-contentful-paint-lcp) for more details.
</docs-step>
<docs-step title="Include Width and Height">
In order to prevent [image-related layout shifts](mdc:https:/web.dev/css-web-vitals/#images-and-layout-shifts), NgOptimizedImage requires that you specify a height and width for your image, as follows:

```html
<img ngSrc="cat.jpg" width="400" height="200">
```
For **responsive images** (images which you've styled to grow and shrink relative to the viewport), the `width` and `height` attributes should be the intrinsic size of the image file. For responsive images it's also important to [set a value for `sizes`.](mdc:#responsive-images)
...
```
You can use the [object-fit](mdc:https:/developer.mozilla.org/docs/Web/CSS/object-fit) CSS property to change how the image will fill its container. If you style your image with `object-fit: "contain"`, the image will maintain its aspect ratio and be "letterboxed" to fit the element. If you set `object-fit: "cover"`, the element will retain its aspect ratio, fully fill the element, and some content may be "cropped" off.

See visual examples of the above at the [MDN object-fit documentation.](mdc:https:/developer.mozilla.org/docs/Web/CSS/object-fit)

You can also style your image with the [object-position property](mdc:https:/developer.mozilla.org/docs/Web/CSS/object-position) to adjust its position within its containing element.
...
5) If you believe this image might be your [LCP element](mdc:https:/web.dev/lcp), add the `priority` attribute to the image element.

You can adjust how the background image fills the container as described in the [Using fill mode](mdc:#using-fill-mode) section.
## Using placeholders
...
You can typically fix this by adding `height: auto` or `width: auto` to your image styles. For more information, see the [web.dev article on the `<img>` tag](mdc:https:/web.dev/patterns/web-vitals-patterns/images/img-tag).
...
A [`preconnect` resource hint](mdc:https:/web.dev/preconnect-and-dns-prefetch) for your image origin ensures that the LCP image loads as quickly as possible.

Preconnect links are automatically generated for domains provided as an argument to a [loader](mdc:#optional-set-up-a-loader). If an image origin cannot be automatically identified, and no preconnect link is detected for the LCP image, `NgOptimizedImage` will warn during development. In that case, you should manually add a resource hint to `index.html`. Within the `<head>` of the document, add a `link` tag with `rel="preconnect"`, as shown below:
...
```
See more information on automatic preconnect generation [here](mdc:#why-is-a-preconnect-element-not-being-generated-for-my-image-domain).
### Request images at the correct size with automatic `srcset`

Defining a [`srcset` attribute](mdc:https:/developer.mozilla.org/docs/Web/API/HTMLImageElement/srcset) ensures that the browser requests an image at the right size for your user's viewport, so it doesn't waste time downloading an image that's too large. `NgOptimizedImage` generates an appropriate `srcset` for the image, based on the presence and value of the [`sizes` attribute](mdc:https:/developer.mozilla.org/docs/Web/API/HTMLImageElement/sizes) on the image tag.

#### Fixed-size images

If your image should be "fixed" in size  (i.e. the same size across devices, except for [pixel density](mdc:https:/web.dev/codelab-density-descriptors)), there is no need to set a `sizes` attribute. A `srcset` can be generated automatically from the image's width and height attributes with no further input required.
...
If your image should be responsive (i.e. grow and shrink according to viewport size), then you will need to define a [`sizes` attribute](mdc:https:/developer.mozilla.org/docs/Web/API/HTMLImageElement/sizes) to generate the `srcset`.
...
If you find that the above does not cover your desired image behavior, see the documentation on [advanced sizes values](mdc:#advanced-sizes-values).
...
By default, `NgOptimizedImage` sets `loading=lazy` for all images that are not marked `priority`. You can disable this behavior for non-priority images by setting the `loading` attribute. This attribute accepts values: `eager`, `auto`, and `lazy`. [See the documentation for the standard image `loading` attribute for details](mdc:https:/developer.mozilla.org/docs/Web/API/HTMLImageElement/loading#value).
...
For additional information about the `sizes` attribute, see [web.dev](mdc:https:/web.dev/learn/design/responsive-images/#sizes) or [mdn](mdc:https:/developer.mozilla.org/docs/Web/API/HTMLImageElement/sizes).

## Configuring an image loader for `NgOptimizedImage`

A "loader" is a function that generates an [image transformation URL](mdc:https:/web.dev/image-cdns/#how-image-cdns-use-urls-to-indicate-optimization-options) for a given image file. When appropriate, `NgOptimizedImage` sets the size, format, and image quality transformations for an image.
...
|:--- |:--- |:--- |
| Cloudflare Image Resizing | `provideCloudflareLoader` | [Documentation](mdc:https:/developers.cloudflare.com/images/image-resizing) |
| Cloudinary | `provideCloudinaryLoader` | [Documentation](mdc:https:/cloudinary.com/documentation/resizing_and_cropping) |
| ImageKit | `provideImageKitLoader` | [Documentation](mdc:https:/docs.imagekit.io) |
| Imgix | `provideImgixLoader` | [Documentation](mdc:https:/docs.imgix.com) |
| Netlify | `provideNetlifyLoader` | [Documentation](mdc:https:/docs.netlify.com/image-cdn/overview) |
...
For a step-by-step process for migration from `background-image` to `NgOptimizedImage`, see the [How to migrate your background image](mdc:#how-to-migrate-your-background-image) section above.
...
```
If you use a variable to pass the domain string to the loader, or you're not using a loader, the static analysis will not be able to identify the domain, and no preconnect link will be generated. In this case you should manually add a preconnect link to the document head, as [described above](mdc:#add-resource-hints).
### Can I use two different image domains in the same page?

The [image loaders](mdc:#configuring-an-image-loader-for-ngoptimizedimage) provider pattern is designed to be as simple as possible for the common use case of having only a single image CDN used within a component. However, it's still very possible to manage multiple image CDNs using a single provider.

To do this, we recommend writing a [custom image loader](mdc:#custom-loaders) which uses the [`loaderParams` property](mdc:#the-loaderparams-property) to pass a flag that specifies which image CDN should be used, and then invokes the appropriate loader based on that flag.
...
If you're waiting on this feature, please upvote the Github issue [here](mdc:https:/github.com/angular/angular/issues/56594).
...
TIP: Want to know more about Angular Signals? See the [In-depth Signals guide](mdc:guide/signals) for the full details.
...
IMPORTANT: `resource` is [experimental](mdc:reference/releases#experimental). It's ready for you to try, but it might change before it is stable.
...
The `loader` property defines a `ResourceLoader`— an async function that retrieves some state. The resource calls the loader every time the `params` computation produces a new value, passing that value to the loader. See [Resource loaders](mdc:#resource-loaders) below for more details.
...
| `abortSignal` | An [`AbortSignal`](mdc:https:/developer.mozilla.org/en-US/docs/Web/API/AbortSignal). See [Aborting requests](mdc:#aborting-requests) below for details. |
...
See [`AbortSignal` on MDN](mdc:https:/developer.mozilla.org/en-US/docs/Web/API/AbortSignal) for more details on request cancellation with `AbortSignal`.
...
This topic covers basic scenarios of how a class can act as a dependency. Angular also allows you to use functions, objects, primitive types such as string or Boolean, or any other types as dependencies. For more information, see [Dependency providers](mdc:guide/di/dependency-injection-providers).
...
- [**Preferred**: At the application root level using `providedIn`](mdc:#preferred-at-the-application-root-level-using-providedin)
- [At the Component level](mdc:#at-the-component-level)
- [At the application root level using `ApplicationConfig`](mdc:#at-the-application-root-level-using-applicationconfig)
- [`NgModule` based applications](mdc:#ngmodule-based-applications)
...
To understand all edge-cases, see [Hierarchical injectors](mdc:guide/di/hierarchical-dependency-injection).
...
You can use the `inject` function in any [injection context](mdc:guide/di/dependency-injection-context). Most of the time, this is in a class property initializer or a class constructor for components, directives, services, and pipes.
...
1. Run this [Angular CLI](mdc:tools/cli) command:
...
Knowing when you are in an injection context will allow you to use the [`inject`](mdc:api/core/inject) function to inject instances.

## Class constructors

Every time the DI system instantiates a class, it does so in an injection context. This is handled by the framework itself. The constructor of the class is executed in that runtime context, which also allows injection of a token using the [`inject`](mdc:api/core/inject) function.
...
Some APIs are designed to be run in an injection context. This is the case, for example, with router guards. This allows the use of [`inject`](mdc:api/core/inject) within the guard function to access a service.
...
Calling [`inject`](mdc:api/core/inject) or calling `assertInInjectionContext` outside of an injection context will throw [error NG0203](mdc:errors/NG0203).
...
For more information on `@Optional()`, see the [`@Optional()` section](mdc:#optional) of this guide.
...
Here is an example of the case where the component router configuration includes a non-default [location strategy](mdc:guide/routing#location-strategy) by listing its provider in the `providers` list of the `ApplicationConfig`.
...
```
HELPFUL: See the [resolution rules](mdc:#resolution-rules) section to understand the relationship between the `EnvironmentInjector` tree, the `ModuleInjector` and the `ElementInjector` tree.
...
It may also be visible at child component/directives based on visibility rules described in the [resolution rules](mdc:#resolution-rules) section.
...
If you can set it up on your own, skip ahead to [Modifying service availability](mdc:#visibility-of-provided-tokens).
...
* The token is used in the value position of a [content query](mdc:guide/components/queries#content-queries).
...
HELPFUL: Libraries should use [tree-shakable providers](mdc:guide/di/dependency-injection#providing-dependency) for all services, providing dependencies at the root level rather than in components or modules.
...
`toSignal` by default needs to run in an [injection context](mdc:guide/di/dependency-injection-context), such as during construction of a component or service. If an injection context is not available, you can manually specify the `Injector` to use instead.
...
`toObservable` by default needs to run in an [injection context](mdc:guide/di/dependency-injection-context), such as during construction of a component or service. If an injection context is not available, you can manually specify the `Injector` to use instead.
...
TIP: This guide assumes you're familiar with [component and directive outputs](mdc:guide/components/outputs).
...
Before you can use `HttpClient` in your app, you must configure it using [dependency injection](mdc:guide/di).
...
```
By default, `HttpClient` uses the [`XMLHttpRequest`](mdc:https:/developer.mozilla.org/docs/Web/API/XMLHttpRequest) API to make requests. The `withFetch` feature switches the client to use the [`fetch`](mdc:https:/developer.mozilla.org/docs/Web/API/Fetch_API) API instead.
...
`withInterceptors` configures the set of interceptor functions which will process requests made through `HttpClient`. See the [interceptor guide](mdc:guide/http/interceptors) for more information.

### `withInterceptorsFromDi()`

`withInterceptorsFromDi` includes the older style of class-based interceptors in the `HttpClient` configuration. See the [interceptor guide](mdc:guide/http/interceptors) for more information.
...
Including `withJsonpSupport` enables the `.jsonp()` method on `HttpClient`, which makes a GET request via the [JSONP convention](mdc:https:/en.wikipedia.org/wiki/JSONP) for cross-domain loading of data.

HELPFUL: Prefer using [CORS](mdc:https:/developer.mozilla.org/docs/Web/HTTP/CORS) to make cross-domain requests instead of JSONP when possible.

### `withXsrfConfiguration(...)`

Including this option allows for customization of `HttpClient`'s built-in XSRF security functionality. See the [security guide](mdc:best-practices/security) for more information.

### `withNoXsrfProtection()`

Including this option disables `HttpClient`'s built-in XSRF security functionality. See the [security guide](mdc:best-practices/security) for more information.
...
`HttpClient` has methods corresponding to the different HTTP verbs used to make requests, both to load data and to apply mutations on the server. Each method returns an [RxJS `Observable`](mdc:https:/rxjs.dev/guide/observable) which, when subscribed, sends the request and then emits the results when the server responds.
...
Fetching data from a backend often requires making a GET request using the [`HttpClient.get()`](mdc:api/common/http/HttpClient#get) method. This method takes two arguments: the string endpoint URL from which to fetch, and an *optional options* object to configure the request.
...
| `'text'` | string data |
| `'arraybuffer'` | [`ArrayBuffer`](mdc:https:/developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/ArrayBuffer) containing the raw response bytes |
| `'blob'` | [`Blob`](mdc:https:/developer.mozilla.org/docs/Web/API/Blob) instance |
...
The [`HttpClient.post()`](mdc:api/common/http/HttpClient#post) method behaves similarly to `get()`, and accepts an additional `body` argument before its options:
...
| [`ArrayBuffer`](mdc:https:/developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/ArrayBuffer) | raw data from the buffer |
| [`Blob`](mdc:https:/developer.mozilla.org/docs/Web/API/Blob) | raw data with the `Blob`'s content type |
| [`FormData`](mdc:https:/developer.mozilla.org/docs/Web/API/FormData) | `multipart/form-data` encoded data |
| [`HttpParams`](mdc:api/common/http/HttpParams) or [`URLSearchParams`](mdc:https:/developer.mozilla.org/docs/Web/API/URLSearchParams) | `application/x-www-form-urlencoded` formatted string |
...
Reactive forms differ from [template-driven forms](mdc:guide/forms/template-driven-forms) in distinct ways.
...
See the [Forms Overview](mdc:guide/forms) for detailed comparisons between the two paradigms.
...
These properties and methods of the underlying [AbstractControl](mdc:api/forms/AbstractControl 'API reference') class are used to control form state and determine when to display messages when handling [input validation](mdc:#validating-form-input 'Learn more about validating form input').

Read about other `FormControl` properties and methods in the [API Reference](mdc:api/forms/FormControl 'Detailed syntax reference').
...
When using the `setValue()` method with a [form group](mdc:#grouping-form-controls) or [form array](mdc:#creating-dynamic-forms) instance, the value needs to match the structure of the group or array.
...
| Form groups | Details                                                                                                                                                                                                                                                |
| :---------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Form group  | Defines a form with a fixed set of controls that you can manage together. Form group basics are discussed in this section. You can also [nest form groups](mdc:#creating-nested-form-groups 'See more about nesting groups') to create more complex forms. |
| Form array  | Defines a dynamic form, where you can add and remove controls at run time. You can also nest form arrays to create more complex forms. For more about this option, see [Creating dynamic forms](mdc:#creating-dynamic-forms).                              |
...
The button in the preceding snippet also has a `disabled` binding attached to it to disable the button when `profileForm` is invalid. You aren't performing any validation yet, so the button is always enabled. Basic form validation is covered in the [Validating form input](mdc:#validating-form-input) section.
</docs-step>
...
Form validation is covered more extensively in the [Form Validation](mdc:guide/forms/form-validation) guide.
...
For more on form validation, visit the [Form Validation](mdc:guide/forms/form-validation) guide.
</docs-step>
</docs-workflow>
...
For complete syntax details, see the API reference documentation for the [Forms package](mdc:api#forms 'API reference').
...
As background for this guide, you should already be familiar with [Angular Reactive Forms](mdc:guide/forms/reactive-forms).
...
Angular provides many APIs for interacting with this `FormGroup`. For example, you may call `login.value`, `login.controls`, `login.patchValue`, etc. (For a full API reference, see the [API documentation](mdc:api/forms/FormGroup).)
...
These improvements currently apply only to *reactive* forms (not [*template-driven* forms](mdc:guide/forms/template-driven-forms)).
...
This control will be automatically inferred to have the type `FormControl<string|null>`. TypeScript will automatically enforce this type throughout the [`FormControl` API](mdc:api/forms/FormControl), such as `email.value`, `email.valueChanges`, `email.setValue(...)`, etc.
...
On any `FormGroup`, it is [possible to disable controls](mdc:api/forms/FormGroup). Any disabled control will not appear in the group's value.
...
Template-driven forms use [two-way data binding](mdc:guide/templates/two-way-binding) to update the data model in the component as changes are made in the template and vice versa.
...
Template-driven forms are a great choice for small or simple forms, while reactive forms are more scalable and suitable for complex forms. For a comparison of the two approaches, see [Choosing an approach](mdc:guide/forms#choosing-an-approach)
...
- Share information across HTML elements using [template reference variables](mdc:guide/templates/variables#template-reference-variables)
...
1. Handle form submission using the [`ngSubmit`](mdc:api/forms/NgForm#properties) output property of the form.
...
1. The sample form uses some style classes from [Twitter Bootstrap](mdc:https:/getbootstrap.com/css): `container`, `form-group`, `form-control`, and `btn`.
...
When you imported the `FormsModule` in your component, Angular automatically created and attached an [NgForm](mdc:api/forms/NgForm) directive to the `<form>` tag in the template (because `NgForm` has the selector `form` that matches `<form>` elements).

To get access to the `NgForm` and the overall form status, declare a [template reference variable](mdc:guide/templates/variables#template-reference-variables).
...
1. To confirm that two-way data binding works for the entire actor model, add a new text binding with the [`json`](mdc:api/common/JsonPipe) pipe at the top to the component's template, which serializes the data to a string.
...
- Each `<input>` element has an `id` property.
  ...
  This is a [standard HTML feature](mdc:https:/developer.mozilla.org/docs/Web/HTML/Element/label).
...
The template reference variable (`#name`) is set to `"ngModel"` because that is the value of the [`NgModel.exportAs`](mdc:api/core/Directive#exportAs) property. This property tells Angular how to link a reference variable to a directive.
</docs-step>
...
Bind the form's [`ngSubmit`](mdc:api/forms/NgForm#properties) event property to the actor-form component's `onSubmit()` method.
...
To add validation to a template-driven form, you add the same validation attributes as you would with [native HTML form validation](mdc:https:/developer.mozilla.org/docs/Web/Guide/HTML/HTML5/Constraint_validation).
...
* The `<input>` element carries the HTML validation attributes: `required` and `minlength`.
    ...
    For more information, see the [Custom validators](mdc:#defining-custom-validators) section.
...
* `#name="ngModel"` exports `NgModel` into a local variable called `name`.
    ...
    For a full list of control properties, see the [AbstractControl](mdc:api/forms/AbstractControl) API reference.
...
You can choose to [write your own validator functions](mdc:#defining-custom-validators), or you can use some of Angular's built-in validators.
...
For a full list of built-in validators, see the [Validators](mdc:api/forms/Validators) API reference.
...
A cross-field validator is a [custom validator](mdc:#defining-custom-validators "Read about custom validators") that compares the values of different fields in a form and accepts or rejects them in combination.
...
The validator retrieves the child controls by calling the `FormGroup`'s [get](mdc:api/forms/AbstractControl#get) method, then compares the values of the `name` and `role` controls.
...
This `@if` displays the error if the `FormGroup` has the cross validation error returned by the `unambiguousRoleValidator` validator, but only if the user finished [interacting with the form](mdc:#control-status-css-classes).

### Adding cross-validation to template-driven forms

For a template-driven form, you must create a directive to wrap the validator function.
You provide that directive as the validator using the [`NG_VALIDATORS` token](mdc:api/forms/NG_VALIDATORS), as shown in the following example.
...
To learn more about `FormControl` options, see the [AbstractControlOptions](mdc:api/forms/AbstractControlOptions) API reference.
...
By default, Angular disables [native HTML form validation](mdc:https:/developer.mozilla.org/docs/Web/Guide/HTML/Constraint_validation) by adding the `novalidate` attribute on the enclosing `<form>` and uses directives to match these attributes with validator functions in the framework.
...
See the [API docs](mdc:api/forms/NgForm#native-dom-validation-ui) for details.
...
The switch uses directives with the [`formControlName`](mdc:api/forms/FormControlName "FormControlName directive API reference") and [`formGroup`](mdc:api/forms/FormGroupDirective "FormGroupDirective API reference") selectors.
...
Learn about how you can [define routes using Angular router](mdc:guide/routing/define-routes).
# Define routes
...
IMPORTANT: Parameters are distinct from information in the URL's [query string](mdc:https:/en.wikipedia.org/wiki/Query_string).
Learn more about [query parameters in Angular in this guide](mdc:guide/routing/read-route-state#query-parameters).
...
In this example, URLs such as `/user/leeroy` and `/user/jenkins` render the `UserProfile` component. This component can then read the `id` parameter and use it to perform additional work, such as fetching data. See [reading route state guide](mdc:guide/routing/read-route-state) for details on reading route parameters.
...
See [Reading route state](mdc:guide/routing/read-route-state) for details on reading route parameters.
...
The `loadComponent` property accepts a loader function that returns a Promise that resolves to an Angular component. In most cases, this function uses the standard [JavaScript dynamic import API](mdc:https:/developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/import). You can, however, use any arbitrary async loader function.
...
You can associate a **title** with each route. Angular automatically updates the [page title](mdc:https:/developer.mozilla.org/en-US/docs/Web/HTML/Element/title) when a route activates. Always define appropriate page titles for your application, as these titles are necessary to create an accessible experience.
...
Each route has a `providers` property that lets you provide dependencies to that route's content via [dependency injection](mdc:guide/di).
...
See the [Dependency injection guide](mdc:guide/di) for more information about providers and injection in Angular.
...
You can read this static data by injecting the `ActivatedRoute`. See [Reading route state](mdc:guide/routing/read-route-state) for details.

### Dynamic data with data resolvers

When you need to provide dynamic data to a route, check out the [guide on route data resolvers](mdc:guide/router/route-data-resolvers).
...
Learn how to [display the contents of your routes with Outlets](mdc:guide/routing/show-routes-with-outlets).
...
Check out the [API docs for RouterOutlet](mdc:api/router/RouterOutlet?tab=api) if you’d like to learn more.

## Next steps

Learn how to [navigate to routes](mdc:guide/routing/navigate-to-routes) with Angular Router.
# Navigate to routes
...
`router.navigate()` supports both simple and complex routing scenarios, allowing you to pass route parameters, [query parameters](mdc:guide/routing/read-route-state#query-parameters), and control navigation behavior.
...
Learn how to [read route state](mdc:guide/routing/read-route-state) to create responsive and context-aware components.
# Read route state
...
Check out the [`ActivatedRoute` API docs](mdc:api/router/ActivatedRoute) for a complete list of what you can access with in the route.
...
Check out the [`ActivatedRoute` API docs](mdc:api/router/ActivatedRoute) and [`ActivatedRouteSnapshot` API docs](mdc:api/router/ActivatedRouteSnapshot) for a complete list of all properties you can access.
...
You can [define route parameters](mdc:guide/routing/define-routes#define-url-paths-with-route-parameters) by prefixing the parameter name with a colon (`:`).
...
For more information, check out the [official docs on QueryParamsHandling](mdc:api/router/QueryParamsHandling).
...
For more information, check out the official docs for [isActiveMatchOptions](mdc:api/router/IsActiveMatchOptions).
...
For more information, check out the [API docs for RouterLinkActive](mdc:api/router/RouterLinkActive).
...
To do so, you use the [withComponentInputBinding](mdc:api/router/withComponentInputBinding) feature with `provideRouter` or the `bindToComponentInputs` option of `RouterModule.forRoot`.
...
To display a 404 page, set up a [wildcard route](mdc:guide/routing/common-router-tasks#setting-up-wildcard-routes) with the `component` property set to the component you'd like to use for your 404 page as follows:
...
To use route guards, consider using [component-less routes](mdc:api/router/Route#componentless-routes) as this facilitates guarding child routes.
...
HELPFUL: For more information on providers and the bootstrap process, see [Dependency Injection](mdc:guide/di/dependency-injection-providers).
...
Note that we are also specifying a value for the `routerLinkActive`'s `ariaCurrentWhenActive`. This makes sure that visually impaired users (which may not perceive the different styling being applied) can also identify the active button. For more information see the Accessibility Best Practices [Active links identification section](mdc:best-practices/a11y#active-links-identification).
...
{path: '', redirectTo: '/heroes-list', pathMatch: 'full'},
    ...
    | `pathMatch`  | This property instructs Angular on how much of the URL to match. For this tutorial, you should set this property to `full`. This strategy is recommended when you have an empty string for a path. For more information about this property, see the [Route API documentation](mdc:api/router/Route). |
...
HELPFUL: This content is based on [Custom Route Matching with the Angular Router](mdc:https:/medium.com/@brandontroberts/custom-route-matching-with-the-angular-router-fbdd48665483), by [Brandon Roberts](mdc:https:/twitter.com/brandontroberts).
...
NOTE: By default, Angular prerenders your entire application and generates a server file. To disable this and create a fully static app, set `outputMode` to `static`. To enable SSR, update the server routes to use `RenderMode.Server`. For more details, see [`Server routing`](mdc:#server-routing) and [`Generate a fully static application`](mdc:#generate-a-fully-static-application).

## Server routing

### Configuring server routes

You can create a server route config by declaring an array of [`ServerRoute`](mdc:api/ssr/ServerRoute 'API reference') objects. This configuration typically lives in a file named `app.routes.server.ts`.
...
You can add this config to your application with [`provideServerRendering`](mdc:api/ssr/provideServerRendering 'API reference') using the [`withRoutes`](mdc:api/ssr/withRoutes 'API reference') function:
...
When using the [App shell pattern](mdc:ecosystem/service-workers/app-shell), you must specify the component to be used as the app shell for client-side rendered routes. To do this, use the [`withAppShell`](mdc:api/ssr/withAppShell 'API reference') feature:
...
The server routing configuration lets you specify how each route in your application should render by setting a [`RenderMode`](mdc:api/ssr/RenderMode 'API reference'):
...
Applications that support installable, offline experiences with [service workers](mdc:https:/developer.mozilla.org/en-US/docs/Web/API/Service_Worker_API) can rely on client-side rendering without needing to communicate with a server.
...
Angular handles redirects specified by the [`redirectTo`](mdc:api/router/Route#redirectTo 'API reference') property in route configurations, differently on the server-side.

**Server-Side Rendering (SSR)**
Redirects are performed using standard HTTP redirects (e.g., 301, 302) within the server-side rendering process.

**Prerendering (SSG)**
Redirects are implemented as "soft redirects" using [`<meta http-equiv="refresh">`](mdc:https:/developer.mozilla.org/en-US/docs/Web/HTML/Element/meta#refresh) tags in the prerendered HTML.

### Customizing build-time prerendering (SSG)

When using [`RenderMode.Prerender`](mdc:api/ssr/RenderMode#Prerender 'API reference'), you can specify several configuration options to customize the prerendering and serving process.

#### Parameterized routes

For each route with [`RenderMode.Prerender`](mdc:api/ssr/RenderMode#Prerender 'API reference'), you can specify a [`getPrerenderParams`](mdc:api/ssr/ServerRoutePrerenderWithParams#getPrerenderParams 'API reference') function. This function lets you control which specific parameters produce separate prerendered documents.

The [`getPrerenderParams`](mdc:api/ssr/ServerRoutePrerenderWithParams#getPrerenderParams 'API reference') function returns a `Promise` that resolves to an array of objects. Each object is a key-value map of route parameter name to value. For example, if you define a route like `post/:id`, `getPrerenderParams ` could return the array `[{id: 123}, {id: 456}]`, and thus render separate documents for `post/123` and `post/456`.

The body of [`getPrerenderParams`](mdc:api/ssr/ServerRoutePrerenderWithParams#getPrerenderParams 'API reference') can use Angular's [`inject`](mdc:api/core/inject 'API reference') function to inject dependencies and perform any work to determine which routes to prerender. This typically includes making requests to fetch data to construct the array of parameter values.
...
Because [`getPrerenderParams`](mdc:api/ssr/ServerRoutePrerenderWithParams#getPrerenderParams 'API reference') exclusively applies to [`RenderMode.Prerender`](mdc:api/ssr/RenderMode#Prerender 'API reference'), this function always runs at _build-time_. `getPrerenderParams` must not rely on any browser-specific or server-specific APIs for data.

IMPORTANT: When using [`inject`](mdc:api/core/inject 'API reference') inside `getPrerenderParams`, please remember that `inject` must be used synchronously. It cannot be invoked within asynchronous callbacks or following any `await` statements. For more information, refer to [`runInInjectionContext`](mdc:api/core/runInInjectionContext).

#### Fallback strategies

When using [`RenderMode.Prerender`](mdc:api/ssr/RenderMode#Prerender 'API reference') mode, you can specify a fallback strategy to handle requests for paths that haven't been prerendered.
...
- **[`REQUEST`](mdc:api/core/REQUEST 'API reference'):** Provides access to the current request object, which is of type [`Request`](mdc:https:/developer.mozilla.org/en-US/docs/Web/API/Request) from the Web API. This allows you to access headers, cookies, and other request information.
- **[`RESPONSE_INIT`](mdc:api/core/RESPONSE_INIT 'API reference'):** Provides access to the response initialization options, which is of type [`ResponseInit`](mdc:https:/developer.mozilla.org/en-US/docs/Web/API/Response/Response#parameters) from the Web API. This allows you to set headers and the status code for the response dynamically. Use this token to set headers or status codes that need to be determined at runtime.
- **[`REQUEST_CONTEXT`](mdc:api/core/REQUEST_CONTEXT 'API reference'):** Provides access to additional context related to the current request. This context can be passed as the second parameter of the [`handle`](mdc:api/ssr/AngularAppEngine#handle 'API reference') function. Typically, this is used to provide additional request-related information that is not part of the standard Web API.
...
[`HttpClient`](mdc:api/common/http/HttpClient) cached outgoing network requests when running on the server. This information is serialized and transferred to the browser as part of the initial HTML sent from the server. In the browser, `HttpClient` checks whether it has data in the cache and if so, reuses it instead of making a new HTTP request during initial application rendering. `HttpClient` stops using the cache once an application becomes [stable](mdc:api/core/ApplicationRef#isStable) while running in a browser.

By default, `HttpClient` caches all `HEAD` and `GET` requests which don't contain `Authorization` or `Proxy-Authorization` headers. You can override those settings by using [`withHttpTransferCacheOptions`](mdc:api/platform-browser/withHttpTransferCacheOptions) when providing hydration.
...
The `@angular/ssr/node` extends `@angular/ssr` specifically for Node.js environments. It provides APIs that make it easier to implement server-side rendering within your Node.js application. For a complete list of functions and usage examples, refer to the [`@angular/ssr/node` API reference](mdc:api/ssr/node/AngularNodeAppEngine) API reference.
...
The `@angular/ssr` provides essential APIs for server-side rendering your Angular application on platforms other than Node.js. It leverages the standard [`Request`](mdc:https:/developer.mozilla.org/en-US/docs/Web/API/Request) and [`Response`](mdc:https:/developer.mozilla.org/en-US/docs/Web/API/Response) objects from the Web API, enabling you to integrate Angular SSR into various server environments. For detailed information and examples, refer to the [`@angular/ssr` API reference](mdc:api/ssr/AngularAppEngine).
...
Hydration improves application performance by avoiding extra work to re-create DOM nodes. Instead, Angular tries to match existing DOM elements to the applications structure at runtime and reuses DOM nodes when possible. This results in a performance improvement that can be measured using [Core Web Vitals (CWV)](mdc:https:/web.dev/learn-core-web-vitals) statistics, such as reducing the First Input Delay ([FID](mdc:https:/web.dev/fid)) and Largest Contentful Paint ([LCP](mdc:https:/web.dev/lcp)), as well as Cumulative Layout Shift ([CLS](mdc:https:/web.dev/cls)). Improving these numbers also affects things like SEO performance.

Without hydration enabled, server-side rendered Angular applications will destroy and re-render the application's DOM, which may result in a visible UI flicker. This re-rendering can negatively impact [Core Web Vitals](mdc:https:/web.dev/learn-core-web-vitals) like [LCP](mdc:https:/web.dev/lcp) and cause a layout shift. Enabling hydration allows the existing DOM to be re-used and prevents a flicker.

## How do you enable hydration in Angular

Hydration can be enabled for server-side rendered (SSR) applications only. Follow the [Angular SSR Guide](mdc:guide/ssr) to enable server-side rendering first.
...
HELPFUL: You will likely need to fix instances of Direct DOM Manipulation before hydration will fully work either by switching to Angular constructs or by using `ngSkipHydration`. See [Constraints](mdc:#constraints), [Direct DOM Manipulation](mdc:#direct-dom-manipulation), and [How to skip hydration for particular components](mdc:#how-to-skip-hydration-for-particular-components) for more details.
...
Event replay supports _native browser events_, for example `click`, `mouseover`, and `focusin`. If you'd like to learn more about JSAction, the library that powers event replay, you can read more [on the readme](mdc:https:/github.com/angular/angular/tree/main/packages/core/primitives/event-dispatch#readme).

---

This feature ensures a consistent user experience, preventing user actions performed before Hydration from being ignored. NOTE: if you have [incremental hydration](mdc:guide/incremental-hydration) enabled, event replay is automatically enabled under the hood.
...
This is because Angular is unaware of these DOM changes and cannot resolve them during the hydration process. Angular will expect a certain structure, but it will encounter a different structure when attempting to hydrate. This mismatch will result in hydration failure and throw a DOM mismatch error ([see below](mdc:#errors)).

It is best to refactor your component to avoid this sort of DOM manipulation. Try to use Angular APIs to do this work, if you can. If you cannot refactor this behavior, use the `ngSkipHydration` attribute ([described below](mdc:#how-to-skip-hydration-for-particular-components)) until you can refactor into a hydration friendly solution.
...
If you are uncertain about whether your HTML is valid, you can use a [syntax validator](mdc:https:/validator.w3.org) to check it.
...
There are several hydration related errors you may encounter ranging from node mismatches to cases when the `ngSkipHydration` was used on an invalid host node. The most common error case that may occur is due to direct DOM manipulation using native APIs that results in hydration being unable to find or match the expected DOM tree structure on the client that was rendered by the server. The other case you may encounter this type of error was mentioned in the [Valid HTML structure](mdc:#valid-html-structure) section earlier. So, make sure the HTML in your templates are using valid structure, and you'll avoid that error case.

For a full reference on hydration related errors, visit the [Errors Reference Guide](mdc:errors).

## How to skip hydration for particular components

Some components may not work properly with hydration enabled due to some of the aforementioned issues, like [Direct DOM Manipulation](mdc:#direct-dom-manipulation). As a workaround, you can add the `ngSkipHydration` attribute to a component's tag in order to skip hydrating the entire component.
...
Application stability is an important part of the hydration process. Hydration and any post-hydration processes only occur once the application has reported stability. There are a number of ways that stability can be delayed. Examples include setting timeouts and intervals, unresolved promises, and pending microtasks. In those cases, you may encounter the [Application remains unstable](mdc:errors/NG0506) error, which indicates that your app has not yet reached the stable state after 10 seconds. If you're finding that your application is not hydrating right away, take a look at what is impacting application stability and refactor to avoid causing these delays.

## I18N

HELPFUL: Support for internationalization with hydration is currently in [developer preview](mdc:reference/releases#developer-preview). By default, Angular will skip hydration for components that use i18n blocks, effectively re-rendering those components from scratch.

To enable hydration for i18n blocks, you can add [`withI18nSupport`](mdc:api/platform-browser/withI18nSupport) to your `provideClientHydration` call.
...
Many third party scripts, such as ad trackers and analytics, modify the DOM before hydration can occur. These scripts may cause hydration errors because the page no longer matches the structure expected by Angular. Prefer deferring this type of script until after hydration whenever possible. Consider using [`AfterNextRender`](mdc:api/core/afterNextRender) to delay the script until post-hydration processes have occurred.

## Incremental Hydration

Incremental hydration is an advanced form of hydration that allows for more granular control over when hydration happens. See the [incremental hydration guide](mdc:guide/incremental-hydration) for more information.
# Incremental Hydration

**Incremental hydration** is an advanced type of [hydration](mdc:guide/hydration) that can leave sections of your application dehydrated and _incrementally_ trigger hydration of those sections as they are needed.

## Why use incremental hydration?

Incremental hydration is a performance improvement that builds on top of full application hydration. It can produce smaller initial bundles while still providing an end-user experience that is comparable to a full application hydration experience. Smaller bundles improve initial load times, reducing [First Input Delay (FID)](mdc:https:/web.dev/fid) and [Cumulative Layout Shift (CLS)](mdc:https:/web.dev/cls).
...
You can enable incremental hydration for applications that already use server-side rendering (SSR) with hydration. Follow the [Angular SSR Guide](mdc:guide/ssr) to enable server-side rendering and the [Angular Hydration Guide](mdc:guide/hydration) to enable hydration first.
...
| Trigger                                             | Description                                                            |
| --------------------------------------------------- | ---------------------------------------------------------------------- |
| [`hydrate on idle`](mdc:#hydrate-on-idle)               | Triggers when the browser is idle.                                     |
| [`hydrate on viewport`](mdc:#hydrate-on-viewport)       | Triggers when specified content enters the viewport                    |
| [`hydrate on interaction`](mdc:#hydrate-on-interaction) | Triggers when the user interacts with specified element                |
| [`hydrate on hover`](mdc:#hydrate-on-hover)             | Triggers when the mouse hovers over specified area                     |
| [`hydrate on immediate`](mdc:#hydrate-on-immediate)     | Triggers immediately after non-deferred content has finished rendering |
| [`hydrate on timer`](mdc:#hydrate-on-timer)             | Triggers after a specific duration                                     |
...
[Intersection Observer API](mdc:https:/developer.mozilla.org/docs/Web/API/Intersection_Observer_API).
...
Incremental hydration has the same constraints as full-application hydration, including limits on direct DOM manipulation and requiring valid HTML structure. Visit the [Hydration guide constraints](mdc:guide/hydration#constraints) section for more details.
...
The Angular CLI downloads and installs everything you need to test an Angular application with [Jasmine testing framework](mdc:https:/jasmine.github.io).

The project you create with the CLI is immediately ready to test.
Just run the [`ng test`](mdc:cli/test) CLI command:

```shell
ng test
```
The `ng test` command builds the application in *watch mode*,
and launches the [Karma test runner](mdc:https:/karma-runner.github.io).
...
The test output is displayed in the browser using [Karma Jasmine HTML Reporter](mdc:https:/github.com/dfederm/karma-jasmine-html-reporter).
...
```
HELPFUL: Read more about Karma configuration in the [Karma configuration guide](mdc:http:/karma-runner.github.io/6.4/config/configuration-file.html).
...
|                                                                    | Details |
|:---                                                                |:---     |
| [Code coverage](mdc:guide/testing/code-coverage)                       | How much of your app your tests are covering and how to specify required amounts. |
| [Testing services](mdc:guide/testing/services)                         | How to test the services your application uses.                                   |
| [Basics of testing components](mdc:guide/testing/components-basics)    | Basics of testing Angular components.                                             |
| [Component testing scenarios](mdc:guide/testing/components-scenarios)  | Various kinds of component testing scenarios and use cases.                       |
| [Testing attribute directives](mdc:guide/testing/attribute-directives) | How to test your attribute directives.                                            |
| [Testing pipes](mdc:guide/testing/pipes)                               | How to test pipes.                                                                |
| [Debugging tests](mdc:guide/testing/debugging)                            | Common testing bugs.                                                              |
| [Testing utility APIs](mdc:guide/testing/utility-apis)                 | Angular testing features.                                                         |
...
To enable this, open the [Karma](mdc:https:/karma-runner.github.io) test platform configuration file, `karma.conf.js`, and add the `check` property in the `coverageReporter:` key.
...
```
HELPFUL: Read more about creating and fine tuning Karma configuration in the [testing guide](mdc:guide/testing#configuration).

The `check` property causes the tool to enforce a minimum of 80% code coverage when the unit tests are run in the project.

Read more on coverage configuration options in the [karma coverage documentation](mdc:https:/github.com/karma-runner/karma-coverage/blob/master/docs/configuration.md).
# Testing services
...
Instead, mock the dependency, use a dummy value, or create a [spy](mdc:https:/jasmine.github.io/tutorials/your_first_suite#section-Spies) on the pertinent service method.
...
Your application relies on Angular [dependency injection (DI)](mdc:guide/di) to create services.
...
The `TestBed` creates a dynamically-constructed Angular *test* module that emulates an Angular [@NgModule](mdc:guide/ngmodules).

The `TestBed.configureTestingModule()` method takes a metadata object that can have most of the properties of an [@NgModule](mdc:guide/ngmodules).
...
Notice how the test uses [*destructuring assignment*](mdc:https:/developer.mozilla.org/docs/Web/JavaScript/Reference/Operators/Destructuring_assignment) to extract the setup variables that it needs.
...
Although this testing guide follows the traditional style and the default [CLI schematics](mdc:https:/github.com/angular/angular-cli) generate test files with `beforeEach()` and `TestBed`, feel free to adopt *this alternative approach* in your own projects.

## Testing HTTP services

Data services that make HTTP calls to remote servers typically inject and delegate to the Angular [`HttpClient`](mdc:guide/http/testing) service for XHR calls.
...
While the *code sample* accompanying this guide demonstrates `HttpClientTestingModule`, this page defers to the [Http guide](mdc:guide/http/testing), which covers testing with the `HttpClientTestingModule` in detail.
...
HELPFUL: Because `compileComponents` is asynchronous, it uses the [`waitForAsync`](mdc:api/core/testing/waitForAsync) utility function imported from `@angular/core/testing`.

Refer to the [waitForAsync](mdc:guide/testing/components-scenarios#waitForAsync) section for more details.
...
`TestBed.createComponent()` creates an instance of the `BannerComponent`, adds a corresponding element to the test-runner DOM, and returns a [`ComponentFixture`](mdc:#componentfixture).
...
The [ComponentFixture](mdc:api/core/testing/ComponentFixture) is a test harness for interacting with the created component and its corresponding element.
...
The application might be running on a *non-browser platform*, such as the server or a [Web Worker](mdc:https:/developer.mozilla.org/docs/Web/API/Web_Workers_API), where the element might have a diminished API or not exist at all.
...
* The `By.css()` static method selects `DebugElement` nodes with a [standard CSS selector](mdc:https:/developer.mozilla.org/docs/Learn/CSS/Building_blocks/Selectors 'CSS selectors').
...
It gives the tester an opportunity to inspect and change the state of the component *before Angular initiates data binding and calls [lifecycle hooks](mdc:guide/components/lifecycle)*.
...
For example, if you run the `BannerComponent` tests in a web coding environment such as [plunker](mdc:https:/plnkr.co), you'll see a message like this one:
...
To correct the problem, call `compileComponents()` as explained in the following [Calling compileComponents](mdc:#calling-compilecomponents) section.
...
HELPFUL: For a use case in which `TestBed.inject()` does not work, see the [*Override component providers*](mdc:#override-component-providers) section that explains when and why you must get the service from the component's injector instead.
...
XHR calls within a test are rare, but if you need to call XHR, see the [`waitForAsync()`](mdc:#waitForAsync) section.
...
You do have to call [tick()](mdc:api/core/testing/tick) to advance the virtual clock.

Calling [tick()](mdc:api/core/testing/tick) simulates the passage of time until all pending asynchronous activities finish.
In this case, it waits for the observable's `setTimeout()`.

The [tick()](mdc:api/core/testing/tick) function accepts `millis` and `tickOptions` as parameters. The `millis` parameter specifies how much the virtual clock advances and defaults to `0` if not provided.
...
```
The [tick()](mdc:api/core/testing/tick) function is one of the Angular testing utilities that you import with `TestBed`.
...
[window as any](mdc:'&lowbar;&lowbar;zone&lowbar;symbol__fakeAsyncPatchLock') = true;
...
The [RxJS `defer()` operator](mdc:http:/reactivex.io/documentation/operators/defer.html) returns an observable.
...
To flush the first quote from the observable, you call [tick()](mdc:api/core/testing/tick).
...
Instead of calling [tick()](mdc:api/core/testing/tick), it calls `fixture.whenStable()`.
...
Because the [template](mdc:#dashboard-hero-component) passes the hero name through the Angular `UpperCasePipe`, the test must match the element value with the upper-cased name.
...
The default is a partial [left-button mouse event object](mdc:https:/developer.mozilla.org/docs/Web/API/MouseEvent/button) accepted by many handlers including the `RouterLink` directive.
...
The test suite setup provided the same router harness [described above](mdc:#routing-component).
...
As explained in [Calling `compileComponents()`](mdc:#calling-compilecomponents), these tests could be run in a non-CLI environment where Angular would have to compile them in the browser.
...
The `HeroDetailComponent` is part of the `HeroModule` [Feature Module](mdc:guide/ngmodules/feature-modules) that aggregates more of the interdependent pieces including the `SharedModule`.
...
The [previous test configuration](mdc:#import-a-feature-module) replaces the real `HeroService` with a `TestHeroService` that intercepts server requests and fakes their responses.
...
Notice that `TestBed.configureTestingModule` no longer provides a fake `HeroService` because it's [not needed](mdc:#spy-stub).
...
The [override metadata object](mdc:guide/testing/utility-apis#metadata-override-object) is a generic defined as follows:
...
1. Reveal the Karma browser window.
    See [Set up testing](mdc:guide/testing#set-up-testing) if you need help with this step.
...
The [`TestBed`](mdc:#testbed-api-summary) and [`ComponentFixture`](mdc:#component-fixture-api-summary) classes are covered separately.

Here's a summary of the stand-alone functions, in order of likely utility:

| Function                     | Details |
|:---                          |:---     |
| `waitForAsync`               | Runs the body of a test \(`it`\) or setup \(`beforeEach`\) function within a special *async test zone*. See [waitForAsync](mdc:guide/testing/components-scenarios#waitForAsync).                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `fakeAsync`                  | Runs the body of a test \(`it`\) within a special *fakeAsync test zone*, enabling a linear control flow coding style. See [fakeAsync](mdc:guide/testing/components-scenarios#fake-async).                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `tick`                       | Simulates the passage of time and the completion of pending asynchronous activities by flushing both *timer* and *micro-task* queues within the *fakeAsync test zone*.  The curious, dedicated reader might enjoy this lengthy blog post, ["*Tasks, microtasks, queues and schedules*"](mdc:https:/jakearchibald.com/2015/tasks-microtasks-queues-and-schedules).  Accepts an optional argument that moves the virtual clock forward by the specified number of milliseconds, clearing asynchronous activities scheduled within that timeframe. See [tick](mdc:guide/testing/components-scenarios#tick). |
| `inject`                     | Injects one or more services from the current `TestBed` injector into a test function. It cannot inject a service provided by the component itself. See discussion of the [debugElement.injector](mdc:guide/testing/components-scenarios#get-injected-services).                                                                                                                                                                                                                                                                                                                                                                          |
...
| `ComponentFixtureAutoDetect` | A provider token for a service that turns on [automatic change detection](mdc:guide/testing/components-scenarios#automatic-change-detection).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `debugElement`      | The `DebugElement` associated with the root element of the component. <br /> The `debugElement` provides insight into the component and its DOM element during test and debugging. It's a critical property for testers. The most interesting members are covered [below](mdc:#debug-element-details). |
...
| `whenStable`        | Returns a promise that resolves when the fixture is stable. <br /> To resume testing after completion of asynchronous activity or asynchronous change detection, hook that promise. See [whenStable](mdc:guide/testing/components-scenarios#whenstable).                                                                                                                                                                                                                                                                                                  |
...
| `query`               | Calling `query(predicate: Predicate<DebugElement>)` returns the first `DebugElement` that matches the [predicate](mdc:#query-predicate) at any depth in the subtree.                                                                                                                                                                                                                                        |
| `queryAll`            | Calling `queryAll(predicate: Predicate<DebugElement>)` returns all `DebugElements` that matches the [predicate](mdc:#query-predicate) at any depth in subtree.                                                                                                                                                                                                                                              |
...
```
Component harnesses are especially useful for shared UI widgets. Developers often write tests that depend on private implementation details of widgets, such as DOM structure and CSS classes. Those dependencies make tests brittle and hard to maintain. Harnesses offer an alternative— a supported API that interacts with the widget the same way an end-user does. Widget implementation changes now become less likely to break user tests. For example, [Angular Material](mdc:https:/material.angular.dev/components/categories) provides a test harness for each component in the library.
...
| Harness environment authors | Developers who want to add support for using component harnesses in additional testing environments. For information on supported testing environments out-of-the-box, see the [test harness environments and loaders](mdc:guide/testing/using-component-harnesses#test-harness-environments-and-loaders). | [Adding support for additional testing environments](mdc:guide/testing/component-harnesses-testing-environments) |

For the full API reference, please see the [Angular CDK's component harness API reference page](mdc:api#angular_cdk_testing).
# Using component harnesses in tests

## Before you start

TIP: This guide assumes you've already read the [component harnesses overview guide](mdc:guide/testing/component-harnesses-overview). Read that first if you're new to using component harnesses.

### CDK Installation

The [Component Dev Kit (CDK)](mdc:https:/material.angular.dev/cdk/categories) is a set of behavior primitives for building components. To use the component harnesses, first install `@angular/cdk` from npm. You can do this from your terminal using the Angular CLI:
...
- End-to-end tests with [WebDriver](mdc:https:/developer.mozilla.org/en-US/docs/Web/WebDriver)
...
Additional testing environments require custom bindings. See the [adding harness support for additional testing environments guide](mdc:guide/testing/component-harnesses-testing-environments) for more information.

### Using the loader from `TestbedHarnessEnvironment` for unit tests

For unit tests you can create a harness loader from [TestbedHarnessEnvironment](mdc:api/cdk/testing/TestbedHarnessEnvironment). This environment uses a [component fixture](mdc:api/core/testing/ComponentFixture) created by Angular's `TestBed`.
...
While every harness defines an API specific to its corresponding component, they all share a common base class, [ComponentHarness](mdc:api/cdk/testing/ComponentHarness). This base class defines a static property, `hostSelector`, that matches the harness class to instances of the component in the DOM.
...
As an example, the following is a test for a component that uses the [Angular Material slider component harness](mdc:https:/material.angular.dev/components/slider/api#MatSliderHarness):
...
By default, test harnesses runs Angular's [change detection](mdc:https:/angular.dev/best-practices/runtime-performance) before reading the state of a DOM element and after interacting with a DOM element.
...
The Angular team recommends using [await](mdc:https:/developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/async_function) to improve the test readability. Calling `await` blocks the execution of your test until the associated `Promise` resolves.
...
```
While `ComponentHarness` subclasses require only the `hostSelector` property, most harnesses should also implement a static `with` method to generate `HarnessPredicate` instances. The [filtering harnesses section](mdc:guide/testing/using-component-harnesses#filtering-harnesses) covers this in more detail.
...
See the [ComponentHarness API reference page](mdc:api/cdk/testing/ComponentHarness) for the full list details of the different `locatorFor` methods.
...
`TestElement` has a number of methods to interact with the underlying DOM, such as `blur()`, `click()`, `getAttribute()`, and more. See the [TestElement API reference page](mdc:api/cdk/testing/TestElement) for the full list of methods.
...
See the [ComponentHarness API reference page](mdc:api/cdk/testing/ComponentHarness) for the full list of the different locatorFor methods.
...
[MDN's CSS Animations guide](mdc:https:/developer.mozilla.org/en-US/docs/Web/CSS/CSS_animations/Using_CSS_animations)
[W3Schools CSS3 Animations guide](mdc:https:/www.w3schools.com/css/css3_animations.asp)
[The Complete CSS Animations Tutorial](mdc:https:/www.lambdatest.com/blog/css-animations-tutorial)
[CSS Animation for Beginners](mdc:https:/thoughtbot.com/blog/css-animation-for-beginners)

and a couple of videos:
[Learn CSS Animation in 9 Minutes](mdc:https:/www.youtube.com/watch?v=z2LQYsZhsFw)
[Net Ninja CSS Animation Tutorial Playlist](mdc:https:/www.youtube.com/watch?v=jgw82b5Y2MU&list=PL4cUxeGkcC9iGYgmEd2dm3zAKzyCGDtM5)
...
Triggering the `open` or `closed` state is done by toggling classes on the element in your component. You can find examples of how to do this in our [template guide](mdc:guide/templates/binding#css-class-and-style-property-bindings).

You can see similar examples in the template guide for [animating styles directly](mdc:guide/templates/binding#css-style-properties).
...
If you don't have to worry about supporting all browsers, you can also check out `calc-size()`, which is the true solution to animating auto height. See [MDN's docs](mdc:https:/developer.mozilla.org/en-US/docs/Web/CSS/calc-size) and (this tutorial)[https://frontendmasters.com/blog/one-of-the-boss-battles-of-css-is-almost-won-transitioning-to-auto/] for more information.
...
2. Use the [`prefers-reduced-motion`](mdc:https:/developer.mozilla.org/en-US/docs/Web/CSS/@media/prefers-reduced-motion) media query to ensure no animations play for users that prefer less animation.
...
[`OnAnimationStart`](mdc:https:/developer.mozilla.org/en-US/docs/Web/API/Element/animationstart_event)
[`OnAnimationEnd`](mdc:https:/developer.mozilla.org/en-US/docs/Web/API/Element/animationend_event)
[`OnAnimationIteration`](mdc:https:/developer.mozilla.org/en-US/docs/Web/API/Element/animationitration_event)
[`OnAnimationCancel`](mdc:https:/developer.mozilla.org/en-US/docs/Web/API/Element/animationcancel_event)

[`OnTransitionStart`](mdc:https:/developer.mozilla.org/en-US/docs/Web/API/Element/transitionstart_event)
[`OnTransitionRun`](mdc:https:/developer.mozilla.org/en-US/docs/Web/API/Element/transitionrun_event)
[`OnTransitionEnd`](mdc:https:/developer.mozilla.org/en-US/docs/Web/API/Element/transitionend_event)
[`OnTransitionCancel`](mdc:https:/developer.mozilla.org/en-US/docs/Web/API/Element/transitioncancel_event)

The Web Animations API has a lot of additional functionality. [Take a look at the documentation](mdc:https:/developer.mozilla.org/en-US/docs/Web/API/Web_Animations_API) to see all the available animation APIs.
...
HELPFUL: The Router's native View Transitions integration is currently in [developer preview](mdc:reference/releases#developer-preview). Native View Transitions are also a relatively new feature so there may be limited support in some browsers.
...
If you’re curious to read more about the details of the browser API, the [Chrome Explainer](mdc:https:/developer.chrome.com/docs/web-platform/view-transitions) is an invaluable resource.
...
View transitions are a [progressive enhancement](mdc:https:/developer.mozilla.org/en-US/docs/Glossary/Progressive_Enhancement). If the browser does not support the API, the Router will perform the DOM updates without calling `startViewTransition` and the navigation will not be animated.
...
[Try the “count” example on StackBlitz](mdc:https:/stackblitz.com/edit/stackblitz-starters-2dnvtm?file=src%2Fmain.ts)
...
* [Chrome Explainer](mdc:https:/developer.chrome.com/docs/web-platform/view-transitions/same-document#transitioning_elements_dont_need_to_be_the_same_dom_element)
* [Angular Example on StackBlitz](mdc:https:/stackblitz.com/edit/stackblitz-starters-dh8npr?file=src%2Fmain.ts)

### Custom entry and exit animations

* [Chrome Explainer](mdc:https:/developer.chrome.com/docs/web-platform/view-transitions/same-document#custom_entry_and_exit_transitions)
* [Angular Example on StackBlitz](mdc:https:/stackblitz.com/edit/stackblitz-starters-8kly3o)

### Async DOM updates and waiting for content

* [Chrome Explainer](mdc:https:/developer.chrome.com/docs/web-platform/view-transitions/same-document#async_dom_updates_and_waiting_for_content)
...
* [Chrome Explainer](mdc:https:/developer.chrome.com/docs/web-platform/view-transitions/same-document#view-transition-types)
* [Angular Example on StackBlitz](mdc:https:/stackblitz.com/edit/stackblitz-starters-vxzcam)

### Handle multiple view transition styles with a class name on the view transition root (deprecated)

* [Chrome Explainer](mdc:https:/developer.chrome.com/docs/web-platform/view-transitions/same-document#changing-on-navigation-type)
* [Angular Example on StackBlitz](mdc:https:/stackblitz.com/edit/stackblitz-starters-nmnzzg?file=src%2Fmain.ts)

### Transitioning without freezing other animations

* [Chrome Explainer](mdc:https:/developer.chrome.com/docs/web-platform/view-transitions/same-document#transitioning-without-freezing)
* [Angular Example on StackBlitz](mdc:https:/stackblitz.com/edit/stackblitz-starters-76kgww)

### Animating with Javascript

* [Chrome Explainer](mdc:https:/developer.chrome.com/docs/web-platform/view-transitions/same-document#animating-with-javascript)
* [Angular Example on StackBlitz](mdc:https:/stackblitz.com/edit/stackblitz-starters-cklnkm)
...
The animation [triggers and transitions](mdc:guide/animations/transition-and-triggers)
...
The animations package allowed you to define various states using the [`state()`](mdc:api/animations/state) function within a component. Examples might be an `open` or `closed` state containing the styles for each respective state within the definition. For example:
...
In the animations package, you could target specific elements by using the `query()` function to find specific elements by a CSS class name, similar to [`document.querySelector()`](mdc:https:/developer.mozilla.org/en-US/docs/Web/API/Element/querySelector). This is unnecessary in a native CSS animation world. Instead, you can use your CSS selectors to target sub-classes and apply any desired `transform` or `animation`.
...
You can use view transitions to animate between routes. See the [Route Transition Animations Guide](mdc:guide/animations/route-animations) to get started.# Angular without ZoneJS (Zoneless)
...
use [ChangeDetectionStrategy.OnPush](mdc:best-practices/skipping-subtrees#using-onpush).
...
not using ZoneJS must make Angular aware of these with the [PendingTasks](mdc:api/core/PendingTasks) service. Serialization
...
In addition, the [pendingUntilEvent](mdc:api/core/rxjs-interop/pendingUntilEvent#) helper in `rxjs-interop` ensures
...
1. Improve the [Angular developer experience](mdc:#improving-the-angular-developer-experience) and
2. Improve the [framework’s performance](mdc:#fast-by-default).
...
* [Zoneless change detection](mdc:guide/zoneless)
* [Resource API](mdc:guide/signals/resource)
* [httpResource](mdc:api/common/http/httpResource)

### Production ready

* [Linked Signal API](mdc:guide/signals/linked-signal)
* [Incremental hydration](mdc:guide/incremental-hydration)
* [Effect API](mdc:api/core/effect)
* [Event replay with SSR](mdc:api/platform-browser/withEventReplay)
* [Route-level render mode](mdc:guide/ssr)
...
For information about our versioning policy and practices —including support and deprecation practices, as well as the release schedule— see [Angular versioning and releases](mdc:reference/releases "Angular versioning and releases").

HELPFUL: If you are currently using AngularJS, see [Upgrading from AngularJS](mdc:https:/angular.io/guide/upgrade "Upgrading from Angular JS").
*AngularJS* is the name for all v1.x versions of Angular.

## Getting notified of new releases

To be notified when new releases are available, follow [@angular](mdc:https:/x.com/angular "@angular on X") on X (formerly Twitter) or subscribe to the [Angular blog](mdc:https:/blog.angular.dev "Angular blog").

## Learning about new features

What's new? What's changed? We share the most important things you need to know on the Angular blog in [release announcements](mdc: https:/blog.angular.dev/ "Angular blog - release announcements").

To review a complete list of changes, organized by version, see the [Angular change log](mdc:https:/github.com/angular/angular/blob/main/CHANGELOG.md "Angular change log").
...
The most recent stable released version of Angular appears [on npm](mdc:https:/www.npmjs.com/package/@angular/core "Angular on npm") under "Version." For example, `16.2.4`.

You can also find the most current version of Angular by using the CLI command [`ng update`](mdc:cli/update).
By default, [`ng update`](mdc:cli/update)(without additional arguments) lists the updates that are available to you.

## Updating your environment and apps

To make updating uncomplicated, we provide complete instructions in the interactive [Angular Update Guide](mdc:update-guide).
...
For simple updates, the CLI command [`ng update`](mdc:cli/update) is all you need.
Without additional arguments, [`ng update`](mdc:cli/update) lists the updates that are available to you and provides recommended steps to update your application to the most current version.

[Angular Versioning and Releases](mdc:reference/releases#versioning "Angular Release Practices, Versioning") describes the level of change that you can expect based on a release's version number.
It also describes supported update paths.

## Resource summary

* Release announcements:
    [Angular blog - release announcements](mdc:https:/blog.angular.dev/ "Angular blog announcements about recent releases")

* Release details:
    [Angular change log](mdc:https:/github.com/angular/angular/blob/main/CHANGELOG.md "Angular change log")

* Update instructions:
    [Angular Update Guide](mdc:update-guide)

* Update command reference:
    [Angular CLI `ng update` command reference](mdc:cli/update)

* Versioning, release, support, and deprecation practices:
    [Angular versioning and releases](mdc:reference/releases "Angular versioning and releases")
# `HttpClient` security

`HttpClient` includes built-in support for two common HTTP security mechanisms: XSSI protection and XSRF/CSRF protection.

TIP: Also consider adopting a [Content Security Policy](mdc:https:/developer.mozilla.org/docs/Web/HTTP/Headers/Content-Security-Policy) for your APIs.

## XSSI protection

Cross-Site Script Inclusion (XSSI) is a form of [Cross-Site Scripting](mdc:https:/en.wikipedia.org/wiki/Cross-site_scripting) attack where an attacker loads JSON data from your API endpoints as `<script>`s on a page they control. Different JavaScript techniques can then be used to access this data.
...
[Cross-Site Request Forgery (XSRF or CSRF)](mdc:https:/en.wikipedia.org/wiki/Cross-site_request_forgery) is an attack technique by which the attacker can trick an authenticated user into unknowingly executing actions on your website.

`HttpClient` supports a [common mechanism](mdc:https:/en.wikipedia.org/wiki/Cross-site_request_forgery#Cookie-to-header_token) used to prevent XSRF attacks. When performing HTTP requests, an interceptor reads a token from a cookie, by default `XSRF-TOKEN`, and sets it as an HTTP header, `X-XSRF-TOKEN`. Because only code that runs on your domain could read the cookie, the backend can be certain that the HTTP request came from your client application and not an attacker.
...
CSRF protection is only needed for requests that can change state on the backend. By their nature, CSRF attacks cross domain boundaries, and the web's [same-origin policy](mdc:https:/developer.mozilla.org/docs/Web/Security/Same-origin_policy) will prevent an attacking page from retrieving the results of authenticated GET requests.
