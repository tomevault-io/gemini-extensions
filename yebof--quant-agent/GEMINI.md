...
- **Env vars（可选调节）**：
  ...
  - **`OPENAI_BASE_URL`（可选，OpenAI-兼容中转站路由，2026-06-08 加）**：设了它,所有 OpenAI(`gpt-`/`o*-`)调用就走这个 base_url 而不是 api.openai.com——同样的 chat/completions 协议,只是换 host+key(`base.py` OpenAI 分支显式读取,空/未设 → SDK 默认)。**必须带 relay 服务的 API 路径前缀**(网关一般是 `.../v1`,SDK 会再拼 `/chat/completions`)。当前生产用一个中转站跑全部 gpt-5.5,base_url + key 都在本机 `.env`(格式 `https://<relay-host>/v1`)、**不写进仓库**。**现在是 HTTPS + 公网受信任证书(Google Trust Services 签发),加密且证书校验通过**(早期曾短暂用明文 HTTP / Caddy 自签,均已弃用)。key 等同密码、外泄就去中转站后台 revoke。failover 仍直连 Anthropic(不经中转)。DeepSeek 用自己的 `_DEEPSEEK_BASE_URL`,不受 `OPENAI_BASE_URL` 影响
  - **`OPENAI_CA_BUNDLE`（可选，2026-06-08 加）**：当中转站用**私有/自签 CA**(如 Caddy internal,公网信任链不认)时,指向该 CA 的 PEM,OpenAI 客户端就用它做**完整校验**(链 + hostname,不是关校验)。只作用于 OpenAI 客户端,Anthropic failover 仍用公共信任链。**当前中转站是公网证书,不需要设这个**——留作将来私有 CA relay 的兜底。坚决不用「关证书校验」那条路
