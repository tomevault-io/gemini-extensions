...
Tailwind v4 is configured via the `@tailwindcss/vite` plugin. `src/frontend/src/index.css` contains only `@import "tailwindcss"` plus custom CSS variables (`:root`/`.dark` theme tokens and the `html` base rule). Any utility class used in JSX/TSX is automatically included in the build — no manual CSS additions needed.
