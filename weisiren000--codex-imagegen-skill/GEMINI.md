...
- CLI fallback also reads `$CODEX_HOME/skills/.system/imagegen/config.json` when present. Supported fields:
  ```json
  {
  ...
  ```
- For compatibility, `base_url` is accepted as an alias of `url`, and `api_key` is accepted as an alias of `key`.
- Precedence is: command-line `--model` > `OPENAI_IMAGE_MODEL` > config `model` > default; `OPENAI_API_KEY` overrides config `key`; `OPENAI_BASE_URL` overrides config `url`.
