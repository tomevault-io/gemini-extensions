...
Before calling work complete after code or doc changes, run verification in this order unless the user explicitly narrows the scope:
- `npm run format`
- `npm run lint:fix`
- `npm run lint`
- `npm run typecheck`
- relevant targeted tests when applicable
- `npm test` for repository-level confirmation

Use `lint:fix` first so the tool applies safe automatic fixes immediately. Resolve any remaining lint issues manually after that.
