...
To plug a sampler into Gibbs, implement:
  ...
  - Optionally, `keeps_linked_layout(sampler)` — return `false` if the sampler holds no parameter vector in linked space (the default is `true`). Its block is then compared at the values' native shape, deriving no `Bijectors` transform, which a distribution defining no link cannot supply anyway. `MH`, `PG`/`CSMC` and `GibbsConditional` answer `false`, and `externalsampler` answers from its own `unconstrained` type parameter rather than from the sampler it wraps, since that is the space the wrapper's `LogDensityFunction` is built in.
