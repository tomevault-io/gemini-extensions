...
- **`@velloo/cli`** — `velloo` binary (citty). Subcommands: `init`, `run`, `design`, `mcp`, `stop`, `status`, `connect`, `login`, `logout`, `publish`, `emit`, `render`, `capture`, `browser`, `export`, `upgrade`, `theme`, `completions` (plus the env-gated `trace`). A different operation is a subcommand, never a mode flag: `design list|add|remove|move|rename|upgrade|bind|set-app-root` manages the repo's designs (`design add` is `runInit` with the path pre-chosen), and `publish list|remove`, `capture list|delete`, `browser install` and `theme export` follow the same `<noun> <verb>` shape. A command that also takes a positional of its own (`publish [design]`) wraps itself in `withSubcommands` (`subcommands.ts`), which makes the subcommand names reserved words for that positional. The design argument is a positional named `design` where it is the command's subject and a `--design` flag where the positional is something else (`emit <screen>`) — never `folder`. The lazy subcommand table lives in `commands/registry.ts` — shared by `cli.ts` and the shell-completions generator (`completions/`), which introspects the citty defs so completions never drift. `init` is interactive (clack-driven wizard); the `wizard/` module composes the prompts and the `scan/` module produces screens from a host app's route structure. **A design's name lives only in its `.design/config.json`.** Resolution (`design.ts`) reads **only the directory a command runs in** — never a `velloo.json` above it, so a project nested in another (`app/admin` inside `app`) is independent — through `findDesigns` (`packages/server/src/designs.ts`): that directory's `velloo.json` lists design *paths* (plus repo-level preferences such as feedback consent, never per-design config), and `manifest.ts` owns writing it. A design folder outside the checkout (managed storage or a chosen path) is a **local design**, recorded solely in `~/.velloo/designs/.locations/` (`packages/server/src/project-location.ts`). Never write a local design into `velloo.json` or into the checkout (no project agent configs, no guidance files) — a cloned repo must not be able to point Velloo outside itself.
