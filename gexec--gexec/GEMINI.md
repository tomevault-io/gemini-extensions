...
Environment variables follow the pattern `GEXEC_<SECTION>_<KEY>` (uppercased).

Key config sections (`pkg/config/config.go`):

| Section | Env prefix | Purpose |
|---|---|---|
| `server` | `GEXEC_SERVER_*` | HTTP listen address, TLS, frontend path |
| `database` | `GEXEC_DATABASE_*` | Driver, host, credentials |
| `upload` | `GEXEC_UPLOAD_*` | File upload (local or S3) |
| `token` | `GEXEC_TOKEN_*` | JWT secret and expiry |
| `encrypt` | `GEXEC_ENCRYPT_*` | Encryption passphrase |
| `log` | `GEXEC_LOG_*` | Log level, pretty printing |
| `scim` | `GEXEC_SCIM_*` | SCIM provisioning |
| `admin` | `GEXEC_ADMIN_*` | Initial admin user bootstrap |
| `metrics` | `GEXEC_METRICS_*` | Prometheus metrics endpoint |
| `runner` | `GEXEC_RUNNER_*` | Runner → server connection |
| `cleanup` | `GEXEC_CLEANUP_*` | Background cleanup job |
...
GEXEC_TOKEN_SECRET=<32-char random string>
GEXEC_ENCRYPT_PASSPHRASE=<32-char random string>
```
...
- Security issues → email `gexec@webhippie.de` before opening a public issue.
