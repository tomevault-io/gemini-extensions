...
- SLD defaults to Euclidean kNN distance in RO rotation-vector space. SO(3) geodesic SLD is explicit opt-in only; the resolved metric must be recorded.
...
### Animate (Phase 1-4 implemented)

- `cryorole animate` is a downstream display/export command; it must not modify raw/canonical landscapes, SLD, selections, source metadata, or reconstructed maps.
- Trajectory interpolation must occur in SO(3), using quaternion SLERP or an equivalent rotation-native method. Euler angles are input/display derivatives and must not be linearly interpolated.
- Canonical waypoints must be mapped through the recorded canonical frame back to raw RO before deriving a physical density transform.
- Landscape coordinate space, baseline RO, ChimeraX scene basis, and pivot coordinate frame must be explicit and recorded; do not infer physical frame compatibility from matching `raw`/`canonical` labels.
- Baseline RO and map/session frame are explicit user assertions; do not silently assume that a saved session represents identity RO or that a canonical RV frame is automatically a physical scene transform.
- Moving-density transforms must be absolute transforms derived from the saved baseline for every frame, never accumulated frame by frame.
- Animation output must state that transformed density is a rigid-body rendering, not a new reconstruction at each trajectory position.
- Animation landscape backgrounds must reuse the public `cryorole visualize` display policy for filtering, color scaling, point ordering, axis limits, and legacy style; do not maintain a divergent renderer policy.
- Linux headless ChimeraX execution must use `--offscreen`; process return code alone is insufficient evidence of success, so require an explicit renderer-completion artifact plus validated output frames.
- Phase 4 may compose only after successful ChimeraX completion and strict
  landscape/structure frame validation. MP4 success additionally requires
  FFmpeg success and FFprobe semantic validation.
- Animation never automatically removes landscape, structure, or composite
  frames.
...
- Do not treat canonical Euler/RV display coordinates as raw physical RO when rendering an animation.
