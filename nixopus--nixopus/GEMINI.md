...
import (
    ...
    domainController "github.com/nixopus/nixopus/api/internal/features/[domain]/controller"
)
...
### Typed Error Responses (Preferred)
Fuego's `HTTPError.Err` field has `json:"-"` and is **never** sent to the client.
Always use the `Detail` field for user-visible messages. Prefer typed errors for standard status codes:
...
// 500 Internal Server Error (or non-standard codes) — use HTTPError with Detail
return nil, fuego.HTTPError{Err: err, Detail: err.Error(), Status: http.StatusInternalServerError}
```

When there is no underlying error, provide a descriptive detail message:
```go
return nil, fuego.UnauthorizedError{Detail: "authentication required"}
