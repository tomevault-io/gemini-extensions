...
2.  **(First run or after code changes) Publish Module:**
    ...
    spacetime publish vibe-multiplayer -y
    ```
...
*   **Connection:** Use `DbConnection.builder()...build();` pattern in a top-level `useEffect(..., [])` hook in `App.tsx`:
    ...
    DbConnection.builder()
      ...
      .withDatabaseName('vibe-multiplayer')   // NOT withModuleName
...
2. **Implementation Phase**:
   ...
   - Build and publish: `spacetime build && spacetime publish vibe-multiplayer -y`
