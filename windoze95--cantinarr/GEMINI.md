...
- Merges to `main` publish `ghcr.io/windoze95/cantinarr` (`latest`; version tags on `v*` releases) once that commit's CI run is green. A `v*` tag on a merged `main` commit additionally auto-creates the GitHub Release that the server's update checker consumes. Images are stamped with `git describe` output, so `latest` images know the nearest release they contain and compare on release cadence, never per merge. The app renders that comparison nowhere today — the app-wide "update available" banner is off — so a release never nags anyone in-app.
