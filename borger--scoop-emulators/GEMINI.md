...
| Checkver 404 / repo moved | Release page moved or repo renamed | Run `autofix-manifest.ps1` - it auto-detects latest via GitHub API |
| Version scheme changed | Project switched versioning format | `autofix-manifest.ps1` detects scheme (numeric/date/semantic) from recent releases |
| Release tag doesn't match | Exact version tag missing in repo | `autofix-manifest.ps1` uses pattern matching to find closest release |
...
- **Enhanced recovery mechanisms:**
  - Detects and recovers from version scheme changes (numeric → date-based, etc.)
  - Attempts recovery when checkver itself returns 404
  - Uses `Repair-VersionPattern` to analyze recent releases and identify versioning style
  - Uses `Find-ReleaseByPatternMatch` to find releases when exact version tags missing
  - Tries alternative URL formats (`v$version` and `$version` prefixes) when lookups fail
...
- **File Encoding:** Always use `New-Object System.Text.UTF8Encoding $false` (never `Set-Content`, `Out-File`, or `[System.Text.Encoding]::UTF8` without no-BOM flag)
- **Version Scheme Awareness:** `autofix-manifest.ps1` now auto-detects version format changes—no manual regex fixes needed in most cases
