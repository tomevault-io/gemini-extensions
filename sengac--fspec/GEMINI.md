...
```
spec/
├── GEMINI.md                    # This file - specification process guide
...
### Warning Messages for Placeholder Detection

When prefill is detected, fspec emits a warning message that is:
...
```xml
**IMPORTANT:** PREFILL DETECTED in generated feature file.
...
✗ Failed to update estimate: **IMPORTANT:** ACDD VIOLATION: Cannot estimate story work unit without completed feature file.
...
✗ Failed to update estimate: **IMPORTANT:** ACDD VIOLATION: Cannot estimate work unit with incomplete feature file.
...
1. **NO Markdown-Based Specifications**
   ...
   - Exception: FOUNDATION.md, TAGS.md, and GEMINI.md are meta-documentation
...
- Blocking hook failures emit **IMPORTANT:** blocks wrapping stderr
- This makes failures highly visible in Gemini CLI
...
- Stderr wrapped in **IMPORTANT:** blocks for AI visibility
...
When a blocking virtual hook fails, stderr is wrapped in a warning block:

```xml
**IMPORTANT:** BLOCKING HOOK FAILURE: Virtual hook 'eslint' for AUTH-001 failed.
...
This makes failures **highly visible** to AI agents in Gemini CLI, ensuring they address issues before continuing.
...
- Handles conflicts with AI-assisted resolution via warning messages
...
When checkpoint restoration causes conflicts, AI receives a warning message:

```xml
**IMPORTANT:** CHECKPOINT CONFLICT RESOLUTION REQUIRED
