...
```
- 若返回 `pending: true` → 按「每轮处理」流程执行，处理完后再次执行监听循环
- 若返回 `pending: false`（超时 300s）→ 再次执行循环继续等待
...
bash 长轮询循环 + `/api/wait_pending` 驱动（server.py 内置），检测到 `pending: true` 时自动执行。
...
- `<tokens>` 内为 token 计数：`in` 输入 token，`out` 输出 token，`total` 合计。`round_deliver.py` 在生成完成后自动采集并附加到 response.txt。
...
7. **不中断监听循环**：规划完成后继续长轮询循环，等待下一轮用户输入
...
- 角色卡存放在 `{ROOT}/角色卡/` 目录下，每个子文件夹为一张卡
- `current-card.txt` 记录当前激活卡名
- 切换卡时调用 `/api/play` 并重新执行导入管线
- 每张卡独立维护 `chat_log.json`、`memory/`、`variables.json`
...
以下规则适用于所有文风。踩中一条即视为不合格。`round_deliver.py` 会对高频违规词做程序化正则检测，命中则强制 retry。

> **完整约束规则集**见 `{ROOT}/skills/styles/constraints.md`（禁用词库 200+、替换规则 12 条、禁止行为 24 条、抗缺陷指南 9 种）。以下为核心摘要。

### 禁用句式（零容忍，程序化检测）
- **破折号（——）**：正文中绝对禁止，无论任何语境
- **"不是……而是……"**：先否后肯句式全部禁止，包括"与其说是X不如说是Y"
- **"XX……不，是……"**：自我纠正式转折禁止
- **"但是/然而" 开头**：禁止用作句子或段落开头词
- **动物比喻**：禁止"就像/如同/仿佛" + 任何动物（小兽/猎物/小猫/野兽等）
- **"像在陈述一个事实"**：禁止
- 禁止「不容XX」「最后一根稻草」等网文套话
- 禁止在普通名词上加暗示性引号（如：有"营养"的）

### 禁用全知修饰词
禁止在动作描写前加全知视角副词：不自觉地、下意识地、不由自主地、情不自禁、鬼使神差、微不可察、极力掩饰、不易察觉、近乎、若有若无、不经意间。
→ 裸写动作本身，让读者自己判断。

### 禁用八股微表情
禁止：瞳孔微缩、喉结滚动、睫毛颤动、呼吸一滞、身体一僵、指节泛白、耳尖滴血、红得滴血。
→ 换成角色自己的习惯小动作，或者直接不写。普通人的紧张不是这样表现的。

### 禁用身体部位八股
禁止：指节、指关节、骨节、手关节、尾椎、尾骨、脊椎、脊柱、神经末梢、四肢百骸。
→ 替换：指节/关节 → 手指；脊椎/脊柱/脊背 → 背部；弧度 → 线条；弓起 → 弯起。

### 禁用虚假精确
禁止：几不可见、几不可闻、几不可察、几不可查、难以察觉、不易察觉、难以言喻、细若蚊呐。
→ 要么写清楚到底是什么，要么不写。
...
### 禁用极端标签化情感词
禁止：崩溃、绝望、沦陷、虔诚、崇拜、臣服、支配、征服、占有、驯化、猎物、玩物、祭品、共犯、献祭、仪式、审判、深渊。
→ 换成朴素情绪词：撑不住、陷进去、很在意、佩服、听他的、想要、习惯了、盯上了。

### 禁用动物化/意象化套话
禁止：小兽、幼兽、母兽、猎物、小猫、野兽般、涟漪、心湖、深潭、古井、巨浪、餍足、旖旎、氤氲。
→ 直接描写具体状态，不套比喻壳。

### 禁用动作套话
禁止：战栗、颤栗、痉挛、绞杀、研磨、裹挟着、铺天盖地的、狂风暴雨般的、画圈、划圈、搔刮。
→ 写具体的、属于这个角色的动作。
