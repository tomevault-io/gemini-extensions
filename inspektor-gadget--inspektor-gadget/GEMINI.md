...
- The chart source is in `charts/gadget/`; `charts/bin/` is gitignored and generated by `make build` in `charts/`.
- In `values.yaml`, the `config` key holds Inspektor Gadget configuration (gadget-level resources like `gadgetConfigMaps`, `operator`, `daemonLogLevel`, etc.), while root-level keys hold Kubernetes resource settings (`image`, `nodeSelector`, `tolerations`, `additionalEnv`, etc.).
- Schema validation is in `charts/gadget/values.schema.json` — update it when adding new values.
- Lint with `cd charts && make lint`; test rendering with `cd charts && make template`.
