...
cd ../.worktrees/<name>/lab
```

New experiments MUST be developed in a worktree (see "Git Worktrees" in the root `../AGENTS.md`). Keeps main clean and lets multiple experiments run in parallel without conflicts.
...
- **Unit tests:** `npx vitest run` (TS, from `../packages/core` or `../web/frontend`) or `pytest` (Python) must stay green.
...
EXPERIMENTS.md is the single source of truth for benchmark numbers and writeups. The root README only carries the shipped-model headline.
...
- Update the one-line summary in the root `../README.md` if the headline metric changed.
...
- [ ] Root README.md updated only if the shipped-model headline metric changed — otherwise it stays untouched
...
Always use `../.venv/bin/python` — the system Python lacks project dependencies. The venv lives at the repo root, one level above `lab/`.
