...
- `#E4E4E7` on `#080C14` = ~15:1 ✓
- `#2DD4BF` on `#080C14` = ~10:1 ✓
- `#A1A1AA` on `#080C14` = ~7:1 ✓
- Never rely on color alone — use labels, shapes, position
- Provide focus-visible outlines: `outline: 2px solid #2DD4BF; outline-offset: 2px`
- All motion behind `prefers-reduced-motion` media query

---

## File & Asset Conventions

- Brand assets live in `public/brand/` (or project-appropriate static dir)
- CSS custom properties preferred: `--cosmic-bg`, `--cosmic-surface`, `--cosmic-accent`, etc.
- Tailwind: extend theme config with cosmic tokens if using Tailwind
- Images: WebP preferred, fallback to PNG. Compress aggressively.
- OG images: 1200×630, dark with teal accent and Space Grotesk text
...
- Open-source intelligence (OSINT) database of military hardware
- Category accent: Amber `#FBBF24` for OSINT classification
- Data-heavy — prioritize clean tables, search interfaces, and filterable lists
- Cosmic effects: moderate. The star/space theme actually fits well with military/satellite imagery — lean into it slightly more than a pure data tool
- Amber accent for category badges and classification markers; teal for interactive elements
- SECURITY: No personal info in public-facing content
...
Never add placeholder implementations, TODO stubs, mock-only features, or partial code that doesn't work end-to-end. Every change must be complete, tested, and production-safe.

- No `// TODO: implement later` stubs that ship
- No fake API calls that return hardcoded data pretending to be real
- No "skeleton" functions that silently do nothing
- No commented-out blocks left as "future work"
- If a feature isn't ready, don't merge it. Period.

### Never Half-Do It
Every PR must leave the codebase in a working state. If you start a feature, finish it or revert. No partial migrations, no half-wired routes, no UI that links to nothing.

### Never Be Unsafe
- NEVER commit secrets, API keys, tokens, or passwords to code
