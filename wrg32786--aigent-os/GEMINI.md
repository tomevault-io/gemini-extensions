...
- `memory/` — Operational memory (priorities, decisions, delegation, session log) — `HEAT_INDEX.json` is produced on demand by the memory-heat daemon (`node daemons/memory-heat/compute-heat.js`); it is not wired to any live hook
...
- `memory-heat/compute-heat.js` — computes vault heat scores, writes `HEAT_INDEX.json`. Run on demand (not wired to a hook; the retired `/close` skill used to invoke it). See `vault/concepts/Memory Decay Doctrine.md` and `daemons/memory-heat/README.md`.
...
## Session Lifecycle — mostly automatic

- **Resume is automatic.** `/resume` fires on `SessionStart(clear)`, and a warm pointer-reinject runs on every other SessionStart source — so a cleared or continued session re-grounds itself. You rarely need to invoke `/resume` by name; do so for a deliberate named-capsule resume or a handoff.
- **State-banking is automatic.** A rolling, best-effort capsule write runs on every `Stop` (`daemons/stop-capsule-writer.mjs`). Invoke `/context-capsule` explicitly only for a deliberate mid-session checkpoint or a clean end-of-thread finalize.
- **The capsule is best-effort autosave, never a gate.** Never tell the operator to wait on capsule machinery. `/open` and `/close` are retired — don't suggest them.
- **The hooks handle the rest.** Auto-capture, token tracking, and context-pressure self-refresh run automatically via Claude Code hooks. The operator doesn't need to think about them.

## Session rhythm

**Offer to bank the thread.** When the operator signals they're wrapping up — "done", "thanks, that's all", "heading out", "goodnight", or similar — offer once: "Want me to bank this thread before you go?" If yes, run `/context-capsule` to finalize a clean resume-ready capsule. Plain words like "close up" trigger the same capsule write. If the operator just leaves, the every-`Stop` autosave already captured a rolling capsule and the next session's `/resume` recovers automatically.
