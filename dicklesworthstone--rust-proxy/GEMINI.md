...
We are early-stage and prioritize correctness, safety, and simplicity over compatibility shims. Fix issues directly; avoid wrapper layers that preserve old behavior.
...
| `src/config.rs` | Config structures, defaults, provider inference |
| `src/state.rs` | Stats tracking and periodic flush |
| `src/iptables.rs` | iptables/ipset integration |
| `src/proxy.rs` | Transparent proxy and upstream CONNECT handling |
| `src/dns.rs` | Domain-to-IPv4 resolution |
