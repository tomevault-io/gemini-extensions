...
|--------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `<Module>..cctor` has fewer `call`s than there are registration shims, or a `[ModuleInitializer]` never runs | A manually version-pinned package left stale `obj/project.assets.json` state in the consumer. MSBuild retained the old `<Compile>` set. **Not** a Roslyn bug |
| Reverse registration "never fires" for a whole namespace                                                     | Same as above. The per-build fixture versions prevent this for the sample fixtures.                                                                                |
| Types missing from a manually rebuilt package                                                                 | Its NuGet cache still contains the prior contents for the same version.                                                                                            |
| Reverse bindings generated against an API that no longer exists                                              | A manually version-pinned dependency cache was read by `nugetExtractApi` during `packNuget`. Clear that dependency before packing.                                 |
...
| Every layer's unit tests are green, but the real pipeline is broken end to end                                | A unit test built its own hand-written `RirClass`/`RirFile` fixture instead of a real one produced by `NugetMetadataReader`. In ADR-072, the plugin's generic-codegen unit tests stayed green against a hand-built `RirClass` while the real reader was still emitting `typeParameters: []` and `instantiations: []` for the same input. Only `scripts/verify.sh`, which runs the real reader, caught it. A green unit test suite proves the generator is correct **given** its input; it proves nothing about whether the reader ever produces that input |
