...
11. **Consult the STYLEGUIDE**: Always read `STYLEGUIDE.md` before tweaking or editing any XMP preset. It documents non-obvious slider interactions, structural safeguards, and proven look recipes that must be respected.
12. **Keep preset tally up to date**: Every time you add, remove, or rename presets, update the total count and per-category counts in README.md. Use `find Presets -name '*.xmp' | wc -l` for the total and `ls Presets/{Category}/*.xmp | wc -l` per category. Also update the Featured Film Stocks list with any new stocks added. The README is the repo's front page — stale counts erode credibility.
