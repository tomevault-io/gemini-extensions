...
```bash
python -m py_compile jupyter_ai_acp_client/__init__.py  # Check single file for syntax errors
```
...
**Python** (in `jupyter_ai_acp_client/*.py` files):
...
- Backend processing in `jupyter_ai_acp_client/routes.py`
...
- **✅ Do**: `jupyter_ai_acp_client/` with underscores, all lowercase
...
```typescript
const PLUGIN_ID = 'jupyter_ai_acp_client:plugin';
```
...
export namespace CommandIDs {
  export const openPanel = 'jupyter_ai_acp_client:open-panel';
  export const refreshData = 'jupyter_ai_acp_client:refresh-data';
}
...
const plugin: JupyterFrontEndPlugin<void> = {
  id: 'jupyter_ai_acp_client:plugin',
  autoStart: true,
...
- Backend routes: `jupyter_ai_acp_client/routes.py` (class `DataRouteHandler`)
- Frontend logic: `src/` directory
- Python package: `jupyter_ai_acp_client/` directory
...
1. **Read the backend first** — Check `jupyter_ai_acp_client/routes.py` to understand the existing API contract
...
Create RESTful endpoints in `jupyter_ai_acp_client/routes.py`:
...
def setup_route_handlers(web_app):
    ...
    data_route = url_path_join(base_url, "jupyter_ai_acp_client", "data")
...
- Define matching endpoint path strings (e.g., `"hello"`, `"get-data"`) in both `jupyter_ai_acp_client/routes.py` and `src/api.ts` to ensure routes sync between backend and frontend
...
2. **Write the code** (TypeScript in `src/`, styles in `style/`, Python in `jupyter_ai_acp_client/`)
...
**After editing Python** (files in `jupyter_ai_acp_client/`):
...
- Ask user to search for the extension ID (`jupyter_ai_acp_client`) to see if it loaded
...
ls -la jupyter_ai_acp_client/labextension/  # Should contain bundled extension
```
...
- Backend routes: `jupyter_ai_acp_client/routes.py`
- Server extension config: `jupyter-config/server-config/jupyter_ai_acp_client.json`
...
**❌ Don't**: Write frontend interfaces without first checking the backend API contract in `jupyter_ai_acp_client/routes.py`
...
```css
.jp-jupyter-ai-acp-client-widget {
  padding: 8px;
}
```
...
**❌ Don't**: Use absolute imports like `from jupyter_ai_acp_client.routes import ...`
...
- **Plugin ID** (in `src/index.ts`): `'jupyter_ai_acp_client:plugin'`
- **Command IDs** (in `src/commands.ts` or `src/index.ts`): `'jupyter_ai_acp_client:command-name'`
...
- **CSS classes** (in `style/index.css`): `.jp-jupyter-ai-acp-client-ClassName`
- **API routes** (in `jupyter_ai_acp_client/routes.py`): `url_path_join(base_url, "jupyter_ai_acp_client", "endpoint")`
