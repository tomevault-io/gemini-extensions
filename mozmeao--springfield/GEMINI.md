...
* When reviewing a PR that features migrations, note the relevant app and cardinal number of any NEW/added migrations in the changeset - e.g. cms/migrations/0223_foo_bar_baz.py means there's a migration numbered 0023 in the cms app. Check the main branch and if that migration number is already in use for that app, add a comment onthe PR warning that we face a migration numbering conflict. However, if there is a merge migration in the same changeset and that already allows for two migrations with the same number, that is likley OK, so instead add a note reminding the user to be sure the merge migration is correct.
