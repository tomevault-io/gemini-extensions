...
> Conventions for using Bitget.Net library when working with Bitget in C#/.NET. Apply when generating code that interacts with the Bitget API.
...
```csharp
var futuresSub = await socketClient.FuturesApiV2.SubscribeToTickerUpdatesAsync(
    BitgetProductTypeV2.UsdtFutures,
    "BTCUSDT",
    update => { /* update.Data.First().LastPrice */ });
```

For Unified/UTA sockets:
...
Same pattern works against other CryptoExchange.Net libraries. Shared socket interfaces do not expose unsubscribe; keep the concrete socket client and call `socketClient.UnsubscribeAsync(sub.Data)`.
...
Prefer project helpers instead of hand-rolling stream merge logic:

- `BitgetSpotSymbolOrderBook`
- `BitgetFuturesSymbolOrderBook`
- `BitgetUnifiedSymbolOrderBook`
- `BitgetOrderBookFactory`
- `BitgetUserDataTracker`
- `BitgetTrackerFactory`

## Hard rules

- Never write raw `HttpClient` to Bitget endpoints.
...
- Prefer `BitgetRestClient` and `BitgetSocketClient`.
- Always use `BitgetCredentials("key", "secret", "passPhrase")`, not generic credentials without passphrase.
- Always store WebSocket subscriptions and unsubscribe on shutdown.
- Use V2 roots: `SpotApiV2`, `FuturesApiV2`, `CopyTradingFuturesV2`; use `UnifiedApi` for Unified/UTA.
- Use `SpotApiV2.Margin` for spot margin.
- Use compact Bitget symbols such as `BTCUSDT`.
- Include `BitgetProductTypeV2` on futures calls.
- Include margin asset where futures methods require it.
- Use the correct enum namespace.
- Do not invent `UtaApi`; the Unified/UTA root is `UnifiedApi`.
...
- Source of truth: `Bitget.Net/Interfaces/Clients/**` and `Bitget.Net/Enums/**`
