...
- 工作期间应定期检查项目目录体积，避免编译缓存、临时文件、调试输出、重复构建目录和无效引用长期堆积并影响检索、构建与 Agent 工作效率。
- 默认重点检查 `target/debug`、`target/release/deps`、`target/release/build`、`target/release/.fingerprint`、`target/release/incremental`、前端 `dist`、Vite 缓存及其他明确可再生目录。
- 修复或开发完成后，用户需要测试时必须保留或重新生成默认 `target/release` 下的最新应用，不得只把可测试构建放在 `target-rebuild` 等临时目录。
- 清理前必须核实绝对路径位于本项目内；如文件被占用，只结束可执行路径明确位于本项目构建目录中的 CCP 相关进程。
- 只清理确认可再生的产物。不得删除源码、规格、验收文档、主题包、用户数据、本地配置、未提交改动、`.git` 历史或仍被项目使用的依赖与资源。
- 禁止使用 `git clean -fdX`、通配符递归删除或其他可能覆盖整个工作区的粗放清理方式；应使用经过核实的明确路径逐项处理。
- `.codegraph` 是代码知识图谱运行目录，默认保留；仅在确认相关进程已停止且文件确属过期时清理 PID 或日志。
- 清理结束后应重新统计主要目录体积，并确认默认 Release 可执行文件仍存在；向用户报告释放空间、保留产物和未处理目录。
