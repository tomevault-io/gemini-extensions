...
- Palette sources are **pluggable** — each source is `{ id, label, search(query) -> results }`. Adding a new source (e.g. headings in current file) means adding a source, not forking the palette.
